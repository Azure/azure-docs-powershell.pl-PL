...
### ByTopologyAndServiceNames (domyślny)
```
...
## Opis
Polecenie cmdlet **New-AzDeploymentManagerServiceUnit** tworzy usługę w ramach usługi w topologii usług i zwraca obiekt reprezentujący tę jednostkę.
Określ jednostkę usługi, podając jej nazwę, nazwę usługi, topologię usługi, a następnie nazwę grupy zasobów. 

Polecenie cmdlet zwraca obiekt serviceunit. Możesz zmodyfikować ten obiekt lokalnie, a następnie zastosować zmiany w usłudze za pomocą polecenia cmdlet Set-AzDeploymentManagerService.
...
To polecenie cmdlet tworzy nową jednostkę usługi o nazwie ContosoService2Storage w ContosoResourceGroup w obszarze ContosoService2 usługi w topologii głównej. Pliki szablonów i parametrów są zdefiniowane jako ścieżki względne w lokalizacji źródłowej artefaktu, do której odwołuje się ContosoServiceTopology topologii usług. Zasoby zdefiniowane w tym szablonie należy wdrożyć w service2ResourceGroup grupy zasobów docelowych z trybem wdrażania ustawionym na wartość przyrostowa.
...
To polecenie cmdlet tworzy nową jednostkę usługi o nazwie ContosoService2Storage w ContosoResourceGroup w obszarze ContosoService2 usługi w topologii głównej. Odwołania szablonu i parametrów są podane jako identyfikator URI SAS jako identyfikator zasobu źródłowego artefaktu nie podano w ContosoServiceTopology1ie topologii usług. Zasoby zdefiniowane w tym szablonie należy wdrożyć w service2ResourceGroup grupy zasobów docelowych, korzystając z trybu wdrażania ustawionego na ukończone.
...
### -DeploymentMode
Tryb wdrażania, który ma być używany podczas wdrażania zasobów w jednostce usługi.
...
### — Lokalizacja
Lokalizacja zasobu jednostki usług.
...
Wymaga odwołania do ArtifactSource w usłudze servicetopology.
...
### -ParametersUri
Identyfikator URI SAS do pliku parametrów.
Jeśli w usłudze servicetopology odwołuje się ArtifactSourceId, określ ścieżkę względną przy użyciu ParametersArtifactSourceRelativePath.
...
### -ServiceName
Nazwa usługi, do której należy ta jednostka usługi.
...
### -Servicetopologyname
Nazwa topologii usługi, do której należy ta jednostka usługi.
...
### -TargetResourceGroup
Określa lokalizację, do której będą wdrażane zasoby w ramach jednostki usługi.
...
### -TemplateUri
Identyfikator URI SAS do pliku szablonu.
Jeśli w usłudze servicetopology odwołuje się ArtifactSourceId, określ ścieżkę względną przy użyciu TemplateArtifactSourceRelativePath.
