---
Module Name: Az.IotHub
Module Guid: 51a3a993-fe39-4f8f-a437-2cb751584af5
Download Help Link: https://docs.microsoft.com/en-us/powershell/module/az.iothub
Help Version: 3.1.2.0
Locale: en-US
content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/IotHub/IotHub/help/Az.IotHub.md
original_content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/IotHub/IotHub/help/Az.IotHub.md
ms.openlocfilehash: ba7e171dfa925460d9b8aa7eadcd018cc388c219
ms.sourcegitcommit: 04221336bc9eed46c05ed1e828a6811534d4b4ab
ms.translationtype: MT
ms.contentlocale: pl-PL
ms.lasthandoff: 12/08/2020
ms.locfileid: "98344902"
---
# AZ. IotHub, moduł
## Opis
Polecenia do zarządzania usługą Azure IotHubs

## AZ. IotHub polecenia cmdlet
### [Dodaj-AzIotHubCertificate](Add-AzIotHubCertificate.md)
Utwórz/zaktualizuj certyfikat centrum usługi Azure IoT.

### [Dodaj-AzIotHubConfiguration](Add-AzIotHubConfiguration.md)
Dodaj konfigurację automatycznej zarządzania urządzeniami IoT w docelowym Centrum IoT.

### [Dodaj-AzIotHubDeployment](Add-AzIotHubDeployment.md)
Dodaj wdrożenie programu IoT Edge w docelowym Centrum IoT.

### [Dodaj-AzIotHubDevice](Add-AzIotHubDevice.md)
Utwórz urządzenie w centrum IoT.

### [Dodaj-AzIotHubDeviceChildren](Add-AzIotHubDeviceChildren.md)
Dodaj urządzenia niebędące krawędziami jako elementy podrzędne do urządzenia brzegowego.

### [Dodaj-AzIotHubEventHubConsumerGroup](Add-AzIotHubEventHubConsumerGroup.md)
Tworzy grupę odbiorców eventhub.

### [Dodaj-AzIotHubKey](Add-AzIotHubKey.md)
Tworzy klucz IotHub.

### [Dodaj-AzIotHubMessageEnrichment](Add-AzIotHubMessageEnrichment.md)
Tworzy wzbogacanie wiadomości dla wybranych punktów końcowych w centrum IoT.

### [Dodaj-AzIotHubModule](Add-AzIotHubModule.md)
Utwórz moduł na docelowym urządzeniu IoT w centrum IoT.

### [Dodaj-AzIotHubRoute](Add-AzIotHubRoute.md)
Tworzenie trasy w centrum IoT

### [Dodaj-AzIotHubRoutingEndpoint](Add-AzIotHubRoutingEndpoint.md)
Dodawanie punktu końcowego do centrum IoT

### [Get-AzIotHub](Get-AzIotHub.md)
Pobiera informacje o IotHubs w ramach abonamentu.

### [Get-AzIotHubCertificate](Get-AzIotHubCertificate.md)
Wyświetla listę wszystkich certyfikatów lub określony certyfikat zawarty w centrum usługi Azure IoT. 

### [Get-AzIotHubCertificateVerificationCode](Get-AzIotHubCertificateVerificationCode.md)
Generuje kod weryfikacyjny dla certyfikatu centrum usługi Azure IoT. 

### [Get-AzIotHubConfiguration](Get-AzIotHubConfiguration.md)
Lista wszystkich lub określonych automatycznych konfiguracji zarządzania urządzeniami IoT.

### [Get-AzIotHubConnectionString](Get-AzIotHubConnectionString.md)
Pobiera connectionStrings IotHub.

### [Get-AzIotHubDeployment](Get-AzIotHubDeployment.md)
Zawiera listę wszystkich lub konkretnego wdrożenia programu IoT Edge.

### [Get-AzIotHubDevice](Get-AzIotHubDevice.md)
Wyświetla listę wszystkich urządzeń lub konkretne urządzenie zawarte w centrum usługi Azure IoT. 

### [Get-AzIotHubDeviceChildren](Get-AzIotHubDeviceChildren.md)
Drukowanie listy rozdzielonych przecinkami urządzeń podrzędnych.

### [Get-AzIotHubDeviceConnectionString](Get-AzIotHubDeviceConnectionString.md)
Uzyskaj parametry połączenia z docelowym urządzeniem IoT w centrum IoT.

### [Get-AzIotHubDeviceParent](Get-AzIotHubDeviceParent.md)
Pobierz urządzenie nadrzędne określonego urządzenia.

### [Get-AzIotHubDeviceTwin](Get-AzIotHubDeviceTwin.md)
Pobiera sznurek urządzenia.

### [Get-AzIotHubDistributedTracing](Get-AzIotHubDistributedTracing.md)
Uzyskaj ustawienia śledzenia rozproszone dla urządzenia.

### [Get-AzIotHubEventHubConsumerGroup](Get-AzIotHubEventHubConsumerGroup.md)
Pobiera wszystkie consumergroups eventhub.

### [Get-AzIotHubJob](Get-AzIotHubJob.md)
Pobiera informacje o zadaniu programu IotHub.

### [Get-AzIotHubKey](Get-AzIotHubKey.md)
Pobiera klucz IotHub.

### [Get-AzIotHubMessageEnrichment](Get-AzIotHubMessageEnrichment.md)
Wyświetla wszystkie wzbogacenia wiadomości lub inne wzbogacanie wiadomości dla Centrum IoT.

### [Get-AzIotHubModule](Get-AzIotHubModule.md)
Zapoznaj się z informacjami na temat modułu urządzenia IoT lub modułów list znajdujących się na urządzeniu IoT w centrum IoT.

### [Get-AzIotHubModuleConnectionString](Get-AzIotHubModuleConnectionString.md)
Pobierz parametry połączenia z docelowym modułem urządzenia IoT w centrum IoT.

### [Get-AzIotHubModuleTwin](Get-AzIotHubModuleTwin.md)
Pobiera sznurek modułowy urządzenia IoT.

### [Get-AzIotHubQuotaMetric](Get-AzIotHubQuotaMetric.md)
Pobiera metryki przydziału dla IotHub.

### [Get-AzIotHubRegistryStatistic](Get-AzIotHubRegistryStatistic.md)
Pobiera RegistryStatistics dla IotHub.

### [Get-AzIotHubRoute](Get-AzIotHubRoute.md)
Uzyskiwanie informacji na temat trasy w centrum IoT

### [Get-AzIotHubRoutingEndpoint](Get-AzIotHubRoutingEndpoint.md)
Uzyskaj informacje na temat wszystkich punktów końcowych Centrum IoT

### [Get-AzIotHubValidSku](Get-AzIotHubValidSku.md)
Pobiera wszystkie prawidłowe jednostki SKU, do których ta IotHub może przejść.

### [Invoke-AzIotHubConfigurationMetricsQuery](Invoke-AzIotHubConfigurationMetricsQuery.md)
Wywołaj zapytanie metryki konfiguracji urządzenia IoT.

### [Invoke-AzIotHubDeploymentMetricsQuery](Invoke-AzIotHubDeploymentMetricsQuery.md)
Wywołanie zapytania metryki wdrażania programu IoT Edge.

### [Invoke-AzIotHubDeviceMethod](Invoke-AzIotHubDeviceMethod.md)
Wywołaj metodę bezpośrednią na urządzeniu.

### [Invoke-AzIotHubManualFailover](Invoke-AzIotHubManualFailover.md)
Wywoływanie procesu przełączania awaryjnego dla Centrum IoT w regionie odzyskiwania po awarii geograficznej.

### [Invoke-AzIotHubModuleMethod](Invoke-AzIotHubModuleMethod.md)
Wywoływanie metody modułu Edge.

### [Invoke-AzIotHubQuery](Invoke-AzIotHubQuery.md)
Zbadaj Centrum IoT za pomocą wydajnego języka SQL.

### [Nowe — AzIotHub](New-AzIotHub.md)
Tworzy nowy IotHub.

### [Nowe — AzIotHubExportDevice](New-AzIotHubExportDevice.md)
Umożliwia utworzenie nowego zadania eksportowania urządzeń.

### [Nowe — AzIotHubImportDevice](New-AzIotHubImportDevice.md)
Umożliwia utworzenie nowego zadania importu urządzeń.

### [Nowe — AzIotHubKey](New-AzIotHubKey.md)
Wygeneruj klucz centrum usługi Azure IoT.

### [Nowe — AzIotHubSasToken](New-AzIotHubSasToken.md)
Generuj token SAS dla docelowego Centrum IoT, urządzenia lub modułu.

### [Remove-AzIotHub](Remove-AzIotHub.md)
Usuwa IotHub.

### [Remove-AzIotHubCertificate](Remove-AzIotHubCertificate.md)
Usuwa certyfikat centrum usługi Azure IoT.

### [Remove-AzIotHubConfiguration](Remove-AzIotHubConfiguration.md)
Usuwanie konfiguracji urządzenia IoT.

### [Remove-AzIotHubDeployment](Remove-AzIotHubDeployment.md)
Usuwanie wdrożenia przeglądarki IoT.

### [Remove-AzIotHubDevice](Remove-AzIotHubDevice.md)
Usuwanie urządzenia Centrum IoT.

### [Remove-AzIotHubDeviceChildren](Remove-AzIotHubDeviceChildren.md)
Usuwanie urządzeń nienależących do brzegu jako dzieci z określonego urządzenia brzegowego.

### [Remove-AzIotHubEventHubConsumerGroup](Remove-AzIotHubEventHubConsumerGroup.md)
Usuwa odbiorcę eventhub.

### [Remove-AzIotHubKey](Remove-AzIotHubKey.md)
Usuwa klucz IotHub.

### [Remove-AzIotHubMessageEnrichment](Remove-AzIotHubMessageEnrichment.md)
Usuwanie wiadomości w centrum IoT.

### [Remove-AzIotHubModule](Remove-AzIotHubModule.md)
Usuń moduły na docelowym urządzeniu IoT w centrum IoT.

### [Remove-AzIotHubRoute](Remove-AzIotHubRoute.md)
Usuwanie trasy w centrum IoT

### [Remove-AzIotHubRoutingEndpoint](Remove-AzIotHubRoutingEndpoint.md)
Usuwanie punktu końcowego Centrum IoT

### [Send-AzIotHubDevice2CloudMessage](Send-AzIotHubDevice2CloudMessage.md)
Wyślij wiadomość od urządzenia do chmury.

### [Set-AzIotHub](Set-AzIotHub.md)
Aktualizuje właściwości IotHub.

### [Set-AzIotHubConfiguration](Set-AzIotHubConfiguration.md)
Zaktualizuj modyfikowalne pola rejestracji konfiguracji.

### [Set-AzIotHubDeployment](Set-AzIotHubDeployment.md)
Aktualizowanie modyfikowalnych pól wdrożenia programu IoT Edge.

### [Set-AzIotHubDevice](Set-AzIotHubDevice.md)
Zaktualizuj urządzenie z Centrum IoT.

### [Set-AzIotHubDeviceParent](Set-AzIotHubDeviceParent.md)
Ustaw urządzenie nadrzędne określonego urządzenia.

### [Set-AzIotHubDistributedTracing](Set-AzIotHubDistributedTracing.md)
Aktualizowanie opcji śledzenia rozproszone dla urządzenia.

### [Set-AzIotHubEdgeModule](Set-AzIotHubEdgeModule.md)
Ustawianie modułów brzegowych na urządzeniu z jedną krawędzią.

### [Set-AzIotHubMessageEnrichment](Set-AzIotHubMessageEnrichment.md)
Aktualizowanie wzbogacania wiadomości w centrum IoT.

### [Set-AzIotHubModule](Set-AzIotHubModule.md)
Zaktualizuj moduł na docelowym urządzeniu IoT w centrum IoT.

### [Set-AzIotHubRoute](Set-AzIotHubRoute.md)
Aktualizowanie trasy w centrum IoT

### [Set-AzIotHubVerifiedCertificate](Set-AzIotHubVerifiedCertificate.md)
Weryfikuje certyfikat centrum usługi Azure IoT. 

### [Test — AzIotHubRoute](Test-AzIotHubRoute.md)
Trasy testowe w centrum IoT

### [Update-AzIotHub](Update-AzIotHub.md)
Zaktualizuj centrum usługi Azure IoT.

### [Update-AzIotHubDeviceTwin](Update-AzIotHubDeviceTwin.md)
Aktualizuje znaczniki i wymagane właściwości urządzenia z sznurem.

### [Update-AzIotHubModuleTwin](Update-AzIotHubModuleTwin.md)
Aktualizuje znaczniki i odpowiednie właściwości modułu urządzenia IoT — sznurek.

