---
Module Name: Az.Resources
Module Guid: ab3ca893-26fe-44b0-bd3c-8933df144d7b
Download Help Link: https://docs.microsoft.com/en-us/powershell/module/az.resources
Help Version: 5.5.2.0
Locale: en-US
content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/Resources/Resources/help/Az.Resources.md
original_content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/Resources/Resources/help/Az.Resources.md
ms.openlocfilehash: f4618db7a128f588508613eb98be261fa8b21683
ms.sourcegitcommit: 04221336bc9eed46c05ed1e828a6811534d4b4ab
ms.translationtype: MT
ms.contentlocale: pl-PL
ms.lasthandoff: 12/08/2020
ms.locfileid: "98362621"
---
# AZ. zasoby, moduł
## Opis
W tym temacie są wyświetlane tematy pomocy dotyczące poleceń cmdlet Menedżera zasobów platformy Azure.

## AZ. materiały cmdlet
### [Dodaj-AzADGroupMember](Add-AzADGroupMember.md)
Dodaje użytkownika do istniejącej grupy usługi AD.

### [Eksportowanie — AzResourceGroup](Export-AzResourceGroup.md)
Przechwytuje grupę zasobów jako szablon i zapisuje ją w pliku.

### [Eksportowanie — AzTemplateSpec](Export-AzTemplateSpec.md)
Eksportuje specyfikację szablonu do lokalnego systemu plików.

### [Get-AzADAppCredential](Get-AzADAppCredential.md)
Pobiera listę poświadczeń skojarzonych z aplikacją.

### [Get-AzADApplication](Get-AzADApplication.md)
Wyświetla istniejące aplikacje usługi Azure Active Directory.

### [Get-AzADGroup](Get-AzADGroup.md)
Umożliwia filtrowanie grup usługi Active Directory.

### [Get-AzADGroupMember](Get-AzADGroupMember.md)
Wyświetla listę członków grupy usługi AD w bieżącej dzierżawie.

### [Get-AzADServicePrincipal](Get-AzADServicePrincipal.md)
Umożliwia filtrowanie podmiotów zabezpieczeń usługi Active Directory.

### [Get-AzADSpCredential](Get-AzADSpCredential.md)
Pobiera listę poświadczeń skojarzonych z podmiotem zabezpieczeń usługi.

### [Get-AzADUser](Get-AzADUser.md)
Umożliwia filtrowanie użytkowników usługi Active Directory.

### [Get-AzDenyAssignment](Get-AzDenyAssignment.md)
Wyświetla listę odrzuconych przydziałów usługi Azure RBAC w określonym zakresie.
Domyślnie lista wszystkich Odmów przydziałów w wybranej subskrypcji platformy Azure.
Użyj odpowiednich parametrów, aby wyświetlić listę odrzuconych przydziałów dla określonego użytkownika lub listę odrzuconych przydziałów w określonej grupie zasobów lub zasobie.

### [Get-AzDeployment](Get-AzDeployment.md)
Pobierz wdrożenie

### [Get-AzDeploymentOperation](Get-AzDeploymentOperation.md)
Operacja pobierania wdrożenia

### [Get-AzDeploymentScript](Get-AzDeploymentScript.md)
Pobiera lub wyświetla skrypty wdrażania.

### [Get-AzDeploymentScriptLog](Get-AzDeploymentScriptLog.md)
Pobiera dziennik wykonania skryptu wdrażania.

### [Get-AzDeploymentWhatIfResult](Get-AzDeploymentWhatIfResult.md)
Pobiera szablon ARM What-If wyniku wdrożenia w zakresie abonamentu. 

### [Get-AzLocation](Get-AzLocation.md)
Pobiera wszystkie lokalizacje i dostawców zasobów obsługiwanych dla każdej lokalizacji.

### [Get-AzManagedApplication](Get-AzManagedApplication.md)
Pobiera zarządzane aplikacje

### [Get-AzManagedApplicationDefinition](Get-AzManagedApplicationDefinition.md)
Pobiera definicje aplikacji zarządzanych

### [Get-AzManagementGroup](Get-AzManagementGroup.md)
Pobiera grupy zarządzania

### [Get-AzManagementGroupDeployment](Get-AzManagementGroupDeployment.md)
Pobierz wdrożenie w grupie zarządzania

### [Get-AzManagementGroupDeploymentOperation](Get-AzManagementGroupDeploymentOperation.md)
Pobieranie operacji wdrażania grupy zarządzania

### [Get-AzManagementGroupDeploymentWhatIfResult](Get-AzManagementGroupDeploymentWhatIfResult.md)
Pobiera szablon ARM What-If wyniku wdrożenia w zakresie grupy zarządzania. 

### [Get-AzPolicyAlias](Get-AzPolicyAlias.md)
Get-AzPolicyAlias pobiera i wyprowadza typy zasobów usług Azure Provider, dla których zdefiniowano aliasy i pasują do podanych wartości parametrów. Jeśli nie zostaną podane żadne parametry, wszystkie typy zasobów dostawców zawierające alias będą wyprowadzane.
Przełącznik-ListAvailable modyfikuje to zachowanie, wyświetlając wszystkie pasujące typy zasobów, w tym te, które nie mają aliasów.

### [Get-AzPolicyAssignment](Get-AzPolicyAssignment.md)
Pobiera przypisania zasad.

### [Get-AzPolicyDefinition](Get-AzPolicyDefinition.md)
Pobiera definicje zasad.

### [Get-AzPolicySetDefinition](Get-AzPolicySetDefinition.md)
Pobiera definicje zestawu zasad.

### [Get-AzProviderFeature](Get-AzProviderFeature.md)
Pobiera informacje o funkcjach dostawcy platformy Azure.

### [Get-AzProviderOperation](Get-AzProviderOperation.md)
Pobiera operacje dla dostawcy zasobów platformy Azure, które są zabezpieczane przy użyciu usługi Azure RBAC.

### [Get-AzResource](Get-AzResource.md)
Pobiera zasoby.

### [Get-AzResourceGroup](Get-AzResourceGroup.md)
Pobiera grupy zasobów.

### [Get-AzResourceGroupDeployment](Get-AzResourceGroupDeployment.md)
Pobiera wdrożenia w grupie zasobów.

### [Get-AzResourceGroupDeploymentOperation](Get-AzResourceGroupDeploymentOperation.md)
Pobiera operację wdrożenia grupy zasobów

### [Get-AzResourceGroupDeploymentWhatIfResult](Get-AzResourceGroupDeploymentWhatIfResult.md)
Pobiera szablon ARM What-If wyniku wdrożenia w zakresie grupy zasobów. 

### [Get-AzResourceLock](Get-AzResourceLock.md)
Pobiera blokadę zasobu.

### [Get-AzResourceProvider](Get-AzResourceProvider.md)
Pobiera dostawcę zasobów.

### [Get-AzRoleAssignment](Get-AzRoleAssignment.md)
Wyświetla listę przypisań ról usługi Azure RBAC w określonym zakresie.
Domyślnie lista wszystkich przypisań ról w wybranej subskrypcji platformy Azure.
Użyj odpowiednich parametrów, aby wyświetlić listę przydziałów dla określonego użytkownika lub wyświetlić zadania dotyczące określonej grupy zasobów lub zasobu.

### [Get-AzRoleDefinition](Get-AzRoleDefinition.md)
Lista wszystkich ról usługi Azure RBAC dostępnych do przypisania.

### [Get-AzTag](Get-AzTag.md)
Pobiera wstępnie zdefiniowane Tagi platformy Azure | Pobiera cały zestaw tagów zasobu lub abonamentu.

### [Get-AzTemplateSpec](Get-AzTemplateSpec.md)
Pobiera lub wyświetla specyfikacje szablonu

### [Get-AzTenantDeployment](Get-AzTenantDeployment.md)
Pobierz wdrożenie w zakresie dzierżawy

### [Get-AzTenantDeploymentOperation](Get-AzTenantDeploymentOperation.md)
Pobierz operację wdrażania wdrożenia w zakresie dzierżawy

### [Get-AzTenantDeploymentWhatIfResult](Get-AzTenantDeploymentWhatIfResult.md)
Pobiera szablon ARM What-If wyniku wdrożenia w zakresie dzierżawy. 

### [Invoke-AzResourceAction](Invoke-AzResourceAction.md)
Umożliwia wywołanie akcji dotyczącej zasobu.

### [Przenieś — AzResource](Move-AzResource.md)
Przenoszenie zasobu do innej grupy zasobów lub abonamentu.

### [Nowe — AzADAppCredential](New-AzADAppCredential.md)
Umożliwia dodanie poświadczenia do istniejącej aplikacji.

### [Nowe — AzADApplication](New-AzADApplication.md)
Tworzy nową aplikację usługi Azure Active Directory.

### [Nowe — AzADGroup](New-AzADGroup.md)
Tworzy nową grupę usługi Active Directory.

### [Nowe — AzADServicePrincipal](New-AzADServicePrincipal.md)
Tworzy nowy podmiot zabezpieczeń usługi Azure Active Directory.

### [Nowe — AzADSpCredential](New-AzADSpCredential.md)
Umożliwia dodanie poświadczenia do istniejącego podmiotu usługi.

### [Nowe — AzADUser](New-AzADUser.md)
Tworzy nowego użytkownika usługi Active Directory.

### [Nowe — AzDeployment](New-AzDeployment.md)
Tworzenie wdrożenia

### [Nowe — AzManagedApplication](New-AzManagedApplication.md)
Tworzy aplikację zarządzaną na platformie Azure.

### [Nowe — AzManagedApplicationDefinition](New-AzManagedApplicationDefinition.md)
Tworzy definicję aplikacji zarządzanej.

### [Nowe — AzManagementGroup](New-AzManagementGroup.md)
Tworzy grupę zarządzania

### [Nowe — AzManagementGroupDeployment](New-AzManagementGroupDeployment.md)
Tworzenie wdrożenia w grupie zarządzania

### [Nowe — AzManagementGroupSubscription](New-AzManagementGroupSubscription.md)
Dodaje subskrypcję do grupy zarządzania.

### [Nowe — AzPolicyAssignment](New-AzPolicyAssignment.md)
Tworzy przypisanie zasady.

### [Nowe — AzPolicyDefinition](New-AzPolicyDefinition.md)
Tworzy definicję zasad.

### [Nowe — AzPolicySetDefinition](New-AzPolicySetDefinition.md)
Tworzy definicję zestawu zasad.

### [Nowe — AzResource](New-AzResource.md)
Tworzy zasób.

### [Nowe — AzResourceGroup](New-AzResourceGroup.md)
Tworzy grupę zasobów platformy Azure.

### [Nowe — AzResourceGroupDeployment](New-AzResourceGroupDeployment.md)
Umożliwia dodanie wdrożenia platformy Azure do grupy zasobów.

### [Nowe — AzResourceLock](New-AzResourceLock.md)
Tworzy blokadę zasobu.

### [Nowe — AzRoleAssignment](New-AzRoleAssignment.md)
Umożliwia przypisanie określonej roli RBAC do określonego podmiotu zabezpieczeń w określonym zakresie.

### [Nowe — AzRoleDefinition](New-AzRoleDefinition.md)
Tworzy rolę niestandardową w usłudze Azure RBAC.
Podaj plik definicji roli JSON lub obiekt PSRoleDefinition jako dane wejściowe.
Najpierw użyj polecenia Get-AzRoleDefinition, aby wygenerować obiekt definicji roli punktu odniesienia.
Następnie zmodyfikuj jej właściwości stosownie do potrzeb.
Na koniec Użyj tego polecenia, aby utworzyć rolę niestandardową przy użyciu definicji roli.

### [Nowe — AzTag](New-AzTag.md)
Tworzy wstępnie zdefiniowany tag Azure lub dodaje wartości do istniejącego tagu | Umożliwia utworzenie lub zaktualizowanie całego zestawu tagów zasobu lub abonamentu.

### [Nowe — AzTemplateSpec](New-AzTemplateSpec.md)
Tworzy nową specyfikację szablonu.

### [Nowe — AzTenantDeployment](New-AzTenantDeployment.md)
Tworzenie wdrożenia w zakresie dzierżawy

### [Rejestr — AzProviderFeature](Register-AzProviderFeature.md)
Rejestruje funkcję dostawcy platformy Azure na Twoim koncie.

### [Rejestr — AzResourceProvider](Register-AzResourceProvider.md)
Rejestruje dostawcę zasobów.

### [Remove-AzADAppCredential](Remove-AzADAppCredential.md)
Usuwa poświadczenie z aplikacji.

### [Remove-AzADApplication](Remove-AzADApplication.md)
Usuwa aplikację Azure Active Directory.

### [Remove-AzADGroup](Remove-AzADGroup.md)
Usuwa grupę usługi Active Directory.

### [Remove-AzADGroupMember](Remove-AzADGroupMember.md)
Usuwa użytkownika z grupy usługi AD.

### [Remove-AzADServicePrincipal](Remove-AzADServicePrincipal.md)
Usuwa podmiot zabezpieczeń usługi Azure Active Directory.

### [Remove-AzADSpCredential](Remove-AzADSpCredential.md)
Usuwa poświadczenia z podmiotu zabezpieczeń usługi.

### [Remove-AzADUser](Remove-AzADUser.md)
Usuwa użytkownika usługi Active Directory.

### [Remove-AzDeployment](Remove-AzDeployment.md)
Usuwa wdrożenie i wszystkie skojarzone operacje

### [Remove-AzDeploymentScript](Remove-AzDeploymentScript.md)
Usuwa skrypt wdrażania i skojarzone z nim zasoby.

### [Remove-AzManagedApplication](Remove-AzManagedApplication.md)
Usuwa zarządzaną aplikację

### [Remove-AzManagedApplicationDefinition](Remove-AzManagedApplicationDefinition.md)
Usuwa definicję aplikacji zarządzanej

### [Remove-AzManagementGroup](Remove-AzManagementGroup.md)
Usuwa grupę zarządzania

### [Remove-AzManagementGroupDeployment](Remove-AzManagementGroupDeployment.md)
Usunięcie wdrożenia w grupie zarządzania i wszelkich skojarzonych operacjach

### [Remove-AzManagementGroupSubscription](Remove-AzManagementGroupSubscription.md)
Usuwa abonament z grupy zarządzania.

### [Remove-AzPolicyAssignment](Remove-AzPolicyAssignment.md)
Usuwa przypisanie zasady.

### [Remove-AzPolicyDefinition](Remove-AzPolicyDefinition.md)
Usuwa definicję zasad.

### [Remove-AzPolicySetDefinition](Remove-AzPolicySetDefinition.md)
Usuwa definicję zestawu zasad.

### [Remove-AzResource](Remove-AzResource.md)
Usuwa zasób.

### [Remove-AzResourceGroup](Remove-AzResourceGroup.md)
Usuwa grupę zasobów.

### [Remove-AzResourceGroupDeployment](Remove-AzResourceGroupDeployment.md)
Usuwa wdrożenie grupy zasobów i wszystkie skojarzone z nią operacje.

### [Remove-AzResourceLock](Remove-AzResourceLock.md)
Usuwa blokadę zasobu.

### [Remove-AzRoleAssignment](Remove-AzRoleAssignment.md)
Usuwa przypisanie roli do określonego podmiotu zabezpieczeń, który jest przypisany do określonej roli w określonym zakresie.

### [Remove-AzRoleDefinition](Remove-AzRoleDefinition.md)
Usuwa rolę niestandardową w usłudze Azure RBAC.
Rola do usunięcia jest określona przy użyciu właściwości ID roli.
Usunięcie nie powiedzie się, jeśli istnieją przypisania ról wprowadzone do roli niestandardowej.

### [Remove-AzTag](Remove-AzTag.md)
Usuwa wstępnie zdefiniowane Tagi lub wartości platformy Azure | Usuwa cały zestaw tagów zasobu lub abonamentu.

### [Remove-AzTemplateSpec](Remove-AzTemplateSpec.md)
Usuwa specyfikację szablonu

### [Remove-AzTenantDeployment](Remove-AzTenantDeployment.md)
Usuwa wdrożenie w zakresie dzierżawy i wszystkie skojarzone operacje

### [Zapisz — AzDeploymentScriptLog](Save-AzDeploymentScriptLog.md)
Zapisuje dziennik wykonania skryptu wdrażania na dysku.

### [Zapisz — AzDeploymentTemplate](Save-AzDeploymentTemplate.md)
Zapisuje szablon wdrożenia w pliku.

### [Zapisz — AzManagementGroupDeploymentTemplate](Save-AzManagementGroupDeploymentTemplate.md)
Zapisuje szablon wdrożenia w pliku.

### [Zapisz — AzResourceGroupDeploymentTemplate](Save-AzResourceGroupDeploymentTemplate.md)
Zapisuje szablon rozmieszczania grup zasobów do pliku.

### [Zapisz — AzTenantDeploymentTemplate](Save-AzTenantDeploymentTemplate.md)
Zapisuje szablon wdrożenia w pliku.

### [Set-AzManagedApplication](Set-AzManagedApplication.md)
Aktualizuje zarządzaną aplikację

### [Set-AzManagedApplicationDefinition](Set-AzManagedApplicationDefinition.md)
Aktualizuje definicję aplikacji zarządzanej

### [Set-AzPolicyAssignment](Set-AzPolicyAssignment.md)
Modyfikuje przypisanie zasady.

### [Set-AzPolicyDefinition](Set-AzPolicyDefinition.md)
Modyfikuje definicję zasad.

### [Set-AzPolicySetDefinition](Set-AzPolicySetDefinition.md)
Modyfikowanie definicji zestawu zasad

### [Set-AzResource](Set-AzResource.md)
Modyfikuje zasób.

### [Set-AzResourceGroup](Set-AzResourceGroup.md)
Modyfikuje grupę zasobów.

### [Set-AzResourceLock](Set-AzResourceLock.md)
Modyfikuje blokadę zasobu.

### [Set-AzRoleAssignment](Set-AzRoleAssignment.md)
Aktualizowanie zadania roli.

### [Set-AzRoleDefinition](Set-AzRoleDefinition.md)
Modyfikuje rolę niestandardową w usłudze Azure RBAC.
Podaj zmodyfikowaną definicję roli jako plik JSON lub jako PSRoleDefinition.
Najpierw użyj polecenia Get-AzRoleDefinition, aby pobrać rolę niestandardową, którą chcesz zmodyfikować.
Następnie zmodyfikuj właściwości, które chcesz zmienić.
Na koniec Zapisz definicję roli za pomocą tego polecenia.

### [Set-AzTemplateSpec](Set-AzTemplateSpec.md)
Modyfikuje specyfikację szablonu.

### [Zatrzymaj — AzDeployment](Stop-AzDeployment.md)
Anulowanie uruchomionego wdrożenia

### [Zatrzymaj — AzManagementGroupDeployment](Stop-AzManagementGroupDeployment.md)
Anulowanie uruchomionego wdrożenia w grupie zarządzania

### [Zatrzymaj — AzResourceGroupDeployment](Stop-AzResourceGroupDeployment.md)
Anuluje wdrożenie grupy zasobów.

### [Zatrzymaj — AzTenantDeployment](Stop-AzTenantDeployment.md)
Anulowanie uruchomionego wdrożenia w zakresie dzierżawy

### [Test — AzDeployment](Test-AzDeployment.md)
Sprawdza poprawność wdrożenia.

### [Test — AzManagementGroupDeployment](Test-AzManagementGroupDeployment.md)
Sprawdza poprawność wdrożenia w grupie zarządzania.

### [Test — AzResourceGroupDeployment](Test-AzResourceGroupDeployment.md)
Sprawdza poprawność wdrożenia grupy zasobów.

### [Test — AzTenantDeployment](Test-AzTenantDeployment.md)
Sprawdza poprawność wdrożenia w zakresie dzierżawy.

### [Wyrejestrowanie — AzProviderFeature](Unregister-AzProviderFeature.md)
Wyrejestrowuje funkcję dostawcy platformy Azure na koncie.

### [Wyrejestrowanie — AzResourceProvider](Unregister-AzResourceProvider.md)
Wyrejestrowuje dostawcę zasobów.

### [Update-AzADApplication](Update-AzADApplication.md)
Umożliwia zaktualizowanie istniejącej aplikacji usługi Azure Active Directory.

### [Update-AzADServicePrincipal](Update-AzADServicePrincipal.md)
Aktualizuje istniejącego podmiot zabezpieczeń usługi Azure Active Directory.

### [Update-AzADUser](Update-AzADUser.md)
Aktualizuje istniejącego użytkownika usługi Active Directory.

### [Update-AzManagementGroup](Update-AzManagementGroup.md)
Aktualizuje grupę zarządzania

### [Update-AzTag](Update-AzTag.md)
Selektywne Aktualizowanie zestawu znaczników zasobu lub abonamentu.
