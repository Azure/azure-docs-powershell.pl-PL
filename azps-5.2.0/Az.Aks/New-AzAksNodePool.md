...
### Tworzenie puli węzłów z parametrami domyślnymi
```powershell
PS C:\> New-AzAksNodePool -ResourceGroupName myResouceGroup -ClusterName myCluster -Name mydefault
```

### Tworzenie kontenera systemu Windows Server na AKS
```powershell
...
### -Nazwaklastraname
Nazwa zarządzanego zasobu klastra.
...
### -Clusterobject
Określ obiekt klastra, w którym ma zostać utworzona Pula węzłów.
...
### -Force
Tworzenie puli węzłów nawet jeśli już istnieje
...
### -OsType
OsType, który ma być wykorzystywany do określenia typu OS.
Wybierz pozycję z systemu Linux lub Windows.
Domyślnie jest to Linux.
...
ScaleSetEvictionPolicy do użycia w celu określenia zasad wykluczenia dla zestawu skali maszyny wirtualnej o niskim priorytecie.
Domyślnie można usunąć.
...
### -ScaleSetPriority
ScaleSetPriority, aby określić priorytet zestawu skali maszyny wirtualnej.
Domyślnie na regularny.
...
### -VmSetType
Reprezentuje typy puli węzłów.
Możliwe wartości obejmują: "VirtualMachineScaleSets", "AvailabilitySet"
