---
Module Name: Az.KeyVault
Module Guid: D48CF693-4125-4D2D-8790-1514F44CE325
Download Help Link: https://docs.microsoft.com/en-us/powershell/module/az.keyvault
Help Version: 4.0.0.0
Locale: en-US
content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/KeyVault/KeyVault/help/Az.KeyVault.md
original_content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/KeyVault/KeyVault/help/Az.KeyVault.md
ms.openlocfilehash: 68bbd1131ebd18046d6e974275eddcb75764960b
ms.sourcegitcommit: 04221336bc9eed46c05ed1e828a6811534d4b4ab
ms.translationtype: MT
ms.contentlocale: pl-PL
ms.lasthandoff: 12/08/2020
ms.locfileid: "98357245"
---
# AZ. moduł magazynu
## Opis
Ten temat zawiera tematy pomocy dotyczące poleceń cmdlet magazynu kluczy platformy Azure.

## AZ. polecenia cmdlet dotyczące magazynu poleceń
### [Dodaj-AzKeyVaultCertificate](Add-AzKeyVaultCertificate.md)
Dodaje certyfikat do magazynu kluczy.

### [Dodaj-AzKeyVaultCertificateContact](Add-AzKeyVaultCertificateContact.md)
Umożliwia dodanie kontaktu do powiadomień o certyfikatach.

### [Dodaj-AzKeyVaultKey](Add-AzKeyVaultKey.md)
Tworzy klucz w magazynie kluczy lub importuje klucz do magazynu kluczy.

### [Dodaj-AzKeyVaultManagedStorageAccount](Add-AzKeyVaultManagedStorageAccount.md)
Umożliwia dodanie istniejącego konta usługi Azure Storage do określonego magazynu kluczy dla swoich kluczy, które mają być zarządzane przez usługę magazynu kluczy.

### [Dodaj-AzKeyVaultNetworkRule](Add-AzKeyVaultNetworkRule.md)
Dodaje regułę ograniczającą dostęp do magazynu kluczy na podstawie adresu internetowego klienta.

### [Backup-AzKeyVault](Backup-AzKeyVault.md)
Pełne tworzenie kopii zapasowej zarządzanego modułu HSM.

### [Backup-AzKeyVaultCertificate](Backup-AzKeyVaultCertificate.md)
Wykonywanie kopii zapasowej certyfikatu w magazynie kluczy.

### [Backup-AzKeyVaultKey](Backup-AzKeyVaultKey.md)
Wykonywanie kopii zapasowej klucza w magazynie kluczy.

### [Backup-AzKeyVaultManagedStorageAccount](Backup-AzKeyVaultManagedStorageAccount.md)
Wykonywanie kopii zapasowej konta magazynu zarządzanego przez magazyny.

### [Backup-AzKeyVaultSecret](Backup-AzKeyVaultSecret.md)
Tworzy kopię zapasową klucza tajnego w magazynie kluczy.

### [Eksportowanie — AzKeyVaultSecurityDomain](Export-AzKeyVaultSecurityDomain.md)
Eksportuje dane domeny zabezpieczeń zarządzanego modułu HSM.

### [Get-AzKeyVault](Get-AzKeyVault.md)
Pobiera kluczowe magazyny.

### [Get-AzKeyVaultCertificate](Get-AzKeyVaultCertificate.md)
Pobiera certyfikat z magazynu kluczy.

### [Get-AzKeyVaultCertificateContact](Get-AzKeyVaultCertificateContact.md)
Umożliwia wyświetlenie kontaktów zarejestrowanych w celu otrzymywania powiadomień o certyfikatach dla magazynu kluczy.

### [Get-AzKeyVaultCertificateIssuer](Get-AzKeyVaultCertificateIssuer.md)
Pobiera wystawcę certyfikatu dla magazynu kluczy.

### [Get-AzKeyVaultCertificateOperation](Get-AzKeyVaultCertificateOperation.md)
Pobiera stan operacji certyfikatu.

### [Get-AzKeyVaultCertificatePolicy](Get-AzKeyVaultCertificatePolicy.md)
Pobiera zasady dotyczące certyfikatu w magazynie kluczy.

### [Get-AzKeyVaultKey](Get-AzKeyVaultKey.md)
Pobiera klucze magazynu kluczy.

### [Get-AzKeyVaultManagedHsm](Get-AzKeyVaultManagedHsm.md)
Pobierz HSMs zarządzaną.

### [Get-AzKeyVaultManagedStorageAccount](Get-AzKeyVaultManagedStorageAccount.md)
Pobiera konta usługi Azure Storage zarządzane za pomocą magazynu kluczy.

### [Get-AzKeyVaultManagedStorageSasDefinition](Get-AzKeyVaultManagedStorageSasDefinition.md)
Pobiera definicje SAS magazynu zarządzanego w magazynie kluczy.

### [Get-AzKeyVaultRoleAssignment](Get-AzKeyVaultRoleAssignment.md)
Uzyskiwanie lub wyświetlanie przydziałów ról zarządzanego modułu HSM. Użyj odpowiednich parametrów, aby wyświetlić listę przydziałów dla określonego użytkownika lub definicji roli.

### [Get-AzKeyVaultRoleDefinition](Get-AzKeyVaultRoleDefinition.md)
Wyświetlanie definicji ról określonego zarządzanego modułu HSM w danym zakresie.

### [Get-AzKeyVaultSecret](Get-AzKeyVaultSecret.md)
Pobiera klucze tajne w magazynie kluczy.

### [Import — AzKeyVaultCertificate](Import-AzKeyVaultCertificate.md)
Importuje certyfikat do magazynu kluczy.

### [Import — AzKeyVaultSecurityDomain](Import-AzKeyVaultSecurityDomain.md)
Importuje uprzednio wyeksportowane dane domeny zabezpieczeń do zarządzanego modułu HSM.

### [Nowe — AzKeyVault](New-AzKeyVault.md)
Tworzy magazyn kluczy.

### [Nowe — AzKeyVaultCertificateAdministratorDetail](New-AzKeyVaultCertificateAdministratorDetail.md)
Tworzy obiekt szczegółów administratora certyfikatu w pamięci.

### [Nowe — AzKeyVaultCertificateOrganizationDetail](New-AzKeyVaultCertificateOrganizationDetail.md)
Tworzy obiekt szczegółów organizacji certyfikatu w pamięci.

### [Nowe — AzKeyVaultCertificatePolicy](New-AzKeyVaultCertificatePolicy.md)
Tworzy obiekt zasad certyfikatu w pamięci.

### [Nowe — AzKeyVaultManagedHsm](New-AzKeyVaultManagedHsm.md)
Tworzy zarządzany modułu HSM.

### [Nowe — AzKeyVaultNetworkRuleSetObject](New-AzKeyVaultNetworkRuleSetObject.md)
Utwórz obiekt reprezentujący ustawienia reguły sieci.

### [Nowe — AzKeyVaultRoleAssignment](New-AzKeyVaultRoleAssignment.md)
Umożliwia przypisanie określonej roli RBAC do określonego podmiotu zabezpieczeń w określonym zakresie.

### [Remove-AzKeyVault](Remove-AzKeyVault.md)
Usuwa Magazyn kluczy.

### [Remove-AzKeyVaultAccessPolicy](Remove-AzKeyVaultAccessPolicy.md)
Usuwa wszystkie uprawnienia użytkownika lub aplikacji z magazynu kluczy.

### [Remove-AzKeyVaultCertificate](Remove-AzKeyVaultCertificate.md)
Usuwa certyfikat z magazynu kluczy.

### [Remove-AzKeyVaultCertificateContact](Remove-AzKeyVaultCertificateContact.md)
Umożliwia usunięcie kontaktu zarejestrowanego dla powiadomień o certyfikatach z magazynu kluczy.

### [Remove-AzKeyVaultCertificateIssuer](Remove-AzKeyVaultCertificateIssuer.md)
Usuwa wystawcę certyfikatu z magazynu kluczy.

### [Remove-AzKeyVaultCertificateOperation](Remove-AzKeyVaultCertificateOperation.md)
Usuwa operację certyfikatu z magazynu kluczy.

### [Remove-AzKeyVaultKey](Remove-AzKeyVaultKey.md)
Usuwa klucz w magazynie kluczy.

### [Remove-AzKeyVaultManagedHsm](Remove-AzKeyVaultManagedHsm.md)
Usuwa zarządzany element HSM.

### [Remove-AzKeyVaultManagedStorageAccount](Remove-AzKeyVaultManagedStorageAccount.md)
Umożliwia usunięcie konta usługi Azure Storage zarządzanego w magazynie kluczy oraz wszystkich skojarzonych definicji skojarzeń zabezpieczeń.

### [Remove-AzKeyVaultManagedStorageSasDefinition](Remove-AzKeyVaultManagedStorageSasDefinition.md)
Usuwa definicje SAS magazynu kluczy zarządzane w usłudze Azure Storage.

### [Remove-AzKeyVaultNetworkRule](Remove-AzKeyVaultNetworkRule.md)
Usuwa regułę sieciową z magazynu kluczy.

### [Remove-AzKeyVaultRoleAssignment](Remove-AzKeyVaultRoleAssignment.md)
Usuwa przypisanie roli do określonego podmiotu zabezpieczeń, który jest przypisany do określonej roli w określonym zakresie.

### [Remove-AzKeyVaultSecret](Remove-AzKeyVaultSecret.md)
Usuwa klucz tajny w magazynie kluczy.

### [Restore-AzKeyVault](Restore-AzKeyVault.md)
Umożliwia całkowite przywrócenie zarządzanego modułu HSM z kopii zapasowej.

### [Restore-AzKeyVaultCertificate](Restore-AzKeyVaultCertificate.md)
Przywraca certyfikat w magazynie kluczy z pliku kopii zapasowej.

### [Restore-AzKeyVaultKey](Restore-AzKeyVaultKey.md)
Tworzy klucz w magazynie kluczy na podstawie klucza kopii zapasowej.

### [Restore-AzKeyVaultManagedStorageAccount](Restore-AzKeyVaultManagedStorageAccount.md)
Przywraca zarządzane konto magazynu w magazynie kluczy z pliku kopii zapasowej.

### [Restore-AzKeyVaultSecret](Restore-AzKeyVaultSecret.md)
Tworzy tajemnicę w magazynie kluczy z poziomu tajnego zapasu.

### [Set-AzKeyVaultAccessPolicy](Set-AzKeyVaultAccessPolicy.md)
Udziela lub modyfikuje istniejące uprawnienia dla użytkownika, aplikacji lub grupy zabezpieczeń w celu wykonania operacji za pomocą magazynu kluczy.

### [Set-AzKeyVaultCertificateIssuer](Set-AzKeyVaultCertificateIssuer.md)
Ustawia wystawcę certyfikatu w magazynie kluczy.

### [Set-AzKeyVaultCertificatePolicy](Set-AzKeyVaultCertificatePolicy.md)
Tworzy lub aktualizuje zasady dotyczące certyfikatu w magazynie kluczy.

### [Set-AzKeyVaultManagedStorageSasDefinition](Set-AzKeyVaultManagedStorageSasDefinition.md)
Ustawia definicję podpisu dostępu współużytkowanego (SAS) z magazynem kluczy dla danego magazynu kluczy zarządzanego konta usługi Azure Storage.

### [Set-AzKeyVaultSecret](Set-AzKeyVaultSecret.md)
Tworzy lub aktualizuje klucz tajny w magazynie kluczy.

### [Zatrzymaj — AzKeyVaultCertificateOperation](Stop-AzKeyVaultCertificateOperation.md)
Anuluje operację certyfikatu w magazynie kluczy.

### [Cofanie — AzKeyVaultCertificateRemoval](Undo-AzKeyVaultCertificateRemoval.md)
Odkrycie usuniętego certyfikatu w magazynie kluczy w stanie aktywnym.

### [Cofanie — AzKeyVaultKeyRemoval](Undo-AzKeyVaultKeyRemoval.md)
Umożliwia odkrycie usuniętego klucza w magazynie kluczy w aktywnym stanie.

### [Cofanie — AzKeyVaultManagedStorageAccountRemoval](Undo-AzKeyVaultManagedStorageAccountRemoval.md)
Odkrycie poprzednio usuniętego konta magazynu zarządzanego przez magazyny.

### [Cofanie — AzKeyVaultManagedStorageSasDefinitionRemoval](Undo-AzKeyVaultManagedStorageSasDefinitionRemoval.md)
Odkrycie uprzednio usuniętej definicji skojarzeń SAS magazynu zarządzanych przez magazyny danych.

### [Cofanie — AzKeyVaultRemoval](Undo-AzKeyVaultRemoval.md)
Odkrycie usuniętego magazynu kluczy w stanie aktywnym.

### [Cofanie — AzKeyVaultSecretRemoval](Undo-AzKeyVaultSecretRemoval.md)
Odkrycie usuniętego klucza tajnego w magazynie kluczy w stan aktywny.

### [Update-AzKeyVault](Update-AzKeyVault.md)
Zaktualizuj stan magazynu kluczy platformy Azure.

### [Update-AzKeyVaultCertificate](Update-AzKeyVaultCertificate.md)
Modyfikowanie atrybutów edytowalnych certyfikatu.

### [Update-AzKeyVaultKey](Update-AzKeyVaultKey.md)
Aktualizuje atrybuty klucza w magazynie kluczy.

### [Update-AzKeyVaultManagedHsm](Update-AzKeyVaultManagedHsm.md)
Zaktualizuj stan modułu HSM zarządzanego na platformie Azure.

### [Update-AzKeyVaultManagedStorageAccount](Update-AzKeyVaultManagedStorageAccount.md)
Aktualizowanie edytowalnych atrybutów konta usługi Azure Storage zarządzanego w magazynie kluczy.

### [Update-AzKeyVaultManagedStorageAccountKey](Update-AzKeyVaultManagedStorageAccountKey.md)
Generuje ponownie określony klucz konta usługi Azure Storage zarządzanego w magazynie kluczy.

### [Update-AzKeyVaultNetworkRuleSet](Update-AzKeyVaultNetworkRuleSet.md)
Umożliwia zaktualizowanie zestawu reguł sieci w magazynie kluczy.

### [Update-AzKeyVaultSecret](Update-AzKeyVaultSecret.md)
Aktualizuje atrybuty klucza tajnego w magazynie kluczy.

