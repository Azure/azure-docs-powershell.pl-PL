---
Module Name: Az.RecoveryServices
Module Guid: 4aa53b7e-fcfe-4e22-979c-9a4e6380de58
Download Help Link: https://docs.microsoft.com/en-us/powershell/module/az.recoveryservices
Help Version: 4.1.2.0
Locale: en-US
content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/RecoveryServices/RecoveryServices/help/Az.RecoveryServices.md
original_content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/RecoveryServices/RecoveryServices/help/Az.RecoveryServices.md
ms.openlocfilehash: 1c8725fe89ee5bcad110555cac5de813ba5ec6e5
ms.sourcegitcommit: 04221336bc9eed46c05ed1e828a6811534d4b4ab
ms.translationtype: MT
ms.contentlocale: pl-PL
ms.lasthandoff: 12/08/2020
ms.locfileid: "98370232"
---
# AZ. RecoveryServices, moduł
## Opis
W tym temacie są wyświetlane tematy pomocy dotyczące apletów poleceń usług odzyskiwania platformy Azure.

## AZ. RecoveryServices polecenia cmdlet
### [Dodaj-AzRecoveryServicesAsrReplicationProtectedItemDisk](Add-AzRecoveryServicesAsrReplicationProtectedItemDisk.md)
Dodaj dysk do ochrony już chronionej maszyny wirtualnej platformy Azure.

### [Backup-AzRecoveryServicesBackupItem](Backup-AzRecoveryServicesBackupItem.md)
Rozpoczyna tworzenie kopii zapasowej elementu kopii zapasowej.

### [Copy-AzRecoveryServicesVault](Copy-AzRecoveryServicesVault.md)
Kopiuje dane z magazynu w jednym regionie do magazynu w innym regionie.

### [Disable-AzRecoveryServicesBackupAutoProtection](Disable-AzRecoveryServicesBackupAutoProtection.md)
Wyłącza automatyczne tworzenie kopii zapasowej dla elementu chronionego.

### [Disable-AzRecoveryServicesBackupProtection](Disable-AzRecoveryServicesBackupProtection.md)
Wyłącza ochronę elementu chronionego kopią zapasową.

### [Disable-AzRecoveryServicesBackupRPMountScript](Disable-AzRecoveryServicesBackupRPMountScript.md)
Odinstalowuje wszystkie pliki punktu odzyskiwania.

### [Edit-AzRecoveryServicesAsrRecoveryPlan](Edit-AzRecoveryServicesAsrRecoveryPlan.md)
Umożliwia edytowanie planu odzyskiwania witryny.

### [Enable-AzRecoveryServicesBackupAutoProtection](Enable-AzRecoveryServicesBackupAutoProtection.md)
Polecenie cmdlet **enable-AzRecoveryServicesBackupAutoProtection** konfiguruje automatyczną ochronę bieżących i przyszłych DBS SQL w danym wystąpieniu za pomocą podanych zasad.

### [Enable-AzRecoveryServicesBackupProtection](Enable-AzRecoveryServicesBackupProtection.md)
Umożliwia tworzenie kopii zapasowej elementu z określonymi zasadami ochrony kopii zapasowych.

### [Get-AzRecoveryServicesAsrAlertSetting](Get-AzRecoveryServicesAsrAlertSetting.md)
Pobiera skonfigurowane ustawienia powiadomień usługi Azure Site Recovery dla magazynu.

### [Get-AzRecoveryServicesAsrEvent](Get-AzRecoveryServicesAsrEvent.md)
Pobiera szczegóły zdarzeń odzyskiwania witryny Azure w magazynie.

### [Get-AzRecoveryServicesAsrFabric](Get-AzRecoveryServicesAsrFabric.md)
Zapoznaj się z informacjami na temat sieci szkieletowej odzyskiwania witryny Azure.

### [Get-AzRecoveryServicesAsrJob](Get-AzRecoveryServicesAsrJob.md)
Pobiera szczegóły określonego zadania ASR lub listę ostatnich zadań ASR w magazynie usług Recovery Services.

### [Get-AzRecoveryServicesAsrNetwork](Get-AzRecoveryServicesAsrNetwork.md)
Pobiera informacje o sieciach zarządzanych przez odzyskiwanie witryny dla bieżącego magazynu.

### [Get-AzRecoveryServicesAsrNetworkMapping](Get-AzRecoveryServicesAsrNetworkMapping.md)
Pobiera informacje o mapowaniach sieciowych odzyskiwania witryny dla bieżącego magazynu.

### [Get-AzRecoveryServicesAsrPolicy](Get-AzRecoveryServicesAsrPolicy.md)
Pobiera zasady replikacji ASR.

### [Get-AzRecoveryServicesAsrProtectableItem](Get-AzRecoveryServicesAsrProtectableItem.md)
Pobierz elementy do ochrony w kontenerze ochrona przed Autoodzyskiwaniem.

### [Get-AzRecoveryServicesAsrProtectionContainer](Get-AzRecoveryServicesAsrProtectionContainer.md)
Pobiera kontenery ochrony ASR w magazynie usług Recovery Services.

### [Get-AzRecoveryServicesAsrProtectionContainerMapping](Get-AzRecoveryServicesAsrProtectionContainerMapping.md)
Pobiera mapowania kontenerów ochrony usługi Azure Site Recovery.

### [Get-AzRecoveryServicesAsrRecoveryPlan](Get-AzRecoveryServicesAsrRecoveryPlan.md)
Pobiera plan odzyskiwania lub wszystkie plany odzyskiwania w magazynie usług Recovery Services

### [Get-AzRecoveryServicesAsrRecoveryPoint](Get-AzRecoveryServicesAsrRecoveryPoint.md)
Pobiera dostępne punkty odzyskiwania dla elementu chronionego replikacji.

### [Get-AzRecoveryServicesAsrReplicationProtectedItem](Get-AzRecoveryServicesAsrReplicationProtectedItem.md)
Pobiera właściwości elementów chronionych replikacji odzyskiwania witryny platformy Azure.

### [Get-AzRecoveryServicesAsrServicesProvider](Get-AzRecoveryServicesAsrServicesProvider.md)
Umożliwia wyświetlenie szczegółów dotyczących dostawców usług odzyskiwania ASR zarejestrowanych w magazynie usług Recovery Services.

### [Get-AzRecoveryServicesAsrStorageClassification](Get-AzRecoveryServicesAsrStorageClassification.md)
Pobiera dostępne (odnalezione) klasyfikacje magazynu ASR w magazynie usług Recovery Services.

### [Get-AzRecoveryServicesAsrStorageClassificationMapping](Get-AzRecoveryServicesAsrStorageClassificationMapping.md)
Pobiera mapowania klasyfikacji magazynu ASR.

### [Get-AzRecoveryServicesAsrVaultContext](Get-AzRecoveryServicesAsrVaultContext.md)
Pobiera informacje o ustawieniach magazynu ASR dotyczące magazynu usług odzyskiwania.

### [Get-AzRecoveryServicesAsrvCenter](Get-AzRecoveryServicesAsrvCenter.md)
Pobiera szczegóły serwerów vCenter zarejestrowanych do odnajdowania na serwerze konfiguracji określonym przez sieć szkieletową ASR.

### [Get-AzRecoveryServicesBackupContainer](Get-AzRecoveryServicesBackupContainer.md)
Pobiera kontenery kopii zapasowych.

### [Get-AzRecoveryServicesBackupItem](Get-AzRecoveryServicesBackupItem.md)
Pobiera elementy z kontenera w kopii zapasowej.

### [Get-AzRecoveryServicesBackupJob](Get-AzRecoveryServicesBackupJob.md)
Pobiera zadania kopii zapasowej.

### [Get-AzRecoveryServicesBackupJobDetail](Get-AzRecoveryServicesBackupJobDetail.md)
Pobiera szczegóły zadania kopii zapasowej.

### [Get-AzRecoveryServicesBackupManagementServer](Get-AzRecoveryServicesBackupManagementServer.md)
Pobiera serwery usługi zarządzania zapasami SCDPM i Azure Backup.

### [Get-AzRecoveryServicesBackupProperty](Get-AzRecoveryServicesBackupProperty.md)
Pobiera właściwości kopii zapasowej.

### [Get-AzRecoveryServicesBackupProtectableItem](Get-AzRecoveryServicesBackupProtectableItem.md)
To polecenie spowoduje pobranie wszystkich elementów podlegających ochronie w określonym kontenerze lub we wszystkich zarejestrowanych kontenerach. Będzie on zawierać wszystkie elementy hierarchii aplikacji. Zwraca DBs i ich wyższe jednostki, takie jak instancja, Availability itd.

### [Get-AzRecoveryServicesBackupProtectionPolicy](Get-AzRecoveryServicesBackupProtectionPolicy.md)
Pobiera zasady ochrony kopii zapasowej dla magazynu.

### [Get-AzRecoveryServicesBackupRecoveryLogChain](Get-AzRecoveryServicesBackupRecoveryLogChain.md)
To polecenie zawiera listę punktów początkowych i końcowych nieprzerwanego łańcucha dziennika danego elementu kopii zapasowej. Użyj go do określenia, czy punkt w czasie, do którego użytkownik chce przywrócić bazę danych, jest prawidłowy.

### [Get-AzRecoveryServicesBackupRecoveryPoint](Get-AzRecoveryServicesBackupRecoveryPoint.md)
Pobiera punkty odzyskiwania dla elementu z kopii zapasowej.

### [Get-AzRecoveryServicesBackupRetentionPolicyObject](Get-AzRecoveryServicesBackupRetentionPolicyObject.md)
Pobiera podstawowy obiekt zasad przechowywania.

### [Get-AzRecoveryServicesBackupRPMountScript](Get-AzRecoveryServicesBackupRPMountScript.md)
Umożliwia pobranie skryptu w celu zainstalowania wszystkich plików punktu odzyskiwania.

### [Get-AzRecoveryServicesBackupSchedulePolicyObject](Get-AzRecoveryServicesBackupSchedulePolicyObject.md)
Pobiera obiekt zasad dotyczących harmonogramu podstawowego.

### [Get-AzRecoveryServicesBackupStatus](Get-AzRecoveryServicesBackupStatus.md)
Sprawdza, czy jest wykonywana kopia zapasowa zasobu ARM.

### [Get-AzRecoveryServicesBackupWorkloadRecoveryConfig](Get-AzRecoveryServicesBackupWorkloadRecoveryConfig.md)
To polecenie konstruuje konfigurację odzyskiwania elementu z kopią zapasową, na przykład SQL DB. W obiekcie konfiguracji są przechowywane wszystkie szczegóły, takie jak tryb odzyskiwania, docelowe miejsca docelowe dla parametrów przywracania i aplikacji, takie jak docelowe ścieżki SQL.

### [Get-AzRecoveryServicesVault](Get-AzRecoveryServicesVault.md)
Pobiera listę magazynów usług odzyskiwania.

### [Get-AzRecoveryServicesVaultProperty](Get-AzRecoveryServicesVaultProperty.md)
Zwraca właściwości magazynu usług Recovery Services.

### [Get-AzRecoveryServicesVaultSettingsFile](Get-AzRecoveryServicesVaultSettingsFile.md)
Pobiera plik ustawień magazynu usługi Azure Site Recovery.

### [Import — AzRecoveryServicesAsrVaultSettingsFile](Import-AzRecoveryServicesAsrVaultSettingsFile.md)
Importowanie określonego pliku ustawień magazynu ASR w celu ustawienia kontekstu magazynu (kontekstu sesji programu PowerShell) dla kolejnych operacji ASR w sesji programu PowerShell. 

### [Initialize-AzRecoveryServicesBackupProtectableItem](Initialize-AzRecoveryServicesBackupProtectableItem.md)
To polecenie wyzwala odnajdowanie wszelkich niechronionych elementów danego typu obciążenia w danym kontenerze. Jeśli aplikacja bazy danych nie jest chroniona automatycznie, użyj tego polecenia, aby odkryć nowe DBs, ilekroć zostaną dodane i kontynuować ochronę.

### [Nowe — AzRecoveryServicesAsrAzureToAzureDiskReplicationConfig](New-AzRecoveryServicesAsrAzureToAzureDiskReplicationConfig.md)
Tworzy obiekt mapowania dysku dla dysków maszyny wirtualnej platformy Azure, które mają być replikowane.

### [Nowe — AzRecoveryServicesAsrFabric](New-AzRecoveryServicesAsrFabric.md)
Tworzy sieć szkieletową odzyskiwania witryny Azure.

### [Nowe — AzRecoveryServicesAsrInMageAzureV2DiskInput](New-AzRecoveryServicesAsrInMageAzureV2DiskInput.md)
Tworzy obiekt mapowania dysku na potrzeby replikowania dysków maszyny wirtualnej vMWare.

### [Nowe — AzRecoveryServicesAsrNetworkMapping](New-AzRecoveryServicesAsrNetworkMapping.md)
Tworzy mapowanie sieci funkcji ASR między dwiema sieciami.

### [Nowe — AzRecoveryServicesAsrPolicy](New-AzRecoveryServicesAsrPolicy.md)
Tworzy zasady replikacji odzyskiwania witryny platformy Azure.

### [Nowe — AzRecoveryServicesAsrProtectableItem](New-AzRecoveryServicesAsrProtectableItem.md)
Dodawanie (odnajdowanie) serwera fizycznego do listy elementów, które można chronić.

### [Nowe — AzRecoveryServicesAsrProtectionContainer](New-AzRecoveryServicesAsrProtectionContainer.md)
Tworzy kontener ochrony odzyskiwania witryny platformy Azure w określonej sieci szkieletowej.

### [Nowe — AzRecoveryServicesAsrProtectionContainerMapping](New-AzRecoveryServicesAsrProtectionContainerMapping.md)
Tworzy mapowanie kontenera ochrony witryny platformy Azure, kojarząc określone zasady replikacji z określonym kontenerem ochrony przed Autoodzyskiwaniem.

### [Nowe — AzRecoveryServicesAsrRecoveryPlan](New-AzRecoveryServicesAsrRecoveryPlan.md)
Tworzy plan odzyskiwania ASR.

### [Nowe — AzRecoveryServicesAsrReplicationProtectedItem](New-AzRecoveryServicesAsrReplicationProtectedItem.md)
Umożliwia replikację elementu chronionego przed ASR przez utworzenie elementu chronionego replikacji.

### [Nowe — AzRecoveryServicesAsrStorageClassificationMapping](New-AzRecoveryServicesAsrStorageClassificationMapping.md)
Tworzy mapowanie klasyfikacji magazynu ASR w magazynie usług Recovery Services.

### [Nowe — AzRecoveryServicesAsrvCenter](New-AzRecoveryServicesAsrvCenter.md)
Dodaje serwer vCenter do odnajdowania elementów podlegających ochronie.

### [Nowe — AzRecoveryServicesBackupProtectionPolicy](New-AzRecoveryServicesBackupProtectionPolicy.md)
Tworzy zasady ochrony kopii zapasowych.

### [Nowe — AzRecoveryServicesVault](New-AzRecoveryServicesVault.md)
Tworzy nowy magazyn usług Recovery Services.

### [Rejestr — AzRecoveryServicesBackupContainer](Register-AzRecoveryServicesBackupContainer.md)
Polecenie cmdlet **register-AzRecoveryServicesBackupContainer** rejestruje maszynę wirtualną platformy Azure dla AzureWorkloads przy użyciu określonego elementu obciążeniatype.

### [Remove-AzRecoveryServicesAsrFabric](Remove-AzRecoveryServicesAsrFabric.md)
Usuwa określoną sieć szkieletową odzyskiwania witryny platformy Azure z magazynu usług odzyskiwania.

### [Remove-AzRecoveryServicesAsrNetworkMapping](Remove-AzRecoveryServicesAsrNetworkMapping.md)
Usuwa określone mapowanie sieci funkcji ASR z magazynu usług odzyskiwania.

### [Remove-AzRecoveryServicesAsrPolicy](Remove-AzRecoveryServicesAsrPolicy.md)
Usuwa określone zasady replikacji ASR z magazynu usług odzyskiwania.

### [Remove-AzRecoveryServicesAsrProtectionContainer](Remove-AzRecoveryServicesAsrProtectionContainer.md)
Usuwa określony kontener ochrony z tkaniny.

### [Remove-AzRecoveryServicesAsrProtectionContainerMapping](Remove-AzRecoveryServicesAsrProtectionContainerMapping.md)
Usuwa określone mapowanie kontenera ochrony usługi Azure Site Recovery.

### [Remove-AzRecoveryServicesAsrRecoveryPlan](Remove-AzRecoveryServicesAsrRecoveryPlan.md)
Usuwa określony plan odzyskiwania ASR z magazynu usług Recovery Services.

### [Remove-AzRecoveryServicesAsrReplicationProtectedItem](Remove-AzRecoveryServicesAsrReplicationProtectedItem.md)
Zatrzymuje/wyłącza replikację elementu chronionego replikacji odzyskiwania witryny platformy Azure.

### [Remove-AzRecoveryServicesAsrServicesProvider](Remove-AzRecoveryServicesAsrServicesProvider.md)
Usuwa/wyrejestrowuje określonego dostawcę usług odzyskiwania usługi Azure Site Recovery z magazynu usług odzyskiwania.

### [Remove-AzRecoveryServicesAsrStorageClassificationMapping](Remove-AzRecoveryServicesAsrStorageClassificationMapping.md)
Usuwa określone mapowanie klasyfikacji magazynu ASR.

### [Remove-AzRecoveryServicesAsrvCenter](Remove-AzRecoveryServicesAsrvCenter.md)
Umożliwia usunięcie serwera vCenter z sieci szkieletowej ASR i zatrzymanie wykrywania maszyn wirtualnych z serwera vCenter.

### [Remove-AzRecoveryServicesBackupProtectionPolicy](Remove-AzRecoveryServicesBackupProtectionPolicy.md)
Usuwa zasady ochrony kopii zapasowej z magazynu.

### [Remove-AzRecoveryServicesVault](Remove-AzRecoveryServicesVault.md)
Usuwa magazyn usług Recovery Services.

### [Restart-AzRecoveryServicesAsrJob](Restart-AzRecoveryServicesAsrJob.md)
Ponowne uruchamianie zadania odzyskiwania witryny Azure.

### [Restore-AzRecoveryServicesBackupItem](Restore-AzRecoveryServicesBackupItem.md)
Przywraca dane i konfigurację elementu kopii zapasowej do punktu odzyskiwania.

### [Życiorys — AzRecoveryServicesAsrJob](Resume-AzRecoveryServicesAsrJob.md)
Wznawia zawieszone zadanie odzyskiwania witryny platformy Azure.

### [Set-AzRecoveryServicesAsrAlertSetting](Set-AzRecoveryServicesAsrAlertSetting.md)
Skonfiguruj ustawienia powiadomień usługi Azure Site Recovery (powiadomienie e-mail) dla magazynu.

### [Set-AzRecoveryServicesAsrReplicationProtectedItem](Set-AzRecoveryServicesAsrReplicationProtectedItem.md)
Ustawia właściwości odzyskiwania, takie jak Sieć docelowa i rozmiar maszyny wirtualnej dla określonego elementu chronionego replikacji.

### [Set-AzRecoveryServicesAsrVaultContext](Set-AzRecoveryServicesAsrVaultContext.md)
Umożliwia ustawienie kontekstu magazynu usług odzyskiwania, który ma być stosowany do kolejnych operacji odzyskiwania witryny platformy Azure w bieżącej sesji programu PowerShell.

### [Set-AzRecoveryServicesBackupProperty](Set-AzRecoveryServicesBackupProperty.md)
Ustawia właściwości zarządzania kopią zapasową.

### [Set-AzRecoveryServicesBackupProtectionPolicy](Set-AzRecoveryServicesBackupProtectionPolicy.md)
Modyfikuje zasady ochrony kopii zapasowej.

### [Set-AzRecoveryServicesVaultContext](Set-AzRecoveryServicesVaultContext.md)
Ustawia kontekst magazynu.

### [Set-AzRecoveryServicesVaultProperty](Set-AzRecoveryServicesVaultProperty.md)
Aktualizuje właściwości magazynu.

### [Start — AzRecoveryServicesAsrApplyRecoveryPoint](Start-AzRecoveryServicesAsrApplyRecoveryPoint.md)
Zmienia punkt odzyskiwania elementu chronionego przed przejściem do trybu failover.

### [Start — AzRecoveryServicesAsrCommitFailoverJob](Start-AzRecoveryServicesAsrCommitFailoverJob.md)
Rozpoczyna akcję zatwierdzania trybu failover dla obiektu odzyskiwania witryny.

### [Start — AzRecoveryServicesAsrPlannedFailoverJob](Start-AzRecoveryServicesAsrPlannedFailoverJob.md)
Rozpoczynanie planowanej pracy w trybie failover.

### [Start — AzRecoveryServicesAsrResynchronizeReplicationJob](Start-AzRecoveryServicesAsrResynchronizeReplicationJob.md)
Rozpoczyna ponowną synchronizację replikacji.

### [Start — AzRecoveryServicesAsrSwitchProcessServerJob](Start-AzRecoveryServicesAsrSwitchProcessServerJob.md)
Przełączanie replikacji z jednego serwera przetwarzania do innego w celu równoważenia obciążenia.

### [Start — AzRecoveryServicesAsrTestFailoverCleanupJob](Start-AzRecoveryServicesAsrTestFailoverCleanupJob.md)
Rozpoczyna operację sprawdzania oczyszczania w trybie failover.

### [Start — AzRecoveryServicesAsrTestFailoverJob](Start-AzRecoveryServicesAsrTestFailoverJob.md)
Rozpoczyna operację testowej pracy w trybie failover.

### [Start — AzRecoveryServicesAsrUnplannedFailoverJob](Start-AzRecoveryServicesAsrUnplannedFailoverJob.md)
Rozpoczyna nieplanowaną operację pracy awaryjnej.

### [Zatrzymaj — AzRecoveryServicesAsrJob](Stop-AzRecoveryServicesAsrJob.md)
Zatrzymuje zadanie odzyskiwania witryny Azure.

### [Zatrzymaj — AzRecoveryServicesBackupJob](Stop-AzRecoveryServicesBackupJob.md)
Anuluje uruchomione zadanie.

### [Cofanie — AzRecoveryServicesBackupItemDeletion](Undo-AzRecoveryServicesBackupItemDeletion.md)
Jeśli element kopii zapasowej zostanie usunięty i zaprezentować go w stanie nieusuniętym, to polecenie przełączy element z powrotem do stanu, w którym dane są zawsze zachowywane. 

### [Wyrejestrowanie — AzRecoveryServicesBackupContainer](Unregister-AzRecoveryServicesBackupContainer.md)
Wyrejestrowuje serwer z systemem Windows lub inny kontener z magazynu.

### [Wyrejestrowanie — AzRecoveryServicesBackupManagementServer](Unregister-AzRecoveryServicesBackupManagementServer.md)
Wyrejestrowuje serwer SCDPM lub zapasowy serwer z magazynu.

### [Update-AzRecoveryServicesAsrMobilityService](Update-AzRecoveryServicesAsrMobilityService.md)
Aktualizacje agenta usługi mobilności push na chronione komputery.

### [Update-AzRecoveryServicesAsrNetworkMapping](Update-AzRecoveryServicesAsrNetworkMapping.md)
Umożliwia zaktualizowanie określonego mapowania sieci usługi Azure Site Recovery.

### [Update-AzRecoveryServicesAsrPolicy](Update-AzRecoveryServicesAsrPolicy.md)
Aktualizuje zasady replikacji odzyskiwania witryny platformy Azure.

### [Update-AzRecoveryServicesAsrProtectionContainerMapping](Update-AzRecoveryServicesAsrProtectionContainerMapping.md)
Zaktualizuj mapowanie kontenera ochrony usługi Azure Site Recovery.

### [Update-AzRecoveryServicesAsrProtectionDirection](Update-AzRecoveryServicesAsrProtectionDirection.md)
Aktualizuje kierunek replikacji określonego elementu chronionego replikacji lub planu odzyskiwania. Służy do ponownego zabezpieczania/odtwarzania kopii zapasowej w replikowanym elemencie lub planie odzyskiwania.

### [Update-AzRecoveryServicesAsrRecoveryPlan](Update-AzRecoveryServicesAsrRecoveryPlan.md)
Aktualizuje zawartość planu odzyskiwania witryny platformy Azure.

### [Update-AzRecoveryServicesAsrServicesProvider](Update-AzRecoveryServicesAsrServicesProvider.md)
Odświeża (serwer odświeżania) informacje otrzymane od dostawcy usług Azure Site Recovery.

### [Update-AzRecoveryServicesAsrvCenter](Update-AzRecoveryServicesAsrvCenter.md)
Aktualizowanie szczegółów odnajdowania dla zarejestrowanego vCenter.

### [Wait-AzRecoveryServicesBackupJob](Wait-AzRecoveryServicesBackupJob.md)
Oczekuje na zakończenie zadania kopii zapasowej.

