...
To polecenie tworzy szablon obrazu maszyny wirtualnej.
...
Maksymalny czas oczekiwania na poczekanie na utworzenie szablonu obrazu.
...
### -Dostosowywanie
Określa właściwości opisujące kroki dostosowywania obrazu, takie jak źródło obrazu itd. Aby skonstruować, zobacz sekcję notatki na temat dostosowywania właściwości i tworzenie tabeli skrótów.
...
### -DefaultProfile
Region HideParameter poświadczenia, konto, dzierżawca i abonament używane do komunikacji z usługą Azure.
...
### -Dystrybuuj
Miejsce docelowe dystrybucji, w którym ma się znaleźć wyjście obrazu.
Aby skonstruować, zobacz sekcję notatki do ROZPOWSZECHNIAnia właściwości i tworzenie tabeli skrótów.
...
Pełne informacje na temat stanu ostatniego uruchomienia.
...
Pełny komunikat o błędzie dotyczący niepowodzenia obsługi.
...
Aby skonstruować, zobacz sekcję notatki dla właściwości źródła i Utwórz tablicę skrótów.
...
Identyfikator tożsamości przypisanej do użytkownika.
...
### -VnetConfigSubnetId
Identyfikator zasobu już istniejącej podsieci.
...
### Microsoft. Azure. PowerShell. polecenia. ImageBuilder. models. Api20200214. IImageTemplate
...
Dostosowywanie <IImageTemplateCustomizer [] >: określa właściwości używane do opisania kroków dostosowywania obrazu, takich jak źródło obrazów itp.
  - `Type <String>`: Typ narzędzia do dostosowywania, którego chcesz użyć na obrazie. Na przykład "Shell" może być konfiguratorem powłoki
  - `[Name <String>]`: Przyjazna nazwa, która umożliwia określenie kontekstu działania tego kroku dostosowania

ROZPOWSZECHNIj <IImageTemplateDistributor [] >: elementy docelowe dystrybucji, w których ma się znaleźć wyjście obrazu.
  - `RunOutputName <String>`: Nazwa, która ma być używana dla skojarzonych RunOutput.
  - `Type <String>`: Typ dystrybucji.
...
Źródło <IImageTemplateSource> : zawiera opis źródła obrazu maszyny wirtualnej umożliwiającego tworzenie, dostosowywanie i dystrybucję.
  - `Type <String>`: Określa typ obrazu źródłowego, od którego należy zacząć.
