---
Module Name: Az.Synapse
Module Guid: 89eceb4f-9916-4ec5-8499-d5cca97a9cae
Download Help Link: https://docs.microsoft.com/en-us/powershell/module/az.synapse
Help Version: 0.0.1.0
Locale: en-US
content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/Synapse/Synapse/help/Az.Synapse.md
original_content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/Synapse/Synapse/help/Az.Synapse.md
ms.openlocfilehash: 0a991dfc2f1584f478f001648a2a24b616937884
ms.sourcegitcommit: 04221336bc9eed46c05ed1e828a6811534d4b4ab
ms.translationtype: MT
ms.contentlocale: pl-PL
ms.lasthandoff: 12/08/2020
ms.locfileid: "98325486"
---
# AZ. Synapse, moduł
## Opis
W tematach w tej sekcji opisano polecenia cmdlet usługi Azure PowerShell dla usługi Azure Synapse Analytics.

## AZ. Synapse polecenia cmdlet
### [Dodaj-AzSynapseTriggerSubscription](Add-AzSynapseTriggerSubscription.md)
Zasubskrybuj wyzwalacz zdarzeń na zdarzenia usługi zewnętrznej.

### [Eksportowanie — AzSynapseNotebook](Export-AzSynapseNotebook.md)
Eksportuje notbooks.

### [Get-AzSynapseActivityRun](Get-AzSynapseActivityRun.md)
Pobiera informacje o działaniach dotyczących uruchamiania rurociągu.

### [Get-AzSynapseDataFlow](Get-AzSynapseDataFlow.md)
Pobiera informacje o przepływach danych w obszarze roboczym.

### [Get-AzSynapseDataset](Get-AzSynapseDataset.md)
Pobiera informacje o zestawach danych w obszarze roboczym.

### [Get-AzSynapseFirewallRule](Get-AzSynapseFirewallRule.md)
Pobiera regułę zapory Synapse Analytics.

### [Get-AzSynapseIntegrationRuntime](Get-AzSynapseIntegrationRuntime.md)
Pobiera informacje o zasobach w czasie wykonywania integracji.

### [Get-AzSynapseIntegrationRuntimeKey](Get-AzSynapseIntegrationRuntimeKey.md)
Pobiera klucze dla środowiska uruchomieniowego integracji.

### [Get-AzSynapseIntegrationRuntimeMetric](Get-AzSynapseIntegrationRuntimeMetric.md)
Pobiera dane metryczne dla środowiska wykonawczego integracji. 

### [Get-AzSynapseIntegrationRuntimeNode](Get-AzSynapseIntegrationRuntimeNode.md)
Pobiera informacje o węźle środowisko uruchomieniowe integracji.

### [Get-AzSynapseLinkedService](Get-AzSynapseLinkedService.md)
Pobiera informacje o usługach połączonych w obszarze roboczym.

### [Get-AzSynapseNotebook](Get-AzSynapseNotebook.md)
Pobiera informacje o notesach w obszarze roboczym.

### [Get-AzSynapsePipeline](Get-AzSynapsePipeline.md)
Pobiera informacje o rurociągach w obszarze roboczym.

### [Get-AzSynapsePipelineRun](Get-AzSynapsePipelineRun.md)
Pobiera informacje o przebiegach potokowych.

### [Get-AzSynapseRoleAssignment](Get-AzSynapseRoleAssignment.md)
Pobiera przypisanie roli Synapse Analytics.

### [Get-AzSynapseRoleDefinition](Get-AzSynapseRoleDefinition.md)
Pobiera definicję roli Synapse Analytics.

### [Get-AzSynapseSparkJob](Get-AzSynapseSparkJob.md)
Pobiera zadanie Synapse analiza Spark.

### [Get-AzSynapseSparkPool](Get-AzSynapseSparkPool.md)
Pobiera pulę Synapse analizy Spark.

### [Get-AzSynapseSparkSession](Get-AzSynapseSparkSession.md)
Pobiera sesję Synapse Analytics Spark.

### [Get-AzSynapseSparkStatement](Get-AzSynapseSparkStatement.md)
Pobiera instrukcję Synapse Analytics Spark.

### [Get-AzSynapseSqlActiveDirectoryAdministrator](Get-AzSynapseSqlActiveDirectoryAdministrator.md)
Pobiera informacje o administratorze usługi Azure AD dla obszaru roboczego analizy Synapse.

### [Get-AzSynapseSqlAdvancedThreatProtectionSetting](Get-AzSynapseSqlAdvancedThreatProtectionSetting.md)
Pobiera zaawansowane ustawienia ochrony przed zagrożeniami dla obszaru roboczego.

### [Get-AzSynapseSqlAuditSetting](Get-AzSynapseSqlAuditSetting.md)
Pobiera ustawienia inspekcji obszaru roboczego analizy usługi Azure Synapse.

### [Get-AzSynapseSqlDatabase](Get-AzSynapseSqlDatabase.md)
Ta funkcja jest w ograniczonym podglądzie, początkowo dostępna tylko w przypadku niektórych abonamentów. Pobiera bazę danych SQL analizy Synapse.

### [Get-AzSynapseSqlPool](Get-AzSynapseSqlPool.md)
Pobiera pulę SQL Synapse Analytics.

### [Get-AzSynapseSqlPoolAuditSetting](Get-AzSynapseSqlPoolAuditSetting.md)
Pobiera ustawienia inspekcji puli SQL usługi Azure Synapse Analytics.

### [Get-AzSynapseSqlPoolRestorePoint](Get-AzSynapseSqlPoolRestorePoint.md)
Pobiera różne punkty przywracania, z których można przywrócić pulę SQL analizy Synapse.

### [Get-AzSynapseSqlTransparentDataEncryptionProtector](Get-AzSynapseSqlTransparentDataEncryptionProtector.md)
Pobiera funkcję ochrony przezroczystego szyfrowania danych (TDE).

### [Get-AzSynapseSqlVulnerabilityAssessmentSetting](Get-AzSynapseSqlVulnerabilityAssessmentSetting.md)
Pobiera ustawienia oceny luki w obszarze roboczym.

### [Get-AzSynapseTrigger](Get-AzSynapseTrigger.md)
Pobiera informacje o wyzwalaczach w obszarze roboczym.

### [Get-AzSynapseTriggerRun](Get-AzSynapseTriggerRun.md)
Zwraca informacje o uruchomionych wyzwalaczach.

### [Get-AzSynapseTriggerSubscriptionStatus](Get-AzSynapseTriggerSubscriptionStatus.md)
Uzyskaj stan subskrypcji wyzwalacza zdarzeń na określone zdarzenia usługi zewnętrznej.

### [Get-AzSynapseWorkspace](Get-AzSynapseWorkspace.md)
Pobiera obszar roboczy analizy Synapse.

### [Invoke-AzSynapseIntegrationRuntimeUpgrade](Invoke-AzSynapseIntegrationRuntimeUpgrade.md)
Uaktualnia środowisko uruchomieniowe integracji na samym własnym poziomie.

### [Invoke-AzSynapsePipeline](Invoke-AzSynapsePipeline.md)
Wywołuje potok w celu uruchomienia go.

### [Invoke-AzSynapseSparkStatement](Invoke-AzSynapseSparkStatement.md)
Wywołuje instrukcję Synapse Analytics Spark.

### [Nowe — AzSynapseFirewallRule](New-AzSynapseFirewallRule.md)
Umożliwia utworzenie reguły zapory Synapse Analytics.

### [Nowe — AzSynapseIntegrationRuntimeKey](New-AzSynapseIntegrationRuntimeKey.md)
Ponowne generowanie klucza środowiska uruchomieniowego w ramach samodzielnej integracji.

### [Nowe — AzSynapseRoleAssignment](New-AzSynapseRoleAssignment.md)
Tworzy przypisanie roli analiza Synapse.

### [Nowe — AzSynapseSparkPool](New-AzSynapseSparkPool.md)
Tworzy pulę Synapse analizy Spark.

### [Nowe — AzSynapseSqlDatabase](New-AzSynapseSqlDatabase.md)
Ta funkcja jest w ograniczonym podglądzie, początkowo dostępna tylko w przypadku niektórych abonamentów. Tworzy bazę danych SQL analizy Synapse.

### [Nowe — AzSynapseSqlPool](New-AzSynapseSqlPool.md)
Tworzy pulę SQL analizy Synapse.

### [Nowe — AzSynapseSqlPoolRestorePoint](New-AzSynapseSqlPoolRestorePoint.md)
Tworzy nowy punkt przywracania w puli SQL usługi Azure Synapse Analytics.

### [Nowe — AzSynapseWorkspace](New-AzSynapseWorkspace.md)
Umożliwia utworzenie obszaru roboczego analizy Synapse.

### [Remove-AzSynapseDataFlow](Remove-AzSynapseDataFlow.md)
Usuwa przepływ danych z obszaru roboczego.

### [Remove-AzSynapseDataset](Remove-AzSynapseDataset.md)
Umożliwia usunięcie zestawu danych z obszaru roboczego.

### [Remove-AzSynapseFirewallRule](Remove-AzSynapseFirewallRule.md)
Usuwa regułę zapory Synapse Analytics.

### [Remove-AzSynapseIntegrationRuntime](Remove-AzSynapseIntegrationRuntime.md)
Usuwa środowisko uruchomieniowe integracji.

### [Remove-AzSynapseIntegrationRuntimeNode](Remove-AzSynapseIntegrationRuntimeNode.md)
Usuwanie węzła o podanej nazwie w środowisku wykonawczym integracji.

### [Remove-AzSynapseLinkedService](Remove-AzSynapseLinkedService.md)
Usuwa połączoną usługę z obszaru roboczego.

### [Remove-AzSynapseNotebook](Remove-AzSynapseNotebook.md)
Usuwanie notesu z obszaru roboczego.

### [Remove-AzSynapsePipeline](Remove-AzSynapsePipeline.md)
Usuwa potok z obszaru roboczego.

### [Remove-AzSynapseRoleAssignment](Remove-AzSynapseRoleAssignment.md)
Usuwa przypisanie roli analiza Synapse.

### [Remove-AzSynapseSparkPool](Remove-AzSynapseSparkPool.md)
Usuwa pulę Synapse analizy Spark.

### [Remove-AzSynapseSqlActiveDirectoryAdministrator](Remove-AzSynapseSqlActiveDirectoryAdministrator.md)
Umożliwia usunięcie administratora usługi Azure AD dla obszaru roboczego analizy Synapse.

### [Remove-AzSynapseSqlDatabase](Remove-AzSynapseSqlDatabase.md)
Ta funkcja jest w ograniczonym podglądzie, początkowo dostępna tylko w przypadku niektórych abonamentów. Usuwa bazę danych SQL Synapse Analytics.

### [Remove-AzSynapseSqlPool](Remove-AzSynapseSqlPool.md)
Usuwa pulę SQL Synapse Analytics.

### [Remove-AzSynapseTrigger](Remove-AzSynapseTrigger.md)
Usuwa wyzwalacz z obszaru roboczego.

### [Remove-AzSynapseTriggerSubscription](Remove-AzSynapseTriggerSubscription.md)
Anulowanie subskrypcji wyzwalacza zdarzeń na zdarzenia usługi zewnętrznej.

### [Remove-AzSynapseWorkspace](Remove-AzSynapseWorkspace.md)
Usuwa obszar roboczy analizy Synapse.

### [Resetowanie — AzSynapseSparkSessionTimeout](Reset-AzSynapseSparkSessionTimeout.md)
Resetuje limit czasu sesji usługi Synapse Analytics Spark.

### [Resetowanie — AzSynapseSqlAdvancedThreatProtectionSetting](Reset-AzSynapseSqlAdvancedThreatProtectionSetting.md)
Usuwa zaawansowane ustawienia ochrony przed zagrożeniami z obszaru roboczego.

### [Resetowanie — AzSynapseSqlAuditSetting](Reset-AzSynapseSqlAuditSetting.md)
Usuwa ustawienia inspekcji obszaru roboczego analizy usługi Azure Synapse.

### [Resetowanie — AzSynapseSqlPoolAuditSetting](Reset-AzSynapseSqlPoolAuditSetting.md)
Usuwa ustawienia inspekcji puli SQL usługi Azure Synapse Analytics.

### [Resetowanie — AzSynapseSqlVulnerabilityAssessmentSetting](Reset-AzSynapseSqlVulnerabilityAssessmentSetting.md)
Czyści ustawienia oceny usterki w obszarze roboczym.

### [Restore-AzSynapseSqlPool](Restore-AzSynapseSqlPool.md)
Przywraca pulę SQL Synapse Analytics.

### [Życiorys — AzSynapseSqlPool](Resume-AzSynapseSqlPool.md)
Wznawia pulę Synapse analizy SQL.

### [Set-AzSynapseDataFlow](Set-AzSynapseDataFlow.md)
Tworzenie lub aktualizowanie przepływu danych w obszarze roboczym.

### [Set-AzSynapseDataset](Set-AzSynapseDataset.md)
Tworzenie lub aktualizowanie zestawu danych w obszarze roboczym.

### [Set-AzSynapseIntegrationRuntime](Set-AzSynapseIntegrationRuntime.md)
Aktualizuje środowisko uruchomieniowe integracji.

### [Set-AzSynapseLinkedService](Set-AzSynapseLinkedService.md)
Łączy magazyn danych lub usługę w chmurze z obszarem roboczym.

### [Set-AzSynapseNotebook](Set-AzSynapseNotebook.md)
Umożliwia utworzenie lub zaktualizowanie notesu w obszarze roboczym.

### [Set-AzSynapsePipeline](Set-AzSynapsePipeline.md)
Tworzy potok w obszarze roboczym.

### [Set-AzSynapseSqlActiveDirectoryAdministrator](Set-AzSynapseSqlActiveDirectoryAdministrator.md)
Inicjuje obsługę administracyjną administratora usługi Azure AD dla puli Synapse analiz SQL.

### [Set-AzSynapseSqlAuditSetting](Set-AzSynapseSqlAuditSetting.md)
Zmienia ustawienia inspekcji obszaru roboczego analizy usługi Azure Synapse.

### [Set-AzSynapseSqlPoolAuditSetting](Set-AzSynapseSqlPoolAuditSetting.md)
Zmienia ustawienia inspekcji puli SQL usługi Azure Synapse Analytics.

### [Set-AzSynapseSqlTransparentDataEncryptionProtector](Set-AzSynapseSqlTransparentDataEncryptionProtector.md)
Ustawia funkcję ochrony przezroczystego szyfrowania danych (TDE) dla obszaru roboczego analizy Synapse.

### [Set-AzSynapseTrigger](Set-AzSynapseTrigger.md)
Tworzy wyzwalacz w obszarze roboczym.

### [Start — AzSynapseSparkSession](Start-AzSynapseSparkSession.md)
Rozpoczyna sesję Synapse Analytics Spark.

### [Start — AzSynapseTrigger](Start-AzSynapseTrigger.md)
Uruchamianie wyzwalacza w obszarze roboczym.

### [Zatrzymaj — AzSynapsePipelineRun](Stop-AzSynapsePipelineRun.md)
Zatrzymuje uruchamianie rurociągu w obszarze roboczym.

### [Zatrzymaj — AzSynapseSparkJob](Stop-AzSynapseSparkJob.md)
Anuluje zadanie Synapse analiza Spark.

### [Zatrzymaj — AzSynapseSparkSession](Stop-AzSynapseSparkSession.md)
Zatrzymuje sesję Synapse Analytics Spark.

### [Zatrzymaj — AzSynapseSparkStatement](Stop-AzSynapseSparkStatement.md)
Anuluje instrukcję Synapse Analytics Spark.

### [Zatrzymaj — AzSynapseTrigger](Stop-AzSynapseTrigger.md)
Zatrzymuje wyzwalacz w obszarze roboczym.

### [Prześlij — AzSynapseSparkJob](Submit-AzSynapseSparkJob.md)
Przesyła zadanie Synapse analiza Spark.

### [Suspend — AzSynapseSqlPool](Suspend-AzSynapseSqlPool.md)
Wstrzymuje pulę Synapse analizy SQL.

### [Sync-AzSynapseIntegrationRuntimeCredential](Sync-AzSynapseIntegrationRuntimeCredential.md)
Synchronizuje poświadczenia między węzłami w środowisku wykonawczym integracji.

### [Test — AzSynapseSparkPool](Test-AzSynapseSparkPool.md)
Sprawdza, czy istnieje Pula usługi Synapse Analytics Spark.

### [Test — AzSynapseSqlDatabase](Test-AzSynapseSqlDatabase.md)
Ta funkcja jest w ograniczonym podglądzie, początkowo dostępna tylko w przypadku niektórych abonamentów. Sprawdza, czy istnieje baza danych SQL analizy Synapse.

### [Test — AzSynapseSqlPool](Test-AzSynapseSqlPool.md)
Sprawdza, czy istnieje Pula SQL analizy Synapse.

### [Test — AzSynapseWorkspace](Test-AzSynapseWorkspace.md)
Sprawdza, czy istnieje obszar roboczy analizy Synapse.

### [Update-AzSynapseFirewallRule](Update-AzSynapseFirewallRule.md)
Umożliwia zaktualizowanie reguły zapory Synapse Analytics.

### [Update-AzSynapseIntegrationRuntime](Update-AzSynapseIntegrationRuntime.md)
Aktualizuje środowisko uruchomieniowe integracji.

### [Update-AzSynapseIntegrationRuntimeNode](Update-AzSynapseIntegrationRuntimeNode.md)
Umożliwia zaktualizowanie węzła środowiska uruchomieniowego integracji na samym własnym poziomie.

### [Update-AzSynapseSparkPool](Update-AzSynapseSparkPool.md)
Umożliwia zaktualizowanie puli Synapse analiz Spark.

### [Update-AzSynapseSqlAdvancedThreatProtectionSetting](Update-AzSynapseSqlAdvancedThreatProtectionSetting.md)
Aktualizuje zaawansowane ustawienia ochrony przed zagrożeniami w obszarze roboczym.

### [Update-AzSynapseSqlDatabase](Update-AzSynapseSqlDatabase.md)
Ta funkcja jest w ograniczonym podglądzie, początkowo dostępna tylko w przypadku niektórych abonamentów. Aktualizuje bazę danych SQL Synapse Analytics.

### [Update-AzSynapseSqlPool](Update-AzSynapseSqlPool.md)
Aktualizuje pulę SQL Synapse Analytics.

### [Update-AzSynapseSqlVulnerabilityAssessmentSetting](Update-AzSynapseSqlVulnerabilityAssessmentSetting.md)
Umożliwia zaktualizowanie ustawień oceny luk w obszarze roboczym.

### [Update-AzSynapseWorkspace](Update-AzSynapseWorkspace.md)
Umożliwia zaktualizowanie obszaru roboczego analizy Synapse.

### [Wait-AzSynapseSparkJob](Wait-AzSynapseSparkJob.md)
Oczekuje na ukończenie zadania usługi Synapse Analytics Spark.

