...
## STRESZCZENIe
Ponowne generowanie klucza dostępu CommunicationService.
PrimaryKey i SecondaryKey nie mogą być ponownie generowane w tym samym czasie.
...
### Przykład 1. regeneruje klucz podstawowy za pomocą IRegenerateKeyParameters Hashtable
```powershell
...
Unieważnia poprzedni klucz podstawowy, ponownie Wygeneruj nowy i zwróci go.

### Przykład 2: generuje ponownie klucz pomocniczy przy użyciu klucza KeyType.
```powershell
...
Unieważnia poprzedni klucz pomocniczy, ponownie Wygeneruj nowy i zwróci go.
...
### -KeyType
Właściwość keyType do ponownego wygenerowania.
Musi mieć wartość "podstawowy" lub "pomocniczy" (bez uwzględniania wielkości liter).
...
### -Parameter
Parametry zawiera opis żądania ponownego wygenerowania kluczy dostępu do skonstruowania, zobacz sekcję notatki dla właściwości parametrów i Utwórz tablicę skrótów.
...
### Microsoft. Azure. PowerShell. cmdlet. Communication. models. Api20200820Preview. IRegenerateKeyParameters
...
PARAMETR <IRegenerateKeyParameters> : parametry zawiera opis żądania ponownego generowania kluczy dostępu
  - `[KeyType <KeyType?>]`: Właściwość keyType, która ma zostać wygenerowana ponownie. Musi mieć wartość "podstawowy" lub "pomocniczy" (bez uwzględniania wielkości liter).
