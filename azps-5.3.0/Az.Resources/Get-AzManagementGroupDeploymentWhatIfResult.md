...
## Opis
Polecenie cmdlet **Get-AzManagementGroupDeploymentWhatIfResult** pobiera szablon ARM What-If wyniku wdrożenia szablonu w określonym zakresie grupy zarządzania. Zwraca listę zmian wskazujących, które zasoby zostaną zaktualizowane po zastosowaniu wdrożenia bez wprowadzania zmian w rzeczywistych zasobach. Aby określić format wyniku zwracanego, użyj parametru *ResultFormat* .

## Przykłady

### Przykład 1: uzyskiwanie What-If wyniku w zakresie grupy zarządzania
```powershell
...
To polecenie pobiera What-If wynik w zakresie grupy zarządzania przy użyciu pliku szablonu niestandardowego oraz pliku parametrów na dysku.
W poleceniu użyto parametru *Location* w celu określenia miejsca, w którym mają być przechowywane dane wdrożenia.
W poleceniu użyto parametru *ManagementGroupId* w celu określenia grupy zarządzania, w której szablon zostanie wdrożony.
...
### Przykład 2: uzyskiwanie wyników What-If w zakresie grupy zarządzania przy użyciu ResourceIdOnly
```powershell
PS C:\> Get-AzManagementGroupDeploymentWhatIfResult `
    -DeploymentName "deploy-01" `
    -Location "West US" `
...
### -ExcludeChangeType
Lista rozdzielanych przecinkami typów zmian zasobów, które mają być wykluczone z wyników What-If.
...
Jeśli ten parametr nie jest określony, domyślnie jest określana nazwa pliku szablonu, gdy jest dostarczany plik szablonu; Domyślnie jest to bieżąca godzina, w której jest dostarczany obiekt szablonu, na przykład "20131223140835".
