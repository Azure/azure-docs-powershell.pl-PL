...
## Opis
Polecenie cmdlet **New-AzVMConfig** tworzy konfigurowalny lokalny obiekt maszyny wirtualnej dla platformy Azure.
Innych poleceń cmdlet można użyć w celu skonfigurowania obiektu maszyny wirtualnej, takiego jak Set-AzVMOperatingSystem, Set-AzVMSourceImage, Add-AzVMNetworkInterface i Set-AzVMOSDisk.
...
Obiekt zestawu dostępności zawiera Właściwość ID. <br>
Maszyny wirtualne określone w tym samym zbiorze dostępności są przydzielane do różnych węzłów, aby zmaksymalizować dostępność. <br>
...
Aby uzyskać więcej informacji na temat planowanej konserwacji platformy Azure, zobacz [Planowana konserwacja maszyn wirtualnych na platformie Azure](https://docs.microsoft.com/azure/virtual-machines/virtual-machines-windows-planned-maintenance?toc=%2fazure%2fvirtual-machines%2fwindows%2ftoc.json) <br>
Obecnie maszyna wirtualna może być dodana do zestawu dostępności tylko w czasie tworzenia. Zestaw dostępności, do którego jest dodawana maszyna wirtualna, powinien znajdować się w tej samej grupie zasobów co zasób zestawu dostępności. Istniejącej maszyny wirtualnej nie można dodać do zestawu dostępności. <br>
Ta właściwość nie może istnieć wraz z właściwościami innymi niż null. virtualMachineScaleSet, odwołanie.
...
### -EnableUltraSSD
Umożliwia posiadanie jednego lub większej liczby zarządzanych dysków z danymi przy użyciu UltraSSD_LRS typu konta magazynu na maszynie wirtualnej.
...
Możliwe wartości dla systemu Windows Server są następujące:
- Windows_Client
- Windows_Server możliwych wartości dla systemu operacyjnego serwera Linux są następujące: 
- RHEL_BYOS (dla RHEL) 
- SLES_BYOS (dla SUSE) 
...
### -Zone
Określa listę stref dostępności dla maszyny wirtualnej. Dozwolone wartości są zależne od możliwości obszaru tego regionu. Dozwolone wartości będą zwykle 1, 2, 3.
