---
Module Name: Az.Storage
Module Guid: da67eaa7-4cb1-4bfa-a194-8bf3faae8ac5
Download Help Link: https://docs.microsoft.com/en-us/powershell/module/az.storage
Help Version: 4.2.3.0
Locale: en-US
content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/Storage/Storage.Management/help/Az.Storage.md
original_content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/Storage/Storage.Management/help/Az.Storage.md
ms.openlocfilehash: f1b3e5b27aa89c61bbe2c15c4c35455540f6d255
ms.sourcegitcommit: 68451baa389791703e666d95469602c5652609ee
ms.translationtype: MT
ms.contentlocale: pl-PL
ms.lasthandoff: 01/05/2021
ms.locfileid: "98374185"
---
# AZ.
## Opis
Ten temat zawiera tematy pomocy dotyczące apletów poleceń usługi Azure Storage Management.

## AZ. w magazynie polecenia cmdlet
### [Dodaj-AzRmStorageContainerLegalHold](Add-AzRmStorageContainerLegalHold.md)
Dodaje znaczniki blokady prawnej do kontenera obiektów BLOB magazynowania

### [Dodaj-AzStorageAccountManagementPolicyAction](Add-AzStorageAccountManagementPolicyAction.md)
Umożliwia dodanie akcji do obiektu grupy akcji ManagementPolicy lub utworzenie obiektu grupy akcji ManagementPolicy z akcją. Obiekt może być wykorzystywany w nowych — AzStorageAccountManagementPolicyRule.

### [Dodaj-AzStorageAccountNetworkRule](Add-AzStorageAccountNetworkRule.md)
 Dodawanie IpRules lub VirtualNetworkRules do właściwości NetworkRule konta magazynu

### [Blisko AzStorageFileHandle](Close-AzStorageFileHandle.md)
Zamyka uchwyty plików udziału plików, katalogu plików lub pliku.

### [Disable-AzStorageBlobDeleteRetentionPolicy](Disable-AzStorageBlobDeleteRetentionPolicy.md)
Wyłącz zasadę usuwania zasad przechowywania dla usługi blob magazynu Azure.

### [Disable-AzStorageBlobRestorePolicy](Disable-AzStorageBlobRestorePolicy.md)
Wyłącza zasady przywracania obiektu BLOB na koncie magazynu.

### [Disable-AzStorageDeleteRetentionPolicy](Disable-AzStorageDeleteRetentionPolicy.md)
Wyłącz zasadę usuwania zasad przechowywania dla usługi blob magazynu Azure.

### [Disable-AzStorageStaticWebsite](Disable-AzStorageStaticWebsite.md)
Wyłącz statyczną witrynę sieci Web dla konta usługi Azure Storage.

### [Enable-AzStorageBlobDeleteRetentionPolicy](Enable-AzStorageBlobDeleteRetentionPolicy.md)
Włącz zasady dotyczące przechowywania obiektów BLOB usługi Azure Storage.

### [Enable-AzStorageBlobRestorePolicy](Enable-AzStorageBlobRestorePolicy.md)
Umożliwia włączanie zasad przywracania obiektu BLOB na koncie magazynu.

### [Enable-AzStorageDeleteRetentionPolicy](Enable-AzStorageDeleteRetentionPolicy.md)
Włącz zasady dotyczące przechowywania obiektów BLOB usługi Azure Storage.

### [Enable-AzStorageStaticWebsite](Enable-AzStorageStaticWebsite.md)
Włącz statyczną witrynę sieci Web dla konta usługi Azure Storage.

### [Get-AzDataLakeGen2ChildItem](Get-AzDataLakeGen2ChildItem.md)
Wyświetla katalogi podrzędne i pliki z katalogu lub katalogu głównego systemu plików.

### [Get-AzDataLakeGen2Item](Get-AzDataLakeGen2Item.md)
Pobiera szczegóły pliku lub katalogu w systemie plików.

### [Get-AzDataLakeGen2ItemContent](Get-AzDataLakeGen2ItemContent.md)
Pobieranie pliku.

### [Get-AzRmStorageContainer](Get-AzRmStorageContainer.md)
Pobiera lub Wyświetla kontenery blob magazynu.

### [Get-AzRmStorageContainerImmutabilityPolicy](Get-AzRmStorageContainerImmutabilityPolicy.md)
Pobiera ImmutabilityPolicy kontenerów blob magazynu.

### [Get-AzRmStorageShare](Get-AzRmStorageShare.md)
Pobiera lub wyświetla udziały plików magazynu.

### [Get-AzStorageAccount](Get-AzStorageAccount.md)
Pobiera konto magazynu.

### [Get-AzStorageAccountKey](Get-AzStorageAccountKey.md)
Pobiera klucze dostępu dla konta usługi Azure Storage.

### [Get-AzStorageAccountManagementPolicy](Get-AzStorageAccountManagementPolicy.md)
Pobiera zasady zarządzania konta usługi Azure Storage.

### [Get-AzStorageAccountNameAvailability](Get-AzStorageAccountNameAvailability.md)
Sprawdza dostępność nazwy konta magazynu.

### [Get-AzStorageAccountNetworkRuleSet](Get-AzStorageAccountNetworkRuleSet.md)
Pobieranie właściwości NetWorkRule konta magazynu

### [Get-AzStorageBlob](Get-AzStorageBlob.md)
Zawiera listę obiektów BLOB w kontenerze.

### [Get-AzStorageBlobContent](Get-AzStorageBlobContent.md)
Pobiera obiekt blob magazynu.

### [Get-AzStorageBlobCopyState](Get-AzStorageBlobCopyState.md)
Pobiera stan kopii obiektu blob magazynu platformy Azure.

### [Get-AzStorageBlobQueryResult](Get-AzStorageBlobQueryResult.md)
Stosuje prostą instrukcję SQL (Structured Query Language) do zawartości obiektu BLOB i zapisuje tylko przeszukiwany podzestaw danych w pliku lokalnym.

### [Get-AzStorageBlobServiceProperty](Get-AzStorageBlobServiceProperty.md)
Pobiera właściwości usługi dla usług blob magazynu platformy Azure.

### [Get-AzStorageContainer](Get-AzStorageContainer.md)
Zawiera listę kontenerów magazynu.

### [Get-AzStorageContainerStoredAccessPolicy](Get-AzStorageContainerStoredAccessPolicy.md)
Pobiera zasady programu Access lub zasady dotyczące kontenera magazynu platformy Azure.

### [Get-AzStorageCORSRule](Get-AzStorageCORSRule.md)
Pobiera reguły specyfikacji CORS dla typu usługi magazynu.

### [Get-AzStorageEncryptionScope](Get-AzStorageEncryptionScope.md)
Uzyskiwanie lub wyświetlanie listy zakresów szyfrowania z konta magazynu.

### [Get-AzStorageFile](Get-AzStorageFile.md)
Wyświetla listę katalogów i plików ścieżki.

### [Get-AzStorageFileContent](Get-AzStorageFileContent.md)
Umożliwia pobranie zawartości pliku.

### [Get-AzStorageFileCopyState](Get-AzStorageFileCopyState.md)
Pobiera stan operacji kopiowania.

### [Get-AzStorageFileHandle](Get-AzStorageFileHandle.md)
Zawiera listę dojść do plików udziału plików, katalogu plików lub pliku.

### [Get-AzStorageFileServiceProperty](Get-AzStorageFileServiceProperty.md)
Pobiera właściwości usługi dla usług plików magazynu platformy Azure.

### [Get-AzStorageObjectReplicationPolicy](Get-AzStorageObjectReplicationPolicy.md)
Pobiera lub wyświetla zasady replikacji obiektów dla konta magazynu.

### [Get-AzStorageQueue](Get-AzStorageQueue.md)
Zawiera listę kolejek przechowywania.

### [Get-AzStorageQueueStoredAccessPolicy](Get-AzStorageQueueStoredAccessPolicy.md)
Pobiera zasady lub zasady dostępu do kolejki magazynu platformy Azure.

### [Get-AzStorageServiceLoggingProperty](Get-AzStorageServiceLoggingProperty.md)
Pobiera właściwości rejestrowania usług Azure Storage Services.

### [Get-AzStorageServiceMetricsProperty](Get-AzStorageServiceMetricsProperty.md)
Pobiera właściwości metryk dla usługi Azure Storage.

### [Get-AzStorageServiceProperty](Get-AzStorageServiceProperty.md)
Pobiera właściwości usług Azure Storage Services.

### [Get-AzStorageShare](Get-AzStorageShare.md)
Pobiera listę udziałów plików.

### [Get-AzStorageShareStoredAccessPolicy](Get-AzStorageShareStoredAccessPolicy.md)
Pobiera zasady dostępu do udziału w magazynie.

### [Get-AzStorageTable](Get-AzStorageTable.md)
Wyświetla listę tabel magazynu.

### [Get-AzStorageTableStoredAccessPolicy](Get-AzStorageTableStoredAccessPolicy.md)
Pobiera zasady lub zasady dostępu do tabeli magazynu platformy Azure.

### [Get-AzStorageUsage](Get-AzStorageUsage.md)
Pobiera użycie zasobu magazynu dla bieżącej subskrypcji.

### [Invoke-AzStorageAccountFailover](Invoke-AzStorageAccountFailover.md)
Wywołuje tryb failover konta magazynu.

### [Lock — AzRmStorageContainerImmutabilityPolicy](Lock-AzRmStorageContainerImmutabilityPolicy.md)
Blokuje ImmutabilityPolicy kontenerów blob magazynu

### [Przenieś — AzDataLakeGen2Item](Move-AzDataLakeGen2Item.md)
Przenoszenie pliku lub katalogu do innego pliku lub katalogu na tym samym koncie magazynu.

### [Nowe — AzDataLakeGen2Item](New-AzDataLakeGen2Item.md)
Tworzenie pliku lub katalogu w systemie plików.

### [Nowe — AzRmStorageContainer](New-AzRmStorageContainer.md)
Tworzy kontener obiektu blob magazynu

### [Nowe — AzRmStorageShare](New-AzRmStorageShare.md)
Umożliwia utworzenie udziału plików magazynu.

### [Nowe — AzStorageAccount](New-AzStorageAccount.md)
Tworzy konto magazynu.

### [Nowe — AzStorageAccountKey](New-AzStorageAccountKey.md)
Generuje ponownie klucz magazynu dla konta usługi Azure Storage.

### [Nowe — AzStorageAccountManagementPolicyFilter](New-AzStorageAccountManagementPolicyFilter.md)
Umożliwia utworzenie obiektu filtru reguły ManagementPolicy, którego można użyć w funkcji New-AzStorageAccountManagementPolicyRule.

### [Nowe — AzStorageAccountManagementPolicyRule](New-AzStorageAccountManagementPolicyRule.md)
Tworzy obiekt reguły ManagementPolicy, który można użyć w programie Set-AzStorageAccountManagementPolicy.

### [Nowe — AzStorageAccountSASToken](New-AzStorageAccountSASToken.md)
Umożliwia utworzenie tokenu SAS na poziomie konta.

### [Nowe — AzStorageBlobQueryConfig](New-AzStorageBlobQueryConfig.md)
Tworzy obiekt konfiguracji zapytania BLOB, który może być wykorzystywany w programie Get-AzStorageBlobQueryResult.

### [Nowe — AzStorageBlobRangeToRestore](New-AzStorageBlobRangeToRestore.md)
Tworzy obiekt zakresu BLOB w celu przywrócenia konta magazynu.

### [Nowe — AzStorageBlobSASToken](New-AzStorageBlobSASToken.md)
Generuje token SAS dla obiektu BLOB usługi Azure Storage.

### [Nowe — AzStorageContainer](New-AzStorageContainer.md)
Tworzy kontener magazynu platformy Azure.

### [Nowe — AzStorageContainerSASToken](New-AzStorageContainerSASToken.md)
Generuje token SAS dla kontenera magazynu platformy Azure.

### [Nowe — AzStorageContainerStoredAccessPolicy](New-AzStorageContainerStoredAccessPolicy.md)
Tworzy zapisane zasady dostępu dla kontenera magazynu platformy Azure.

### [Nowe — AzStorageContext](New-AzStorageContext.md)
Tworzy kontekst usługi Azure Storage.

### [Nowe — AzStorageDirectory](New-AzStorageDirectory.md)
Tworzy katalog.

### [Nowe — AzStorageEncryptionScope](New-AzStorageEncryptionScope.md)
Tworzy zakres szyfrowania dla konta magazynu.

### [Nowe — AzStorageFileSASToken](New-AzStorageFileSASToken.md)
Generuje token podpisu dostępu współdzielonego dla pliku magazynu.

### [Nowe — AzStorageObjectReplicationPolicyRule](New-AzStorageObjectReplicationPolicyRule.md)
Tworzenie reguły zasad replikacji obiektów.

### [Nowe — AzStorageQueue](New-AzStorageQueue.md)
Umożliwia utworzenie kolejki magazynu.

### [Nowe — AzStorageQueueSASToken](New-AzStorageQueueSASToken.md)
Generuje token podpisu dostępu współdzielonego dla kolejki magazynu platformy Azure.

### [Nowe — AzStorageQueueStoredAccessPolicy](New-AzStorageQueueStoredAccessPolicy.md)
Tworzy zapisane zasady dostępu dla kolejki magazynu platformy Azure.

### [Nowe — AzStorageShare](New-AzStorageShare.md)
Umożliwia utworzenie udziału plików.

### [Nowe — AzStorageShareSASToken](New-AzStorageShareSASToken.md)
Wygeneruj token podpisu dostępu współdzielonego dla udziału magazynu platformy Azure.

### [Nowe — AzStorageShareStoredAccessPolicy](New-AzStorageShareStoredAccessPolicy.md)
Tworzy zapisane zasady dostępu w udziale miejsca do magazynowania.

### [Nowe — AzStorageTable](New-AzStorageTable.md)
Tworzy tabelę magazynu.

### [Nowe — AzStorageTableSASToken](New-AzStorageTableSASToken.md)
Generuje token SAS dla tabeli magazynu platformy Azure.

### [Nowe — AzStorageTableStoredAccessPolicy](New-AzStorageTableStoredAccessPolicy.md)
Tworzy zapisane zasady dostępu do tabeli magazynu platformy Azure.

### [Remove-AzDataLakeGen2AclRecursive](Remove-AzDataLakeGen2AclRecursive.md)
Usuń listę ACL rekursywnie w określonej ścieżce. 

### [Remove-AzDataLakeGen2Item](Remove-AzDataLakeGen2Item.md)
Usuwanie pliku lub katalogu.

### [Remove-AzRmStorageContainer](Remove-AzRmStorageContainer.md)
Usuwa kontener obiektu blob magazynu

### [Remove-AzRmStorageContainerImmutabilityPolicy](Remove-AzRmStorageContainerImmutabilityPolicy.md)
Usuwa ImmutabilityPolicy kontenera obiektu blob magazynu z odblokowanymi zasadami

### [Remove-AzRmStorageContainerLegalHold](Remove-AzRmStorageContainerLegalHold.md)
Usuwa z kontenera obiektów blob magazynu prawnego Tagi

### [Remove-AzRmStorageShare](Remove-AzRmStorageShare.md)
Umożliwia usunięcie udziału plików magazynu.

### [Remove-AzStorageAccount](Remove-AzStorageAccount.md)
Umożliwia usunięcie konta magazynu z platformy Azure.

### [Remove-AzStorageAccountManagementPolicy](Remove-AzStorageAccountManagementPolicy.md)
Usuwa zasady zarządzania konta usługi Azure Storage.

### [Remove-AzStorageAccountNetworkRule](Remove-AzStorageAccountNetworkRule.md)
Usuwanie IpRules lub VirtualNetworkRules z właściwości NetWorkRule konta magazynu

### [Remove-AzStorageBlob](Remove-AzStorageBlob.md)
Umożliwia usunięcie określonego obiektu blob magazynu.

### [Remove-AzStorageContainer](Remove-AzStorageContainer.md)
Usuwa określony kontener magazynu.

### [Remove-AzStorageContainerStoredAccessPolicy](Remove-AzStorageContainerStoredAccessPolicy.md)
Usuwa zapisane zasady dostępu z kontenera magazynu platformy Azure.

### [Remove-AzStorageCORSRule](Remove-AzStorageCORSRule.md)
Usuwa składnik CORS dla usługi magazynu.

### [Remove-AzStorageDirectory](Remove-AzStorageDirectory.md)
Usuwa katalog.

### [Remove-AzStorageFile](Remove-AzStorageFile.md)
Usuwa plik.

### [Remove-AzStorageObjectReplicationPolicy](Remove-AzStorageObjectReplicationPolicy.md)
Usuwa określone zasady replikacji obiektów z konta magazynu.

### [Remove-AzStorageQueue](Remove-AzStorageQueue.md)
Usuwa kolejkę magazynu.

### [Remove-AzStorageQueueStoredAccessPolicy](Remove-AzStorageQueueStoredAccessPolicy.md)
Usuwa zapisane zasady dostępu z kolejki usługi Azure Storage.

### [Remove-AzStorageShare](Remove-AzStorageShare.md)
Usuwa udział plików.

### [Remove-AzStorageShareStoredAccessPolicy](Remove-AzStorageShareStoredAccessPolicy.md)
Usuwa zapisane zasady dostępu z udziału magazynu.

### [Remove-AzStorageTable](Remove-AzStorageTable.md)
Usuwa tabelę magazynu.

### [Remove-AzStorageTableStoredAccessPolicy](Remove-AzStorageTableStoredAccessPolicy.md)
Usuwa zapisane zasady dostępu z tabeli magazynu platformy Azure.

### [Restore-AzRmStorageShare](Restore-AzRmStorageShare.md)
Przywraca usunięty udział pliku.

### [Restore-AzStorageBlobRange](Restore-AzStorageBlobRange.md)
Przywraca konto magazynu dla określonych zakresów obiektów BLOB.

### [REVOKE-AzStorageAccountUserDelegationKeys](Revoke-AzStorageAccountUserDelegationKeys.md)
Odwołaj wszystkie klucze delegowania użytkowników konta magazynu.

### [Set-AzCurrentStorageAccount](Set-AzCurrentStorageAccount.md)
Modyfikuje bieżące konto magazynu określonego abonamentu.

### [Set-AzDataLakeGen2AclRecursive](Set-AzDataLakeGen2AclRecursive.md)
Ustawianie listy ACL rekursywnie w określonej ścieżce. 

### [Set-AzDataLakeGen2ItemAclObject](Set-AzDataLakeGen2ItemAclObject.md)
Tworzy/aktualizuje obiekt Gen2 ACL elementu datalake, którego można użyć w Update-AzDataLakeGen2Item polecenia cmdlet.

### [Set-AzRmStorageContainerImmutabilityPolicy](Set-AzRmStorageContainerImmutabilityPolicy.md)
Tworzy lub aktualizuje ImmutabilityPolicy kontenery blob magazynu.

### [Set-AzStorageAccount](Set-AzStorageAccount.md)
Modyfikuje konto magazynu.

### [Set-AzStorageAccountManagementPolicy](Set-AzStorageAccountManagementPolicy.md)
Tworzy lub modyfikuje zasady zarządzania konta usługi Azure Storage.

### [Set-AzStorageBlobContent](Set-AzStorageBlobContent.md)
Wysyła plik lokalny do obiektu BLOB usługi Azure Storage.

### [Set-AzStorageContainerAcl](Set-AzStorageContainerAcl.md)
Ustawia uprawnienie dostępu publicznego do kontenera magazynu.

### [Set-AzStorageContainerStoredAccessPolicy](Set-AzStorageContainerStoredAccessPolicy.md)
Ustawia zapisane zasady dostępu dla kontenera magazynu platformy Azure.

### [Set-AzStorageCORSRule](Set-AzStorageCORSRule.md)
Ustawia reguły CORS dla typu usługi magazynu.

### [Set-AzStorageFileContent](Set-AzStorageFileContent.md)
Umożliwia przekazywanie zawartości pliku.

### [Set-AzStorageObjectReplicationPolicy](Set-AzStorageObjectReplicationPolicy.md)
Tworzy lub aktualizuje określone zasady replikacji obiektów na koncie magazynu.

### [Set-AzStorageQueueStoredAccessPolicy](Set-AzStorageQueueStoredAccessPolicy.md)
Ustawia zapisane zasady dostępu dla kolejki magazynu platformy Azure.

### [Set-AzStorageServiceLoggingProperty](Set-AzStorageServiceLoggingProperty.md)
Modyfikuje rejestrowanie w usłudze Azure Storage Services.

### [Set-AzStorageServiceMetricsProperty](Set-AzStorageServiceMetricsProperty.md)
Modyfikuje właściwości metryk dla usługi Azure Storage.

### [Set-AzStorageShareQuota](Set-AzStorageShareQuota.md)
Umożliwia ustawienie pojemności magazynowania dla udziału.

### [Set-AzStorageShareStoredAccessPolicy](Set-AzStorageShareStoredAccessPolicy.md)
Aktualizuje zapisane zasady dostępu w udziale magazynu.

### [Set-AzStorageTableStoredAccessPolicy](Set-AzStorageTableStoredAccessPolicy.md)
Ustawia zasady dostępu przechowywane dla tabeli magazynu platformy Azure.

### [Start — AzStorageBlobCopy](Start-AzStorageBlobCopy.md)
Rozpoczyna Kopiowanie obiektu BLOB.

### [Start — AzStorageBlobIncrementalCopy](Start-AzStorageBlobIncrementalCopy.md)
Rozpocznij operację przyrostowa kopiowania z migawki obiektu BLOB strony do określonego obiektu BLOB strony docelowej.

### [Start — AzStorageFileCopy](Start-AzStorageFileCopy.md)
Rozpoczyna kopiowanie pliku źródłowego.

### [Zatrzymaj — AzStorageBlobCopy](Stop-AzStorageBlobCopy.md)
Zatrzymuje operację kopiowania.

### [Zatrzymaj — AzStorageFileCopy](Stop-AzStorageFileCopy.md)
Zatrzymuje operację kopiowania w określonym pliku docelowym.

### [Update-AzDataLakeGen2AclRecursive](Update-AzDataLakeGen2AclRecursive.md)
Aktualizuj listę ACL rekursywnie w określonej ścieżce. 

### [Update-AzDataLakeGen2Item](Update-AzDataLakeGen2Item.md)
Aktualizowanie pliku lub katalogu na właściwościach, metadanych, uprawnieniach, listach ACL i właścicielu.

### [Update-AzRmStorageContainer](Update-AzRmStorageContainer.md)
Modyfikuje kontener obiektu BLOB przechowywania

### [Update-AzRmStorageShare](Update-AzRmStorageShare.md)
Modyfikuje udział plików magazynu.

### [Update-AzStorageAccountNetworkRuleSet](Update-AzStorageAccountNetworkRuleSet.md)
Aktualizowanie właściwości NetworkRule konta magazynu

### [Update-AzStorageBlobServiceProperty](Update-AzStorageBlobServiceProperty.md)
Modyfikuje właściwości usługi dla usługi blob magazynu Azure.

### [Update-AzStorageEncryptionScope](Update-AzStorageEncryptionScope.md)
Modyfikowanie zakresu szyfrowania dla konta magazynu.

### [Update-AzStorageFileServiceProperty](Update-AzStorageFileServiceProperty.md)
Modyfikuje właściwości usługi dla usługi plików magazynu platformy Azure.

### [Update-AzStorageServiceProperty](Update-AzStorageServiceProperty.md)
Modyfikuje właściwości usługi Azure Storage.

