...
## STRESZCZENIe
Tworzy klaster pamięci podręcznej Redis enterpise i skojarzoną z nią bazę danych
...
### Przykład 1. Tworzenie Redis pamięci podręcznej przedsiębiorstwa
```powershell
...
To polecenie tworzy Redisą pamięć podręczną dla przedsiębiorstw o nazwie moja pamięć.

### Przykład 2: Tworzenie Redis pamięci podręcznej przedsiębiorstwa za pomocą opcjonalnych parametrów
```powershell
...
To polecenie tworzy Redisą pamięć podręczną o nazwie mój bufor, korzystając z opcjonalnych parametrów.
...
### -Pojemność
Rozmiar klastra usługi RedisEnterprise.
Domyślna wartość 2 lub 3 w zależności od jednostki SKU.
Prawidłowymi wartościami są (2, 4, 6,...) dla wersji Enterprise SKU i (3, 9, 15,...) dla wersji Flash (wersje SKU).
...
### -MinimumTlsVersion
Minimalna wersja protokołu TLS dla klastra do obsługi, na przykład 1,2
...
### -SKU
Typ klastra RedisEnterprise do wdrożenia.
Możliwe wartości: (Enterprise_E10, EnterpriseFlash_F300 itd.)
...
### -Zone
Strefy, w których zostanie wdrożony ten klaster.
