...
## STRESZCZENIe
Zmienia ustawienia inspekcji operacji pomocy technicznej firmy Microsoft dotyczące programu Azure SQL Server.
...
## Opis
Polecenie cmdlet **Set-AzSqlServerMSSupportAudit** powoduje zmianę ustawień inspekcji operacji pomocy technicznej firmy Microsoft dla programu Azure SQL Server.
Aby użyć polecenia cmdlet, użyj parametrów *ResourceGroupName* oraz *nazwa_serwera* w celu zidentyfikowania serwera.
Gdy magazyn obiektów BLOB jest miejscem docelowym dla dzienników inspekcji, określ parametr *StorageAccountResourceId* , aby określić konto magazynu dla dzienników inspekcji.

## Przykłady

### Przykład 1: Włączanie zasad inspekcji operacji pomocy technicznej Microsoft dotyczących magazynu obiektów BLOB dla programu Azure SQL Server
```powershell
...
### Przykład 2: wyłączanie zasad inspekcji operacji pomocy technicznej firmy Microsoft w usłudze Azure SQL Server
```powershell
PS C:\>Set-AzSqlServerMSSupportAudit -ResourceGroupName "ResourceGroup01" -ServerName "Server01" -BlobStorageTargetState Disabled
```

### Przykład 3: Włączanie zasad inspekcji operacji pomocy technicznej Microsoft dotyczących usługi centrum zdarzeń w usłudze Azure SQL Server
```powershell
...
### Przykład 4: wyłączanie zasad inspekcji usług centrum zdarzeń w usłudze Microsoft dla programu Azure SQL Server
```powershell
PS C:\>Set-AzSqlServerMSSupportAudit -ResourceGroupName "ResourceGroup01" -ServerName "Server01" -EventHubTargetState Disabled
```

### Przykład 5: Włączanie zasad inspekcji operacji pomocy technicznej dla usługi Microsoft log w usłudze Azure SQL Server
```powershell
...
### Przykład 6: wyłączenie zasad inspekcji operacji pomocy technicznej dla usługi Microsoft log w usłudze Azure SQL Server
```powershell
...
### Przykład 7: wyłączanie, za pośrednictwem rurociągu, zasady inspekcji operacji pomocy technicznej firmy Microsoft dotyczące usługi SQL Server
```powershell
...
### Przykład 8: wyłączanie wysyłania rekordów inspekcji operacji pomocy technicznej firmy Microsoft w usłudze Azure SQL Server do magazynu obiektów blob i włączanie wysyłania ich do analizy dzienników.
```powershell
...
### Przykład 9: Włączanie wysyłania rekordów inspekcji operacji pomocy technicznej firmy Microsoft w programie Azure SQL Server do magazynu obiektów blob, centrum zdarzeń i analizy dzienników.
```powershell
...
Wskazuje, czy Magazyn obiektów BLOB jest miejscem docelowym dla rekordów inspekcji operacji pomocy technicznej firmy Microsoft.
...
### -EventHubTargetState
Wskazuje, czy centrum zdarzeń jest miejscem docelowym dla rekordów inspekcji operacji pomocy technicznej firmy Microsoft.
...
Wskazuje, czy Analiza dzienników jest lokalizacją docelową dla rekordów inspekcji operacji pomocy technicznej firmy Microsoft.
...
### -PassThru
Określa, czy na końcu wykonania polecenia cmdlet mają być wyprowadzane zasady inspekcji działań pomocy technicznej firmy Microsoft.
...
### -WorkspaceResourceId
Identyfikator obszaru roboczego (identyfikator zasobu obszaru roboczego analiza dziennika) dla obszaru roboczego analizy dzienników, do którego chcesz wysłać dzienniki inspekcji operacji pomocy technicznej firmy Microsoft. Przykład:/subscriptions/4b9e8510-67ab-4e9a-95a9-e2f1e570ea9c/resourceGroups/insights-integration/providers/Microsoft.OperationalInsights/workspaces/viruela2
...
### Microsoft. Azure. Commands. SQL. Audit. model. ServerDevOpsAuditModel
