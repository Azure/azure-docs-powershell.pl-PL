...
## Opis
Polecenie cmdlet **New-AzSqlDatabaseExport** wyeksportuje bazę danych SQL Azure jako plik BACPAC do konta magazynu.
W celu pobrania informacji o stanie dla tego żądania można wysłać żądanie uzyskania stanu bazy danych — eksport.
To polecenie cmdlet jest również obsługiwane przez usługę bazy danych SQL Server undatabase na platformie Azure.

> [!IMPORTANT]
> Aby móc korzystać z tego polecenia cmdlet, Zapora w usłudze Azure SQL Server musi być skonfigurowana tak, aby można było uzyskać dostęp do tego serwera za pomocą usług platformy Azure i zasobów. Jeśli nie zostanie to skonfigurowane, będą występować błędy GatewayTimeout.
...
Wartość domyślna to SQL, jeśli nie jest ustawiony typ uwierzytelniania.
Dopuszczalne wartości tego parametru to:
- Instrukcji.
Uwierzytelnianie SQL.
Ustaw *AdministratorLogin* i *AdministratorLoginPassword* na nazwę użytkownika i hasło administratora SQL. 
- ADPassword.
Uwierzytelnianie usługi Azure Active Directory.
Ustaw *AdministratorLogin* i *AdministratorLoginPassword* na nazwę użytkownika i hasło administratora usługi Azure AD.
...
Identyfikator zasobu programu SQL Server umożliwiający utworzenie prywatnego linku
...
Identyfikator zasobu konta magazynu umożliwiający utworzenie prywatnego linku
...
- SharedAccessKey.
Ta wartość używa klucza (SAS) (Shared Access Signature).
...
### -StorageUri
Określa łącze obiektu BLOB jako adres URL pliku BACPAC.
...
### -UseNetworkIsolation
Jeśli jest ustawiona, utworzy prywatne łącze dla konta magazynu i/lub programu SQL Server
