...
### UpdateExpandedEventHub (domyślny)
```
...
### Przykład 1: Aktualizowanie istniejącego połączenia danych EventHub
```powershell
...
Powyższe polecenie aktualizuje istniejące połączenie danych EventHub o nazwie "myeventhubdc" dla bazy danych "mykustodatabase" w klastrze "testnewkustocluster".

### Przykład 2: Aktualizowanie istniejącego połączenia danych programu EventGrid
```powershell
...
Powyższe polecenie aktualizuje istniejące połączenie danych programu EventGrid o nazwie "myeventgriddc" dla bazy danych "mykustodatabase" w klastrze "testnewkustocluster".

### Przykład 3: Aktualizowanie istniejącego połączenia danych programu IotHub
```powershell
...
Powyższe polecenie aktualizuje istniejące połączenie danych programu IotHub o nazwie "myiothubdc" dla bazy danych "mykustodatabase" w klastrze "testnewkustocluster".

### Przykład 4: Aktualizowanie istniejącego połączenia danych EventHub za pomocą tożsamości
```powershell
...
### Przykład 5: Aktualizowanie istniejącego połączenia danych EventGrid przy użyciu tożsamości
```powershell
...
### Przykład 6: Aktualizowanie istniejącego połączenia danych IotHub za pomocą tożsamości
```powershell
...
### -Compression
Typ kompresji komunikaty centrum zdarzeń.
...
### -Odbiorca
Grupa użytkowników centrum wydarzenia/IoT.
...
### -DataFormat
Format danych wiadomości.
Opcjonalnie można dodać format danych do każdej wiadomości.
...
### -EventHubResourceId
Identyfikator zasobu centrum zdarzeń, który ma zostać wykorzystany do utworzenia połączenia danych/siatki zdarzeń, jest skonfigurowany w celu wysyłania zdarzeń.
...
### -EventSystemProperty
Właściwości systemowe centrum wydarzeń/IoT.
...
### -IgnoreFirstRecord
Jeśli ustawiono wartość PRAWDA, oznacza to, że pokarmowanie powinno ignorować pierwszy rekord każdego pliku.
...
### -IotHubResourceId
Identyfikator zasobu Centrum IoT, który ma zostać wykorzystany do utworzenia połączenia danych.
...
### -Kind
Rodzaj punktu końcowego połączenia danych
...
### -MappingRuleName
Reguła mapowania, która ma być używana do spożycia danych.
Opcjonalnie informacje dotyczące mapowania można dodawać do poszczególnych wiadomości.
...
### -TableName
Tabela, w której dane powinny być spożywane.
Opcjonalnie informacje dotyczące tabeli można dodawać do poszczególnych wiadomości.
...
### Microsoft. Azure. PowerShell. polecenia. Kusto. models. Api20200918. IDataConnection
