...
## OPIS
Polecenie **cmdlet Remove-AzRelayAuthorizationRule** usuwa regułę autoryzacji dla danych obiektów przekazywania (Przestrzeń nazw/WcfRelay/HybridConnection).
...
Usuwa regułę autoryzacji `AuthoRule1` przestrzeni `TestNameSpace-Relay1` nazw.
...
Usuwa regułę `AuthoRule1` autoryzacji pliku WcfRelay `TestWcfRelay` z przestrzeni `TestNameSpace-Relay1` nazw.
...
Usuwa regułę `AuthoRule1` autoryzacji połączenia hybrydowego `TestHybridConnection` z przestrzeni `TestNameSpace-Relay1` nazw.
