...
### Przykład 1. Aktualizowanie istniejącego klastra według nazwy
```powershell
...
Powyższe polecenie aktualizuje klaster Kusto "testnewkustocluster" znaleziony w grupie zasobów "testrg".

### Przykład 2. Aktualizowanie istniejącego klastra według nazwy
```powershell
...
Powyższe polecenie aktualizuje klaster "testnewkustocluster" znaleziony w grupie zasobów "testrg" za pomocą klucza zarządzanego przez klienta.
...
### -EnableCryption
Wartość logiczna wskazująca, czy dyskerowanie klastrów jest szyfrowane.
...
Wartość logiczna wskazująca, czy jest włączone podwójne szyfrowanie.
...
Wartość logiczna wskazująca, czy funkcja przesyłania strumieniowego jest włączona.
...
Typ "SystemAssigned, UserAssigned" obejmuje zarówno tożsamość utworzoną niejawnie, jak i zestaw tożsamości przypisanych przez użytkownika.
Wpisanie typu "Brak" spowoduje usunięcie wszystkich tożsamości.
...
Lista tożsamości użytkowników skojarzonych z klastrem Kusto.
Odwołania do kluczy słownika tożsamości użytkownika będą ARM identyfikatorami zasobów w postaci: '/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.ManagedIdentity/userAssignedIdentities/{identityName}'.
...
Użytkownikowi przypisano tożsamość (ARM identyfikator zasobu), która ma dostęp do klucza.
...
Aby skonstruować, zobacz sekcję NOTES, aby uzyskać informacje o właściwościach LANGUAGEEXTENSIONVALUE i utworzyć tabelę skrótów.
...
Wartość logiczna wskazująca, czy funkcja zoptymalizowanego skalowania automatycznego jest włączona, czy nie.
...
Liczba minimalnych dozwolonych wystąpień.
...
Wersja zdefiniowanego szablonu, na przykład 1.
...
Dzierżawy zewnętrzne klastrów.
Aby skonstruować, zobacz sekcję NOTATEK, aby uzyskać informacje o właściwościach TRUSTEDEXTERNALTENANT i utworzyć tabelę skrótów.
...
Identyfikator publicznego adresu IP usługi zarządzania danymi.
...
LANGUAGEEXTENSIONVALUE <ILanguageExtension[]>: Lista rozszerzeń językowych.
  - `[Name <LanguageExtensionName?>]`: nazwa rozszerzenia języka.

TRUSTEDEXTERNALTENANT <ITrustedExternalTenant[]>: Dzierżawy zewnętrzne klastrów.
  - `[Value <String>]`: identyfikator GUID reprezentujący dzierżawę zewnętrzną.
