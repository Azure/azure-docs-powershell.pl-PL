...
### Przykład 1. Uzyskiwanie publicznych adresów IP na poziomie wystąpienia dla danej nazwy usługi w chmurze.
```powershell
...
Pobiera publiczne adresy IP na poziomie wystąpienia dla danej nazwy usługi w chmurze.

### Przykład 2. Uzyskaj publiczne adresy IP na poziomie wystąpienia dla danego obiektu usługi w chmurze.
```powershell
...
Pobiera publiczne adresy IP na poziomie wystąpienia dla danego obiektu usługi w chmurze.
...
<CloudService>CLOUDSERVICE: wystąpienie usługi CloudService.
  ...
  - `[ExtensionProfile <ICloudServiceExtensionProfile>]`: w tym artykule opisano profil rozszerzenia usługi w chmurze.
    - `[Extension <IExtension[]>]`: Lista rozszerzeń dla usługi w chmurze.
      ...
      - `[TypeHandlerVersion <String>]`: określa wersję rozszerzenia. Określa wersję rozszerzenia. Jeśli ten element nie jest określony lub jako wartość zostanie użyta gwiazdka (*), zostanie użyta najnowsza wersja rozszerzenia. Jeśli wartość jest określona z głównym numerem wersji i gwiazdką jako pomocniczy numer wersji (X.), wybrana jest najnowsza wersja pomocnicza określonej wersji głównych. Jeśli podano główny numer wersji i numer wersji pomocniczej (X.Y), wybrana jest określona wersja rozszerzenia. Jeśli jest określona wersja, w wystąpieniu roli jest wykonywane automatyczne uaktualnianie.
