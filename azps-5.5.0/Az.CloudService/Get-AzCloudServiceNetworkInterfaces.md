...
### Przykład 1. Uzyskiwanie interfejsów sieciowych według nazwy usługi w chmurze
```powershell
...
Pobiera wszystkie interfejsy sieciowe dla danej nazwy usługi w chmurze.

### Przykład 2. Uzyskiwanie interfejsów sieciowych za pomocą obiektu usługi w chmurze
```powershell
...
Pobiera wszystkie interfejsy sieciowe dla danego obiektu usługi w chmurze.

### Przykład 3. Uzyskiwanie interfejsów sieciowych według obiektu usługi w chmurze i nazwy wystąpienia roli.
```powershell
PS C:\> Get-AzCloudServiceNetworkInterfaces -CloudService $cs -RoleInstanceName WebRole1_IN_0

```

Pobiera wszystkie interfejsy sieciowe dla danego obiektu usługi w chmurze i nazwy wystąpienia roli.

## PARAMETERS

### — CloudService
Wystąpienie usługi CloudService.
...
<CloudService>CLOUDSERVICE: wystąpienie usługi CloudService.
  ...
  - `[ExtensionProfile <ICloudServiceExtensionProfile>]`: w tym artykule opisano profil rozszerzenia usługi w chmurze.
    - `[Extension <IExtension[]>]`: Lista rozszerzeń dla usługi w chmurze.
      ...
      - `[TypeHandlerVersion <String>]`: określa wersję rozszerzenia. Określa wersję rozszerzenia. Jeśli ten element nie zostanie określony lub jako wartość zostanie użyta gwiazdka (*), zostanie użyta najnowsza wersja rozszerzenia. Jeśli wartość jest określona z głównym numerem wersji i gwiazdką jako pomocniczy numer wersji (X.), wybrana jest najnowsza wersja pomocnicza określonej wersji głównych. Jeśli określono główny numer wersji i numer wersji pomocniczej (X.Y), wybrana jest określona wersja rozszerzenia. Jeśli jest określona wersja, automatyczne uaktualnianie jest wykonywane w wystąpieniu roli.
