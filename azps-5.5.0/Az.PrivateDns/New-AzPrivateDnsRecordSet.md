...
## OPIS
Polecenie New-AzPrivateDnsRecordSet cmdlet tworzy nowy zestaw rekordów DNS (Private Domain Name System) o określonej nazwie i wpisz go w określonej strefie prywatnej. Obiekt RecordSet to zestaw prywatnych rekordów DNS o takiej samej nazwie i typie. Pamiętaj, że nazwa jest względna dla strefy prywatnej, a nie do w pełni kwalifikowanej nazwy. Parametr PrivateDnsRecord określa rekordy w zestawie rekordów. Ten parametr pobiera tablicę prywatnych rekordów DNS skonstruowanych przy użyciu parametru New-AzPrivateDnsRecordConfig. Za pomocą operatora potoku można przekazać obiekt PSPrivateDnsZone do tego polecenia cmdlet lub przekazać obiekt PSPrivateDnsZone jako parametr Zone lub określić strefę według jej parametru ResourceId lub możesz też określić strefę według nazwy. Możesz użyć parametru Confirm i $ConfirmPreference zmienną programu Windows PowerShell, aby określić, czy polecenie cmdlet monituje o potwierdzenie. Jeśli zgodny zestaw rekordów już istnieje (nazwa i typ rekordu), musisz określić parametr Zastąp, w przeciwnym razie polecenie cmdlet nie utworzy nowego zestawu rekordów.
