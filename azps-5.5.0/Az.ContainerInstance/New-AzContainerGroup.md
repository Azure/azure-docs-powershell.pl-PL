...
## SYNOPSIS
Tworzy grupę kontenera.

## SKŁADNIA

### CreateContainerGroupBaseParamSet (domyślne)
```
...
## OPIS
Polecenia **cmdlet New-AzContainerGroup** tworzą grupę kontenera.
...
Te polecenia tworzą grupę kontenerów przy użyciu najnowszego obrazu nginx i żąda publicznego adresu IP z otwarciem portu 8000.
...
Te polecenia tworzą grupę kontenerów i uruchamiają skrypt niestandardowy wewnątrz kontenera.

### Przykład 3. Tworzy grupę kontenera uruchamianą do ukończenia.
```
...
Te polecenia tworzą grupę kontenerów, która drukuje słowo "witaj" i zatrzymuje się.

### Przykład 4. Tworzenie grupy kontenera przy użyciu obrazu w rejestrze kontenerów platformy Azure
```
...
Te polecenia tworzą grupę kontenerów przy użyciu obrazu nginx w rejestrze kontenerów platformy Azure.

### Przykład 5. Tworzenie grupy kontenerów przy użyciu obrazu w niestandardowym rejestrze obrazów kontenera
```
PS C:\> $secpasswd = ConvertTo-SecureString "PlainTextPassword" -AsPlainText -Force
...
Te polecenia tworzą grupę kontenerów przy użyciu obrazu niestandardowego z rejestru obrazów kontenera niestandardowego.

### Przykład 6. Tworzy grupę kontenerów, która umożliwia uchwytowanie woluminu plików platformy Azure
```
PS C:\> $secpasswd = ConvertTo-SecureString "PlainTextPassword" -AsPlainText -Force
PS C:\> $mycred = New-Object System.Management.Automation.PSCredential ("username", $secpasswd)
...
Te polecenia tworzą grupę kontenerów, w ramach których jest chowywny podany udział plików platformy `/mnt/azfile` Azure.
...
Te polecenia tworzą grupę kontenerów z przypisaną tożsamością systemu przy użyciu najnowszego obrazu nginx i żąda publicznego adresu IP z otwarciem portu 8000.
...
To polecenie tworzy grupę kontenerów z przypisaną tożsamością systemu i przypisaną przez użytkownika przy użyciu najnowszego obrazu nginx i żąda publicznego adresu IP z otwarciem portu 8000.
...
Poświadczenia konta magazynu udziału plików platformy Azure do instalacji, gdzie nazwa użytkownika jest nazwą konta magazynu, a kluczem jest klucz konta magazynu.
...
Ścieżka instalacji dla woluminu plików platformy Azure.
...
Nazwa udziału plików platformy Azure do instalacji.
...
### - Command
Polecenie uruchamiane w kontenerze.
...
### - IdentityId
Identyfikatory tożsamości przypisane przez użytkownika
...
### — Port
Port(y) do otwarcia. Domyślne: [80]
...
### -RegistryCredential
Poświadczenia rejestru kontenera niestandardowego.
...
Niestandardowy serwer logowania kontenera.
...
### -RestartPolicy
Zasady ponownego uruchamiania kontenera. Domyślne: Zawsze
