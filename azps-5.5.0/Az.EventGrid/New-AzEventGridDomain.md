...
Tworzy domenę siatki zdarzeń (Domain1) w określonej lokalizacji geograficznej westus2 w grupie zasobów \` \` \` \` \` MyResourceGroupName (Nazwa_grupy \` zasobów).
...
Tworzy domenę siatki zdarzeń (Domain1) w określonej lokalizacji geograficznej westus2, w grupie zasobów MyResourceGroupName z określonymi tagami \` \` \` \` "Department" (Dział) i \` \` "Environment" (Środowisko).
