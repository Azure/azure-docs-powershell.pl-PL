...
### Przykład 1. Tworzy nowy pakiet MSIX w buforze hosta za pomocą aliasu pakietu
```powershell
...
To polecenie dodaje pakiet MSIX z określonej ścieżki obrazu do usługi HostPool

### Przykład 2. Tworzy nowy pakiet MSIX w buforze hosta
```powershell
...
To polecenie dodaje pakiet MSIX w określonym buforze hosta
...
### — DisplayName
Przyjazna dla użytkownika nazwa wyświetlana w portalu.
...
### — ImagePath
Ścieżka obrazu VHD/CIM w dziele sieciowym.
...
### —IsActive
Upewnij się, że ta wersja pakietu jest aktywna w całej buforze hosta.
...
### -LastUpdated
Data ostatniej aktualizacji pakietu, która została znaleziona w appxmanifest.xml.
...
Aby skonstruować, zobacz sekcję NOTATKI, aby uzyskać informacje o właściwościach PACKAGEAPPLICATION i utworzyć tabelę skrótu.
...
Aby skonstruować, zobacz sekcję NOTES, aby uzyskać informacje o właściwościach PACKAGEDEPENDENCY i utworzyć tabelę skrótów.
...
### -PackageFamilyName
Spakuj nazwę rodziny z appxmanifest.xml.
...
### -PackageRelativePath
Ścieżka względna do pakietu wewnątrz obrazu.
...
PACKAGEAPPLICATION <IMsixPackageApplications[]>: Lista aplikacji pakietu. 
  - `[AppId <String>]`: Identyfikator aplikacji pakietu, który został znaleziony w appxmanifest.xml.
  - `[AppUserModelId <String>]`: Służy do aktywowania aplikacji pakietu. Składa się z nazwy pakietu i pola ApplicationID. Znaleziono w appxmanifest.xml.
  - `[Description <String>]`: Opis aplikacji pakietu.
  - `[FriendlyName <String>]`: przyjazna dla użytkownika nazwa.
  - `[IconImageName <String>]`: przyjazna dla użytkownika nazwa.
  - `[RawIcon <Byte[]>]`: ikona ciągu 64-bitowego jako tablica bajtowa.
  - `[RawPng <Byte[]>]`: ikona ciągu 64-bitowego jako tablica bajtowa.

PACKAGEDEPENDENCY <IMsixPackageDependencies[]>: Lista zależności pakietów. 
  - `[DependencyName <String>]`: Nazwa zależności pakietu.
  - `[MinVersion <String>]`: Wymagana wersja dependency.
