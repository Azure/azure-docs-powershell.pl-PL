...
## OPIS
Polecenie **cmdlet Get-AzResourceGroupDeploymentWhatIfResult** pobiera ARM szablonu What-If wynik wdrożenia szablonu w określonym zakresie grupy zasobów. Zwraca ona listę zmian wskazujących, które zasoby zostaną zaktualizowane, jeśli wdrożenie zostanie zastosowane bez dokonywania zmian w zasobach rzeczywistych. Aby określić format zwracanych wyników, użyj *parametru ResultFormat.*

## PRZYKŁADY

### Przykład 1. Uzyskiwanie What-If w zakresie grupy zasobów
```powershell
...
To polecenie uzyskuje What-If w zakresie określonej grupy zasobów przy użyciu pliku szablonu niestandardowego i pliku parametru na dysku.
To polecenie używa *parametru ResourceGroupName* w celu określenia grupy zasobów, w której szablon zostanie wdrożony.
...
### Przykład 2. Uzyskiwanie What-If w zakresie grupy zasobów przy użyciu tylko zasobu
```powershell
PS C:\> Get-AzResourceGroupDeploymentWhatIfResult `
    -DeploymentName "deploy-01" `
    -ResourceGroupName "myRG1" `
...
### -ExcludeChangeType
Typy zmian zasobów rozdzielane przecinkami, które mają być wykluczone What-If wyników.
