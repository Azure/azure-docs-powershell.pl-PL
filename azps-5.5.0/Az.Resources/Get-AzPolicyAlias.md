...
## SYNOPSIS
Get-AzPolicyAlias pobiera i wyprowadza zasoby dostawców platformy Azure, które mają zdefiniowane aliasy i są zgodne z określonymi wartościami parametrów. Jeśli nie podano parametrów, zostaną wyprowadzne wszystkie typy zasobów dostawcy zawierające alias.
...
Podana wartość dopasowania jest dopasowana, jeśli ciąg docelowy zawiera ciąg, w przypadku porównania bez uwzględniania liter.
...
* Dodatkowe właściwości są dostępne w wyniku i mogą być wyświetlane, rurując dane wyjściowe `Format-List` do. Na przykład: `Get-AzPolicyAlias -NamespaceMatch 'Web' -ResourceTypeMatch site -PathMatch cert | Format-List`
