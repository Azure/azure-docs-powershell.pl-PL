...
## SYNOPSIS
Aktualizowanie listy ACL sieci usługi signalr.
...
## OPIS
Zaktualizuj wartość ACL sieci usługi signalr, w tym domyślną akcję i wartość Acls sieci dla połączenia publicznego i prywatnego.

## PRZYKŁADY

### Zezwalaj na usługę RESTAPI, clientConnection dla sieci publicznej i ustaw domyślną akcję Deny (Odmów)
```powershell
...
### Zezwalaj na połączenie klienta i połączenie z serwerem dla prywatnego połączenia punktu końcowego
```powershell
...
### Odrzucanie połączenia klienta zarówno dla sieci publicznej, jak i prywatnego połączenia punktu końcowego
```powershell
...
### -DefaultAction
Domyślna akcja acl sieci signalr ( zezwalaj lub odmów). Decyduje ona, czy ACL sieci są odrzucane, czy zezwalać na ich działanie. Jeśli na przykład akcja domyślna jest zezwalana, ważne są tylko acla odmów.
...
### -PrivateEndpointName
Nazwy prywatnych punktów końcowych do zaktualizowania
...
### - PublicNetwork
Aktualizowanie ACL sieci publicznej
...
### -ResourceGroupName
Nazwa grupy zasobów.
Ustawienie domyślne będzie używane, jeśli nie zostanie określone.
