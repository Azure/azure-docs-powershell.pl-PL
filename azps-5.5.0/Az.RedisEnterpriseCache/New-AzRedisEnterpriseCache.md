...
## SYNOPSIS
Tworzy klaster pamięci podręcznej Redis Enterpise i skojarzoną bazę danych.
...
To polecenie tworzy pamięć podręczną Redis Enterprise Cache o nazwie MyCache.

### Przykład 2. Tworzenie pamięci podręcznej Redis Enterprise Cache przy użyciu niektórych parametrów opcjonalnych
```powershell
...
To polecenie tworzy pamięć podręczną Redis Enterprise Cache o nazwie MyCache, używając niektórych parametrów opcjonalnych.
...
### -ClientProtocol
Określa, czy klienci redis mogą łączyć się przy użyciu protokołów TLS-encrypted, czy zwykłego tekstu redis.
Wartością domyślną jest szyfrowanie TLS.
...
### - ClusteringPolicy
Zasady klastrowania — domyślne ustawienie to OSSCluster.
Określony w czasie tworzenia.
...
### -EvictionPolicy
Zasady ponownego wywłasowania — wartością domyślną jest NietrwałaLRU.
...
### — Moduł
Opcjonalny zestaw modułów redis umożliwiających włączenie w tej bazie danych — moduły można dodawać tylko podczas tworzenia.
Aby skonstruować, zobacz sekcję NOTATKI, aby uzyskać informacje o właściwościach modułu i utworzyć tabelę skrótu.
...
### — Strefa
Strefy, w których ten klaster zostanie wdrożony.
...
MODULE <IModule[]>: Opcjonalny zestaw modułów redis umożliwiających włączenie w tej bazie danych — moduły można dodawać tylko podczas tworzenia.
