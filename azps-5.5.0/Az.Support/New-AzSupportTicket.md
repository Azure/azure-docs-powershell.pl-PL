...
Aby uzyskać szczegółową dokumentację na temat konstruowania systemu Payload dla różnych typów [przydziałów, kliknij tutaj](https://aka.ms/supportrpquotarequestpayload)
...
### Przykład 11. Tworzenie biletu obsługi przydziału w celu zwiększenia przydziału dla jednostek DTU dla programu SQL Data Warehouse. Użyj Get-AzSupportService i Get-AzSupportProblemClassification, aby pobrać prawidłowe identyfikatory GUID dla klasyfikacji problemów programu Quota SQL Date Warehouse.
```powershell
...
### Przykład 12. Tworzenie biletu obsługi przydziału w celu zwiększenia przydziału dla serwerów programu SQL Data Warehouse. Użyj Get-AzSupportService i Get-AzSupportProblemClassification, aby pobrać prawidłowe identyfikatory GUID dla klasyfikacji problemów programu SQL Data Warehouse przydziału.
```powershell
PS C:\> New-AzSupportTicket -Name "test1" -Title "Test" -Description "Test" -Severity "minimal" -ProblemClassificationId "/providers/Microsoft.Support/services/{quota_service_guid}/problemClassifications/{sql_datawarehouse_problemClassification_guid}" -QuotaTicketDetail @{QuotaChangeRequestVersion = "1.0" ; QuotaChangeRequestSubType = "Servers" ; QuotaChangeRequests = (@{Region = "westus"; Payload = "{`"NewLimit`":200}"})} -CustomerContactDetail @{FirstName = "first" ; LastName = "last" ; PreferredTimeZone = "pacific standard time" ; PreferredSupportLanguage = "en-us" ; Country = "USA" ; PreferredContactMethod = "Email" ; PrimaryEmailAddress = "user@contoso.com"}
...
### Przykład 14. Utwórz bilet obsługi przydziału w celu zwiększenia przydziału dla podstawowych maszyn wirtualnych dla określonej rodziny maszyny wirtualnej dla usługi Machine Learning. Użyj Get-AzSupportService i Get-AzSupportProblemClassification, aby pobrać prawidłowe identyfikatory GUID dla klasyfikacji problemów z usługą Quota Machine Learning.
```powershell
...
Wywęszony język niestandardowego.
