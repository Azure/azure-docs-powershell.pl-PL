...
Polecenie **cmdlet New-AzApiManagementApiRevision** tworzy poprawkę interfejsu API dla istniejącego interfejsu API w kontekście zarządzania interfejsem API.

## PRZYKŁADY

### Przykład 1. Tworzenie pustej poprawki interfejsu API dla interfejsu API
```powershell
...
To polecenie tworzy poprawkę interfejsu `2` `echo-api` API interfejsu API.

### Przykład 2. Tworzenie poprawki interfejsu API z istniejącego interfejsu API i kopiowanie wszystkich operacji, tagów i zasad
```powershell
...
To polecenie tworzy poprawkę interfejsu `5` `echo-api` API interfejsu API.

## PARAMETERS

### -ApiId
Identyfikator interfejsu API, którego poprawka ma zostać utworzona.
...
### -ApiRevision
Identyfikator poprawek interfejsu API.
...
### -ApiRevisionDescription
Opis poprawek interfejsu API. Ten parametr jest opcjonalny.
...
### -ServiceUrl
Adres URL usługi sieci Web, która ujawnia interfejs API w usłudze Backend. Ten adres URL będzie używany tylko w usłudze Azure API Management i nie będzie publiczny. Musi to być od 1 do 2000 znaków. Ten parametr jest wymagany.
...
### -SourceApiRevision
Identyfikator poprawki interfejsu API źródłowego interfejsu API. Ten parametr jest opcjonalny.
