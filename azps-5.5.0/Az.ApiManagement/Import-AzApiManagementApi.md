...
## OPIS
Polecenie cmdlet **Import-AzApiManagementApi** importuje interfejs API zarządzania interfejsem Azure API Management z pliku lub adresu URL w języku WADL (Web Application Description Language), w języku WSDL (Web Services Description Language) lub w formacie Swagger.
...
### Przykład 2. Importowanie interfejsu API z pliku Swagger
```powershell
PS C:\>$ApiMgmtContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
To polecenie importuje interfejs API z określonego pliku swaggera.
...
### Przykład 4. Importowanie interfejsu API z linku Open Api
```powershell
...
To polecenie importuje interfejs API z określonego łącza specyfikacji Open 3.0.

### Przykład 5. Importowanie interfejsu API z otwartego linku interfejsu API do zestawu apiVersion
...
To polecenie importuje interfejs API z określonego dokumentu specyfikacji Open 3.0 i tworzy nowy interfejs ApiVersion.
...
### -ApiRevision
Identyfikator poprawki interfejsu API. Ten parametr jest opcjonalny. Jeśli nie zostanie określona, importowanie zostanie wykonane na aktualnie aktywnej poprawce lub w nowym interfejsie API.
...
### -ApiType
Ten parametr jest opcjonalny z domyślną wartością Http. Opcja mydła ma zastosowanie tylko podczas importowania pliku WSDL i powoduje utworzenie interfejsu API przekazać protokołu SOAP.
...
### -ApiVersionSetId
Identyfikator zasobu dla powiązanego zestawu wersji interfejsu API. Ten parametr jest opcjonalny.
...
### — Ścieżka
Określa ścieżkę interfejsu API sieci Web jako ostatnią część publicznego adresu URL interfejsu API.
Ten adres URL jest używany przez klientów interfejsu API do wysyłania żądań do usługi sieci Web.
Musi to być od 1 do 400 znaków.
...
### — Protokół
Protokoły WEB API (http, https). Protokoły, dla których jest dostępny interfejs API. Ten parametr jest opcjonalny. Jeśli zostanie podany, zastąpi on protokoły określone w dokumencie specyfikacji.
...
### -ServiceUrl
Adres URL usługi sieci Web eksponujące interfejs API. Ten adres URL będzie używany tylko w usłudze Azure API Management i nie będzie publiczny. Ten parametr jest opcjonalny. Jeśli zostanie podany, zastąpi ono adres ServiceUrl określony w dokumencie Specyfikacje.
...
### -WsdlEndpointName
Nazwa lokalna punktu końcowego WSDL (portu), który ma zostać zaimportowany. Musi to być od 1 do 400 znaków. Ten parametr jest opcjonalny i wymagany tylko do importowania pliku Wsdl. Wartość domyślna to $null.
...
### -WsdlServiceName
Nazwa lokalna usługi WSDL do zaimportowania. Musi to być od 1 do 400 znaków. Ten parametr jest opcjonalny i wymagany tylko do importowania pliku Wsdl. Wartość domyślna to $null.
...
### System.Nullable'1[[Microsoft.Azure.Commands.ApiManagement.ServiceManagement.Models.PsApiManagementApiType, Microsoft.Azure.PowerShell.Cmdlet.ApiManagement.ServiceManagement, Version=1.0.0.0, Culture=neutral, PublicKeyToken=null]]
