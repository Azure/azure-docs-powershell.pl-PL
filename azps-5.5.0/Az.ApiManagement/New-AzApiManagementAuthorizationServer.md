...
## OPIS
Polecenie **cmdlet New-AzApiManagementAuthorizationServer** tworzy serwer autoryzacji usługi Azure API Management.
...
Tworzy serwer autoryzacji. (wygenerowane automatycznie)
...
Określa tablicę metod wysyłania tokenu dostępu.
...
Określa punkt końcowy autoryzacji do uwierzytelniania właścicieli zasobów i uzyskiwania uprawnień autoryzacji.
...
Prawidłowe wartości to: GET, POST.
...
psdx_paramvalues podstawowe i podstawowe.
...
### -ClientId
Określa identyfikator klienta konsoli deweloperskiej, która jest aplikacją kliencyjną.
...
Określa punkt końcowy rejestracji klienta, który służy do rejestrowania klientów na serwerze autoryzacji i uzyskiwania poświadczeń klienta.
...
### -ClientSecret
Określa klucz tajny klienta konsoli dewelopera, który jest aplikacją kliencyjną.
...
### -DefaultScope
Określa domyślny zakres dla serwera autoryzacji.
...
### — Nazwa
Określa nazwę serwera autoryzacji do utworzenia.
...
Musisz określić ten parametr, jeśli parametr ResourceOwnerPassword jest określony przez parametr *GrantTypes.*
...
Ten parametr należy określić, jeśli parametr ResourceOwnerPassword jest określony przez parametr *GrantTypes.*
...
### - ServerId
Określa identyfikator serwera autoryzacji do utworzenia.
...
### -SupportState
Wskazuje, czy parametr *województwa ma być* obsługujący.
...
### -TokenBodyParameters
Określa dodatkowe parametry treści przy **użyciu formatu aplikacji/x-www-form-urlencoded.**
...
### -TokenEndpointUrl
Określa adres URL punktu końcowego tokenu, który jest używany przez klientów do uzyskiwania tokenów dostępu w zamian za prezentowanie uprawnień autoryzacji lub odświeżanie tokenów.
