...
## OPIS
To polecenie spowoduje usunięcie określonego punktu końcowego chmury z grupy synchronizacji. Ten proces nie dotyczy plików platformy Azure, które współużytkuje odwołania do punktów końcowych w chmurze. To polecenie jest przeznaczone tylko do zlikwidowania. Usunięcie punktu końcowego w chmurze jest operacją szkodliwej. Punkty końcowe serwera nie mogą być synchronizowane bez co najmniej jednego punktu końcowego w chmurze. Tej operacji nie należy wykonywać w celu rozwiązania problemów z synchronizacją. Ponowne dodanie tego udziału plików platformy Azure do tej samej grupy synchronizacji jako punktu końcowego chmury może powodować konflikty plików i inne niezamierzone konsekwencje.
...
To polecenie spowoduje usunięcie punktu końcowego chmury.
...
### -InputObject
Obiekt wejściowy CloudEndpoint, który zwykle przechodzi przez potok.
...
### -PassThru
W normalnym wykonywaniu to polecenie cmdlet nie zwraca żadnej wartości przy sukcesie. Jeśli podasz parametr PassThru, po pomyślnym wykonaniu polecenie cmdlet zapisze wartość w potoku.
