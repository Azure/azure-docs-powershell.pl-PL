...
Uruchamia operację zasad replikacji aktualizacji przy użyciu określonych parametrów i zwraca zadanie asr służące do śledzenia operacji.
...
Uruchamia operację zasad azure aktualizacji azure na replikację przy użyciu określonych parametrów i zwraca zadanie asr służące do śledzenia operacji.
...
Uruchamia zasady replikacji platformy Azure Azure przy użyciu określonych parametrów i zwraca zadanie asr służące do śledzenia operacji.

## PARAMETERS

### -ApplicationConsistentSnapshotFrequencyInHours
Określa częstotliwość (w godzinach), z jaką mają być tworzyć spójne punkty odzyskiwania aplikacji.
...
### -AzureToV Jaka jest
Określa odzyskiwanie po awarii z platformy Azure do vMWare.
...
### -HyperVToAzure
Przełącz parametr wskazujący, że określone zasady są używane do replikowania maszyn wirtualnych hyper-V do platformy Azure.
...
### -MultiVmSyncStatus
Określa stan synchronizacji wieluVm dla zasad.
...
Określa punkty odzyskiwania liczb, które mają być zachowywane.
...
Określa identyfikator konta magazynu platformy Azure dla miejsca docelowego replikacji. Używany jako konto docelowego magazynu dla replikacji, jeśli nie podano alternatywy podczas włączania replikacji przy użyciu New-AzRecoveryServicesASRReplicationProtectedItem cmdlet.
...
Czas (w godzinach), aby zachować punkty odzyskiwania po utworzeniu.
...
### -ReplicaDeletion
Określa, czy replika maszyny wirtualnej powinna zostać usunięta przy wyłączaniu replikacji z zarządzanej witryny maszyny wirtualnej do innej.
...
### -ReplicationPort
Określa port używany na replikację.
...
Nie może być późniejsza niż 24 godziny od rozpoczęcia zadania.
...
Wartość progowa RPO w minutach, dla których ma być ostrzegana.
...
Przełącz parametr wskazujący, że określone zasady są używane do replikowania zarządzanych maszyn wirtualnych WIRTUALNEJ funkcji Hyper-V między dwiema witrynami hyper-V.
...
### -VChatToAzure
Przełącz parametr wskazujący, że określone zasady są używane do replikowania wirtualnych maszyn wirtualnych do platformy Azure.
