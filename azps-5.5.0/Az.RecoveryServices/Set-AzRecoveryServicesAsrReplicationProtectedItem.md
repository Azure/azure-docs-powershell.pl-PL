...
Określa szczegóły konfiguracji testu trybu failover i nic trybu failover.
...
### -LicenseType
Specyfikuj wybór typu licencji, który ma być używany dla maszyn wirtualnych systemu Windows Server. Jeśli masz prawo do używania korzyści z używania hybrydowego systemu Azure (HUB) do migracji i chcesz określić, że ustawienie CENTRUM ma być używane w przypadku awarii tego chronionego elementu, ustaw dla tego chronionego elementu typ licencji: WindowsServer.
...
Aby wrócić do wartości domyślnych, możesz określić wartość NotSelected (NieS wybrana).
...
Identyfikator zasobu usługi w chmurze odzyskiwania w celu trybu failover tej maszyny wirtualnej.
...
Określa identyfikator sieciowej grupy zabezpieczeń, która ma zostać skojarzona z siecią NIC odzyskiwania.
...
Określa statyczny adres IP, który powinien zostać przypisany do podstawowej karty NIC podczas odzyskiwania.
...
Określa identyfikator publicznego zasobu adresu IP, który ma zostać skojarzony z skojarzoną z siecią NIC odzyskiwania.
...
Określa, czy maszynę wirtualną platformy Azure utworzoną w ramach trybu failover należy używać zarządzanych dyskietek.
