...
## SYNOPSIS
Tworzy konfigurację ASR NIC zawierającą szczegóły konfiguracji związanej z trybu failover i testowanie jej.
...
## OPIS
Polecenie **cmdlet New-AzRecoveryServicesAsrVMNicConfig** tworzy obiekt konfiguracji ASR NIC zawierający szczegóły związane z trybu failover i testowanie trybu failover. W przypadku, gdy żadne informacje nie zostaną przekazane, odpowiednie wartości są wybierane z elementu chronionego replikacją, aby uniknąć aktualizowania tych wartości do wartości null.
...
Tworzy obiekt ASRVmNicConfig z trybu failover i przetestuj faiover sieciowe ustawienia skonfigurowane dla nic. Każda właściwość, która nie jest przekazywana powyżej, jest pobierana z przekazywanego elementu chronionego.
...
Tworzy obiekt ASRVmNicConfig z testowymi ustawieniami sieci faiover skonfigurowanymi dla zmiany nazwy przez nic. Każda właściwość, która nie jest przekazywana powyżej, jest pobierana z przekazywanego elementu chronionego.


### Przykład 3

Tworzy konfigurację ASR NIC zawierającą szczegóły konfiguracji związanej z trybu failover i testowanie jej. (wygenerowane automatycznie)
...
Określa, czy tryb sieci przyspieszonej jest włączony w przypadku karty NIC odzyskiwania.
...
Określa, czy tryb sieci przyspieszonej jest włączony w testowej chmurze trybu failover.
...
### - NicId
Określ identyfikator GUID NIC ASR.
...
Określa identyfikatory pul adresów zaplecza dla karty NIC odzyskiwania.
...
Określa identyfikator NSG skojarzonego z skojarzoną z siecią NIC odzyskiwania.
...
### -RecoveryNicName
Określa nazwę nic odzyskiwania.
...
Określa nazwę grupy zasobów NIC odzyskiwania.
...
Określa adres IP sieci NIC odzyskiwania.
...
Określa identyfikator publicznego adresu IP skojarzonego z siecią NIC odzyskiwania.
...
Określanie elementu chronionego replikacją asr.
...
### -ReuseExistingNic
Określa, czy podczas trybu failover można używać istniejącej karty nic.
...
Określa identyfikator NSG skojarzony z testową skojarzoną z skojarzoną z testową skojarzoną z siecią NIC trybu failover.
...
### -TfoNicName
Określa nazwę testu trybu failover NIC.
...
Określa nazwę testowej grupy zasobów NIC trybu failover.
...
### -TfoNicDressIPAddress
Określa adres IP testowej nic trybu failover.
...
Określa identyfikator publicznego adresu IP skojarzonego z testową skojarzoną z siecią NIC trybu failover.
...
### -TfoReuseExistingNic
Określa, czy podczas testowania trybu failover może być używana istniejąca karta NIC.
...
### -TfoVMSubnetName
Określa nazwę testowej podsieci trybu failover.
