...
To polecenie usuwa obiekt blob oznaczony jego nazwą.
...
To polecenie używa symbolu wieloznacznego gwiazdki (*) i potoku w celu pobrania obiektów blob lub obiektów blob, a następnie ich usuwa.

### Przykład 4. Usuwanie wersji pojedynczego obiektu blob
```
PS C:\> Remove-AzStorageBlob -Container "containername" -Blob blob2 -VersionId "2020-07-03T16:19:16.2883167Z"
```

To polecenie usuwa pojedynczy obiekt blob o numerze VersionId.

### Przykład 5. Usuwanie migawki pojedynczego obiektu blob
```
PS C:\> Remove-AzStorageBlob -Container "containername" -Blob blob1 -SnapshotTime "2020-07-06T06:56:06.8588431Z"
```

To polecenie usuwa migawkę pojedynczego obiektu blob za pomocą migawki czasu migawki.
...
Aby go uzyskać, Get-AzStorageContainer polecenie cmdlet.
...
Aby utworzyć New-AzStorageContext cmdlet, możesz użyć polecenia cmdlet.
...
### - DeleteSnapshot
Określa, że wszystkie migawki mają zostać usunięte, ale nie obiekt blob podstawowy.
Jeśli ten parametr nie jest określony, obiekt blob podstawowy i jego migawki są usuwane razem.
Użytkownik zostanie wyświetlony monit o potwierdzenie operacji usunięcia.
...
Określa profil platformy Azure, który ma być odczytywany przez polecenie cmdlet.
Jeśli nie zostanie określone, polecenie cmdlet odczyta dane z profilu domyślnego.
