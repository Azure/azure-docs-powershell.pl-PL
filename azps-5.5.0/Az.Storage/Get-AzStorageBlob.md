...
## OPIS
Polecenie **cmdlet Get-AzStorageBlob** wyświetla listę obiektów blob w określonym kontenerze na koncie magazynu platformy Azure.
...
To polecenie używa nazwy obiektu blob i symbolu wieloznacznego w celu uzyskania obiektu blob.

### Przykład 2. Uzyskiwanie obiektów blob w kontenerze przy użyciu potoku
```
...
To polecenie używa potoku w celu uzyskania wszystkich obiektów blob (w tym obiektów blob w stanie Usunięte) w kontenerze.

### Przykład 3. Uzyskiwanie obiektów blob według prefiksu nazwy
```
PS C:\>Get-AzStorageBlob -Container "ContainerName" -Prefix "blob"
```

To polecenie używa prefiksu nazwy do uzyskania obiektów blob.

### Przykład 4. Lista obiektów blob w wielu partiach
```
...
W tym przykładzie parametry *MaxCount* i *ContinuationToken* są używane do listy obiektów blob magazynu platformy Azure w wielu partiach.
Pierwsze cztery polecenia przypiszą wartości do zmiennych, których będzie można użyć w tym przykładzie.
Piąte polecenie określa instrukcji **Do-While,** która używa polecenia cmdlet **Get-AzStorageBlob** w celu uzyskania obiektów blob.
Instrukcja zawiera token kontynuacji przechowywany w $Token zmiennej.
...
### Przykład 5. Pobierz wszystkie obiekty blob w kontenerze zawierają wersję obiektów blob
```
...
To polecenie pobiera wszystkie obiekty blob w kontenerze, w tym wersję obiektów blob.

### Przykład 6. Uzyskiwanie wersji pojedynczego obiektu blob
```
...
To polecenie pobiera pojedynczy obiekt blob z polem VersionId.

### Przykład 7. Uzyskiwanie migawki pojedynczego obiektu blob
```
...
To polecenie pobiera migawkę pojedynczego obiektu blob z migawką MigawkaGodziny.

## PARAMETERS

### — obiekt blob
Określa nazwę lub deseń nazw, których można używać do wyszukiwania z symbolami wieloznacznych.
Jeśli nie określono nazwy obiektu blob, polecenie cmdlet wyświetla listę wszystkich obiektów blob w określonym kontenerze.
Jeśli dla tego parametru zostanie określona wartość, polecenie cmdlet wyświetla listę wszystkich obiektów blob o nazwach, które pasują do tego parametru. Ten parametr obsługuje symbole wieloznaczne w dowolnym miejscu ciągu.
...
### — kontekst
Określa konto magazynu platformy Azure, z którego chcesz uzyskać listę obiektów blob.
Możesz użyć polecenia cmdlet New-AzStorageContext, aby utworzyć kontekst magazynu.
...
Użyj tego parametru i *parametru MaxCount,* aby wyświetlić listę obiektów blob w wielu partiach.
...
### - IncludeDeleted
Dołącz usunięty obiekt blob, domyślnie obiekt blob get nie będzie uwzględniał usuniętego obiektu blob.
...
### - IncludeVersion
Wersje obiektów blob będą wyświetlane tylko wtedy, gdy ten parametr istnieje, domyślnie get blob nie będzie zawierał wersji obiektów blob.
...
### —Prefiks
Określa prefiks dla nazw obiektów blob, które mają zostać uzyskać.
Ten parametr nie obsługuje wyszukiwania przy użyciu wyrażeń regularnych ani symboli wieloznacznych.
Oznacza to, że jeśli kontener zawiera tylko obiekty blob o nazwach "My", "MyBlob1" i "MyBlob2" i określisz "-Prefix My*", polecenie cmdlet nie zwróci żadnych obiektów blob.
Jeśli jednak określisz "-Prefix My", polecenie cmdlet zwróci wartości "My", "MyBlob1" i "MyBlob2".
