...
## OPIS
Polecenie New-AzEventHubAuthorizationRule cmdlet tworzy nową regułę autoryzacji centrum zdarzeń.
...
Tworzy regułę autoryzacji MyAuthRuleName, która udziela praw Dosłuchiwanie i Wysyłanie do przestrzeni nazw MyNamespaceName z grupą zasobów \` \` \` \` \` MyResourceGroupName. \`
...
Tworzy regułę autoryzacji MyAuthRuleName, która udziela uprawnień Dosłuchiwanie i Wysyłanie do centrum zdarzeń MyEventHubName w przestrzeni nazw MyNamespaceName z grupą zasobów \` \` \` \` \` \` \` MyResourceGroupName. \`
