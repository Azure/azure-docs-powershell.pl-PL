...
W witrynie Name.SqlDataBaseDnsSuffix (na przykład Name.database.windows.net) i Name.secondary.SqlDataBaseDnsSuffix są tworzone dwa punkty końcowe TDS usługi Azure SQL Database. Te punkty końcowe mogą być używane do łączenia się odpowiednio z regionami podstawowymi i pomocniczmi grupy trybu failover. Jeśli na region podstawowy ma wpływ awarii, zostanie wyzwolona automatyczna funkcja trybu failover dla punktów końcowych i baz danych zgodnie z zasadami grupy trybu failover wystąpienia i okresem prolongaty.
...
To polecenie tworzy nową grupę trybu failover wystąpienia z zasadami trybu failover "Automatic" dla pary Zarządzane wystąpienie.
...
Nazwa grupy zasobów pomocniczych grupy Wystąpienie trybu failover.
