...
### ByTopologyAndServiceNames (domyślne)
```
...
## OPIS
Polecenie **cmdlet New-AzDeploymentManagerServiceUnit** tworzy usługę w topologii usługi i zwraca obiekt reprezentujący jednostkę usługi.
Określ jednostkę usługi według jej nazwy, nazwy usługi, topologii usługi, w których się znajduje, oraz nazwy grupy zasobów. 

Polecenie cmdlet zwraca obiekt ServiceUnit. Możesz zmodyfikować ten obiekt lokalnie, a następnie zastosować zmiany do usługi za pomocą Set-AzDeploymentManagerService cmdlet.
...
To polecenie cmdlet tworzy nową jednostkę usługi o nazwie ContosoService2Storage w grupie ContosoResourceGroup w obszarze usługi ContosoService2 w topologii ContosoServiceTopology w lokalizacji Centralnej Stanów Zjednoczonych. Pliki szablonu i parametrów są zdefiniowane jako ścieżki względne do lokalizacji źródła artefaktu, do których odwołuje się topologia usługi ContosoServiceTopology. Zasoby zdefiniowane w tym szablonie należy wdrożyć w grupie docelowej service2ResourceGroup z trybem wdrażania ustawionym na wartość Przyrostowe.
...
To polecenie cmdlet tworzy nową jednostkę usługi o nazwie ContosoService2Storage w grupie ContosoResourceGroup w obszarze usługi ContosoService2 w topologii ContosoServiceTopology w lokalizacji Centralnej Stanów Zjednoczonych. Odwołania do szablonu i parametrów są udostępniane jako wartości Uri SAS, ponieważ w topologii usługi ContosoServiceTopology1 nie podano wartości ResourceId źródła artefaktu. Zasoby zdefiniowane w tym szablonie należy wdrożyć w grupie docelowej service2ResourceGroup z trybem wdrażania ustawionym na wartość Ukończono.
...
### -DeploymentMode
Tryb wdrażania do użycia podczas wdrażania zasobów w jednostce usługi.
...
### — Lokalizacja
Lokalizacja zasobu jednostki usługi.
...
Ścieżka do pliku parametrów względem źródła artefaktu.
Wymaga odwoływać się do źródła ArtifactSource w ologii ServiceTopologii.
...
### -ParametersUri
Uri SAS do pliku parametrów.
Jeśli w ologii ServiceTopologii dowoływano się do wartości ArtifactSourceId, określ ścieżkę względną przy użyciu parametru ParametersArtifactSourceRelativePath.
...
### -ServiceName
Nazwa usługi, do których należy ta jednostka usługi.
...
### -ServiceTopologyName
Nazwa topologii usługi, do których należy ta jednostka usługi.
...
### -TargetResourceGroup
Określa lokalizację, w której zostaną wdrożone zasoby w ramach jednostki usługi.
...
Ścieżka do pliku szablonu względem źródła artefaktu.
Wymaga odwoływać się do źródła ArtifactSource w ologii ServiceTopologii.
...
### -TemplateUri
Uri SAS do pliku szablonu.
Jeśli w ologii ServiceTopologii dowoływano się do wartości ArtifactSourceId, określ ścieżkę względną przy użyciu szablonu TemplateArtifactSourceRelativePath.
