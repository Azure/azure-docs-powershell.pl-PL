...
### Przykład 1. Uzyskiwanie aplikacji funkcji według nazwy i zatrzymywanie jej.
```powershell
PS C:\> Get-AzFunctionApp -Name MyAppName -ResourceGroupName MyResourceGroupName | Stop-AzFunctionApp -Force
```

To polecenie pobiera aplikację funkcji według nazwy i zatrzymuje ją.

### Przykład 2. Zatrzymywanie działania aplikacji funkcji według nazwy.
```powershell
PS C:\> Stop-AzFunctionApp -Name MyAppName -ResourceGroupName MyResourceGroupName -Force
```

To polecenie zatrzymuje aplikację funkcji według nazwy.
...
### — Wymuszanie
Wymusza zatrzymanie działania aplikacji funkcji przez polecenie cmdlet bez monitu o potwierdzenie.
