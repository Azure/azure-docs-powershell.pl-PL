...
## SYNOPSIS
Tworzy obiekt VHubRoute, który może być przekazywany jako parametr do New-AzVHubRouteTable użytkownika.
...
Powyższe polecenie spowoduje utworzenie obiektu usługi VHubRoute z wartością nextHop jako określoną zaporą, którą można następnie dodać do zasobu tabeli VHubRouteTable.
...
Powyższe polecenie spowoduje utworzenie obiektu usługi VHubRoute z następnąhop jako określonym hubVnetConnection, który można następnie dodać do zasobu tabeli VHubRouteTable.
...
```
Powyższe polecenia uzyskają konfigurację RoutingConfiguration istniejącej już usługi AzVHubRoute, a następnie dodają trasę statyczną na połączeniu. Jeśli natomiast masz nadzieję na utworzenie nowego połączenia przy użyciu trasy statycznej, zobacz przykład 1 w [tym miejscu.](New-AzRoutingConfiguration.md)
## PARAMETERS
