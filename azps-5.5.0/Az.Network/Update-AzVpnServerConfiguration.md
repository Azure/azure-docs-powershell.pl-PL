...
### ByVpnServerConfigurationName (Domyślna)
```
...
## OPIS
Polecenie cmdlet **Update-AzVpnServerConfiguration** umożliwia aktualizowanie istniejącej konfiguracji VpnServerConfiguration przy użyciu różnych parametrów VpnProtocols, VpnAuthenticationTypes, IpsecPolicies oraz ustawianie wybranych parametrów powiązanych z uwierzytelnianiem vpn zgodnie z wymaganiami klienta w zakresie łączności typu Point to site.
...
Powyższe polecenie zaktualizuje istniejącą konfigurację VpnServerConfiguration przy użyciu połączenia VpnProtocol jako IkeV2.

## PARAMETERS

### -AadAudience
Odbiorcy AAD do uwierzytelniania P2S AAD.
...
### -AadIssuer
Wystawca AAD dla uwierzytelniania P2S AAD.
...
### - AadTenant
Dzierżawa AAD do uwierzytelniania P2S AAD.
...
### -InputObject
Obiekt konfiguracji serwera vpn do modyfikacji
...
Lista ścieżek plików RadiusClientRootCertificate
...
### -RadiusServerList
P2S Zewnętrzne serwery o wielu promieniach.
...
### -ResourceId
Identyfikator zasobu Azure dla konfiguracji serwera vpn.
...
Lista zasad IPSec dla konfiguracji vpnServerConfiguration.
...
Lista vpnClientCertificates, które mają zostać odwołane ścieżki plików
...
Lista elementów VpnClientRootCertificates, które mają zostać dodane ścieżki plików
