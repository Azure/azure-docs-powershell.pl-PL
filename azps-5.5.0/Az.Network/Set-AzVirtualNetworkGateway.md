...
## OPIS
Polecenie **cmdlet Set-AzVirtualNetworkGateway** aktualizuje bramę sieci wirtualnej.
...
Pierwsze polecenie otrzymuje wirtualną bramę sieciową o nazwie Gateway01 należącą do grupy zasobów ResourceGroup001 i przechowuje ją u zmiennej o nazwie $Gateway Drugie polecenie aktualizuje wirtualną bramę sieciową przechowywaną w zmiennym $Gateway.
To polecenie ustawia również asn na 1337.
...
Pierwsze polecenie otrzymuje wirtualną bramę sieciową o nazwie Gateway01 należącą do grupy zasobów ResourceGroup001 i przechowuje ją w zmiennej o nazwie $Gateway Drugie polecenie tworzy obiekt zasad ipsec sieci Vpn zgodnie z określonymi parametrami ipsec.
Trzecie polecenie aktualizuje bramę sieci wirtualnej przechowywaną w zmiennym $Gateway.
To polecenie ustawia również niestandardowe zasady ipsec sieci vpn określone w obiekcie $vpnclientipsecpolicy w wirtualnej bramie sieciowej.
...
Pierwsze polecenie otrzymuje wirtualną bramę sieciową o nazwie Gateway01 należącą do grupy zasobów ResourceGroup001 i przechowuje je w zmiennej o nazwie $Gateway Drugie polecenie aktualizuje bramę sieci wirtualnej o tagach @{ testtagKey="SomeTagKey"; testtagValue="SomeKeyValue" }.

### Przykład 4. Dodawanie/aktualizowanie konfiguracji uwierzytelniania AAD dla usługi VpnClient istniejącej bramy sieci wirtualnej
```
PS C:\>$Gateway = Get-AzVirtualNetworkGateway -ResourceGroupName "ResourceGroup001" -Name "Gateway001"
...
Pierwsze polecenie otrzymuje wirtualną bramę sieciową o nazwie Gateway01 należącą do grupy zasobów ResourceGroup001 i przechowuje je w zmiennej o nazwie $Gateway Drugie polecenie aktualizuje bramę sieci wirtualnej Gateway01 przy użyciu params konfiguracji uwierzytelniania AAD:aadTenantUri, aadAudienceId, aadIssuerUri for VpnClient.
Trzecie polecenie usuwa konfigurację uwierzytelniania AAD z usługi VpnClient bramy sieci wirtualnej.

### Przykład 5. Dodawanie/aktualizowanie adresów IpConfigurationBgpPeeringAddresses do istniejącej bramy sieci wirtualnej
```
PS C:\>$Gateway = Get-AzVirtualNetworkGateway -ResourceGroupName "ResourceGroup001" -Name "Gateway001"
...
Pierwsze polecenie otrzymuje wirtualną bramę sieciową o nazwie Gateway01 należącą do grupy zasobów ResourceGroup001 i przechowuje ją u zmiennej o nazwie $Gateway Drugie polecenie przypisuje wartość identyfikatora IpConfiguration Bramy sieci wirtualnej Gateway01 do zmiennej ipconfigurationId1.
Trzecie polecenie przypisuje listę adresów do listy adresowej1.
Czwarte polecenie utworzyło obiekt PSIpConfigurationBgpPeeringAddress.
Piąte polecenie ustawiło tę nową utworzoną konfigurację PSIpConfigurationBgpPeeringAddress na Adres IpConfigurationBgpPeeringAddresses i zaktualizuj bramę.
...
### — Asn
AsN bramy sieci wirtualnej, która służy do skonfigurowania sesji BGP w przypadku podkołowy IPsec
...
Oflaguj, aby wyłączyć funkcję Aktywne aktywne w bramie sieci wirtualnej
...
### -GatewayDefaultSite
Witryna domyślna do wymusania na wymusze wydychania.
Jeśli zostanie określona witryna domyślna, cały ruch internetowy z sieci vnet bramy jest przekierowyowyny do tej witryny.
...
### -RadiusServerSecret
Tajny serwer o promieniu zewnętrznym P2S.
...
Oflaguj, aby usunąć uwierzytelnianie AAD dla klienta P2S z bramy sieci wirtualnej.
...
Obiekt bramy sieci wirtualnej, w przypadku których są bazowe modyfikacje.
Można to pobrać za pomocą programu Get-AzVirtualNetworkGateway
...
### -VpnClientAddressPool
Miejsce na adresy, z których można przydzielić adresy IP klientów sieci VPN.
Nie powinno to nakładać się na sieć wirtualną ani zakresy lokalne.
...
### -VpnClientProtocol
Lista protokołów odsyłki klienta VPN P2S
...
Klient SIECI VPN prezentujący certyfikat, który jest taki, jak jeden z nich, zostanie mu z dala od komputera.
...
### -VpnClientRootCertificates
Lista certyfikatów głównych klienta sieci VPN, których można używać do uwierzytelniania klienta sieci VPN.
Klienci sieci VPN łączący muszą prezentować certyfikaty wygenerowane na podstawie jednego z tych certyfikatów głównych.
