...
Utwórz podstawowy typ węzła z 3 węzłami.
...
Utwórz podstawowy typ węzła z 5 węzłami i określanie właściwości położenia, zastosowań, aplikacji i portów pierwotnych.
...
### — Pojemność
Tagi wydajności zastosowane do węzłów typu węzła jako pary klucz/wartość są używane przez menedżera zasobów klastrów do zrozumienia, ile zasobu ma węzeł.
Aktualizacja spowoduje zastąpienie bieżących wartości.
...
### — DiskSize
Rozmiar dysku każdej maszyny wirtualnej typu węzła w kb/s.
...
### -PlacementProperty
Tagi położenia stosowane do węzłów typu węzła jako pary klucz/wartość, których można używać do wskazywania miejsc, w których mają być uruchamiane określone usługi (obciążenie pracą).
Aktualizacja spowoduje zastąpienie bieżących wartości.
...
### — podstawowy
Określ, czy typ węzła jest podstawowy.
W tym typie węzła będą uruchamiane usługi systemowe.
Tylko jeden typ węzła powinien być oznaczony jako podstawowy.
Podstawowy typ węzła nie może być usuwany ani zmieniany dla istniejących klastrów.
...
### -VmImageOffer
Typ oferty portalu Azure Virtual Machines Marketplace.
Domyślne: WindowsServer.
...
### -VmImagePublisher
Obraz wydawcy witryny Azure Virtual Machines Marketplace.
Domyślne: MicrosoftWindowsServer.
...
### -VmImageVersion
Obraz wersji witryny Azure Virtual Machines Marketplace.
Domyślne: najnowsze.
