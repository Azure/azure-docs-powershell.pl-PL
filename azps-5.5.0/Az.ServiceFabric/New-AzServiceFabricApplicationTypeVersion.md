...
## OPIS
To polecenie cmdlet tworzy nową wersję typu aplikacji przy użyciu pakietu określonego w parametrze -PackageUrl. To powinno być dostępne za pośrednictwem punktu końcowego REST dla usługi Azure Resource Manager do użycia podczas wdrażania i zawierało pakiet aplikacji spakowany i spakowany z rozszerzeniem sfpkg. To polecenie spowoduje utworzenie typu aplikacji, jeśli jeszcze nie istnieje.
...
W tym przykładzie utworzysz wersję typu aplikacji "v1" pod typem "testAppType". Wersja w pliku manifestu aplikacji zawarta w pakiecie powinna mieć tę samą wersję, co wersja określona w pliku -Version.
