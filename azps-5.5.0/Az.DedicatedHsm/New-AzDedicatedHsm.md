...
### Przykład 1. Tworzenie dedykowanego modułu HSM w istniejącej sieci wirtualnej
```powershell
...
To polecenie tworzy dedykowany moduł HSM w istniejącej sieci wirtualnej.

**UWAGA:** Obecnie istnieje ograniczenie, które zwraca przed pełnym inicjowaniem obsługi `New-AzDedicatedHsm` systemu HSM na platformie Azure.
Dlatego po utworzeniu nowego hsm należy sprawdzić jego stan i upewnić się, że jest przed `Get-AzDedicatedHsm` `Provisioning State` jego `Succeeded` użyciem.
...
### — Lokalizacja
Obsługiwana lokalizacja platformy Azure, w której należy utworzyć dedykowany moduł HSM.
...
### - NetworkInterface
Określa listę identyfikatorów zasobów dla interfejsów sieciowych skojarzonych ze dedykowanym modułem HSM.
Aby skonstruować, zobacz sekcję NOTATKI, aby uzyskać informacje o właściwościach NETWORKINTERFACE i utworzyć tabelę skrótów.
...
### - StampId
To pole będzie używane, gdy JNW nie obsługuje stref dostępności.
...
### -SubnetId
Identyfikator ARM w postaci /subscriptions/{SubscriptionId}/resourceGroups/{ResourceGroupName}/...
...
### Microsoft.Azure.PowerShell.Cmdlet.DedicatedHsm.Models.Api20181031.IDedicatedHsm
...
NETWORKINTERFACE <INetworkInterface[]>: Określa listę identyfikatorów zasobów dla interfejsów sieciowych skojarzonych ze dedykowanym modułem HSM.
  - `[PrivateIPAddress <String>]`: prywatny adres IP interfejsu
