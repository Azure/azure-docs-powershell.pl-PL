...
## OPIS
Tworzy lub aktualizuje metadane wystąpienia usługi.

## PRZYKŁADY

### Przykład 1. Tworzy nową usługę Azure healthcareapis fhir o nazwie MyService w grupie zasobów MyResourceGroup w lokalizacji westus2 z przepływnością oferty dla usługi azure healthcareapis = 400
```powershell
...
### Przykład 2. Tworzy nową usługę Azure healthcareapis fhir o nazwie MyService w grupie zasobów MyResourceGroup w lokalizacji westus2 z przepływnością oferty w usłudze azure healthcareapis = 400 oraz uri klucza magazynu kluczy "https:// \<my-keyvault> \<my-key> .vault.azure.net/keys/".
```powershell
...
Określ nagłówki HTTP, których można używać podczas żądania.
W dowolnym nagłówku użyj " *".
...
Określ, jak długo wynik żądania może być buforowany w ciągu kilku sekund.
...
Określ adresy URL witryn pochodzenia, które mogą uzyskać dostęp do tego interfejsu API, lub użyj "*", aby zezwolić na dostęp z dowolnej witryny.
...
### -AlegosKeyVaultKeyUri
HealthcareApis Fhir Service JednaksKeyVaultKeyUri.
URI klucza zarządzanego przez klienta dla bazy danych kopii zapasowej.
...
### -NamysłOfferThroughput
HealthcareApis Fhir Service DosOfferThroughput.
...
Nazwa konta magazynu eksportu usługi HealthcareApis Fhir.
...
### — Rodzaj
Rodzaj usługi HealthcareApis.
Wartość domyślna to Fhir
...
### — Lokalizacja
Lokalizacja usługi HealthcareApis.
...
### - ManagedIdentity
Używasz tożsamości zarządzanej?
...
### -PublicNetworkAccess
Typ dostępu sieciowego dla usługi Fhir. Często `Enabled` lub `Disabled` .
...
### — Tag
Znaczniki konta usługi HealthcareApis Fhir.
