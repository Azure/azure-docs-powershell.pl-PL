...
## SYNOPSIS
Pobiera metryk użycia puli dla konta wsadowego.
...
## OPIS
Polecenie **cmdlet Get-AzBatchPoolUsageMetric** pobiera metryki użycia, agregowane według puli w poszczególnych interwałach czasowych dla określonego konta.
Możesz uzyskać statystykę dla określonej puli i przedziału czasu.

## PRZYKŁADY

### Przykład 1. Uzyskiwanie metryk użycia puli dla przedziału czasu
```
...
Następne dwa polecenia tworzą obiekty **DateTime** przy użyciu Get-Date cmdlet.
Polecenia przechowują te wartości w $StartTime i $EndTime zmiennych do użycia z końcowym poleceniem.
Ostatnie polecenie zwraca wszystkie metryki użycia puli, zagregowane według puli, w interwale czasu między określonym czasem rozpoczęcia i zakończenia.

### Przykład 2. Uzyskiwanie metryk użycia puli przy użyciu filtru
```
...
To polecenie określa ciąg filtru określający tę pulę i używa tej samej $Context wartości, co w poprzednim przykładzie.
...
Określ czas o co najmniej dwie godziny wcześniej.
Jeśli nie określisz czasu zakończenia, to polecenie cmdlet użyje ostatniego dostępnego interwału agregacji.
...
### — Filtr
Określa klauzulę filtru OData, która ma być filtrowana w celu filtrowania metryk zwracanych przez to polecenie cmdlet.
Jedyną prawidłową właściwością **jest poolId z** wartością ciągu.
Możliwe operacje są następujące: eq, ge, gt, le, lt, startswith.
...
Określ czas co najmniej o dwie i pół godziny wcześniej.
Jeśli nie określisz czasu rozpoczęcia, to polecenie cmdlet będzie korzystać z ostatniego dostępnego interwału agregacji.
