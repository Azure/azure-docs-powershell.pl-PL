...
## Opis
Rozpocznij operację przyrostowa kopiowania z migawki obiektu BLOB strony do określonego obiektu BLOB strony docelowej.
Zobacz więcej szczegółów dotyczących funkcji https://docs.microsoft.com/en-us/rest/api/storageservices/fileservices/incremental-copy-blob .

## Przykłady

### Przykład 1. Rozpocznij operację przyrostowej kopii według nazwy obiektu BLOB i czasu migawki
```
...
To polecenie rozpoczyna przyrostowe operacje kopiowania według nazwy obiektu BLOB i czasu migawki

### Przykład 2. Rozpocznij operację przyrostowej kopii przy użyciu źródłowego identyfikatora URI
```
...
To polecenie rozpoczyna przyrostową operację kopiowania przy użyciu źródłowego identyfikatora URI

### Przykład 3: Rozpoczynanie przyrostowej operacji kopiowania przy użyciu rurociągu kontenera z GetAzureStorageContainer
```
...
To polecenie rozpoczyna przyrostową operację kopiowania przy użyciu potoku kontenera od GetAzureStorageContainer

### Przykład 4: Rozpoczynanie przyrostowej operacji kopiowania z obiektu CloudPageBlob do docelowego obiektu BLOB przy użyciu nazwy obiektu BLOB
```
...
To polecenie rozpoczyna przyrostową operację kopiowania z obiektu CloudPageBlob do docelowego obiektu BLOB z nazwą obiektu BLOB.

## PARAMETRÓW

### -AbsoluteUri
Bezwzględny identyfikator URI do źródła. Należy zauważyć, że poświadczenia powinny być podane w identyfikatorze URI, jeśli źródło wymaga dowolnych danych.
...
Maksymalny czas wykonywania wszystkich żądań po stronie klienta (w sekundach).
...
### -CloudBlob
Obiekt CloudBlob z biblioteki klienta magazynu platformy Azure. Możesz je utworzyć lub użyć Get-AzureStorageBlob polecenia cmdlet.
...
### -CloudBlobContainer
Obiekt CloudBlobContainer z biblioteki klienta magazynu platformy Azure. Możesz je utworzyć lub użyć Get-AzureStorageContainer polecenia cmdlet.
...
### -Context
Kontekst źródłowy usługi Azure Storage. Możesz je utworzyć, New-AzureStorageContext polecenie cmdlet.
...
### -DestContext
Kontekst miejsca docelowego usługi Azure Storage. Możesz je utworzyć, New-AzureStorageContext polecenie cmdlet.
...
Upłynął limit czasu serwera dla każdego żądania (w sekundach).
...
### Microsoft. platformy windowsazure. Storage. blob. CloudPageBlob
Microsoft. platformy windowsazure. Storage. blob. CloudBlobContainer system. String Microsoft. platformy windowsazure. Commands. Common. Storage. AzureStorageContext
