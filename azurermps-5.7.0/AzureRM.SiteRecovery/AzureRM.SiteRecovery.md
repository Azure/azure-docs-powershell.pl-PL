---
Module Name: AzureRM.SiteRecovery
Module Guid: d1de7560-48e1-48f3-bc8c-4eea3af2bbe1
Download Help Link:
  object Object: 
Help Version:
  object Object: 
Locale: en-US
content_git_url: https://github.com/Azure/azure-powershell/blob/preview/src/ResourceManager/SiteRecovery/Commands.SiteRecovery/help/AzureRM.SiteRecovery.md
original_content_git_url: https://github.com/Azure/azure-powershell/blob/preview/src/ResourceManager/SiteRecovery/Commands.SiteRecovery/help/AzureRM.SiteRecovery.md
ms.openlocfilehash: 4922f8a62d44eb180995937fbe59a3d3ca40acbb
ms.sourcegitcommit: 43f4bdf2a59dd82fd881512aa9761bf72eb5703c
ms.translationtype: MT
ms.contentlocale: pl-PL
ms.lasthandoff: 04/23/2019
ms.locfileid: "93522653"
---
# Moduł AzureRM. SiteRecovery
## Opis
W tym temacie opisano tematy pomocy dotyczące apletów poleceń odzyskiwania witryny platformy Azure.

## Polecenia cmdlet AzureRM. SiteRecovery
### [Edit-AzureRmSiteRecoveryRecoveryPlan](Edit-AzureRmSiteRecoveryRecoveryPlan.md)
Umożliwia edytowanie planu odzyskiwania witryny.

### [Get-AzureRmSiteRecoveryFabric](Get-AzureRmSiteRecoveryFabric.md)
Pobierz właściwości sieci szkieletowej odzyskiwania witryny Azure.

### [Get-AzureRmSiteRecoveryJob](Get-AzureRmSiteRecoveryJob.md)
Pobiera informacje o operacjach dotyczących bieżącego magazynu odzyskiwania witryny.

### [Get-AzureRmSiteRecoveryNetwork](Get-AzureRmSiteRecoveryNetwork.md)
Pobiera informacje o sieciach zarządzanych przez odzyskiwanie witryny dla bieżącego magazynu.

### [Get-AzureRmSiteRecoveryNetworkMapping](Get-AzureRmSiteRecoveryNetworkMapping.md)
Pobiera informacje o mapowaniach sieciowych odzyskiwania witryny dla bieżącego magazynu.

### [Get-AzureRmSiteRecoveryPolicy](Get-AzureRmSiteRecoveryPolicy.md)
Pobiera zasady ochrony przed odzyskiwaniem witryny.

### [Get-AzureRmSiteRecoveryProtectableItem](Get-AzureRmSiteRecoveryProtectableItem.md)
Pobierz elementy do zabezpieczenia w kontenerze ochrony.

### [Get-AzureRmSiteRecoveryProtectionContainer](Get-AzureRmSiteRecoveryProtectionContainer.md)
Pobiera kontenery ochrony dla bieżącego magazynu odzyskiwania witryny.

### [Get-AzureRmSiteRecoveryProtectionContainerMapping](Get-AzureRmSiteRecoveryProtectionContainerMapping.md)
Pobiera mapowania kontenerów ochrony usługi Azure Site Recovery.

### [Get-AzureRmSiteRecoveryProtectionEntity](Get-AzureRmSiteRecoveryProtectionEntity.md)
Pobiera listę jednostek chronionych lub chronionych w bieżącym magazynie odzyskiwania witryny.

### [Get-AzureRmSiteRecoveryRecoveryPlan](Get-AzureRmSiteRecoveryRecoveryPlan.md)
Pobiera plan odzyskiwania w ramach odzyskiwania witryny.

### [Get-AzureRmSiteRecoveryRecoveryPoint](Get-AzureRmSiteRecoveryRecoveryPoint.md)
Pobiera dostępne punkty odzyskiwania dla elementu chronionego replikacji.

### [Get-AzureRmSiteRecoveryReplicationProtectedItem](Get-AzureRmSiteRecoveryReplicationProtectedItem.md)
Pobiera właściwości elementów chronionych przy odzyskiwaniu witryny Azure.

### [Get-AzureRmSiteRecoveryServer](Get-AzureRmSiteRecoveryServer.md)
Pobiera informacje o serwerach odzyskiwania witryny zarejestrowanych w bieżącym magazynie.

### [Get-AzureRmSiteRecoveryServicesProvider](Get-AzureRmSiteRecoveryServicesProvider.md)
Pobiera informacje dotyczące dostawców usługi Azure Site Recovery w magazynie.

### [Get-AzureRmSiteRecoverySite](Get-AzureRmSiteRecoverySite.md)
Pobiera witryny Hyper-V z magazynu odzyskiwania witryny.

### [Get-AzureRmSiteRecoveryStorageClassification](Get-AzureRmSiteRecoveryStorageClassification.md)
Pobiera klasyfikacje przestrzeni dyskowej w ramach odzyskiwania witryny.

### [Get-AzureRmSiteRecoveryStorageClassificationMapping](Get-AzureRmSiteRecoveryStorageClassificationMapping.md)
Pobiera mapowanie klasyfikacji magazynu w ramach odzyskiwania witryny.

### [Get-AzureRmSiteRecoveryVault](Get-AzureRmSiteRecoveryVault.md)
Pobiera magazyny odzyskiwania witryny z bieżącej subskrypcji.

### [Get-AzureRmSiteRecoveryVaultSettings](Get-AzureRmSiteRecoveryVaultSettings.md)
Pobiera informacje o ustawieniach bieżącego magazynu odzyskiwania witryny.

### [Get-AzureRmSiteRecoveryVaultSettingsFile](Get-AzureRmSiteRecoveryVaultSettingsFile.md)
Pobiera plik ustawień magazynu odzyskiwania witryny.

### [Get-AzureRmSiteRecoveryVM](Get-AzureRmSiteRecoveryVM.md)
Pobiera informacje o maszynach wirtualnych zarządzanych przez odzyskiwanie witryny.

### [Import — AzureRmSiteRecoveryVaultSettingsFile](Import-AzureRmSiteRecoveryVaultSettingsFile.md)
Importuje plik ustawień magazynu odzyskiwania witryny.

### [Nowe — AzureRmSiteRecoveryFabric](New-AzureRmSiteRecoveryFabric.md)
Tworzy sieć szkieletową odzyskiwania witryny Azure.

### [Nowe — AzureRmSiteRecoveryNetworkMapping](New-AzureRmSiteRecoveryNetworkMapping.md)
Tworzy mapowanie między sieciami wirtualnymi.

### [Nowe — AzureRmSiteRecoveryPolicy](New-AzureRmSiteRecoveryPolicy.md)
Tworzy zasady replikacji odzyskiwania witryny.

### [Nowe — AzureRmSiteRecoveryProtectionContainerMapping](New-AzureRmSiteRecoveryProtectionContainerMapping.md)
Tworzy mapowanie kontenera ochrony witryny platformy Azure, kojarząc zasady z kontenerem ochrony.

### [Nowe — AzureRmSiteRecoveryRecoveryPlan](New-AzureRmSiteRecoveryRecoveryPlan.md)
Tworzy plan odzyskiwania witryny w ramach odzyskiwania witryny.

### [Nowe — AzureRmSiteRecoveryReplicationProtectedItem](New-AzureRmSiteRecoveryReplicationProtectedItem.md)
Włączanie replikacji elementu do ochrony przez utworzenie elementu chronionego

### [Nowe — AzureRmSiteRecoverySite](New-AzureRmSiteRecoverySite.md)
Tworzy witrynę odzyskiwania witryny.

### [Nowe — AzureRmSiteRecoveryStorageClassificationMapping](New-AzureRmSiteRecoveryStorageClassificationMapping.md)
Tworzy mapowanie klasyfikacji magazynu w ramach odzyskiwania witryny.

### [Nowe — AzureRmSiteRecoveryVault](New-AzureRmSiteRecoveryVault.md)
Tworzy magazyn usług odzyskiwania witryny.

### [Remove-AzureRmSiteRecoveryFabric](Remove-AzureRmSiteRecoveryFabric.md)
Usuwa sieć szkieletową odzyskiwania witryny Azure.

### [Remove-AzureRmSiteRecoveryNetworkMapping](Remove-AzureRmSiteRecoveryNetworkMapping.md)
Usuwa mapowanie sieci z bieżącego magazynu odzyskiwania witryny.

### [Remove-AzureRmSiteRecoveryPolicy](Remove-AzureRmSiteRecoveryPolicy.md)
Usuwa zasady replikacji odzyskiwania witryny.

### [Remove-AzureRmSiteRecoveryProtectionContainerMapping](Remove-AzureRmSiteRecoveryProtectionContainerMapping.md)
Usuwa mapowanie kontenera ochrony usługi Azure Site Recovery.

### [Remove-AzureRmSiteRecoveryRecoveryPlan](Remove-AzureRmSiteRecoveryRecoveryPlan.md)
Usuwa plan odzyskiwania witryny z witryny odzyskiwania witryny.

### [Remove-AzureRmSiteRecoveryReplicationProtectedItem](Remove-AzureRmSiteRecoveryReplicationProtectedItem.md)
Usuwa element chroniony replikacji odzyskiwania witryny platformy Azure.

### [Remove-AzureRmSiteRecoveryServer](Remove-AzureRmSiteRecoveryServer.md)
Usuwa serwer odzyskiwania witryny z bieżącego magazynu.

### [Remove-AzureRmSiteRecoveryServicesProvider](Remove-AzureRmSiteRecoveryServicesProvider.md)
Usuwa dostawcę usług Azure Site Recovery Services.

### [Remove-AzureRmSiteRecoverySite](Remove-AzureRmSiteRecoverySite.md)
Usuwa witrynę odzyskiwania witryny z bieżącego magazynu.

### [Remove-AzureRmSiteRecoveryStorageClassificationMapping](Remove-AzureRmSiteRecoveryStorageClassificationMapping.md)
Usuwa mapowanie klasyfikacji magazynu z odzyskiwania witryny.

### [Remove-AzureRmSiteRecoveryVault](Remove-AzureRmSiteRecoveryVault.md)
Usuwa magazyn odzyskiwania witryny.

### [Restart-AzureRmSiteRecoveryJob](Restart-AzureRmSiteRecoveryJob.md)
Ponowne uruchamianie zadania odzyskiwania witryny Azure.

### [Życiorys — AzureRmSiteRecoveryJob](Resume-AzureRmSiteRecoveryJob.md)
Wznawia wstrzymane zadanie odzyskiwania witryny.

### [Set-AzureRmSiteRecoveryProtectionEntity](Set-AzureRmSiteRecoveryProtectionEntity.md)
Umożliwia ustawienie stanu jednostki ochrony przed odzyskiwaniem witryny.

### [Set-AzureRmSiteRecoveryReplicationProtectedItem](Set-AzureRmSiteRecoveryReplicationProtectedItem.md)
Ustawia właściwości odzyskiwania, takie jak Sieć docelowa i rozmiar maszyny wirtualnej dla elementu chronionego replikacji.

### [Set-AzureRmSiteRecoveryVaultSettings](Set-AzureRmSiteRecoveryVaultSettings.md)
Ustawia kontekst magazynu odzyskiwania witryny dla kolejnych operacji.

### [Set-AzureRmSiteRecoveryVM](Set-AzureRmSiteRecoveryVM.md)
Ustawia opcje po stronie odzyskiwania dla jednostki ochrony przed odzyskiwaniem witryny.

### [Start — AzureRmSiteRecoveryApplyRecoveryPoint](Start-AzureRmSiteRecoveryApplyRecoveryPoint.md)
Zmienia punkt odzyskiwania dla uszkodzonego elementu chronionego przed zatwierdzeniem operacji przełączania awaryjnego.

### [Start — AzureRmSiteRecoveryCommitFailoverJob](Start-AzureRmSiteRecoveryCommitFailoverJob.md)
Rozpoczyna akcję zatwierdzania trybu failover dla obiektu odzyskiwania witryny.

### [Start — AzureRmSiteRecoveryPlannedFailoverJob](Start-AzureRmSiteRecoveryPlannedFailoverJob.md)
Rozpoczyna operację planowanej pracy awaryjnej odzyskiwania witryny.

### [Start — AzureRmSiteRecoveryPolicyAssociationJob](Start-AzureRmSiteRecoveryPolicyAssociationJob.md)
Uruchamianie zadania skojarzenia zasad replikacji witryny.

### [Start — AzureRmSiteRecoveryPolicyDissociationJob](Start-AzureRmSiteRecoveryPolicyDissociationJob.md)
Rozpoczyna zadanie tworzenia skojarzenia w zasadach replikacji skojarzonych z kontenerem ochrona przed odzyskiwaniem witryny.

### [Start — AzureRmSiteRecoveryTestFailoverJob](Start-AzureRmSiteRecoveryTestFailoverJob.md)
Rozpoczyna test pracy awaryjnej jednostki ochrony przed odzyskiwaniem witryny.

### [Start — AzureRmSiteRecoveryUnplannedFailoverJob](Start-AzureRmSiteRecoveryUnplannedFailoverJob.md)
Rozpoczyna nieplanowaną pracę awaryjną dla jednostki ochrony odzyskiwania witryny lub planu odzyskiwania.

### [Zatrzymaj — AzureRmSiteRecoveryJob](Stop-AzureRmSiteRecoveryJob.md)
Zatrzymuje zadanie odzyskiwania witryny.

### [Update-AzureRmSiteRecoveryPolicy](Update-AzureRmSiteRecoveryPolicy.md)
{{Tutaj ręcznie wprowadź Update-AzureRmSiteRecoveryPolicy opis}}

### [Update-AzureRmSiteRecoveryProtectionDirection](Update-AzureRmSiteRecoveryProtectionDirection.md)
Umożliwia zaktualizowanie serwera źródłowego i docelowego w celu ochrony obiektu odzyskiwania witryny.

### [Update-AzureRmSiteRecoveryRecoveryPlan](Update-AzureRmSiteRecoveryRecoveryPlan.md)
Aktualizuje plan odzyskiwania w witrynie odzyskiwanie witryny.

### [Update-AzureRmSiteRecoveryServer](Update-AzureRmSiteRecoveryServer.md)
Odświeża serwer odzyskiwania witryny.

### [Update-AzureRmSiteRecoveryServicesProvider](Update-AzureRmSiteRecoveryServicesProvider.md)
Aktualizuje informacje otrzymane od dostawcy usług Azure Site Recovery.

