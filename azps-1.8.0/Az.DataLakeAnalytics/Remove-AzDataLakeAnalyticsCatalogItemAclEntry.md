...
### RemoveCatalogAclEntryForUser (domyślny)
```
...
## Opis
Polecenie cmdlet **Remove-AzDataLakeAnalyticsCatalogItemAclEntry** usuwa wpis (ACE) z listy kontroli dostępu (ACL) wykazu lub elementu wykazu w usłudze Data Lake Analytics.

## Przykłady

### Przykład 1: Usuwanie listy ACL użytkowników dla wykazu
```powershell
...
To polecenie usuwa wykaz ACL wykazu dla Patti w pełni na koncie contosoadla.

### Przykład 2: Usuwanie listy ACL użytkowników dla bazy danych
```powershell
...
To polecenie usuwa listę ACL bazy danych dla Pattigo pełnego contosoadla konta.
...
### -Group
Usuwanie wpisu ACL wykazu dla grupy.
...
### -ObjectId
Tożsamość użytkownika, który ma zostać usunięty.
...
### -User
Usuwanie wpisu ACL wykazu dla użytkownika.
