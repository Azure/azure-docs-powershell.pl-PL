...
## Opis
Polecenie cmdlet **Add-AzLoadBalancerRuleConfig** umożliwia dodanie konfiguracji reguły do modułu równoważenia obciążenia platformy Azure.
...
Drugie polecenie używa operatora potoku w celu przekazania modułu równoważenia obciążenia w $slb do polecenia **Add-AzLoadBalancerRuleConfig** , które dodaje konfigurację reguły o nazwie NewRule.
Trzecie polecenie zaktualizuje moduł równoważenia obciążenia na platformie Azure za pomocą nowej konfiguracji reguł modułu równoważenia obciążenia.

## PARAMETRÓW

### -BackendAddressPool
Określa pulę adresów zaplecza, która ma zostać skojarzona z konfiguracją reguły modułu równoważenia obciążenia.
...
### -EnableTcpReset
Odbieraj dwukierunkowe Resetowanie protokołu TCP na limicie czasu bezczynności przepływu TCP lub nieoczekiwane zakończenie połączenia. Ten element jest wykorzystywany tylko wtedy, gdy protokół jest ustawiony na protokół TCP.
...
Określa długość czasu (w minutach) przechowywania stanu konwersacji w usłudze równoważenia obciążenia.
...
To polecenie cmdlet umożliwia dodanie konfiguracji reguły do modułu równoważenia obciążenia, który określa ten parametr.
...
### -Protocol (protokół)
Specfies protokół zgodny z regułą modułu równoważenia obciążenia.
