...
### ByExpressRouteConnectionName (domyślny)
```
...
## Opis
Polecenie cmdlet **Set-AzExpressRouteConnection** aktualizuje połączenie Express Route utworzone między obsługą komunikacji bezpośredniej bramy usługi Express Route i lokalnego obwodu trasy Express.
...
Powyższego poziomu spowoduje utworzenie grupy zasobów, wirtualnej sieci WAN, sieci wirtualnej, wirtualnego centrum i ExpressRouteSite w zachodnim obszarze "testRG" w grupie zasobów platformy Azure. Brama ExpressRoute zostanie utworzona później w koncentratorze wirtualnym z dwoma jednostkami skali.

Po utworzeniu bramy jest on połączony z równorzędnym obwodem ExpressRoute, korzystając z polecenia New-AzExpressRouteConnection.

Po zaktualizowaniu połączenie będzie miało inne RoutingWeight, korzystając z polecenia Set-AzExpressRouteConnection.
...
### -AuthorizationKey
Klucz autoryzacji, który ma zostać wykorzystany do utworzenia połączenia bramy usługi ExpressRoute.
...
Włącz zabezpieczenia internetowe dla tego połączenia bramy ExpressRoute
...
### -Inputobject
Obiekt ExpressRouteConnection do zaktualizowania.
...
### -ResourceId
Identyfikator zasobu obiektu ExpressRouteConnection, który ma zostać usunięty.
...
### -RoutingWeight
Waga, która musi być przypisana do tego połączenia dla routingu pakietów.
