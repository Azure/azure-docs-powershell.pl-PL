...
### Przykład 2: Tworzenie zapory zezwalającej na cały ruch HTTPS
```powershell
$rgName = "resourceGroupName"
$vnet = Get-AzVirtualNetwork -ResourceGroupName $rgName -Name "vnet"
$pip = Get-AzPublicIpAddress -ResourceGroupName $rgName -Name "publicIpName"
...
### Przykład 3: DNAT — Przekieruj ruch przeznaczony do 10.1.2.3:80 do 10.2.3.4:8080
```powershell
...
### Przykład 4: Tworzenie zapory bez reguł i z zagrożeniami Intel w trybie alertu
```powershell
$rgName = "resourceGroupName"
$vnet = Get-AzVirtualNetwork -ResourceGroupName $rgName -Name "vnet"
$pip = Get-AzPublicIpAddress -ResourceGroupName $rgName -Name "publicIpName"
...
### Przykład 5: Tworzenie zapory, która zezwala na cały ruch HTTP na porcie 8080, ale blokuje szkodliwe domeny zidentyfikowane przez zagrożenie Intel
```powershell
$rgName = "resourceGroupName"
$vnet = Get-AzVirtualNetwork -ResourceGroupName $rgName -Name "vnet"
$pip = Get-AzPublicIpAddress -ResourceGroupName $rgName -Name "publicIpName"
...
### Przykład 8: Tworzenie zapory, która umożliwia ruch MSSQL do określonej bazy danych SQL
```powershell
$rgName = "resourceGroupName"
$vnet = Get-AzVirtualNetwork -ResourceGroupName $rgName -Name "vnet"
$pip = Get-AzPublicIpAddress -ResourceGroupName $rgName -Name "publicIpName"
...
### Przykład 9: Tworzenie zapory podłączonej do wirtualnego koncentratora
```powershell
$rgName = "resourceGroupName"
...
### Przykład 10: Tworzenie zapory przy użyciu konfiguracji whitelist analizy zagrożeń
```powershell
$rgName = "resourceGroupName"
$vnet = Get-AzVirtualNetwork -ResourceGroupName $rgName -Name "vnet"
$pip = Get-AzPublicIpAddress -ResourceGroupName $rgName -Name "publicIpName"
...
### Przykład 11: Tworzenie zapory z dostosowanym ustawieniami zakresu prywatnego
```powershell
$rgName = "resourceGroupName"
$vnet = Get-AzVirtualNetwork -ResourceGroupName $rgName -Name "vnet"
$pip = Get-AzPublicIpAddress -ResourceGroupName $rgName -Name "publicIpName"
...
### Przykład 14: Tworzenie zapory z serwerami proxy DNS i DNS
```powershell
$rgName = "resourceGroupName"
$vnet = Get-AzVirtualNetwork -ResourceGroupName $rgName -Name "vnet"
$pip = Get-AzPublicIpAddress -ResourceGroupName $rgName -Name "publicIpName"
...
W tym przykładzie jest tworzona Zapora dołączona do sieci wirtualnej "VNET" w tej samej grupie zasobów co Zapora.
Serwer proxy DNS jest włączony dla tej zapory i są dostępne 2 serwery DNS. Wymagane jest również skonfigurowanie serwera proxy DNS dla reguł sieciowych, jeśli istnieją jakieś reguły sieciowe z nazwami FQDN, wtedy serwer proxy DNS również będzie używany.
...
W tym przykładzie jest tworzona Zapora podłączona do koncentratora wirtualnego "koncentrator" w tej samej grupie zasobów co Zapora.
Zapora będzie przypisana 2 publiczne adresy IP, które są tworzone niejawnie.

### 16: Tworzenie zapory z opcją Zezwalaj na aktywną FTP.
```
$rgName = "resourceGroupName"
$vnet = Get-AzVirtualNetwork -ResourceGroupName $rgName -Name "vnet"
$pip = Get-AzPublicIpAddress -ResourceGroupName $rgName -Name "publicIpName"
...
W tym przykładzie jest tworzona Zapora z flagą Zezwalaj na aktywną FTP.
...
Wymaga funkcji proxy DNS dla nazw FQDN w regułach sieciowych.
...
### -AllowActiveFTP
Zezwala na aktywną FTP na zaporze. Domyślnie jest on wyłączony.
...
### -HubIPAddresses
Adresy IP zapory podłączonej do wirtualnego koncentratora
