...
## STRESZCZENIe
Interfejs API umożliwiający zarejestrowanie nowego komputera, a w związku z tym utworzenie prześledzonego zasobu w ARM
...
### Przykład 1: znajduje się na pokładzie komputera, który jest podłączony do komputera.
```powershell
...
Znajduje się na płycie, na którym znajduje się komputer, na którym jesteś podłączony.

### Przykład 2: komputer zdalny jest podłączony do podłączonego urządzenia
```powershell
...
Na komputerze zdalnym jako urządzenie podłączone za pomocą funkcji komunikacji zdalnej programu PowerShell.
Uwaga: obecnie jest obsługiwany tylko system Windows jako element docelowy.
...
### -Proxy
Identyfikator URI serwera proxy, który ma być używany
...
### -PSSession
Po wybraniu tej polecenia narzędzie, które dołączają do komputerów platformy Azure, będzie uruchamiane w ramach każdego PSSession.
Uwaga: Ta funkcja działa tylko w systemie Windows.
...
### -ResourceGroupName
Nazwa grupy zasobów, do której chcesz dodać maszynę.
...
### -Subskrypcji
Identyfikator subskrypcji, do której chcesz dodać maszynę.
