...
## Opis
To polecenie umożliwia utworzenie punktu końcowego chmurowej synchronizacji plików platformy Azure. Punkt końcowy w chmurze to odwołanie do istniejącego udziału plików platformy Azure. Reprezentuje udział pliku i definiuje jego uczestnictwo w synchronizowaniu wszystkich plików w grupie synchronizacja, w której został utworzony punkt końcowy w chmurze.
...
Punkt końcowy w chmurze jest integralnym członkiem grupy synchronizacji, jest to przykład tworzenia jednej z istniejących w istniejącej grupie synchronizacji o nazwie "mySyncGroupName".
...
### -AzureFileShareName
Nazwa udziału konta magazynu (nazwa udziału plików Azure)
...
### -Name (nazwa)
Nazwa punktu końcowego chmury. W przypadku tworzenia za pośrednictwem witryny Azure Portal nazwa jest ustawiana na nazwę z odwołaniami do udostępniania plików w usłudze Azure.
...
Identyfikator dzierżawy konta magazynu (identyfikator katalogu firm)
