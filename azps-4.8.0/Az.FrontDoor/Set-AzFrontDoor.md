...
## Opis
Polecenie cmdlet **Set-AzFrontDoor** umożliwia zaktualizowanie czołowego modułu równoważenia obciążenia. Jeśli parametry wejściowe nie są podane, będą używane stare parametry z obecnego frontu drzwi.

## Przykłady

### Przykład 1: aktualizowanie istniejących drzwi przednich przy użyciu FrontDoorName i ResourceGroupName.
```powershell
...
Zaktualizuj istniejące FrontDoor.

### Przykład 2: aktualizowanie istniejących drzwi przednich za pomocą obiektu PSFrontDoor.
```powershell
...
### Przykład 3: aktualizowanie istniejących drzwi przednich za pomocą ResourceId
```powershell
...
### Przykład 4: aktualizowanie właściwości BackendPoolSetting EnforceCertificateNameCheck istniejących drzwi przednich przy użyciu parametru-DisableCertificateNameCheck Switch
Przednie są one aktualizowane przy użyciu FrontoorName, ResourceGroupName, PSFrontDoor obiektu lub ResourceId. (Na przykład powyżej 3 przykładów) W poniższym przykładzie użyto obiektu PSFrontDoor.
...
### -BackendPoolsSetting
Ustawienia wszystkich backendPools.
...
Czy wyłączyć sprawdzanie nazw certyfikatów dla żądań HTTPS we wszystkich pulach zaplecza. Brak wpływu na żądania inne niż HTTPS.
...
### -EnabledState
Stan operacyjny modułu równoważenia obciążenia przednia drzwi.
...
### -Inputobject
Obiekt z przednią drzwi do zaktualizowania.
...
### -Name (nazwa)
Nazwa przednich drzwi, które mają zostać zaktualizowane.
...
### -ResourceId
Identyfikator zasobu przednich drzwi do zaktualizowania
