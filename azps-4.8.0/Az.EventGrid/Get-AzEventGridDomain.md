...
## Opis
Polecenie cmdlet Get-AzEventGridDomain pobiera szczegóły określonej domeny siatki zdarzeń lub listę wszystkich domen w siatce zdarzeń w bieżącej subskrypcji platformy Azure.
Jeśli nazwa domeny jest podana, zostanie zwrócona wartość szczegóły jednej domeny siatki zdarzeń.
Jeśli nie podano nazwy domeny, zostanie zwrócona Lista domen. Liczba elementów zwróconych na tej liście jest kontrolowana przez parametr najwyższy. Jeśli najszukana wartość nie jest określona lub $null, lista będzie zawierać wszystkie elementy domeny zwrócone jednocześnie. W przeciwnym razie na początku będzie wskazywana Maksymalna liczba elementów, które mają zostać zwrócone na liście.
Jeśli więcej domen jest wciąż dostępnych, w następnej rozmowie należy użyć wartości w NextLink, aby uzyskać następną stronę domen.
...
Pobiera szczegóły Domain1 domeny siatki zdarzeń \` \` w MyResourceGroupName grupy zasobów \` \` .
...
Pobiera szczegóły Domain1 domeny w siatce zdarzeń \` \` w grupie zasób \` MyResourceGroupName \` przy użyciu opcji ResourceID.
...
Wyświetlanie wszystkich domen w siatce zdarzeń w MyResourceGroupName grupy \` zasobów \` bez stronicowania (wszystkie domeny są zwracane w jednym zasobie)
...
Lista domen w siatce zdarzeń (jeśli istnieją) w grupie MyResourceGroupName, która jest taka sama jak w \` \` przypadku programu $odataFilter 10 domen. Jeśli jest dostępnych więcej wyników, $result. NextLink nie będzie $null. Aby uzyskać kolejne strony domen, użytkownik powinien ponownie zadzwonić Get-AzEventGridDomain i używa wyników. NextLink uzyskana z poprzedniej rozmowy. Osoba dzwoniąca powinna zatrzymać się w wyniku. NextLink się $null.
...
Wyświetlanie listy wszystkich domen w siatce zdarzeń w ramach subskrypcji platformy Azure bez stronicowania (wszystkie domeny są zwracane w jednym zasobie)
...
Lista domen w siatce zdarzeń (jeśli istnieją) w subskrypcji platformy Azure, które są zgodne z podaną $odataFilter 20 domenami w danym momencie. Jeśli jest dostępnych więcej wyników, $result. NextLink nie będzie $null. Aby uzyskać kolejne strony domen, użytkownik powinien ponownie zadzwonić Get-AzEventGridDomain i używa wyników. NextLink uzyskana z poprzedniej rozmowy. Osoba dzwoniąca powinna zatrzymać się w wyniku. NextLink się $null.
...
### -NextLink
Łącze do następnej strony, które mają zostać uzyskane.
Ta wartość jest uzyskiwana za pomocą pierwszego Get-AzEventGrid polecenia cmdlet, gdy więcej zasobów jest wciąż dostępnych do zbadania.
...
### -ODataQuery
Kwerenda OData używana do filtrowania wyników listy.
...
### -Początek
Maksymalna liczba zasobów do uzyskania.
Prawidłowa wartość należy do zakresu od 1 do 100.
Jeśli określona jest największa wartość, a dalsze wyniki są nadal dostępne, wynik będzie zawierał link do następnej strony, do której ma zostać przeszukana w NextLink.
Jeśli nie określono najwyższych wartości, pełna lista zasobów zostanie zwrócona jednocześnie.
