...
## Opis
Polecenie cmdlet **New-AzSqlDatabase** tworzy bazę danych SQL Azure.
...
To polecenie tworzy bazę danych o nazwie Database02 w puli elastycznej o nazwie ElasticPool01 na serwerze Server01.

### Przykład 3: Tworzenie bazy danych programu vCore na określonym serwerze
```
...
### Przykład 4: Tworzenie bazy danych serwera na określonym serwerze
```
...
To polecenie tworzy bezserwerową bazę danych o nazwie Database04 na serwerze Server01.
...
Opóźnienie automatycznego wstrzymania w minutach dla bazy danych (tylko na serwerze),-1 w celu rezygnacji
...
### -ComputeModel
Model obliczeniowy dla bazy danych Azure SQL Database. Serwer lub Zainicjowano obsługę administracyjną
...
### -MinimumCapacity
Minimalna pojemność bazy danych jest zawsze przydzielona, jeśli nie została wstrzymana.
W przypadku baz danych platformy Azure SQL bezserwerowych.
...
### -ReadReplicaCount
Liczba replik pomocniczych tylko do odczytu skojarzonych z bazą danych, do której mogą być przekierowywane połączenia z przeznaczeniem aplikacji tylko do odczytu. Ta właściwość jest dostępna tylko w przypadku baz danych z podskalą wersji.
...
### -ReadScale
Jeśli ta opcja jest włączona, połączenia z zastosowanymi aplikacjami, które mają ustawioną wartość tylko do odczytu, mogą być przekierowywane do repliki pomocniczej tylko do odczytu. Ta właściwość jest tylko do settable dla kluczowych baz danych Premium i dla firm.
...
### -Tagi
Określa słownik par klucz-wartość w postaci tabeli skrótów, która jest skojarzona z nową bazą danych. Na przykład: @ {Key0 = "value0"; KEY1 = $null; key2 = "wartość2"}
