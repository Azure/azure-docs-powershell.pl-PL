...
## Opis
Polecenie cmdlet **Update-AzDataLakeGen2Item** aktualizuje plik lub katalog na właściwościach, metadanych, uprawnieniach, listach ACL i właścicielu.
...
### Przykład 1: Tworzenie obiektu ACL z 3 wpisami ACL i aktualizowanie listy ACL do wszystkich elementów w systemie plików cyklicznie
```
...
To polecenie najpierw tworzy obiekt ACL z 3 wpisami ACL (Use-Inputobject), aby dodać wpis ACL do istniejącego obiektu ACL, a następnie uzyskać wszystkie elementy w systemie plików i zaktualizować listę ACL elementów.

### Przykład 2: aktualizowanie wszystkich właściwości pliku i wyświetlenie ich
```
...
To polecenie aktualizuje wszystkie właściwości pliku (ACL, uprawnienie, właściciel, Grupa, metadane, właściwość może zostać zaktualizowana o dowolne conbination) i pokazywanie ich w konsoli programu PowerShell.

### Przykład 3: Dodawanie wpisu ACL do katalogu
```
## Get the origin ACL
...
To polecenie pobiera listę ACL z katalogu, aktualizuje/dodaje wpis listy ACL, a następnie przywraca go do katalogu.
Jeśli wpis ACL o tej samej AccessControlType/EntityId/DefaultScope nie istnieje, zostanie dodany nowy wpis ACL, w przeciwnym razie uprawnienia do istniejącego wpisu listy ACL.

## PARAMETRÓW

### -ACL
Ustawia prawa kontroli dostępu POSIX do plików i katalogów.
Utwórz ten obiekt, używając nowego-AzDataLakeGen2ItemAclObject.
...
### -Group
Ustawia grupę będącą właścicielem obiektu BLOB.
...
### -Inputobject
Obiekt usługi Azure datalake Gen2 do zaktualizowania
...
### -Metadata
Określa metadane katalogu lub pliku.
...
### -Właściciel
Ustawia właściciela obiektu BLOB.
...
### -Path
Ścieżka w określonym systemie plików, która powinna zostać zaktualizowana.
Może to być plik lub katalog w formacie "Directory/file.txt" lub "directory1/Directory2/".
Nieokreślenie tego parametru spowoduje zaktualizowanie katalogu głównego systemu plików.
...
### -Uprawnienie
Umożliwia ustawianie uprawnień dostępu do POSIX dla właściciela pliku, grupy będącej właścicielem i innych.
Każdej klasie można udzielić uprawnień Odczyt, zapis lub wykonywanie.
Obsługa symboliczna (rwxrw-RW-).
Nieprawidłowy w połączeniu z listą ACL.
...
### -Property
Określa właściwości katalogu lub pliku. Obsługiwane właściwości pliku to: CacheControl, ContentDisposition, ContentEncoding, ContentLanguage, ContentMD5, ContentType.
Obsługiwane właściwości katalogu to: CacheControl, ContentDisposition, ContentEncoding, ContentLanguage.
