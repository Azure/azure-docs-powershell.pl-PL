...
## Opis
To polecenie cmdlet umożliwia tworzenie usług bezstanowych i stanowych w ramach określonej aplikacji. Usługa powinna zostać zamknięta w manifeście aplikacji, a typ musi być taki sam jak w manifeście. Nazwa aplikacji powinna być prefiksem nazwy usługi.
...
W tym przykładzie zostanie utworzona nowa usługa bezstanowa "testApp ~ testService1" z liczbą wystąpień-1 (na wszystkich węzłach).
...
W tym przykładzie zostanie utworzona nowa usługa stanowa "testApp ~ testService2" z elementem docelowym 5 węzłów.
...
### -DefaultMoveCost
Określ domyślny koszt przeniesienia.
Większe koszty sprawiają, że Menedżer zasobów klastra przeniesie replikę podczas próby zrównoważenia klastra
...
### -InstanceCount
Określanie liczby wystąpień dla usługi
...
### -MinReplicaSetSize
Określanie minimalnego rozmiaru zestawu replik dla usługi
...
Wskazuje, że usługa korzysta ze schematu partycji nazwanej.
Usługi korzystające z tego modelu zazwyczaj mają dane, które można przetworzyć w ramach ograniczonego zestawu.
Do typowych przykładów pól danych używanych jako klucze partycji nazwanych są regiony, kody pocztowe, grupy klientów lub inne granice biznesowe.
...
Wskazuje, że usługa używa schematu partycji singleton.
Partycje pojedyncze są zazwyczaj używane, gdy usługa nie wymaga żadnych dodatkowych tras routingu.
...
Wskazuje, że usługa używa schematu partycji UniformInt64.
Oznacza to, że każda partycja należy do zakresu kluczy Int64.
...
Określanie czasu oczekiwania na utratę kworum dla usługi
...
Określanie czasu oczekiwania na ponowne uruchomienie repliki dla usługi
...
Określanie czasu trwania repliki dla usługi
...
### -Stanowy
Korzystanie z usługi akumulującej stan
...
### -Bez Stanów
Korzystanie z usługi bezstanowej
...
Określanie docelowego rozmiaru zestawu replik dla usługi
...
### -Type
Określ nazwę typu usługi aplikacji, która powinna znajdować się w manifeście aplikacji.
