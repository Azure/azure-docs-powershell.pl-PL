...
## OPIS
Scalanie określonych `Reservation` s w `Reservation` nowe. Te dwie `Reservation` scalone muszą mieć te same właściwości.
...
Scalanie dwóch określonych `Reservation` s w jedną `Reservation`
...
### — Rezerwacja
Ciągi dwóch znaków ReservationId do scalenia oddzielone przecinkami
...
### -ReservationId
ReservationOrderId dla `ReservationOrder` tej, która zawiera dwa `Reservation` s
