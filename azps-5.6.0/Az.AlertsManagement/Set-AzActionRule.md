...
## SYNOPSIS
Utwórz lub zaktualizuj regułę akcji.

## SKŁADNIA

### BySimplifiedFormatDiagnosticsActionRule (Domyślna)
```
...
To polecenie cmdlet tworzy regułę akcji dla kompresji z zakresem subskrypcji.
...
To polecenie cmdlet tworzy regułę akcji dla grupy akcji z listą zakresu grup zasobów.
...
To polecenie cmdlet tworzy regułę akcji dla ustawień diagnostycznych z zakresem zasobów.

## PARAMETERS

### -ActionGroupId
Identyfikator grupy akcji, który ma zostać powiadomiony.
...
Oczekiwany format - { \<operation\> : } W przypadku \<comma separated list of values\> np.
Zawiera:grupy inteligentne
...
Oczekiwany format - { \<operation\> : } W przypadku \<comma separated list of values\> np.
Równa się:/subscriptions/ad825170-845c-47db-8f00-11978947b089/resourceGroups/abvarma/providers/microsoft.insights/metricAlerts/test-mrmc-vm-abvarma
...
Oczekiwany format - { \<operation\> : } W przypadku \<comma separated list of values\> np.
Równa się:Platforma,Analiza dziennika
...
### -ReccurenceType
Określa czas trwania, przez który należy chcieć je stosować.
...
### -ReccurentValue
Wartości cykliczne, jeśli mają zastosowanie. W przypadku tygodniowego - \[ 1,2 \] W przypadku miesięcznej - \[ 1,3,5,30\]
...
### -WymuszeńGodziny
Czas zakończenia.
Format 2018-12-09 06:00:00 + Powinien zostać wspomniany w przypadku cyklicznego harmonogramu kompresji — raz, codziennie, co tydzień lub co miesiąc.
...
### - WymuszeniePoczątego Rozpoczęcia
Godzina rozpoczęcia.
Format 2018-12-09 06:00:00 + Powinien zostać wspomniany w przypadku cyklicznego harmonogramu kompresji — raz, codziennie, co tydzień lub co miesiąc.
