...
## OPIS
Polecenie Set-AzConsumptionBudget aktualizuje budżet w ramach subskrypcji lub grupy zasobów.

## PRZYKŁADY

### Przykład 1. Aktualizowanie budżetu o nową kwotę przy użyciu nazwy budżetu na poziomie subskrypcji
```powershell
...
### Przykład 2. Aktualizacja budżetu z powiadomieniem, gdy koszt lub użycie osiągnie próg 90 procent kwoty na poziomie subskrypcji
```powershell
...
### Przykład 3. Aktualizowanie budżetu według nowej kwoty przy użyciu nazwy budżetu na poziomie grupy zasobów
```powershell
...
### — Kategoria
Kategorią budżetu może być koszt lub użycie.
...
### -ContactEmail
Adresy e-mail do wysłania powiadomienia o budżecie po przekroczeniu progu.
...
### - ContactGroup
Grupy akcji, do których będą wysyłać powiadomienia o budżecie po przekroczeniu progu.
...
### -ContactRole
Role kontaktów w celu wysłania powiadomienia o budżecie po przekroczeniu progu.
...
### -EndDate
Data zakończenia okresu budżetu (YYYY-MM-DD w czasie UTC).
...
### -MeterFilter
Rozdzielana przecinkami lista metrów, według których mają być filtrowane dane.
Wymagane, jeśli kategoria jest kategorią użycia.
...
Jeśli nie zostanie określone, powiadomienie jest domyślnie wyłączone.
...
Powiadomienie jest wysyłane, gdy koszt lub użycie przekracza próg.
Zawsze jest to procent i musi być między 0 a 1000.
...
### -ResourceFilter
Rozdzielona przecinkami lista wystąpień zasobów, według których mają być filtrowane.
...
### -ResourceGroupFilter
Rozdzielona przecinkami lista grup zasobów, według których mają być filtrowane dane.
...
### -StartDate
Data rozpoczęcia (YYYY-MM-DD w czasie UTC) okresu budżetu.
Nie przed bieżącym miesiącem dla miesięcznego okresu.
Nie wcześniej niż trzy miesiące w przypadku kwartalnych okresów.
Nie wcześniej niż dwanaście miesięcy w przypadku okresowych okresów.
Przyszła data rozpoczęcia nie większa niż trzy miesiące.
...
### - TimeGrain
Część czasu w budżecie może być miesięczna, kwartalna lub rocznie.
