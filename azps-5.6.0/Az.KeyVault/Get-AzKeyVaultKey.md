...
Po uruchomieniu tego polecenia możesz sprawdzać różne właściwości klucza, poruszając się po $Key obiektu.

### Przykład 5. Uzyskiwanie wszystkich kluczy, które zostały usunięte, ale nie są czyszone dla tego magazynu kluczy
```powershell
...
### Przykład 6. Pobiera klucz ITPfx, który został usunięty, ale nie przeczyszczony dla tego magazynu kluczy.
```powershell
...
Jest to jeden z kroków importowania kluczy chronionych przez HSM do magazynu kluczy platformy Azure. Zobacz https://docs.microsoft.com/azure/key-vault/keys/hsm-protected-keys
...
### -OutFile
Określa plik wyjściowy, dla którego to polecenie cmdlet zapisuje klucz. Klucz publiczny jest domyślnie zapisywany w formacie PEM.
