---
Module Name: Az.KeyVault
Module Guid: D48CF693-4125-4D2D-8790-1514F44CE325
Download Help Link: https://docs.microsoft.com/powershell/module/az.keyvault
Help Version: 4.0.0.0
Locale: en-US
content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/KeyVault/KeyVault/help/Az.KeyVault.md
original_content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/KeyVault/KeyVault/help/Az.KeyVault.md
ms.openlocfilehash: 7948b764f82b6051d3f4c298f4293856efe4f2b1
ms.sourcegitcommit: 4dfb0cc533b83f77afdcfbe2618c1e6c8d221330
ms.translationtype: MT
ms.contentlocale: pl-PL
ms.lasthandoff: 03/04/2021
ms.locfileid: "101960737"
---
# Moduł Az.KeyVault
## Opis
W tym temacie są wyświetlane tematy pomocy dotyczące polecenia cmdlet magazynu kluczy platformy Azure.

## Az.KeyVault Cmdlet
### [Add-AzKeyVaultCertificate](Add-AzKeyVaultCertificate.md)
Dodaje certyfikat do magazynu kluczy.

### [Add-AzKeyVaultCertificateContact](Add-AzKeyVaultCertificateContact.md)
Dodaje kontakt do powiadomień o certyfikatach.

### [Add-AzKeyVaultKey](Add-AzKeyVaultKey.md)
Tworzy klucz w magazynie kluczy lub importuje klucz do magazynu kluczy.

### [Add-AzKeyVaultManagedStorageAccount](Add-AzKeyVaultManagedStorageAccount.md)
Dodaje istniejące konto magazynu platformy Azure do określonego magazynu kluczy, aby jego klucze zostały zarządzane przez usługę magazynu kluczy.

### [Add-AzKeyVaultNetworkRule](Add-AzKeyVaultNetworkRule.md)
Dodaje regułę mającą na celu ograniczenie dostępu do magazynu kluczy na podstawie adresu internetowego klienta.

### [Backup-AzKeyVault](Backup-AzKeyVault.md)
Pełne tworzenie kopii zapasowej zarządzanego systemu HSM.

### [Backup-AzKeyVaultCertificate](Backup-AzKeyVaultCertificate.md)
Kopii zapasowej certyfikatu w magazynie kluczy.

### [Backup-AzKeyVaultKey](Backup-AzKeyVaultKey.md)
Kopię zapasową klucza w magazynie kluczy.

### [Backup-AzKeyVaultManagedStorageAccount](Backup-AzKeyVaultManagedStorageAccount.md)
Kopii zapasowej konta magazynu zarządzanego przez usługę KeyVault.

### [Backup-AzKeyVaultSecret](Backup-AzKeyVaultSecret.md)
Kopię zapasową klucza tajnego w magazynie kluczy.

### [Export-AzKeyVaultSecurityDomain](Export-AzKeyVaultSecurityDomain.md)
Eksportuje dane domeny zabezpieczeń zarządzanego hsm.

### [Get-AzKeyVault](Get-AzKeyVault.md)
Pobiera magazyny kluczy.

### [Get-AzKeyVaultCertificate](Get-AzKeyVaultCertificate.md)
Pobiera certyfikat z magazynu kluczy.

### [Get-AzKeyVaultCertificateContact](Get-AzKeyVaultCertificateContact.md)
Pobiera kontakty zarejestrowane w celu powiadomień o certyfikatach dla magazynu kluczy.

### [Get-AzKeyVaultCertificateIssuer](Get-AzKeyVaultCertificateIssuer.md)
Pobiera wystawcę certyfikatu dla magazynu kluczy.

### [Get-AzKeyVaultCertificateOperation](Get-AzKeyVaultCertificateOperation.md)
Pobiera stan operacji certyfikatu.

### [Get-AzKeyVaultCertificatePolicy](Get-AzKeyVaultCertificatePolicy.md)
Pobiera zasady dla certyfikatu w magazynie kluczy.

### [Get-AzKeyVaultKey](Get-AzKeyVaultKey.md)
Pobiera klucze magazynu kluczy.

### [Get-AzKeyVaultManagedHsm](Get-AzKeyVaultManagedHsm.md)
Uzyskiwanie zarządzanych hsm.

### [Get-AzKeyVaultManagedStorageAccount](Get-AzKeyVaultManagedStorageAccount.md)
Pobiera zarządzane konta magazynu platformy Azure z magazynem kluczy.

### [Get-AzKeyVaultManagedStorageSasDefinition](Get-AzKeyVaultManagedStorageSasDefinition.md)
Pobiera definicje SAS magazynu zarządzanego przez magazyn kluczy.

### [Get-AzKeyVaultRoleAssignment](Get-AzKeyVaultRoleAssignment.md)
Uzyskiwanie lub lista przypisań ról zarządzanego hsm. Użyj odpowiednich parametrów, aby wyświetlić listę przydziałów dla określonego użytkownika lub definicji roli.

### [Get-AzKeyVaultRoleDefinition](Get-AzKeyVaultRoleDefinition.md)
Lista definicji ról danego zarządzanego modułu HSM w danym zakresie.

### [Get-AzKeyVaultSecret](Get-AzKeyVaultSecret.md)
Staje się tajemnicą w magazynie kluczy.

### [Import-AzKeyVaultCertificate](Import-AzKeyVaultCertificate.md)
Importuje certyfikat do magazynu kluczy.

### [Import-AzKeyVaultSecurityDomain](Import-AzKeyVaultSecurityDomain.md)
Wcześniej wyeksportowano dane z domeny zabezpieczeń do zarządzanego systemu HSM.

### [New-AzKeyVault](New-AzKeyVault.md)
Tworzy magazyn kluczy.

### [New-AzKeyVaultCertificateAdministratorDetail](New-AzKeyVaultCertificateAdministratorDetail.md)
Tworzy obiekt szczegółów administratora certyfikatu w pamięci.

### [New-AzKeyVaultCertificateOrganizationDetail](New-AzKeyVaultCertificateOrganizationDetail.md)
Tworzy obiekt szczegółów organizacji certyfikatu w pamięci.

### [New-AzKeyVaultCertificatePolicy](New-AzKeyVaultCertificatePolicy.md)
Tworzy obiekt zasad certyfikatu w pamięci.

### [New-AzKeyVaultManagedHsm](New-AzKeyVaultManagedHsm.md)
Tworzy zarządzany moduł HSM.

### [New-AzKeyVaultNetworkRuleSetObject](New-AzKeyVaultNetworkRuleSetObject.md)
Utwórz obiekt reprezentujący ustawienia reguły sieciowej.

### [New-AzKeyVaultRoleAssignment](New-AzKeyVaultRoleAssignment.md)
Przypisuje określoną rolę RBAC do określonego podmiotu zabezpieczeń w określonym zakresie.

### [Remove-AzKeyVault](Remove-AzKeyVault.md)
Usuwa magazyn kluczy.

### [Remove-AzKeyVaultAccessPolicy](Remove-AzKeyVaultAccessPolicy.md)
Usuwa wszystkie uprawnienia użytkownika lub aplikacji z magazynu kluczy.

### [Remove-AzKeyVaultCertificate](Remove-AzKeyVaultCertificate.md)
Usuwa certyfikat z magazynu kluczy.

### [Remove-AzKeyVaultCertificateContact](Remove-AzKeyVaultCertificateContact.md)
Usuwa kontakt zarejestrowany w celu powiadomienia o certyfikacie z magazynu kluczy.

### [Remove-AzKeyVaultCertificateIssuer](Remove-AzKeyVaultCertificateIssuer.md)
Usuwa wystawcę certyfikatu z magazynu kluczy.

### [Remove-AzKeyVaultCertificateOperation](Remove-AzKeyVaultCertificateOperation.md)
Usuwa operację certyfikatu z magazynu kluczy.

### [Remove-AzKeyVaultKey](Remove-AzKeyVaultKey.md)
Usuwa klucz z magazynu kluczy.

### [Remove-AzKeyVaultManagedHsm](Remove-AzKeyVaultManagedHsm.md)
Usuwa zarządzany moduł HSM.

### [Remove-AzKeyVaultManagedStorageAccount](Remove-AzKeyVaultManagedStorageAccount.md)
Usuwa zarządzane konto magazynu platformy Azure i wszystkie skojarzone definicje sas.

### [Remove-AzKeyVaultManagedStorageSasDefinition](Remove-AzKeyVaultManagedStorageSasDefinition.md)
Usuwa definicje SAS magazynu zarządzanego przez magazyn kluczy platformy Azure.

### [Remove-AzKeyVaultNetworkRule](Remove-AzKeyVaultNetworkRule.md)
Usuwa regułę sieciową z magazynu kluczy.

### [Remove-AzKeyVaultRoleAssignment](Remove-AzKeyVaultRoleAssignment.md)
Usuwa przypisanie roli do określonego podmiotu zabezpieczeń, który jest przypisany do określonej roli w określonym zakresie.

### [Remove-AzKeyVaultSecret](Remove-AzKeyVaultSecret.md)
Usuwa klucz tajny z magazynu kluczy.

### [Restore-AzKeyVault](Restore-AzKeyVault.md)
W pełni przywraca zarządzany moduł HSM z kopii zapasowej.

### [Restore-AzKeyVaultCertificate](Restore-AzKeyVaultCertificate.md)
Przywraca certyfikat w magazynie kluczy z pliku kopii zapasowej.

### [Restore-AzKeyVaultKey](Restore-AzKeyVaultKey.md)
Tworzy klucz w magazynie kluczy z klucza kopii zapasowej.

### [Restore-AzKeyVaultManagedStorageAccount](Restore-AzKeyVaultManagedStorageAccount.md)
Przywraca konto magazynu zarządzanego w magazynie kluczy z pliku kopii zapasowej.

### [Restore-AzKeyVaultSecret](Restore-AzKeyVaultSecret.md)
Tworzy klucz tajny w magazynie kluczy z kopii zapasowej tajnej.

### [Set-AzKeyVaultAccessPolicy](Set-AzKeyVaultAccessPolicy.md)
Udziela lub modyfikuje istniejące uprawnienia użytkownika, aplikacji lub grupy zabezpieczeń do wykonywania operacji z magazynem kluczy.

### [Set-AzKeyVaultCertificateIssuer](Set-AzKeyVaultCertificateIssuer.md)
Ustawia wystawcę certyfikatu w magazynie kluczy.

### [Set-AzKeyVaultCertificatePolicy](Set-AzKeyVaultCertificatePolicy.md)
Tworzy lub aktualizuje zasady dla certyfikatu w magazynie kluczy.

### [Set-AzKeyVaultManagedStorageSasDefinition](Set-AzKeyVaultManagedStorageSasDefinition.md)
Ustawia definicję podpisu SAS (Shared Access Signature) z magazynem kluczy dla danego zarządzanego konta magazynu usługi Azure.

### [Set-AzKeyVaultSecret](Set-AzKeyVaultSecret.md)
Tworzy lub aktualizuje klucz tajny w magazynie kluczy.

### [Stop-AzKeyVaultCertificateOperation](Stop-AzKeyVaultCertificateOperation.md)
Anulowanie operacji certyfikatu w magazynie kluczy.

### [Undo-AzKeyVaultCertificateRemoval](Undo-AzKeyVaultCertificateRemoval.md)
Przywraca usunięty certyfikat z magazynu kluczy do stanu aktywnego.

### [Undo-AzKeyVaultKeyRemoval](Undo-AzKeyVaultKeyRemoval.md)
Przywraca usunięty klucz z magazynu kluczy do stanu aktywnego.

### [Undo-AzKeyVaultManagedStorageAccountRemoval](Undo-AzKeyVaultManagedStorageAccountRemoval.md)
Odzyskiwanie poprzednio usuniętego konta magazynu zarządzanego przez usługę KeyVault.

### [Undo-AzKeyVaultManagedStorageSasDefinitionRemoval](Undo-AzKeyVaultManagedStorageSasDefinitionRemoval.md)
Przywraca usuniętą wcześniej definicję SAS magazynu zarządzanego przez usługę KeyVault.

### [Undo-AzKeyVaultRemoval](Undo-AzKeyVaultRemoval.md)
Przywraca usunięty magazyn kluczy do stanu aktywnego.

### [Undo-AzKeyVaultSecretRemoval](Undo-AzKeyVaultSecretRemoval.md)
Przywraca usunięty klucz tajny w magazynie kluczy do stanu aktywnego.

### [Update-AzKeyVault](Update-AzKeyVault.md)
Zaktualizuj stan magazynu kluczy platformy Azure.

### [Update-AzKeyVaultCertificate](Update-AzKeyVaultCertificate.md)
Modyfikuje atrybuty edytowalne certyfikatu.

### [Update-AzKeyVaultKey](Update-AzKeyVaultKey.md)
Aktualizuje atrybuty klucza w magazynie kluczy.

### [Update-AzKeyVaultManagedHsm](Update-AzKeyVaultManagedHsm.md)
Aktualizowanie stanu zarządzanego systemu Azure HSM.

### [Update-AzKeyVaultManagedStorageAccount](Update-AzKeyVaultManagedStorageAccount.md)
Aktualizuj edytowalne atrybuty zarządzanego konta magazynu platformy Azure w magazynie kluczy.

### [Update-AzKeyVaultManagedStorageAccountKey](Update-AzKeyVaultManagedStorageAccountKey.md)
Ponowne generowanie określonego klucza zarządzanego konta magazynu platformy Azure.

### [Update-AzKeyVaultNetworkRuleSet](Update-AzKeyVaultNetworkRuleSet.md)
Aktualizuje zestaw reguł sieciowych w magazynie kluczy.

### [Update-AzKeyVaultSecret](Update-AzKeyVaultSecret.md)
Aktualizuje atrybuty klucza tajnego w magazynie kluczy.

