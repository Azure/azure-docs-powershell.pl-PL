...
### DefinitionNameSignInName (domyślna)
```
...
## OPIS
Użyj polecenia cmdlet, aby odwołać dostęp do dowolnego podmiotu głównego `Remove-AzKeyVaultRoleAssignment` w danym zakresie i w danej roli. Obiekt przydziału, czyli musi zostać określony główny podmiot. Podmiot zabezpieczeń może być użytkownikiem (za pomocą parametrów SignInName lub ObjectId identyfikuje użytkownika), grupą zabezpieczeń (identyfikując grupę za pomocą parametru ObjectId) lub podmiotem zabezpieczeń usługi (w celu zidentyfikowania parametru ServicePrincipal należy użyć parametrów ApplicationId lub ObjectId). Rola, do których jest przypisany podmiot zabezpieczeń, MUSI zostać określona przy użyciu parametru RoleDefinitionName lub RoleDefinitionId.
...
W tym przykładzie odwołana jest rola " zarządzanego administratora zasad HSM" user1@microsoft.com dla " w zakresie "/keys".
...
W tym przykładzie wszystkie role " user1@microsoft.com " są odwołane we wszystkich zakresach.
...
### -PassThru
Zwraca wartość prawda po przywróceniu HSM.
