...
### Przykład 1. Aktualizacja tagów obszaru roboczego programu Databricks
```powershell
...
To polecenie aktualizuje tagi obszaru roboczego programu Databricks.

### Przykład 2. Włączanie szyfrowania w obszarze roboczym programu Databricks
```powershell
...
Włączenie szyfrowania w obszarze roboczym programu Databricks wymaga trzech kroków:
1.
Zaktualizuj obszar roboczy `-PrepareEncryption` (jeśli tak nie został utworzony).
1.
Znajdź `StorageAccountIdentityPrincipalId` w wyniku ostatniego kroku.
Udzielanie głównych uprawnień podmiotowi zabezpieczeń.
1.
Zaktualizuj ponownie obszar roboczy, aby wypełnić informacje o kluczu szyfrowania:
    - `-EncryptionKeySource`
    - `-EncryptionKeyVaultUri`
    - `-EncryptionKeyName`
    - `-EncryptionKeyVersion`

### Przykład 3. Wyłączanie szyfrowania w obszarze roboczym usługi Databricks
```powershell
...
Aby wyłączyć szyfrowanie, wystarczy ustawić `-EncryptionKeySource` wartość `'Default'` .
...
### -EncryptionKeySource
Klucz szyfrowaniaSource (provider).
Możliwe wartości (bez uwzględniania liter): Domyślne, Microsoft.Keyvault
