...
## OPIS
To polecenie umożliwia zmiany parametrów dostosowywanych punktu końcowego serwera. Na przykład zasady warstwowania chmury i warstwy w chmurze można zmienić w dowolnym momencie. Po utworzeniu punktu końcowego serwera nie można zmienić kilku aspektów punktu końcowego serwera, takich jak ścieżka lokalna.
...
W tym przykładzie wykonuje dwie akcje, ustawia nowe zasady warstwy w chmurze dla określonego punktu końcowego serwera, co powoduje kierowanie serwera do warstw wszystkich plików, do których nie uzyskano dostępu w ciągu ostatnich 30 dni, oraz wyłącza tryb przesyłania danych w trybie offline, który był początkowo włączony w tym punkcie końcowym serwera podczas jego tworzenia. Transfer danych w trybie offline jest używany w ramach współdziałania z usługami migracji zbiorczej, takimi jak usługa Azure Data Box.
