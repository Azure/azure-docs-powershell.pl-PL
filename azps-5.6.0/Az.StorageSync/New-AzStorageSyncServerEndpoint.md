...
## OPIS
To polecenie tworzy nowy punkt końcowy serwera na zarejestrowanym serwerze. Dzięki temu określona ścieżka na serwerze może rozpocząć synchronizację plików z innymi punktami końcowymi w grupie synchronizacji. Jeśli w grupie synchronizacji znajdują się już pliki w innych punktach końcowych, a ta nowo dodana lokalizacja również zawiera pliki, proces uzgadniania spróbuje ustalić, czy pliki w rzeczywistości są takie same w tych samych folderach co w innych punktach końcowych. Przestrzenie nazw będą scalać i uzgadniać pliki, aby zapobiec plikom konfliktów. Jeśli istnieją pliki w innych punktach końcowych serwera, często lepszym rozwiązaniem jest rozpoczęcie od pustej lokalizacji na tym serwerze, aby pliki z chmury zeszły na serwer w procesie automatycznym nazywanym szybkim odzyskiwaniem po awarii. Metadane przestrzeni nazw zostaną najpierw zsynchronizowane w dół, a następnie zostanie pobrany strumień danych każdego pliku. Jeśli użytkownik lub aplikacja zażąda pliku poza kolejnością pobierania, ten plik zostanie odwołany z priorytetem w celu spełnienia żądania dostępu. Opcjonalnie możesz użyć określania warstw w chmurze dla tego punktu końcowego serwera, aby określić, czy ten punkt końcowy powinien stać się pamięcią podręczną pełnego zestawu plików z chmury. W przypadku przechowywania warstw w chmurze pobieranie zawartości pliku zakończy się w punkcie zdefiniowanym przez zasady warstwy w chmurze, które można ustawić.
...
To polecenie tworzy nowy punkt końcowy serwera na zarejestrowanym serwerze i wstawia go do grupy synchronizacji. THis way it is part of a topology of other endpoints and file metadata and content will immediately start to sync between all locations references as endpoints in the sync group.
...
### -OfflineDataTransfer
Parametr danych iniekowany w chmurze
...
Parametr Uri udostępniania pliku danych w chmurze
...
### - ParentObject
Obiekt SyncGroup, który zwykle przechodzi przez parametr.
...
### -ServerResourceId
Zarejestrowany identyfikator zasobu serwera
...
Parametr procentu wolnego miejsca (volume free space)
