...
## OPIS
Polecenie Set-AzApplicationGatewayBackendHttpSetting aktualizuje ustawienia protokołu HTTP (Hypertext Transfer Protocol) dla bramy aplikacji Azure.
Ustawienia protokołu HTTP typu Back-end są stosowane do wszystkich serwerów back-end w puli.

## PRZYKŁADY

### Przykład 1. Aktualizowanie ustawień protokołu HTTP dla bramy aplikacji
```
...
Pierwsze polecenie pobiera bramę aplikacji o nazwie ApplicationGateway01, która należy do grupy zasobów o nazwie ResourceGroup01, i przechowuje ją w zmiennej $AppGw zasobów.
Drugie polecenie aktualizuje ustawienia HTTP bramy aplikacji w zmiennej $AppGw, aby używać portu 88, protokołu HTTP i umożliwia chciwość opartą na plikach cookie.
...
### -ApplicationGateway
Określa obiekt bramy aplikacji, z którym to polecenie cmdlet skojarzy ustawienia protokołu HTTP na zawęzie.
...
Dopuszczalne wartości dla tego parametru to: Wyłączone lub Włączone.
...
### — Nazwa
Określa nazwę obiektu ustawień protokołu HTTP w kopii zapasowej.
...
Jeśli dla tego parametru nie podano żadnej wartości, oznacza to, że ścieżka nie będzie prefiksem.
...
### — Port
Określa port do użycia dla każdego serwera w puli serwerów back-end.
...
### — Dor.
Określa wartość do skojarzenia z ustawieniami protokołu HTTP.
...
### - NaiwnyId
Określa identyfikator współpracownika, który ma skojarzyć z ustawieniami protokołu HTTP na zawęzie.
...
### — Protokół
Określa protokół używany do komunikacji między bramą aplikacji a serwerami back-end.
Dopuszczalne wartości dla tego parametru to: Http i Https.
W tym parametrze jest zróżnicowa wielkość liter.
