...
## OPIS
Polecenie **cmdlet Add-AzApplicationGatewayUrlPathMapConfig** dodaje tablicę mapowań ścieżek adresu URL do puli serwerów back end.

## PRZYKŁADY

### Przykład 1. Dodawanie mapowania ścieżek adresu URL do bramy aplikacji.
```
...
Pierwsze polecenie pobiera bramę aplikacji o nazwie appGwName i przechowuje ją w $appgw zmienną.
Drugie polecenie pobiera pulę adresów zaplecza i przechowuje je w $pool danych.
Trzecie polecenie otrzymuje ustawienia http zaplecza i zapisuje je w $poolSettings zmienną.
Czwarte polecenie tworzy nową konfigurację reguł ścieżki o nazwie rule01 i zapisuje ją w $pathRule zmienną.
Piąte polecenie dodaje do bramy aplikacji konfigurację mapowania ścieżek adresów URL o nazwie url01.
Szóste polecenie aktualizuje bramę aplikacji.

## PARAMETERS

### -ApplicationGateway
Określa bramę aplikacji, do której to polecenie cmdlet dodaje konfigurację mapy ścieżki adresu URL.
...
Identyfikator domyślnego zestawu reguł ponownego edytowego bramy aplikacji
...
### — Nazwa
Określa nazwę mapy ścieżki adresu URL, którą to polecenie cmdlet dodaje do puli serwerów zaplecza.
...
Reguły ścieżki są poufne dla kolejności i są stosowane w określonej kolejności.
