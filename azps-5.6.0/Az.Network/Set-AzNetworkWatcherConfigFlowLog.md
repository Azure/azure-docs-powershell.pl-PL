...
## OPIS
Ta Set-AzNetworkWatcherConfigFlowLog konfiguruje rejestrowanie przepływu dla zasobu docelowego. Do właściwości, które należy skonfigurować, należą: czy rejestrowanie przepływu jest włączone dla udostępnianego zasobu, skonfigurowane konto magazynu do wysyłania dzienników, format rejestrowania przepływu i zasady przechowywania dla dzienników. Obecnie grupy zabezpieczeń sieci są obsługiwane do rejestrowania przepływu. 
...
W tym przykładzie konfigurujemy rejestrowanie przepływu w grupie zabezpieczeń sieci (NSG, Network Security Group) z określonymi dziennikami w wersji 2. W odpowiedzi widać, że dla określonego serwera NSG włączono rejestrowanie przepływu, ustawiono format i nie skonfigurowano żadnych zasad przechowywania. Jeśli region nie obsługuje określonej wersji, aplikacja Network Watcher zapisze domyślną obsługiwaną wersję w regionie.
...
W tym przykładzie konfigurujemy stan rejestrowania przepływu i analizę ruchu dla grupy zabezpieczeń sieci. W odpowiedzi widać, że określona NSG ma włączone rejestrowanie przepływu i analizę ruchu, format domyślny i nie są ustawione żadne zasady przechowywania.

### Przykład 4. Wyłączanie analizy ruchu dla określonego serwera NSG ze skonfigurowanym rejestrowaniem przepływu i analizą ruchu
```
PS C:\> $NW = Get-AzNetworkWatcher -ResourceGroupName NetworkWatcherRg -Name NetworkWatcher_westcentralus
PS C:\> $nsg = Get-AzNetworkSecurityGroup -ResourceGroupName NSGRG -Name appNSG
PS C:\> $storageId = "/subscriptions/bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb/resourceGroups/NSGRG/providers/Microsoft.Storage/storageAccounts/contosostorageacct123"
...
W tym przykładzie wyłączyliśmy analizę ruchu dla grupy zabezpieczeń sieci, w której skonfigurowano wcześniej rejestrowanie przepływu i analizę ruchu. W odpowiedzi widać, że dla określonego serwera NSG włączono rejestrowanie przepływu, ale funkcja Analizy ruchu została wyłączona.
...
Pobiera lub ustawia interwał (w minutach), który określa, jak często usługa TA powinna wykonać analizę przepływu. Obsługiwane wartości to 10 i 60 minut.
