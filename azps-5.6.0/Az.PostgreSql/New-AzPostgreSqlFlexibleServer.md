...
### Przykład 1. Tworzenie nowego serwera elastycznego PostgreSql z argumentami
```powershell
...
### Przykład 2. Tworzenie nowego elastycznego serwera PostgreSql z ustawieniem domyślnym
```powershell
...
To polecenie cmdlet tworzy elastyczny serwer PostgreSql z domyślnymi wartościami parametrów i zapewnianie obsługi administracyjnej serwera w nowej sieci wirtualnej oraz delegowanie podsieci do serwera.
Domyślna wartość lokalizacji to Zachód STANÓW Zjednoczonych 2, SKU to Standard_D2s_v3, warstwa SKU to GeneralPurpose, a rozmiar magazynu to 128GiB.
...
### Przykład 3. Tworzenie nowego elastycznego serwera PostgreSql z siecią wirtualną
```powershell
PS C:\> $Vnet = 'vnetname'
...
To polecenie cmdlet tworzy elastyczny serwer PostgreSql z identyfikatorem vnet lub nazwą sieci vnet podaną przez użytkownika.
Jeśli sieć wirtualna nie istnieje, polecenie cmdlet tworzy sieć.

### Przykład 4. Tworzenie nowego elastycznego serwera PostgreSql z siecią wirtualną i nazwą podsieci
```powershell
...
To polecenie cmdlet tworzy elastyczny serwer PostgreSql z nazwą sieci vnet, nazwą podsieci, prefiksem podsieci i prefiksem podsieci.
Jeśli sieć wirtualna i podsieci wirtualne nie istnieją, polecenie cmdlet utworzy je.

### Przykład 7. Tworzenie nowego elastycznego serwera PostgreSql z publicznym dostępem do wszystkich ip
```powershell
...
To polecenie cmdlet tworzy elastyczny serwer PostgreSql otwarty dla wszystkich adresów IP.

### Przykład 8. Tworzenie nowego elastycznego serwera PostgreSql z zaporą
```powershell
...
To polecenie cmdlet tworzy elastyczny serwer PostgreSql otwarty dla określonych adresów IP.
...
Hasło musi zawierać znaki z trzech z następujących kategorii: angielskie wielkie litery, angielskie małe litery, cyfry i znaki niealalnumeryczne.
...
### - SKU
Nazwa sKU, zwykle warstwa + rodzina + core, na przykład Standard_B1ms, Standard_D2ds_v4.
...
Należy pamiętać, że podsieci zostanie delegowana na serwery Microsoft.DBforPostgreSQL/flexibleServers.
...
### — Strefa
Strefa dostępności do obsługi administracyjnej zasobu.
