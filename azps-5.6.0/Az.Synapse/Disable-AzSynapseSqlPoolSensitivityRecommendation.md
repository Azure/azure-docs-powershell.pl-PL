...
## OPIS
Polecenie Disable-AzSynapseSqlPoolSensitivityRecommendation wyłącza (odrzuca) zalecenia dotyczące wrażliwości kolumn w puli sql.

## PRZYKŁADY

### Przykład 1. Wyłączanie rekomendacji wrażliwości dla danej kolumny w puli sql Synapse Azure.
```powershell
...
### Przykład 2. Wyłącz zalecenia dotyczące wrażliwości dla kolumn, które mają zalecenia wrażliwości w puli sql Synapse Azure przy użyciu funkcji Piping.
```powershell
...
### Przykład 3. Wyłączanie rekomendacji wrażliwości dla danej kolumny w puli sql Synapse Azure przy użyciu połączenia rurowego.
```powershell
