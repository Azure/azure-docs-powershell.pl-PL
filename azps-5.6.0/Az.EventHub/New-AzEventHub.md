...
### EventhubPropertiesSet (domyślne)
```
...
## OPIS
Polecenie New-AzEventHub cmdlet tworzy nowe centrum zdarzeń platformy Azure.
Aby utworzyćhubę zdarzenia z właściwościami przechwytywania opisu, wykonaj poniższe czynności (przykłady 2). 

## PRZYKŁADY

### Przykład 1. Tworzenie nowej aplikacji EventHub
```powershell
PS C:\> New-AzEventHub -ResourceGroupName MyResourceGroupName -NamespaceName MyNamespaceName -Name MyEventHubName
```

Tworzy centrum zdarzeń o nazwie MyEventHubName z 3-dniowym okresem przechowywania wiadomości i dwoma partycjami w lokalizacji WestUS z grupą zasobów \` \` \` \` \` MyResourceGroupName. \`

### Przykład 2. Aktualizowanie aplikacji Eventhub za pomocą funkcji CaptureDescription
```powershell
...
Tworzy centrum zdarzeń o nazwie MyEventHubName z 3-dniowym okresem przechowywania wiadomości, 2 partycjami i właściwościami CaptureDescription w lokalizacji WestUS z grupą zasobów \` \` \` \` \` MyResourceGroupName. \`
...
### -InputObject
Obiekt Wprowadzania w usłudze EventHub
...
Przechowywanie wiadomości w usłudze Eventhub w dniach
