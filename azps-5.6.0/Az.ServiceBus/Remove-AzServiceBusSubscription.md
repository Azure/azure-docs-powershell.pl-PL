...
## OPIS
Polecenie **cmdlet Remove-AzServiceBusSubscription** usuwa subskrypcję do tematu z określonej przestrzeni nazw typu bus usługi.
...
Usuwa subskrypcję do `SB-TopicSubscription-Example1` tematu w określonej przestrzeni nazw `SB-Topic_exampl1` autobusu `SB-Example1` usług.

### Przykład 2. InputObject — używanie zmiennej:
```powershell
PS C:\> $inputobject = Get-AzServiceBusSubscription <params>
PS C:\> Remove-AzServiceBusSubscription -InputObject $inputobject
```

### Przykład 3. InputObject — używanie funkcji rurowych:
```powershell
...
Usuwa subskrypcję z identyfikatora ARM w ciągu $resourceid/string dla parametru -ResourceId. 
...
### -ResourceId
Identyfikator zasobu subskrypcji usługi bus
