...
## OPIS
Funkcja **Enable-AzFrontDoorCustomDomainHttps** włącza protokół HTTPS dla domeny niestandardowej.

## PRZYKŁADY

### Przykład 1. Włączanie protokołu HTTPS dla domeny niestandardowej za pomocą właściwości FrontDoorName i ResourceGroupName przy użyciu certyfikatu zarządzanego front door.
```powershell
...
Włącz protokół HTTPS dla domeny niestandardowej "frontendpointname1-custom-xyz", która jest częścią frontdooru "frontdoor1" w grupie zasobów "resourcegroup1" przy użyciu certyfikatu zarządzanego front door.

### Przykład 2. Włączanie protokołu HTTPS dla domeny niestandardowej za pomocą właściwości FrontDoorName i ResourceGroupName przy użyciu własnego certyfikatu w magazynie kluczy.
```powershell
...
### Przykład 3. Włączanie protokołu HTTPS dla domeny niestandardowej przy użyciu obiektu PSFrontendEndpoint przy użyciu certyfikatu zarządzanego front door.
```powershell
...
Włącz protokół HTTPS dla domeny niestandardowej za pomocą obiektu PSFrontendEndpoint przy użyciu certyfikatu zarządzanego przez front door.

### Przykład 4. Włączanie protokołu HTTPS dla domeny niestandardowej z identyfikatorem zasobu przy użyciu certyfikatu zarządzanego przez front door.
```powershell
...
Włącz protokół HTTPS dla domeny niestandardowej "frontendpointname1-custom-xyz" przy użyciu identyfikatora zasobu $resourceId użyciu certyfikatu zarządzanego front door.
...
Nazwa punktu końcowego frontendu.
...
### -InputObject
Obiekt punktu końcowego frontendu do zaktualizowania.
...
### -MinimumTlsVersion
Minimalna wersja TLS wymagana przez klientów do nawiązania uścisku SSL z drzwiami przednimi.
...
### -ResourceId
Identyfikator zasobu punktu końcowego drzwi frontowych w celu włączenia https
...
### -SecretName
Nazwa klucza magazynu kluczy reprezentująca pełny certyfikat PFX
...
### - SecretVersion
Wersja klucza klucza o kluczu tajnym reprezentująca pełny certyfikat PFX
...
### -VaultId
Identyfikator magazynu kluczy zawierający certyfikat SSL
