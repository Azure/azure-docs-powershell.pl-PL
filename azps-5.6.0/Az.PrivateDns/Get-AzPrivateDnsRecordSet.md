...
## OPIS
Polecenie Get-AzPrivateDnsRecordSet pobiera zestaw rekordów DNS (Private Domain Name System) o określonej nazwie i typie w określonej strefie prywatnej. Jeśli nie określisz parametrów Name (Nazwa) lub RecordType (Typ Rekordu), to polecenie cmdlet zwróci wszystkie zestawy rekordów określonego typu w strefie prywatnej. Jeśli określisz parametr RecordType, ale nie parametr Name, to polecenie cmdlet zwróci wszystkie zestawy rekordów określonego typu rekordu. Za pomocą operatora potoku możesz przekazać obiekt PSPrivateDnsZone do tego polecenia cmdlet lub przekazać obiekt PSPrivateDnsZone jako parametr Zone lub możesz również określić nazwę grupy zasobów i strefy. Możesz także określić strefę prywatną przy użyciu identyfikatora zasobu strefy prywatnej.
...
To polecenie pobiera zestaw rekordów typu A o nazwie www w określonej grupie zasobów i strefie prywatnej, a następnie przechowuje go w $RecordSet zmienną. Ponieważ parametry Name (Nazwa) i RecordType (Typ Rekordu) są określone, zwracany jest tylko jeden obiekt RecordSet.
...
To polecenie pobiera tablicę wszystkich zestawów rekordów typu A w strefie prywatnej o nazwie myzone.com w grupie zasobów o nazwie MyResourceGroup, a następnie przechowuje je w $RecordSets danych.

### Przykład 3. Uzyskiwanie wszystkich zestawów rekordów w strefie prywatnej
```powershell
...
To polecenie pobiera tablicę wszystkich zestawów rekordów w strefie prywatnej o nazwie myzone.com w grupie zasobów o nazwie MyResourceGroup, a następnie przechowuje je w zmiennej $RecordSets zasobów.

### Przykład 4. Uzyskiwanie wszystkich zestawów rekordów w strefie prywatnej przy użyciu obiektu PSPrivateDnsZone
```powershell
...
Ten przykład jest odpowiednikiem powyższego przykładu 3. Tym razem strefa prywatna jest określana przy użyciu obiektu strefy prywatnej.
...
### — Strefa
Obiekt DnsZone reprezentujący strefę, w której należy utworzyć zestaw rekordów.
