...
## OPIS
Polecenie Add-AzPrivateDnsRecordConfig cmdlet dodaje rekord DNS (Private Domain Name System) do obiektu RecordSet. Obiekt RecordSet jest obiektem w trybie offline i zmiany w nim nie zmieniają prywatnych odpowiedzi DNS do momentu uruchomienia polecenia cmdlet Set-AzPrivateDnsRecordSet w celu utrwalenia zmiany w prywatnej usłudze DNS platformy Microsoft Azure. Rekordy SOA są tworzone po utworzeniu prywatnej strefy DNS i usuwane po usunięciu prywatnej strefy DNS. Nie można dodawać ani usuwać rekordów SOA, ale można je edytować. Obiekt RecordSet można przekazać do tego polecenia cmdlet jako parametr lub za pomocą operatora potoku.
...
W tym przykładzie do istniejącego zestawu rekordów został dodano rekord A.
...
Ten przykład powoduje dodanie rekordu AAAAA do istniejącego zestawu rekordów.
...
Ten przykład powoduje dodanie rekordu MX do istniejącego zestawu rekordów.
...
### - Cname
Kanoniczna nazwa rekordu CNAME do dodania.
...
### — Exchange
Host wymiany poczty dla rekordu MX do dodania.
Nie może być względna względem nazwy strefy.
Nie może zawierać kropki kończącej
...
### -Ipv4Address
Adres IPv4 rekordu A do dodania.
...
### -Ipv6Address
Adres IPv6 rekordu AAAA do dodania.
...
### — Priority (Priorytet)
Rekord SRV o priorytecie, który należy dodać.
...
### — Waga
Wartość wagi dla rekordu SRV do dodania.
