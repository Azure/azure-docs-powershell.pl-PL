...
### Przykład 1. Sprawdzanie poprawności parametrów połączenia danych aplikacji EventHub
```powershell
...
Powyższe polecenie sprawdza połączenie danych aplikacji EventHub o nazwie "myeventhubdc" dla bazy danych "mykustodatabase" w klastrze "testnewkustocluster".

### Przykład 2. Sprawdzanie poprawności parametrów połączenia danych z siecią EventGrid
```powershell
...
Powyższe polecenie sprawdza połączenie danych aplikacji EventGrid o nazwie "myeventgriddc" dla bazy danych "mykustodatabase" w klastrze "testnewkustocluster".

### Przykład 3. Sprawdzanie poprawności parametrów połączenia danych aplikacji IotHub
```powershell
...
Powyższe polecenie sprawdza połączenie danych aplikacji IotHub o nazwie "myiothubdc" dla bazy danych "mykustodatabase" w klastrze "testnewkustocluster".

### Przykład 4. Sprawdzanie poprawności parametrów połączenia danych aplikacji EventHub za pomocą tożsamości
```powershell
...
### Przykład 5. Sprawdzanie poprawności parametrów połączenia danych z aplikacji EventGrid za pomocą tożsamości
```powershell
PS C:\> $database = Get-AzKustoDatabase -ResourceGroupName "testrg" -ClusterName "testnewkustocluster" -DatabaseName "mykustodatabase"
...
### Przykład 6. Sprawdzanie poprawności parametrów połączenia danych aplikacji IotHub za pomocą tożsamości
```powershell
PS C:\> $database = Get-AzKustoDatabase -ResourceGroupName "testrg" -ClusterName "testnewkustocluster" -DatabaseName "mykustodatabase" 
...
Nazwa typu zdarzenia magazynu obiektów blob do przetwarzania.
...
### - ConsumerGroup
Grupa klientów z centrum zdarzeń/centrum wydarzeń.
...
### -EventHubResourceId
Identyfikator zasobu centrum zdarzeń używany do tworzenia połączenia danych/siatki zdarzeń jest skonfigurowany do wysyłania zdarzeń.
...
### -EventSystemProperty
Właściwości systemowe centrum zdarzeń/iot.
...
### -IgnoreFirstRecord
Jeśli ta wartość jest ustawiona na prawda, oznacza, że ingestion powinien zignorować pierwszy rekord każdego pliku.
...
### -MappingRuleName
Reguła mapowania, która ma być używana do pozysowywania danych.
Opcjonalnie informacje o mapowaniu można dodać do każdej wiadomości.
...
Nazwa zasad dostępu do udostępniania.
...
### -TableName
Tabela, w której powinny być zbierane dane.
Opcjonalnie do każdej wiadomości można dodać informacje z tabeli.
...
### Microsoft.Azure.PowerShell.cmdlet.kusto.models.api20200918.IDataConnectionValidationResult
