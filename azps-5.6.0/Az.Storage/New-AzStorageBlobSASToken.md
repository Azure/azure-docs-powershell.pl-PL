...
### BlobNameWithPermission (Domyślna)
```
...
## OPIS
Polecenie **cmdlet New-AzStorageBlobSASToken** generuje token SAS (Shared Access Signature) dla obiektu blob magazynu platformy Azure.

## PRZYKŁADY

### Przykład 1. Generowanie tokenu SAS obiektu blob z pełnymi uprawnieniami obiektów blob
```
PS C:\>New-AzStorageBlobSASToken -Container "ContainerName" -Blob "BlobName" -Permission rwd
```

W tym przykładzie jest generowany token SAS obiektu blob z pełnymi uprawnieniami obiektów blob.
...
### Przykład 3. Generowanie tokenu SAS tożsamości użytkownika z kontekstem magazynowania na podstawie uwierzytelniania OAuth
```
...
W tym przykładzie jest generowany token SAS obiektu blob tożsamości użytkownika z kontekstem magazynu na podstawie uwierzytelniania OAuth
...
Gdy kontekst magazynu jest oparty na uwierzytelnieniu OAuth, wygeneruje token SAS obiektu blob tożsamości użytkownika.
...
### -ExpiryTime
Określa, kiedy wygasa podpis dostępu udostępnionego.
...
### — Uprawnienie
Określa uprawnienia dla obiektu blob magazynu. Należy pamiętać, że jest to ciąg, taki jak `rwd` (w przypadku odczytu, zapisu i usunięcia). 
