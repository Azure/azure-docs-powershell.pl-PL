...
## OPIS
Polecenie cmdlet **Close-AzStorageFileHandle** zamyka uchwyty pliku udziału plików, katalogu plików lub pliku.

## PRZYKŁADY

### Przykład 1. Wyświetla listę wszystkich udziałów plików bieżącego konta magazynu i rekurencyjnie zamyka wszystkie uchwyty plików udziałów plików.
```
PS C:\>Get-AzStorageShare | Close-AzStorageFileHandle -CloseAll -Recursive
```

To polecenie wyświetla listę wszystkich udziałów plików bieżącego konta magazynu i rekurencyjnie zamyka wszystkie uchwyty plików udziałów plików.

### Przykład 2. Ponowne zamykanie wszystkich uchwytów plików w katalogu plików i wyświetlanie liczby uchwytów zamkniętego pliku
```
PS C:\>Close-AzStorageFileHandle -ShareName "mysharename" -Path 'dir1/dir2' -Recursive -CloseAll -PassThru
10
```

To polecenie zamyka wszystkie uchwyty plików w katalogu plików i wyświetla liczbę zamkniętych uchwytów plików.

### Przykład 3. Zamknięcie wszystkich uchwytów plików otwieranych 1 dzień temu w katalogu plików
```
...
To polecenie wyświetla listę wszystkich uchwytów pliku w katalogu plików, odfiltrowuje uchwyty otwarte 1 dzień temu, a następnie je zamyka.

### Przykład 4. Zamknięcie wszystkich uchwytów pliku
```
PS C:\>Close-AzStorageFileHandle -ShareName "mysharename" -Path 'dir1/dir2/test.txt' -CloseAll
```

To polecenie zamyka wszystkie uchwyty pliku.
...
### -CloseAll
Wymusz zamknięcie wszystkich uchwytów pliku.
...
### — Plik
Obiekt CloudFile wskazywał plik do zamknięcia.
...
### -PassThru
Zwraca liczbę uchwytów zamkniętego pliku.
