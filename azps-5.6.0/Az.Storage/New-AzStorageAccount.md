...
### Przykład 3. Tworzenie konta magazynu przy użyciu urządzenia Kind StorageV2 oraz generowanie i przypisywanie tożsamości dla usługi Azure KeyVault.
```powershell
...
### Przykład 7. Utworzenie konta magazynu z włączym uwierzytelnianiem usługi domenowej Active Directory w plikach.
```
...
### Przykład 9. Tworzenie konta MinimumTlsVersion i AllowBlobPublicAccess oraz wyłączanie dostępu do klucza udostępnionego
```
...
Polecenie utwórz konto z wartością MinimumTlsVersion, AllowBlobPublicAccess i wyłącz dostęp kluczu udostępnionego do konta, a następnie pokaż 3 właściwości utworzonego konta. 

### Przykład 10. Tworzenie konta magazynu przy użyciu ustawienia RoutingPreference
```powershell
...
- StorageV2. Konto magazynu ogólnego przeznaczenia w wersji 2 (GPv2), które obsługuje obiekty blob, tabele, kolejki, pliki i dyski, z zaawansowanymi funkcjami, takich jak warstwowanie danych.
...
Usługa zastosuje pomocniczą warstwę szyfrowania z kluczami zarządzanymi platformy dla danych w spoczynku.
...
- Premium_LRS. Premium lokalnie nadmiarowa przestrzeń dyskowa.
