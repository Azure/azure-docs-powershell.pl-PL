...
Tworzenie nowej bezpośredniej komunikacji równorzędnej z jednym połączeniem w obiekcie Seattle przy użyciu usługi PeerAsn 65000

### Tworzenie nowej komunikacji równorzędnej programu Exchange
```powershell
#Gets the ASN
PS C:> $asn = Get-AzPeerAsn -PeerName Contoso
...
### Konwertowanie starszej komunikacji równorzędnej na ARM równorzędną
```powershell
#Gets the ASN
PS C:> $asn = Get-AzPeerAsn -PeerName Contoso
...
### - DirectConnection
Utwórz nowe połączenia bezpośrednie, używając New-AzExchangePeeringConnection i potoku do tego polecenia.
...
### - ExchangeConnection
Utwórz nowe połączenia programu Exchange, używając New-AzExchangePeeringConnection i potoku do tego polecenia.
...
### — MicrosoftNetwork
Wybierz sieć firmy Microsoft, z którą chcesz się schować.
...
### -PeeringLocation
Lokalizacja fizyczna inna niż region platformy Azure.
Użyj Get-AzPeeringLocation -Rodzaj \<kind\> użyj nazwy miasta jako klucza.
...
### - SKU
Wybierz Basic_Direct_Free lub Premium_Direct_Free, chyba że jawnie zasuń inną opcję.
...
### — Tag
Tagi do skojarzenia z usługą komunikacji równorzędnej firmy Microsoft.
