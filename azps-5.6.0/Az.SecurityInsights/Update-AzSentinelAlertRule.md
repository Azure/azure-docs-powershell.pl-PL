...
## OPIS
Polecenie **cmdlet Update-AzSentinelAlertRule** aktualizuje regułę analityczna (reguła alertu) w określonym obszarze roboczym.
Można użyć wartości -InputObject, -ResourceId lub -AlertId.  Możesz zaktualizować 1 lub więcej parmaterów proprtery.
...
W tym przykładzie zmieniono ustawienie wartości **AlertRule** na *Disabled* (Wyłączone) i zmieniono nazwę na *Disabled-AlertRuleDisplayName*(Wyłączone).  Wszystkie pozostałe właściwości pozostaną takie same.
...
W tym przykładzie **aktualizacja alertu AlertRule** została wyłączona przy użyciu ustawienia InputObject.  Wszystkie pozostałe właściwości pozostaną takie same.
...
Reguła alertu wyświetla nazwy wyklucz filtru.
...
### -NajbłędszaWyświetlona
Reguła alertu jest wyłączona.
