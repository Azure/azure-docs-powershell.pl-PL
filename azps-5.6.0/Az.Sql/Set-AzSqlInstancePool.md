...
### DefaultSetInstancePoolParameterSet (Domyślne)
```
...
## OPIS
Polecenie **cmdlet Set-AzSqlInstancePool** modyfikuje właściwości puli wystąpień języka Azure SQL.

## PRZYKŁADY

### Przykład 1. Ustawianie typu licencji puli wystąpień
```powershell
...
To polecenie ustawia typ licencji i/lub tagi dla puli wystąpień o nazwie instancePool0.

### Przykład 2. Ustawianie typu licencji puli wystąpień przy użyciu obiektu puli wystąpień
```powershell
...
To polecenie ustawia typ licencji i/lub tagi puli wystąpień przy użyciu obiektu puli wystąpień.

### Przykład 3. Ustawianie typu licencji puli wystąpień przy użyciu identyfikatora zasobu puli wystąpień
```powershell
...
### -InputObject
Obiekt wejściowy puli wystąpień.
...
### -LicenseType
Określenie typu licencji, którego należy użyć.
Dopuszczalne wartości to Cena Bazowa (z rabatem AHB) i LicenseIncluded (bez rabatu AHB).
