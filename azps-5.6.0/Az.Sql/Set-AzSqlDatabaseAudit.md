...
RECEIVE  
REFERENCES  
Ogólnym formularzem definiującym akcję, która ma być poddana inspekcji, jest: [akcja] ON [obiekt] BY [podmiot] Zauważ, że [obiekt] w powyższym formacie może odwoływać się do obiektu, takiego jak tabela, widok lub procedura składowana, albo do całej bazy danych lub schematu. W innych przypadkach używane są odpowiednio formularze DATABASE::[nazwa_bazy danych] i SCHEMA::[nazwa_schematu].
Na przykład:  
SELECT on dbo.myTable by public  
Select on DATABASE::myDatabase by public  
SELECT w schemacie::mySchema by public  
Aby uzyskać więcej informacji, zobacz https://docs.microsoft.com/sql/relational-databases/security/auditing/sql-server-audit-action-groups-and-actions#database-level-audit-actions .
