...
## SYNOPSIS
Tworzy aplikację logiczną w grupie zasobów.
...
## OPIS
Polecenie **cmdlet New-AzLogicApp** tworzy aplikację logiki przy użyciu funkcji aplikacje logic.
Aplikacja logiki to zbiór akcji lub wyzwalaczy zdefiniowanych w definicji aplikacji logiki.
To polecenie cmdlet zwraca obiekt **przepływu** pracy.
Możesz utworzyć aplikację logiki, określając nazwę, lokalizację, definicję aplikacji logiki, grupę zasobów i plan.
Definicja i parametry aplikacji logiki są sformatowane w notacji obiektów JavaScript (JSON).
Jako szablonu definicji i parametrów można użyć aplikacji logiki.
...
### Przykład 1. Tworzenie aplikacji logiki przy użyciu ścieżek plików definicji i parametrów
```
...
To polecenie umożliwia utworzenie aplikacji logiki w określonej grupie zasobów.
Aplikacja logiki zawiera definicję i parametry określone przez ścieżki plików.

### Przykład 2. Tworzenie aplikacji logiki przy użyciu obiektów definicji i parametrów
```
...
To polecenie tworzy aplikację logiki w grupie zasobów określonej grupy zasobów.

### Przykład 3. Tworzenie aplikacji logiki przy użyciu potoku w celu określenia grupy zasobów
```
...
To polecenie pobiera grupę zasobów o nazwie ResourceGroup11 przy użyciu Get-AzResourceGroup cmdlet.
Polecenie przekazuje grupę zasobów do bieżącego polecenia cmdlet przy użyciu operatora potoku.
Bieżące polecenie cmdlet tworzy aplikację logiczną w tej grupie zasobów.
Aplikacja logiki zawiera definicję i parametry określone przez ścieżki plików.

### Przykład 4. Tworzenie aplikacji logiki na podstawie istniejącej aplikacji logiki
```
...
Pierwsze polecenie pobiera aplikację logiki o nazwie LogicApp03 za pomocą Get-AzLogicApp cmdlet.
Polecenie zapisuje aplikację logiki w $Workflow zmienną.
Drugie polecenie tworzy nową aplikację logiki, która korzysta z definicji i parametrów aplikacji logiki przechowywanej w $Workflow.
...
### —Definicja
Określa definicję aplikacji logiki jako obiekt lub ciąg w formacie JSON (JavaScript Object Notation).
...
### — Lokalizacja
Określa lokalizację aplikacji logiki.
Wprowadź lokalizację centrum danych platformy Azure, taką jak Stany Zjednoczone i Azja Południowo-Wschodnia.
Aplikację logiki można umieścić w dowolnej lokalizacji.
...
### -ParameterFilePath
Określa ścieżkę pliku sformatowanych parametrów JSON.
...
### — Parametry
Określa obiekt kolekcji parametrów dla aplikacji logiki.
Określ tabelę skrótu, słownik \<string\> lub \<string, WorkflowParameter\> słownik.
...
### — województwo
Określa stan aplikacji logiki.
Dopuszczalne wartości dla tego parametru to: Włączone i Wyłączone.
