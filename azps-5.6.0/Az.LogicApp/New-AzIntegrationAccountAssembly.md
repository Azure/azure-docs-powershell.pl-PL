...
## SYNOPSIS
Tworzy zestaw konta integracji.
...
Tworzy nowy zestaw przy użyciu pliku lokalnego znajdującego się w ścieżce pliku zawartej w pliku "$localAssemblyFilePath".

### Przykład 2. Tworzenie nowego zestawu przy użyciu danych bajtowych
```powershell
...
Tworzy nowy zestaw przy użyciu tablicy bajtów zawartej w "$assemblyContent".

### Przykład 3. Tworzenie nowego zestawu przy użyciu linku zawartości
```powershell
...
Tworzy nowy zestaw przy użyciu danych bajtowych znajdujących się w adresie URL "$assemblyUrl". Jest to sugerowana metoda tworzenia zespołów o dużych rozmiarach.
