...
## SYNOPSIS
Tworzy pulę w usłudze wsadowej.

## SKŁADNIA

### CloudServiceAndTargetDedicated (domyślne)
```
...
## OPIS
Polecenie **cmdlet New-AzBatchPool** tworzy pulę w usłudze Azure Batch w ramach konta określonego przez *parametr BatchContext.*

## PRZYKŁADY

### Przykład 1. Tworzenie nowej puli przy użyciu zestawu parametrów TargetDedicated przy użyciu parametru CloudServiceConfiguration
```
...
Pula jest skonfigurowana do korzystania z STANDARD_D1_V2 maszyn wirtualnych w wersji systemu operacyjnego dla czteroszpowiątowej rodziny.

### Przykład 2. Tworzenie nowej puli przy użyciu zestawu parametrów TargetDedicated przy użyciu parametru VirtualMachineConfiguration
```
...
To polecenie tworzy nową pulę o identyfikatorze MyPool przy użyciu zestawu parametrów TargetDedicated.
Alokacja docelowa to trzy węzły obliczeniowe.
Pula jest skonfigurowana do używania STANDARD_D1_V2 maszyn wirtualnych z obrazem systemu operacyjnego Centrum danych w systemie operacyjnym Windows-2016.

### Przykład 3. Tworzenie nowej puli przy użyciu zestawu parametrów Autoscale
```
PS C:\>$imageReference = New-Object -TypeName "Microsoft.Azure.Commands.Batch.Models.PSImageReference" -ArgumentList @("WindowsServer", "MicrosoftWindowsServer", "2016-Datacenter", "*")
PS C:\>$configuration = New-Object -TypeName "Microsoft.Azure.Commands.Batch.Models.PSVirtualMachineConfiguration" -ArgumentList @($imageReference, "batch.node.windows amd64")
...
To polecenie tworzy nową pulę z identyfikatorem AutoScalePool przy użyciu zestawu parametrów Autoscale.
Pula jest skonfigurowana do używania STANDARD_D1_V2 maszyn wirtualnych z obrazem systemu operacyjnego centrum danych w systemie Windows-2016, a docelowa liczba węzłów obliczeniowych jest ustalana na podstawie formuły Skala automatyczna.
...
### -AutoScaleEvaluationIntervalation
Określa czas (w minutach), który upływa, zanim rozmiar puli zostanie automatycznie dostosowany zgodnie z formułą Autoskalowanie.
...
### -AutoScaleFormula
Określa formułę automatycznego skalowania puli.
...
Usługa wsadowa instaluje certyfikaty, do których się odwołujesz, na każdym węźle obliczeniowym puli.
...
Określa ustawienia konfiguracji puli na podstawie platformy usług Azure w chmurze.
...
### — Identyfikator
Określa identyfikator puli, która ma być tworzyć.
...
Wskazuje, że to polecenie cmdlet konfiguruje pulę na komunikacji bezpośredniej między dedykowanymi węzłami obliczeniowymi.
...
Określa maksymalną liczbę zadań, które można uruchomić w pojedynczym węźle obliczeniowym.
...
### — Metadane
Określa metadane (jako pary klucz/wartość), które mają być dodane do nowej puli.
...
### - MountConfiguration
Lista systemów plików do instalacji na każdym węźle w puli. Obsługuje to pliki Azure, NFS, CIFS/SMB i blobfuse.
...
### - ResizeTimeout
Określa przechył czasu na przydzielanie węzłów obliczeniowych do puli.
...
### - StartTask
Określa specyfikację zadania rozpoczęcia dla puli.
Zadanie startowe jest uruchamiane, gdy węzeł obliczeniowy łączy się z pulą lub gdy węzeł obliczeniowy jest ponownie uruchamiany lub ponownie uruchamiany.
...
Określa docelową liczbę dedykowanych węzłów obliczeniowych do przydzielenia puli.
...
Określa docelową liczbę węzłów obliczeniowych o niskim priorytecie, które należy przydzielić do puli.
...
Określa zasady planowania zadań, na przykład typ ComputeNodeFillType.
...
### -UserAccount
Lista kont użytkowników, które mają zostać utworzone w każdym węźle w puli.
