...
## SYNOPSIS
Uzyskuje wynik What-If dla wdrożenia w zakresie grupy zarządzania. 
...
## OPIS
Polecenie **cmdlet Get-AzManagementGroupDeploymentWhatIfResult** pobiera wynik szablonu ARM What-If dla wdrożenia szablonu w określonym zakresie grupy zarządzania. Zwraca ona listę zmian wskazujących, które zasoby zostaną zaktualizowane, jeśli wdrożenie zostanie zastosowane bez dokonywania żadnych zmian w zasobach rzeczywistych. Aby określić format zwracanych wyników, użyj *parametru ResultFormat.*

## PRZYKŁADY

### Przykład 1. Uzyskiwanie What-If w zakresie grupy zarządzania
```powershell
...
To polecenie uzyskuje What-If w zakresie grupy zarządzania przy użyciu pliku szablonu niestandardowego i pliku parametru na dysku.
To polecenie używa *parametru Location* w celu określenia, gdzie mają być przechowywane dane wdrożenia.
To polecenie używa *parametru ManagementGroupId* w celu określenia grupy zarządzania, w której szablon zostanie wdrożony.
To polecenie określa plik szablonu za pomocą parametru *TemplateFile.*
To polecenie określa plik parametru szablonu za pomocą parametru *TemplateParametersFile.*
To polecenie używa *parametru ResultFormat* w celu What-If wyniku tak, aby uwzględniał pełne obciążenia zasobów.

### Przykład 2. Uzyskiwanie What-If w zakresie grupy zarządzania przy użyciu tylko zasobu ResourceIdOnly
```powershell
PS C:\> Get-AzManagementGroupDeploymentWhatIfResult `
    -DeploymentName "deploy-01" `
    -Location "West US" `
...
W przypadku, gdy plik szablonu jest podany, domyślnie jest określana jego nazwa domyślna. domyślnie jest to bieżąca godzina, gdy obiekt szablonu jest podany, na przykład "20131223140835".
