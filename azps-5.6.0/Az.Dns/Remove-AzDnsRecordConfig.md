...
## SYNOPSIS
Usuwa rekord DNS z obiektu lokalnego zestawu rekordów.
...
## OPIS
Polecenie **cmdlet Remove-AzDnsRecordConfig** usuwa rekord DNS (Domain Name System) z zestawu rekordów.
Obiekt **RecordSet** jest obiektem offline i zmiany w nim nie zmieniają odpowiedzi DNS do momentu uruchomienia polecenia cmdlet Set-AzDnsRecordSet w celu utrwalenia zmiany w usłudze DNS platformy Microsoft Azure.
Aby można było usunąć rekord, wszystkie pola tego typu muszą być dokładnie zgodne.
Nie można dodawać ani usuwać rekordów SOA.
Rekordy SOA są tworzone automatycznie po utworzeniu strefy DNS i automatyczne usunięciu jej po jej usunięciu.
...
W tym przykładzie jest usuwany rekord A z istniejącego zestawu rekordów.
Jeśli jest to jedyny rekord w zestawie rekordów, wynikiem będzie pusty zestaw rekordów.
Aby całkowicie usunąć zestaw rekordów, zobacz Remove-AzDnsRecordSet.
...
W tym przykładzie jest usuwany rekord AAAA z istniejącego zestawu rekordów.
Jeśli jest to jedyny rekord w zestawie rekordów, wynikiem będzie pusty zestaw rekordów.
Aby całkowicie usunąć zestaw rekordów, zobacz Remove-AzDnsRecordSet.
...
W tym przykładzie jest usuwany rekord CNAME z istniejącego zestawu rekordów.
Ponieważ zestaw rekordów CNAME może zawierać co najwyżej jeden rekord, wynikiem jest pusty zestaw rekordów.
...
W tym przykładzie jest usuwany rekord MX z istniejącego zestawu rekordów.
Nazwa rekordu "@" oznacza zestaw rekordów w wierzchołku strefy.
Jeśli jest to jedyny rekord w zestawie rekordów, wynikiem jest pusty zestaw rekordów.
Aby całkowicie usunąć zestaw rekordów, zobacz Remove-AzDnsRecordSet.

### Przykład 5. Usuwanie rekordu NS ze zestawu rekordów
```
...
W tym przykładzie jest usuwany rekord NS z istniejącego zestawu rekordów.
Jeśli jest to jedyny rekord w zestawie rekordów, wynikiem jest pusty zestaw rekordów.
Aby całkowicie usunąć zestaw rekordów, zobacz Remove-AzDnsRecordSet.

### Przykład 6. Usuwanie rekordu PTR ze zestawu rekordów
```
...
W tym przykładzie jest usuwany rekord PTR z istniejącego zestawu rekordów.
Jeśli jest to jedyny rekord w zestawie rekordów, wynikiem jest pusty zestaw rekordów.
Aby całkowicie usunąć zestaw rekordów, zobacz Remove-AzDnsRecordSet.

### Przykład 7. Usuwanie rekordu SRV ze zestawu rekordów
```
...
W tym przykładzie jest usuwany rekord SRV z istniejącego zestawu rekordów.
Jeśli jest to jedyny rekord w zestawie rekordów, wynikiem jest pusty zestaw rekordów.
Aby całkowicie usunąć zestaw rekordów, zobacz Remove-AzDnsRecordSet.

### Przykład 8. Usuwanie rekordu TXT ze zestawu rekordów
```
...
W tym przykładzie jest usuwany rekord TXT z istniejącego zestawu rekordów.
Jeśli jest to jedyny rekord w zestawie rekordów, wynikiem jest pusty zestaw rekordów.
Aby całkowicie usunąć zestaw rekordów, zobacz Remove-AzDnsRecordSet.
...
### -Nsdname
Określa serwer nazw dla rekordu serwera nazw.
