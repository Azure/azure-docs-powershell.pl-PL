...
## OPIS
W przypadku tokenów SAS urządzenia parametr zasad jest używany tylko do uzyskiwania dostępu do rejestru urządzenia. Dlatego zasady powinny mieć dostęp do odczytu rejestru.
W przypadku tokenów centrum IoT zasady są częścią interfejsu SAS.
...
Wygeneruj token SAS urządzenia przy użyciu zasad właściciela iothub w celu uzyskania dostępu do rejestru urządzenia usługi {iothub_name}.
...
Wygeneruj token SAS modułu za pomocą zasad właściciela iothub w celu uzyskania dostępu do rejestru urządzenia usługi {iothub_name}.
...
### — Czas trwania
Przyszła wygaśnięcie (w sekundach) tokenu do wygenerowania.
