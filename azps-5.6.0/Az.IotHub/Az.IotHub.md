---
Module Name: Az.IotHub
Module Guid: 51a3a993-fe39-4f8f-a437-2cb751584af5
Download Help Link: https://docs.microsoft.com/powershell/module/az.iothub
Help Version: 3.1.2.0
Locale: en-US
content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/IotHub/IotHub/help/Az.IotHub.md
original_content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/IotHub/IotHub/help/Az.IotHub.md
ms.openlocfilehash: 422e2c2d3fabd1dd94e4aa0da666ce91cb3d92b6
ms.sourcegitcommit: 4dfb0cc533b83f77afdcfbe2618c1e6c8d221330
ms.translationtype: MT
ms.contentlocale: pl-PL
ms.lasthandoff: 03/04/2021
ms.locfileid: "101988198"
---
# Moduł Az.IotHub
## Opis
Polecenia do zarządzania usługą Azure IotHubs

## Az.IotHub Cmdlet
### [Add-AziotHubCertificate](Add-AzIotHubCertificate.md)
Utwórz/zaktualizuj certyfikat centrum IoT Azure.

### [Add-AzIotHubConfiguration](Add-AzIotHubConfiguration.md)
Dodaj konfigurację automatycznego zarządzania urządzeniami IoT w docelowym centrum IoT.

### [Add-AzIotHubDeployment](Add-AzIotHubDeployment.md)
Dodaj wdrożenie IoT Edge w docelowym centrum IoT.

### [Add-AzIotHubDevice](Add-AzIotHubDevice.md)
Utwórz urządzenie w Centrum IoT.

### [Add-AzIotHubDevice1](Add-AzIotHubDeviceChildren.md)
Dodaj urządzenia nie edge jako dzieci do urządzenia brzegowego.

### [Add-AzIotHubEventHubConsumerGroup](Add-AzIotHubEventHubConsumerGroup.md)
Tworzy grupę klientów z aplikacji EventHub.

### [Add-AziotHubKey](Add-AzIotHubKey.md)
Tworzy klucz aplikacji IotHub.

### [Add-AziotHubMessageEnrichment](Add-AzIotHubMessageEnrichment.md)
Tworzy wzbogacanie wiadomości dla wybranych punktów końcowych w Centrum IoT.

### [Add-AziotHubModule](Add-AzIotHubModule.md)
Utwórz moduł na urządzeniu docelowym IoT w Centrum IoT.

### [Add-AziotHubRoute](Add-AzIotHubRoute.md)
Tworzenie trasy w centrum IoT

### [Add-AzIotHubRoutingEndpoint](Add-AzIotHubRoutingEndpoint.md)
Dodawanie punktu końcowego do centrum IoT

### [Get-AzIotHub](Get-AzIotHub.md)
Pobiera informacje o usłudze IotHubs w ramach subskrypcji.

### [Get-AzIotHubCertificate](Get-AzIotHubCertificate.md)
Lista wszystkich certyfikatów lub określonego certyfikatu zawartego w centrum Azure IoT Hub. 

### [Get-AzIotHubCertificateVerificationCode](Get-AzIotHubCertificateVerificationCode.md)
Generuje kod weryfikacyjny dla certyfikatu usługi Azure IoT Hub. 

### [Get-AzIotHubConfiguration](Get-AzIotHubConfiguration.md)
Wyświetla listę wszystkich lub określonej konfiguracji automatycznego zarządzania urządzeniami przez usługę IoT.

### [Get-AzIotHubConnectionString](Get-AzIotHubConnectionString.md)
Pobiera połączenia z usługą IotHub.

### [Get-AzIotHubDeployment](Get-AzIotHubDeployment.md)
Lista wszystkich lub konkretnego wdrożenia programu IoT Edge.

### [Get-AzIotHubDevice](Get-AzIotHubDevice.md)
Lista wszystkich urządzeń lub konkretnego urządzenia zawartego w centrum Azure IoT Hub. 

### [Get-AzIotHubDevice Jednakowe](Get-AzIotHubDeviceChildren.md)
Drukowanie rozdzielonych przecinkami listy przypisanych urządzeń dzieci.

### [Get-AzIotHubDeviceConnectionString](Get-AzIotHubDeviceConnectionString.md)
Pobierz parametrów połączenia docelowego urządzenia IoT w centrum Iot.

### [Get-AzIotHubDeviceParent](Get-AzIotHubDeviceParent.md)
Pobierz urządzenie nadrzędne określonego urządzenia.

### [Get-AzIotHubDeviceTwin](Get-AzIotHubDeviceTwin.md)
Pobiera urządzenie.

### [Get-AzIotHubDistributedTracing](Get-AzIotHubDistributedTracing.md)
Pobierz ustawienia śledzenia rozłożonego dla urządzenia.

### [Get-AzIotHubEventHubConsumerGroup](Get-AzIotHubEventHubConsumerGroup.md)
Pobiera wszystkie grupy klientów w usłudze EventHub.

### [Get-AziotHubJob](Get-AzIotHubJob.md)
Pobiera informacje o zadaniu w aplikacji IotHub.

### [Get-AziotHubKey](Get-AzIotHubKey.md)
Pobiera klucz aplikacji IotHub.

### [Get-AzIotHubMessageEnrichment](Get-AzIotHubMessageEnrichment.md)
Zawiera listę wszystkich wzbogacenia wiadomości lub wzbogacania określonej wiadomości w Centrum IoT.

### [Get-AziotHubModule](Get-AzIotHubModule.md)
Szczegółowe informacje na temat modułu lub modułów listy urządzeń IoT znajdujących się na urządzeniu IoT w centrum IoT.

### [Get-AzIotHubModuleConnectionString](Get-AzIotHubModuleConnectionString.md)
Pobierz ciąg połączenia docelowego modułu urządzenia IoT w centrum Iot.

### [Get-AziotHubModuleTwin](Get-AzIotHubModuleTwin.md)
Pobiera moduł urządzenia IoT.

### [Get-AziotHubQuotaMetric](Get-AzIotHubQuotaMetric.md)
Pobiera metryki przydziału dla aplikacji IotHub.

### [Get-AzIotHubRegistryStatistic](Get-AzIotHubRegistryStatistic.md)
Pobiera statystykę rejestru dla aplikacji IotHub.

### [Get-AziotHubRoute](Get-AzIotHubRoute.md)
Uzyskiwanie informacji na temat trasy w centrum IoT Hub

### [Get-AzIotHubRoutingEndpoint](Get-AzIotHubRoutingEndpoint.md)
Uzyskiwanie informacji o wszystkich punktach końcowych Centrum IoT

### [Get-AzIotHubValidSku](Get-AzIotHubValidSku.md)
Pobiera wszystkie prawidłowe jednostki SKU, do których może zostać przejście w tej usłudze IotHub.

### [Invoke-AzIotHubConfigurationMetricsQuery](Invoke-AzIotHubConfigurationMetricsQuery.md)
Wywołaj zapytanie metryczne konfiguracji urządzenia IoT.

### [Invoke-AzIotHubDeploymentMetricsQuery](Invoke-AzIotHubDeploymentMetricsQuery.md)
Wywołaj zapytanie metryczne wdrożenia IoT Edge.

### [Invoke-AzIotHubDeviceMethod](Invoke-AzIotHubDeviceMethod.md)
Wywoływanie metody bezpośredniej na urządzeniu.

### [Invoke-AzIotHubManualFailover](Invoke-AzIotHubManualFailover.md)
Wywołaj proces trybu failover dla centrum IoT do regionu odzyskiwania po awarii sparowanego geograficznie.

### [Invoke-AziotHubModuleMethod](Invoke-AzIotHubModuleMethod.md)
Wywoływanie metody modułu Edge.

### [Invoke-AzIotHubQuery](Invoke-AzIotHubQuery.md)
Wykonywanie zapytań w Centrum IoT przy użyciu zaawansowanego języka SQL.

### [New-AziotHub](New-AzIotHub.md)
Tworzy nową iotHub.

### [New-AzIotHubExportDevice](New-AzIotHubExportDevice.md)
Tworzy nowe zadanie eksportowania urządzeń.

### [New-AzIotHubImportDevice](New-AzIotHubImportDevice.md)
Tworzy nowe zadanie importowania urządzeń.

### [New-AziotHubKey](New-AzIotHubKey.md)
Wygeneruj klucz aplikacji Azure IoT Hub.

### [New-AzIotHubSasToken](New-AzIotHubSasToken.md)
Wygeneruj token SAS dla docelowego centrum IoT Hub, urządzenia lub modułu.

### [Remove-AzIotHub](Remove-AzIotHub.md)
Usuwa element iotHub.

### [Remove-AzIotHubCertificate](Remove-AzIotHubCertificate.md)
Usuwa certyfikat centrum IoT Azure.

### [Remove-AzIotHubConfiguration](Remove-AzIotHubConfiguration.md)
Usuń konfigurację urządzenia IoT.

### [Remove-AzIotHubDeployment](Remove-AzIotHubDeployment.md)
Usuń wdrożenie IoT Edge.

### [Remove-AzIotHubDevice](Remove-AzIotHubDevice.md)
Usuń urządzenie Centrum IoT.

### [Remove-AzIotHubDevice Jednakowe](Remove-AzIotHubDeviceChildren.md)
Usuń urządzenia inne niż krawędzie jako dzieci z określonego urządzenia brzegowego.

### [Remove-AzIotHubEventHubConsumerGroup](Remove-AzIotHubEventHubConsumerGroup.md)
Usuwa grupę konsumencką aplikacji Eventhub.

### [Remove-AziotHubKey](Remove-AzIotHubKey.md)
Usuwa klucz aplikacji IotHub.

### [Remove-AzIotHubMessageEnrichment](Remove-AzIotHubMessageEnrichment.md)
Usuwanie wiadomości wzbogacenie w centrum IoT.

### [Remove-AzIotHubModule](Remove-AzIotHubModule.md)
Usuń moduły na docelowym urządzeniu IoT w Centrum IoT.

### [Remove-AzIotHubRoute](Remove-AzIotHubRoute.md)
Usuwanie trasy w centrum IoT

### [Remove-AzIotHubRoutingEndpoint](Remove-AzIotHubRoutingEndpoint.md)
Usuwanie punktu końcowego dla centrum IoT

### [Send-AzIotHubDevice2CloudMessage](Send-AzIotHubDevice2CloudMessage.md)
Wysyłanie wiadomości z urządzenia do chmury.

### [Set-AziotHub](Set-AzIotHub.md)
Aktualizuje właściwości aplikacji IotHub.

### [Set-AzIotHubConfiguration](Set-AzIotHubConfiguration.md)
Zaktualizuj pola, które można wyciszyć podczas rejestracji konfiguracji.

### [Set-AzIotHubDeployment](Set-AzIotHubDeployment.md)
Aktualizowanie mutable fields of IoT Edge deployment.

### [Set-AzIotHubDevice](Set-AzIotHubDevice.md)
Zaktualizuj urządzenie Centrum IoT.

### [Set-AzIotHubDeviceParent](Set-AzIotHubDeviceParent.md)
Ustaw urządzenie nadrzędne określonego urządzenia.

### [Set-AzIotHubDistributedTracing](Set-AzIotHubDistributedTracing.md)
Aktualizowanie opcji śledzenia rozłożonego dla urządzenia.

### [Set-AziotHubEdgeModule](Set-AzIotHubEdgeModule.md)
Ustaw moduły brzegowe na jednym urządzeniu brzegowym.

### [Set-AziotHubMessageEnrichment](Set-AzIotHubMessageEnrichment.md)
Aktualizowanie wzbogacania wiadomości w centrum IoT.

### [Set-AziotHubModule](Set-AzIotHubModule.md)
Aktualizowanie modułu na urządzeniu docelowym IoT w centrum IoT.

### [Set-AziotHubRoute](Set-AzIotHubRoute.md)
Aktualizowanie trasy w centrum IoT

### [Set-AziotHubVerifiedCertificate](Set-AzIotHubVerifiedCertificate.md)
Sprawdza certyfikat usługi Azure IoT Hub. 

### [Test-AziotHubRoute](Test-AzIotHubRoute.md)
Testowanie tras w centrum IoT

### [Update-AziotHub](Update-AzIotHub.md)
Zaktualizuj centrum Azure IoT Hub.

### [Update-AzIotHubDeviceTwin](Update-AzIotHubDeviceTwin.md)
Aktualizuje tagi i żądane właściwości urządzenia.

### [Update-AziotHubModuleTwin](Update-AzIotHubModuleTwin.md)
Aktualizuje tagi i żądane właściwości modułu urządzenia IoT.

