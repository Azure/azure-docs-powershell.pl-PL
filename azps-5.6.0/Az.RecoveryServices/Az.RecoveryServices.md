---
Module Name: Az.RecoveryServices
Module Guid: 4aa53b7e-fcfe-4e22-979c-9a4e6380de58
Download Help Link: https://docs.microsoft.com/powershell/module/az.recoveryservices
Help Version: 4.1.2.0
Locale: en-US
content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/RecoveryServices/RecoveryServices/help/Az.RecoveryServices.md
original_content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/RecoveryServices/RecoveryServices/help/Az.RecoveryServices.md
ms.openlocfilehash: 60d55b1519b9a31bc9c4c931b411c7c0366e8d47
ms.sourcegitcommit: 4dfb0cc533b83f77afdcfbe2618c1e6c8d221330
ms.translationtype: MT
ms.contentlocale: pl-PL
ms.lasthandoff: 03/04/2021
ms.locfileid: "101974513"
---
# Moduł Az.RecoveryServices
## Opis
W tym temacie są wyświetlane tematy pomocy dotyczące polecenia cmdlet usług Azure Recovery Services.

## Az.RecoveryServices Cmdlet
### [Add-AzRecoveryServicesAsrReplicationProtectedItemServices](Add-AzRecoveryServicesAsrReplicationProtectedItemDisk.md)
Dodaj dysk w celu ochrony już chronionej maszyny wirtualnej platformy Azure.

### [Backup-AzRecoveryServicesBackupItem](Backup-AzRecoveryServicesBackupItem.md)
Uruchamia kopię zapasową elementu kopii zapasowej.

### [Copy-AzRecoveryServicesVault](Copy-AzRecoveryServicesVault.md)
Kopiuje dane z magazynu w jednym regionie do magazynu w innym regionie.

### [Disable-AzRecoveryServicesBackupAutoProtection](Disable-AzRecoveryServicesBackupAutoProtection.md)
Wyłącza automatyczną kopię zapasową elementu, który można chronić.

### [Disable-AzRecoveryServicesBackupProtection](Disable-AzRecoveryServicesBackupProtection.md)
Wyłącza ochronę elementu chronionego kopią zapasową.

### [Disable-AzRecoveryServicesBackupRPMountScript](Disable-AzRecoveryServicesBackupRPMountScript.md)
Dezinstaluje wszystkie pliki punktu odzyskiwania.

### [Edit-AzRecoveryServicesAsrRecoveryPlan](Edit-AzRecoveryServicesAsrRecoveryPlan.md)
Edytuje plan odzyskiwania witryny.

### [Enable-AzRecoveryServicesBackupAutoProtection](Enable-AzRecoveryServicesBackupAutoProtection.md)
Polecenie cmdlet **Enable-AzRecoveryServicesBackupAutoProtection** konfiguruje automatyczną ochronę bieżących i wszystkich przyszłych kodów DB języka SQL w danym wystąpieniu przy użyciu podanych zasad.

### [Enable-AzRecoveryServicesBackupProtection](Enable-AzRecoveryServicesBackupProtection.md)
Umożliwia tworzenie kopii zapasowej elementu z określonymi zasadami ochrony kopii zapasowej.

### [Get-AzRecoveryServicesAsrAlertSetting](Get-AzRecoveryServicesAsrAlertSetting.md)
Pobiera skonfigurowane ustawienia powiadomień usługi Azure Site Recovery dla magazynu.

### [Get-AzRecoveryServicesAsrEvent](Get-AzRecoveryServicesAsrEvent.md)
Pobiera szczegółowe informacje o zdarzeniach odzyskiwania witryny platformy Azure w magazynie.

### [Get-AzRecoveryServicesAsrFabric](Get-AzRecoveryServicesAsrFabric.md)
Uzyskaj szczegółowe informacje na temat struktury Azure Site Recovery Fabric.

### [Get-AzRecoveryServicesAsrJob](Get-AzRecoveryServicesAsrJob.md)
Pobiera szczegółowe informacje o określonym zadaniu asr lub liście ostatnich zadań asr w magazynie usług odzyskiwania.

### [Get-AzRecoveryServicesAsrNetwork](Get-AzRecoveryServicesAsrNetwork.md)
Pobiera informacje o sieciach zarządzanych przez odzyskiwanie witryny dla bieżącego magazynu.

### [Get-AzRecoveryServicesAsrNetworkMapping](Get-AzRecoveryServicesAsrNetworkMapping.md)
Pobiera informacje o mapowaniu sieci odzyskiwania witryny dla bieżącego magazynu.

### [Get-AzRecoveryServicesAsrPolicy](Get-AzRecoveryServicesAsrPolicy.md)
Pobiera zasady replikacji asr.

### [Get-AzRecoveryServicesAsrProtectableItem](Get-AzRecoveryServicesAsrProtectableItem.md)
Pobierz elementy, które można chronić, w kontenerze ochrony asr.

### [Get-AzRecoveryServicesAsrProtectionContainer](Get-AzRecoveryServicesAsrProtectionContainer.md)
Pobiera kontenery ochrony usługi asr w magazynie usług odzyskiwania.

### [Get-AzRecoveryServicesAsrProtectionContainerMapping](Get-AzRecoveryServicesAsrProtectionContainerMapping.md)
Pobiera mapowania kontenerów usługi Azure Site Recovery Protection.

### [Get-AzRecoveryServicesAsrRecoveryPlan](Get-AzRecoveryServicesAsrRecoveryPlan.md)
Pobiera plan odzyskiwania lub wszystkie plany odzyskiwania w magazynie usług odzyskiwania

### [Get-AzRecoveryServicesAsrRecoveryPoint](Get-AzRecoveryServicesAsrRecoveryPoint.md)
Pobiera dostępne punkty odzyskiwania dla elementu chronionego replikacją.

### [Get-AzRecoveryServicesAsrReplicationProtectedItem](Get-AzRecoveryServicesAsrReplicationProtectedItem.md)
Pobiera właściwości elementów chronionych replikacją odzyskiwania witryn platformy Azure.

### [Get-AzRecoveryServicesAsrServicesProvider](Get-AzRecoveryServicesAsrServicesProvider.md)
Pobiera szczegółowe informacje o dostawcach usług odzyskiwania asr zarejestrowanych w magazynie usług odzyskiwania.

### [Get-AzRecoveryServicesAsrStorageClassification](Get-AzRecoveryServicesAsrStorageClassification.md)
Pobiera dostępne (odkryte) klasyfikacje magazynu ASR w magazynie usług odzyskiwania.

### [Get-AzRecoveryServicesAsrStorageClassificationMapping](Get-AzRecoveryServicesAsrStorageClassificationMapping.md)
Pobiera mapowania klasyfikacji magazynu asr.

### [Get-AzRecoveryServicesAsrVaultContext](Get-AzRecoveryServicesAsrVaultContext.md)
Pobiera informacje o ustawieniach magazynu asr dla magazynu usług odzyskiwania.

### [Get-AzRecoveryServicesAsrvCenter](Get-AzRecoveryServicesAsrvCenter.md)
Pobiera szczegółowe informacje o serwerach vCenter zarejestrowanych do odnajdowania na serwerze konfiguracji określonym przez sieć szkieletową ASR.

### [Get-AzRecoveryServicesBackupContainer](Get-AzRecoveryServicesBackupContainer.md)
Pobiera kontenery kopii zapasowej.

### [Get-AzRecoveryServicesBackupItem](Get-AzRecoveryServicesBackupItem.md)
Pobiera elementy z kontenera w kopii zapasowej.

### [Get-AzRecoveryServicesBackupJob](Get-AzRecoveryServicesBackupJob.md)
Otrzymuje zadania kopii zapasowej.

### [Get-AzRecoveryServicesBackupJobDetail](Get-AzRecoveryServicesBackupJobDetail.md)
Pobiera szczegółowe informacje dotyczące zadania kopii zapasowej.

### [Get-AzRecoveryServicesBackupManagementServer](Get-AzRecoveryServicesBackupManagementServer.md)
Pobiera serwery zarządzania kopiami scdpm i kopiami zapasowymi platformy Azure.

### [Get-AzRecoveryServicesBackupProperty](Get-AzRecoveryServicesBackupProperty.md)
Pobiera właściwości kopii zapasowej.

### [Get-AzRecoveryServicesBackupProtectableItem](Get-AzRecoveryServicesBackupProtectableItem.md)
To polecenie spowoduje pobranie wszystkich elementów, które można chronić, w określonym kontenerze lub we wszystkich zarejestrowanych kontenerach. Będzie się on składał ze wszystkich elementów hierarchii aplikacji. Zwraca db i ich jednostki górnej warstwy, takie jak Instance, AvailabilityGroup itp.

### [Get-AzRecoveryServicesBackupProtectionPolicy](Get-AzRecoveryServicesBackupProtectionPolicy.md)
Pobiera zasady ochrony kopii zapasowej dla magazynu.

### [Get-AzRecoveryServicesBackupRecoveryLogChain](Get-AzRecoveryServicesBackupRecoveryLogChain.md)
To polecenie zawiera listę punktów startowych i końcowych nieprzerwanego łańcucha dziennika danego elementu kopii zapasowej. Za jego pomocą można określić, czy punkt w czasie, do którego użytkownik chce przywrócić baza danych, jest prawidłowy, czy nie.

### [Get-AzRecoveryServicesBackupRecoveryPoint](Get-AzRecoveryServicesBackupRecoveryPoint.md)
Pobiera punkty odzyskiwania dla elementu kopii zapasowej.

### [Get-AzRecoveryServicesBackupRetentionPolicyObject](Get-AzRecoveryServicesBackupRetentionPolicyObject.md)
Pobiera obiekt podstawowych zasad przechowywania.

### [Get-AzRecoveryServicesBackupRPMountScript](Get-AzRecoveryServicesBackupRPMountScript.md)
Pobiera skrypt, aby zainstalować wszystkie pliki punktu odzyskiwania.

### [Get-AzRecoveryServicesBackupSchedulePolicyObject](Get-AzRecoveryServicesBackupSchedulePolicyObject.md)
Pobiera obiekt zasad harmonogramu podstawowego.

### [Get-AzRecoveryServicesBackupStatus](Get-AzRecoveryServicesBackupStatus.md)
Sprawdza, czy ARM kopii zapasowej zasobu.

### [Get-AzRecoveryServicesBackupWorkloadRecoveryConfig](Get-AzRecoveryServicesBackupWorkloadRecoveryConfig.md)
To polecenie konstruuje konfigurację odzyskiwania elementu kopii zapasowej, takiego jak baza danych SQL. Obiekt konfiguracji przechowuje wszystkie szczegóły, takie jak tryb odzyskiwania, docelowe miejsca docelowe przywracania i parametry specyficzne dla aplikacji, takie jak docelowe ścieżki fizyczne języka SQL.

### [Get-AzRecoveryServicesVault](Get-AzRecoveryServicesVault.md)
Pobiera listę magazynów usług odzyskiwania.

### [Get-AzRecoveryServicesVaultProperty](Get-AzRecoveryServicesVaultProperty.md)
Zwraca właściwości magazynu usług odzyskiwania.

### [Get-AzRecoveryServicesVaultSettingsFile](Get-AzRecoveryServicesVaultSettingsFile.md)
Pobiera plik ustawień magazynu usługi Azure Site Recovery.

### [Import-AzRecoveryServicesAsrVaultSettingsFile](Import-AzRecoveryServicesAsrVaultSettingsFile.md)
Importuje określony plik ustawień magazynu asr, aby ustawić kontekst magazynu (kontekst sesji programu PowerShell) dla kolejnych operacji ASR w sesji programu PowerShell. 

### [Initialize-AzRecoveryServicesBackupProtectableItem](Initialize-AzRecoveryServicesBackupProtectableItem.md)
To polecenie wyzwala odnajdowanie elementów niechronionych danego typu obciążenia pracą w danym kontenerze. Jeśli aplikacja bazy danych nie jest automatycznie chroniona, użyj tego polecenia do odnajdowania nowych znaków DB po każdym dodaniu i kontynuowaniu ich ochrony.

### [New-AzRecoveryServicesAsrAzureToAzureChatReplicationConfig](New-AzRecoveryServicesAsrAzureToAzureDiskReplicationConfig.md)
Tworzy obiekt mapowania dysku, aby zreplikować dysk maszyn wirtualnych platformy Azure.

### [New-AzRecoveryServicesAsrFabric](New-AzRecoveryServicesAsrFabric.md)
Tworzy szkielet odzyskiwania witryn platformy Azure.

### [New-AzRecoveryServicesAsrInMageAzureV2ChatInput](New-AzRecoveryServicesAsrInMageAzureV2DiskInput.md)
Tworzy obiekt mapowania dysku dla dysków maszyn wirtualnych vMWare, które mają zostać zreplikowane.

### [New-AzRecoveryServicesAsrNetworkMapping](New-AzRecoveryServicesAsrNetworkMapping.md)
Tworzy mapowanie sieci asr między dwiema sieciami.

### [New-AzRecoveryServicesAsrPolicy](New-AzRecoveryServicesAsrPolicy.md)
Tworzy zasady replikacji usługi Azure Site Recovery.

### [New-AzRecoveryServicesAsrProtectableItem](New-AzRecoveryServicesAsrProtectableItem.md)
Dodawanie (odnajdowanie) serwera fizycznego do listy elementów, które można chronić.

### [New-AzRecoveryServicesAsrProtectionContainer](New-AzRecoveryServicesAsrProtectionContainer.md)
Tworzy kontener Azure Site Recovery Protection Container w określonym szkieletie.

### [New-AzRecoveryServicesAsrProtectionContainerMapping](New-AzRecoveryServicesAsrProtectionContainerMapping.md)
Tworzy mapowanie kontenera usługi Azure Site Recovery Protection przez skojarzenie określonych zasad replikacji z określonym kontenerem ochrony usługi ASR.

### [New-AzRecoveryServicesAsrRecoveryPlan](New-AzRecoveryServicesAsrRecoveryPlan.md)
Tworzy plan odzyskiwania po stronie asr.

### [New-AzRecoveryServicesAsrReplicationProtectedItem](New-AzRecoveryServicesAsrReplicationProtectedItem.md)
Umożliwia replikację elementu chronionego przez asr przez utworzenie elementu chronionego replikacją.

### [New-AzRecoveryServicesAsrStorageClassificationMapping](New-AzRecoveryServicesAsrStorageClassificationMapping.md)
Tworzy mapowanie klasyfikacji magazynu asr w magazynie usług odzyskiwania.

### [New-AzRecoveryServicesAsrvCenter](New-AzRecoveryServicesAsrvCenter.md)
Dodaje serwer vCenter w celu odnajdowanie elementów, które można chronić.

### [New-AzRecoveryServicesBackupProtectionPolicy](New-AzRecoveryServicesBackupProtectionPolicy.md)
Tworzy zasady ochrony kopii zapasowej.

### [New-AzRecoveryServicesVault](New-AzRecoveryServicesVault.md)
Tworzy nowy magazyn usług odzyskiwania.

### [Register-AzRecoveryServicesBackupContainer](Register-AzRecoveryServicesBackupContainer.md)
Polecenie cmdlet **Register-AzRecoveryServicesBackupContainer** rejestruje maszynę wirtualną platformy Azure dla usługi AzureWorkloads z określonym typem obciążenia pracą.

### [Remove-AzRecoveryServicesAsrFabric](Remove-AzRecoveryServicesAsrFabric.md)
Usuwa określoną sieć Azure Site Recovery Fabric z magazynu usług odzyskiwania.

### [Remove-AzRecoveryServicesAsrNetworkMapping](Remove-AzRecoveryServicesAsrNetworkMapping.md)
Usuwa wskazane mapowanie sieci asr z magazynu usług odzyskiwania.

### [Remove-AzRecoveryServicesAsrPolicy](Remove-AzRecoveryServicesAsrPolicy.md)
Usuwa określone zasady replikacji asr z magazynu usług odzyskiwania.

### [Remove-AzRecoveryServicesAsrProtectionContainer](Remove-AzRecoveryServicesAsrProtectionContainer.md)
Usuwa określony kontener ochrony z jego struktury szkieletowej.

### [Remove-AzRecoveryServicesAsrProtectionContainerMapping](Remove-AzRecoveryServicesAsrProtectionContainerMapping.md)
Usuwa wskazane mapowanie kontenera ochrony usługi Azure Site Recovery.

### [Remove-AzRecoveryServicesAsrRecoveryPlan](Remove-AzRecoveryServicesAsrRecoveryPlan.md)
Usuwa określony plan odzyskiwania asr z magazynu usług odzyskiwania.

### [Remove-AzRecoveryServicesAsrReplicationProtectedItem](Remove-AzRecoveryServicesAsrReplicationProtectedItem.md)
Zatrzymuje/wyłącza replikację elementu chronionego replikacją usługi Azure Site Recovery.

### [Remove-AzRecoveryServicesAsrServicesProvider](Remove-AzRecoveryServicesAsrServicesProvider.md)
Usuwa/wyrejestrowywuje określonego dostawcę usług odzyskiwania witryny Azure z magazynu usług odzyskiwania.

### [Remove-AzRecoveryServicesAsrStorageClassificationMapping](Remove-AzRecoveryServicesAsrStorageClassificationMapping.md)
Usuwa wskazane mapowanie klasyfikacji magazynu asr.

### [Remove-AzRecoveryServicesAsrvCenter](Remove-AzRecoveryServicesAsrvCenter.md)
Usuwa serwer vCenter z struktury ASR i zatrzymuje odnajdowanie maszyn wirtualnych z serwera vCenter.

### [Remove-AzRecoveryServicesBackupProtectionPolicy](Remove-AzRecoveryServicesBackupProtectionPolicy.md)
Usuwa zasady ochrony kopii zapasowej z magazynu.

### [Remove-AzRecoveryServicesVault](Remove-AzRecoveryServicesVault.md)
Usuwa magazyn usług odzyskiwania.

### [Restart-AzRecoveryServicesAsrJob](Restart-AzRecoveryServicesAsrJob.md)
Powoduje ponowne uruchomienie zadania odzyskiwania witryny platformy Azure.

### [Restore-AzRecoveryServicesBackupItem](Restore-AzRecoveryServicesBackupItem.md)
Przywraca dane i konfigurację elementu kopii zapasowej do punktu odzyskiwania.

### [Resume-AzRecoveryServicesAsrJob](Resume-AzRecoveryServicesAsrJob.md)
Wznawia zawieszone zadanie odzyskiwania witryny platformy Azure.

### [Set-AzRecoveryServicesAsrAlertSetting](Set-AzRecoveryServicesAsrAlertSetting.md)
Skonfiguruj ustawienia powiadomień usługi Azure Site Recovery (powiadomienie e-mail) dla magazynu.

### [Set-AzRecoveryServicesAsrReplicationProtectedItem](Set-AzRecoveryServicesAsrReplicationProtectedItem.md)
Ustawia właściwości odzyskiwania, takie jak sieć docelowa i rozmiar maszyny wirtualnej dla określonego elementu chronionego replikacją.

### [Set-AzRecoveryServicesAsrVaultContext](Set-AzRecoveryServicesAsrVaultContext.md)
Ustawia kontekst magazynu usług odzyskiwania, który ma być używany do kolejnych operacji odzyskiwania witryny platformy Azure w bieżącej sesji programu PowerShell.

### [Set-AzRecoveryServicesBackupProperty](Set-AzRecoveryServicesBackupProperty.md)
Ustawia właściwości zarządzania kopią zapasową.

### [Set-AzRecoveryServicesBackupProtectionPolicy](Set-AzRecoveryServicesBackupProtectionPolicy.md)
Modyfikuje zasady ochrony kopii zapasowej.

### [Set-AzRecoveryServicesVaultContext](Set-AzRecoveryServicesVaultContext.md)
Ustawia kontekst magazynu.

### [Set-AzRecoveryServicesVaultProperty](Set-AzRecoveryServicesVaultProperty.md)
Aktualizuje właściwości magazynu.

### [Start-AzRecoveryServicesAsrApplyRecoveryPoint](Start-AzRecoveryServicesAsrApplyRecoveryPoint.md)
Przed zatwierdzeniem operacji trybu failover zmienia punkt odzyskiwania dla elementu chronionego w trybie failover.

### [Start-AzRecoveryServicesAsrCommitFailoverJob](Start-AzRecoveryServicesAsrCommitFailoverJob.md)
Rozpoczyna akcję zatwierdzania trybu failover dla obiektu odzyskiwanie witryny.

### [Start-AzRecoveryServicesAsrPlannedFailoverJob](Start-AzRecoveryServicesAsrPlannedFailoverJob.md)
Rozpoczyna planowaną operację trybu failover.

### [Start-AzRecoveryServicesAsrResynchronizeReplicationJob](Start-AzRecoveryServicesAsrResynchronizeReplicationJob.md)
Rozpoczyna ponowną synchronizację replikacji.

### [Start-AzRecoveryServicesAsrSwitchProcessServerJob](Start-AzRecoveryServicesAsrSwitchProcessServerJob.md)
Przełączanie replikacji z jednego serwera procesu na inny w celu równoważenia obciążenia.

### [Start-AzRecoveryServicesAsrTestFailoverCleanupJob](Start-AzRecoveryServicesAsrTestFailoverCleanupJob.md)
Rozpoczyna test operacji oczyszczania trybu failover.

### [Start-AzRecoveryServicesAsrTestFailoverJob](Start-AzRecoveryServicesAsrTestFailoverJob.md)
Rozpoczyna testowanie operacji trybu failover.

### [Start-AzRecoveryServicesAsrUnplannedFailoverJob](Start-AzRecoveryServicesAsrUnplannedFailoverJob.md)
Rozpoczyna niezaplanowaną operację trybu failover.

### [Stop-AzRecoveryServicesAsrJob](Stop-AzRecoveryServicesAsrJob.md)
Zatrzymuje zadanie odzyskiwania witryny platformy Azure.

### [Stop-AzRecoveryServicesBackupJob](Stop-AzRecoveryServicesBackupJob.md)
Anulowanie zadania uruchomionego.

### [Undo-AzRecoveryServicesBackupItemDeletion](Undo-AzRecoveryServicesBackupItemDeletion.md)
Jeśli element kopii zapasowej zostanie usunięty i będzie obecne w stanie "miękkiego usunięcia", to polecenie przywróci stan, w którym dane zostaną zachowane na zawsze. 

### [Unregister-AzRecoveryServicesBackupContainer](Unregister-AzRecoveryServicesBackupContainer.md)
Wyrejestruje serwer Windows Server lub inny kontener z magazynu.

### [Unregister-AzRecoveryServicesBackupManagementServer](Unregister-AzRecoveryServicesBackupManagementServer.md)
Wyrejestruje serwer SCDPM lub serwer kopii zapasowej z magazynu.

### [Update-AzRecoveryServicesAsr JednakityService](Update-AzRecoveryServicesAsrMobilityService.md)
Aktualizacje agenta obsługi mobilności na komputerach chronionych.

### [Update-AzRecoveryServicesAsrNetworkMapping](Update-AzRecoveryServicesAsrNetworkMapping.md)
Aktualizuje wskazane mapowanie sieci odzyskiwania witryn platformy Azure.

### [Update-AzRecoveryServicesAsrPolicy](Update-AzRecoveryServicesAsrPolicy.md)
Aktualizuje zasady replikacji usługi Azure Site Recovery.

### [Update-AzRecoveryServicesAsrProtectionContainerMapping](Update-AzRecoveryServicesAsrProtectionContainerMapping.md)
Zaktualizuj mapowanie kontenera ochrony przed odzyskiwaniem witryn platformy Azure.

### [Update-AzRecoveryServicesAsrProtectionDirection](Update-AzRecoveryServicesAsrProtectionDirection.md)
Aktualizuje kierunek replikacji dla określonego elementu chronionego replikacją lub planu odzyskiwania. Służy do ponownego chronienia/odwracania replikowania elementu replikowanego lub planu odzyskiwania.

### [Update-AzRecoveryServicesAsrRecoveryPlan](Update-AzRecoveryServicesAsrRecoveryPlan.md)
Aktualizuje zawartość planu odzyskiwania witryn platformy Azure.

### [Update-AzRecoveryServicesAsrServicesProvider](Update-AzRecoveryServicesAsrServicesProvider.md)
Odświeża (odśwież serwer) informacje otrzymane od dostawcy usług Azure Site Recovery Services.

### [Update-AzRecoveryServicesAsrvCenter](Update-AzRecoveryServicesAsrvCenter.md)
Aktualizowanie szczegółów odnajdowania dla zarejestrowanego centrum vCenter.

### [Wait-AzRecoveryServicesBackupJob](Wait-AzRecoveryServicesBackupJob.md)
Czeka na zakończenie zadania kopii zapasowej.

