...
## OPIS
To polecenie cmdlet umożliwia tworzenie usług nie stanowych lub usług o określonym stanie w określonej aplikacji. Usługa powinna zakończyć działanie w pliku manifestu aplikacji, a typ powinien być taki sam jak ten w pliku manifestu. Nazwa aplikacji powinna być prefiksem nazwy usługi.
...
W tym przykładzie utworzysz nową usługę bez stanową "testApp~testService1" z liczbami wystąpień -1 (we wszystkich węzłach).
...
W tym przykładzie utworzysz nową usługę o stanie "testApp~testService2" z elementem docelowym 5 węzłów.
...
Wyższe koszty sprawiają, że menedżer zasobów klastrów przenosi replikę, gdy próbuje zrównoważyć klaster
...
### -MinReplicaSetSize
Określanie rozmiaru zestawu repliki min dla usługi
...
Wskazuje, że usługa używa nazwanego schematu partycji.
Usługi korzystające z tego modelu zazwyczaj mają dane, które mogą być zasobnikami w ramach zestawu ograniczonego.
Niektóre typowe przykłady pól danych używanych jako nazwane klucze partycji to regiony, kody pocztowe, grupy klientów lub inne granice biznesowe.
...
Wskazuje, że usługa używa schematu partycji singletona.
Partycje singletona są zwykle używane, gdy usługa nie wymaga dodatkowego routingu.
...
Oznacza to, że każda partycja jest właścicielem zakresu kluczy int64.
...
### -NatłokuLossWaitDuration
Określanie czasu oczekiwania na utratę utraty danych dla usługi
...
### - Bez stanowy
Korzystanie z usługi bez stanowej
...
### — Wpisz
Określ nazwę typu usługi aplikacji, która powinna istnieć w pliku manifestu aplikacji.
