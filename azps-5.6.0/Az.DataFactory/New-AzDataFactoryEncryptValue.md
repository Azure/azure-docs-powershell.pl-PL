...
### Przykład 2. Szyfrowanie parametrów połączenia innych niż ODBC, które używa uwierzytelniania systemu Windows.
```
...
Polecenie **Nowy obiekt tworzy** obiekt PSCredential przy użyciu ciągów bezpiecznej nazwy użytkownika i hasła.
...
Polecenie **ConvertTo-SecureString** konwertuje określony ciąg na bezpieczny ciąg.
Polecenie **New-AzDataFactoryEncryptValue** tworzy zaszyfrowaną wartość obiektu przechowywanego w usłudze $Value dla określonego typu usługi, bramy, grupy zasobów i połączonego typu usługi.
