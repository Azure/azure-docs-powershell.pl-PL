...
### GetWithDefaultParameters (domyślne)
```
...
## OPIS
Polecenie **cmdlet Get-AzMetric** pobiera wartości metryczne dla określonego zasobu.

## PRZYKŁADY

### Przykład 1. Uzyskiwanie metryki z podsumowaną wartością wyjściową
```
...
To polecenie pobiera wartości metryczne dla witryny website3 z okresem 1 minuty.

### Przykład 2. Uzyskiwanie metryki ze szczegółowymi danymi wyjściowym
```
...
To polecenie uzyskuje szczegółowe dane wyjściowe metryki Żądania.

### Przykład 4. Uzyskiwanie podsumowanych danych wyjściowych dla określonej metryki przy użyciu określonego filtru wymiaru
```
...
To polecenie pobiera podsumowanie danych wyjściowych metryki PageViews z określonym filtrem wymiaru i typem agregacji.
...
### -DetailedOutput
Wskazuje, że to polecenie cmdlet wyświetla szczegółowe dane wyjściowe.
Domyślnie dane wyjściowe są podsumowywane.
...
### -MetricFilter
Określa metrykę filtru wymiarów metrycznych, dla których mają być kwerendowane metryki.
...
### -MetricNamespace
Określa metrykę przestrzeni nazw, dla których mają być kwerendy.
...
### -OrderBy
Określa agregację do użycia w wynikach sortowania i kierunek sortowania (przykład: suma asc).
...
### -ResultType
Określa typ wyniku do zwrócenia (metadane lub dane).
...
### - TimeGrain
Określa słoj czasu metryki jako obiekt **TimeSpan** w formacie gg:mm:ss.
...
### — Na górze
Określa maksymalną liczbę rekordów do pobrania (wartość domyślna: 10), które mają zostać określone przy $filter.
...
Więcej informacji o obsługiwanych metrykach można znaleźć na stronie: https://docs.microsoft.com/azure/azure-monitor/platform/metrics-supported
