---
Module Name: Az.Monitor
Module Guid: 698c387c-bd6b-41c6-82ce-721f1ef39548
Download Help Link: https://docs.microsoft.com/powershell/module/az.monitor
Help Version: 4.0.4.0
Locale: en-US
content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/Monitor/Monitor/help/Az.Monitor.md
original_content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/Monitor/Monitor/help/Az.Monitor.md
ms.openlocfilehash: deba09da2aa6db63446d69c6e5dfc870f06689da
ms.sourcegitcommit: 4dfb0cc533b83f77afdcfbe2618c1e6c8d221330
ms.translationtype: MT
ms.contentlocale: pl-PL
ms.lasthandoff: 03/04/2021
ms.locfileid: "101980250"
---
# Moduł Az.Monitor
## Opis
Ten temat zawiera tematy pomocy dotyczące polecenia cmdlet usługi Azure Insights.

## Polecenia cmdlet az.monitor
### [Add-AzAutoscaleSetting](Add-AzAutoscaleSetting.md)
Tworzy ustawienie skalowania automatycznego.

### [Add-AzLogProfile](Add-AzLogProfile.md)
Tworzy nowy profil dziennika aktywności. Ten profil służy do archiwizowania dziennika aktywności na koncie magazynu platformy Azure lub strumieniowego przesyłania go do centrum zdarzeń platformy Azure w tej samej subskrypcji. 

### [Add-AzMetricAlertRule](Add-AzMetricAlertRule.md)
Dodaje lub aktualizuje regułę alertów opartą na metrykach.

### [Add-AzMetricAlertRuleV2](Add-AzMetricAlertRuleV2.md)
Dodaje lub aktualizuje regułę alertów W2 (nieistnieczną) opartą na metryki.

### [Add-AzWebtestAlertRule](Add-AzWebtestAlertRule.md)
Dodaje lub aktualizuje regułę alertu testu sieci Web.

### [Disable-AzActivityLogAlert](Disable-AzActivityLogAlert.md)
Wyłącza alert dziennika aktywności i ustawia jego tagi.

### [Enable-AzActivityLogAlert](Enable-AzActivityLogAlert.md)
Włącza alert dziennika aktywności i ustawia jego tagi.

### [Get-AzActionGroup](Get-AzActionGroup.md)
Pobiera grupy akcji.

### [Get-AzActivityLog](Get-AzActivityLog.md)
Pobieranie zdarzeń dziennika aktywności.

### [Get-AzActivityLogAlert](Get-AzActivityLogAlert.md)
Pobiera jeden lub więcej zasobów alertów dziennika aktywności.

### [Get-AzAlertHistory](Get-AzAlertHistory.md)
Pobiera historię alertów.

### [Get-AzAlertRule](Get-AzAlertRule.md)
Pobiera reguły alertów.

### [Get-AzAutoscaleHistory](Get-AzAutoscaleHistory.md)
Pobiera historię autoskalowania.

### [Get-AzAutoscaleSetting](Get-AzAutoscaleSetting.md)
Pobiera ustawienia skalowania automatycznego.

### [Get-AzDiagnosticSetting](Get-AzDiagnosticSetting.md)
Pobiera zarejestrowane kategorie i czas.

### [Get-AzDiagnosticSettingCategory](Get-AzDiagnosticSettingCategory.md)
Pobierz kategorię obsługiwanych ustawień diagnostycznych dla zasobu Azure lub przygotuj listę tych kategorii.

### [Get-AzInsightsPrivateLinkScope](Get-AzInsightsPrivateLinkScope.md)
Uzyskiwanie zakresu prywatnego linku

### [Get-AzInsightsPrivateLinkScopedResource](Get-AzInsightsPrivateLinkScopedResource.md)
Uzyskiwanie prywatnego zasobu z zakresem linków

### [Get-AzLogProfile](Get-AzLogProfile.md)
Pobiera profil dziennika.

### [Get-AzMetric](Get-AzMetric.md)
Pobiera wartości metryczne zasobu.

### [Get-AzMetricAlertRuleV2](Get-AzMetricAlertRuleV2.md)
Pobiera metryczną regułę alertów W2 (nieistnieczną)

### [Get-AzMetricDefinition](Get-AzMetricDefinition.md)
Pobiera definicje metryczne.

### [Get-AzScheduledQueryRule](Get-AzScheduledQueryRule.md)
Pobiera zasoby dotyczące zapytań planowanych

### [New-AzActionGroup](New-AzActionGroup.md)
Tworzy obiekt odwołania ActionGroup w pamięci.

### [New-AzActionGroupReceiver](New-AzActionGroupReceiver.md)
Tworzy nowego odbiorcy grupy akcji.

### [New-AzActivityLogAlertCondition](New-AzActivityLogAlertCondition.md)
Tworzy w pamięci nowy obiekt warunku alertu dziennika aktywności.

### [New-AzAlertRuleEmail](New-AzAlertRuleEmail.md)
Tworzy akcję wiadomości e-mail dla reguły alertu.

### [New-AzAlertRuleWebhook](New-AzAlertRuleWebhook.md)
Tworzy webhook reguły alertu.

### [New-AzAutoscaleNotification](New-AzAutoscaleNotification.md)
Tworzy powiadomienie e-mail o automatycznym skalowaniach.

### [New-AzAutoscaleProfile](New-AzAutoscaleProfile.md)
Tworzy profil autoskalowania.

### [New-AzAutoscaleRule](New-AzAutoscaleRule.md)
Tworzy regułę skalowania automatycznego.

### [New-AzAutoscaleWebhook](New-AzAutoscaleWebhook.md)
Tworzy webhook autoskalowania.

### [New-AzDiagnosticDetailSetting](New-AzDiagnosticDetailSetting.md)
Tworzenie obiektu PSDiagnosticDetailSetting, typ może być metryka lub dziennik

### [New-AzDiagnosticSetting](New-AzDiagnosticSetting.md)
Tworzenie obiektu PSServiceDiagnosticSettings.

### [New-AzInsightsPrivateLinkScope](New-AzInsightsPrivateLinkScope.md)
Tworzenie prywatnego zakresu łączy

### [New-AzInsightsPrivateLinkScopedResource](New-AzInsightsPrivateLinkScopedResource.md)
Tworzenie dla prywatnego zasobu z zakresem linków

### [New-AzMetricAlertRuleV2Criteria](New-AzMetricAlertRuleV2Criteria.md)
Tworzy obiekt kryteriów lokalnych, za pomocą których można utworzyć nowy alert metryczny.

### [New-AzMetricAlertRuleV2DimensionSelection](New-AzMetricAlertRuleV2DimensionSelection.md)
Tworzy obiekt wyboru wymiaru lokalnego, który może służyć do konstruowania metrycznych kryteriów alertów.

### [New-AzMetricFilter](New-AzMetricFilter.md)
Tworzy filtr wymiarów metrycznych, który może być używany do tworzenia zapytań dotyczących metryk.

### [New-AzScheduledQueryRule](New-AzScheduledQueryRule.md)
Tworzy regułę alertu dziennika (typ reguły zaplanowanego zapytania)

### [New-AzScheduledQueryRuleAlertingAction](New-AzScheduledQueryRuleAlertingAction.md)
Tworzy obiekt typu Akcja alertu

### [New-AzScheduledQueryRuleAznsActionGroup](New-AzScheduledQueryRuleAznsActionGroup.md)
Tworzy obiekt typu Azns Action Group

### [New-AzScheduledQueryRuleLogMetricTrigger](New-AzScheduledQueryRuleLogMetricTrigger.md)
Tworzy obiekt wyzwalacza metryki typu Log.

### [New-AzScheduledQueryRuleSchedule](New-AzScheduledQueryRuleSchedule.md)
Tworzy obiekt typu Harmonogram

### [New-AzScheduledQueryRuleSource](New-AzScheduledQueryRuleSource.md)
Tworzy obiekt typu Source

### [New-AzScheduledQueryRuleTriggerCondition](New-AzScheduledQueryRuleTriggerCondition.md)
Tworzy obiekt typu Warunek wyzwalacza

### [Remove-AzActionGroup](Remove-AzActionGroup.md)
Usuwa grupę akcji.

### [Remove-AzActivityLogAlert](Remove-AzActivityLogAlert.md)
Usuwa alert dziennika aktywności.

### [Remove-AzAlertRule](Remove-AzAlertRule.md)
Usuwa regułę alertu.

### [Remove-AzAutoscaleSetting](Remove-AzAutoscaleSetting.md)
Usuwa ustawienie automatycznego skalowania.

### [Remove-AzDiagnosticSetting](Remove-AzDiagnosticSetting.md)
Usuwanie ustawienia diagnostycznego zasobu.

### [Remove-AzInsightsPrivateLinkScope](Remove-AzInsightsPrivateLinkScope.md)
Usuwanie zakresu prywatnego linku

### [Remove-AzInsightsPrivateLinkScopedResource](Remove-AzInsightsPrivateLinkScopedResource.md)
Delete for private link scoped resource

### [Remove-AzLogProfile](Remove-AzLogProfile.md)
Usuwa profil dziennika.

### [Remove-AzMetricAlertRuleV2](Remove-AzMetricAlertRuleV2.md)
Usuwa metryczną regułę alertu V2 (nieistnieczną).

### [Remove-AzScheduledQueryRule](Remove-AzScheduledQueryRule.md)
Usuwa regułę alertu dziennika

### [Set-AzActionGroup](Set-AzActionGroup.md)
Tworzy nową lub aktualizuje istniejącą grupę akcji.

### [Set-AzActivityLogAlert](Set-AzActivityLogAlert.md)
Tworzy nowy lub ustawia istniejący alert dziennika aktywności.

### [Set-AzDiagnosticSetting](Set-AzDiagnosticSetting.md)
Ustawia ustawienia dzienników i metryk dla zasobu.

### [Set-AzScheduledQueryRule](Set-AzScheduledQueryRule.md)
Aktualizuje regułę alertu dziennika

### [Update-AzInsightsPrivateLinkScope](Update-AzInsightsPrivateLinkScope.md)
Aktualizacja dla prywatnego zakresu łączy

### [Update-AzScheduledQueryRule](Update-AzScheduledQueryRule.md)
Aktualizuje regułę alertu dziennika

