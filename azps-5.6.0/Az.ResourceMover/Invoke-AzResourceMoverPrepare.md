...
### Przykład 1. Sprawdzanie zależności przed przygotowaniem zasobów. Uzyskaj wymagane zasoby zależne, które również muszą być przygotowane.
```powershell
...
Sprawdź poprawność zależności przed przygotowaniem zasobów.
Uzyskaj wymagane zasoby zależne, które również muszą być przygotowane.

### Przykład 2. Zainicjuj przygotowywanie zestawu zasobów w kolekcji przenoszenia, używając jako danych wejściowych "MoveResource Name".
```powershell
...
Zainicjuj przygotowywanie zestawu zasobów w kolekcji przenoszenia przy użyciu funkcji "SourceARMID".
