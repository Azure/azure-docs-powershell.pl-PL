...
## Opis
Polecenie cmdlet **Submit-AzDataLakeAnalyticsJob** przesyła zadanie usługi Azure Data Lake Analytics.
...
### Przykład 2: przesyłanie zadania za pomocą parametrów skryptu
```
...
Parametry skryptu U — SQL są poprzedzone powyżej treści skryptu głównego, na przykład: DECLARE @Department String = "Sales"; DECLARE @NumRecords = 1000; DECLARE @StartDateTime DateTime = New DateTime (2017; 12; 6; 0; 0; 0; 0);

## PARAMETRÓW

### — Konto
Nazwa konta usługi Data Lake Analytics, w ramach którego zostanie przesłane zadanie.
...
### -AnalyticsUnits
Jednostki analiz, które mają być używane dla tego zadania. Zwykle więcej jednostek analitycznych przeznaczonych do skryptu powoduje przyspieszenie czasu wykonywania skryptu.
...
### -Compilemode
Typ kompilacji, który ma zostać wykonany dla tego zadania. Prawidłowe wartości: 
- Semantyka (przeprowadzanie tylko testów semantycznych i niezbędnych testów Sanity)
- Pełna (pełna Kompilacja)
- SingleBox (pełna kompilacja prowadzona lokalnie)
...
### -CompileOnly
Wskazuje, że przesłanie zadania powinno być wykonane tylko w przypadku, gdy ustawiono wartość PRAWDA.
...
### -Name (nazwa)
Przyjazna nazwa zadania do przesłania.
...
### -Priority (priorytet)
Priorytet zadania. Jeśli nie określono, priorytet to 1000. Niższy numer oznacza wyższy priorytet zadania.
...
### -Runtime
Opcjonalnie Ustaw wersję środowiska uruchomieniowego do użycia dla zadania. W przypadku pozostawienia tego ustawienia zostanie użyte domyślne środowisko uruchomieniowe.
...
### -ScriptParameter
Parametry skryptu dla tego zadania, jako słownik nazw parametrów (String) do wartości (dowolna kombinacja Byte, bajty, int, uint (lub UInt32), Long, ULONG (lub UInt64), float, Double, decimal, Short (lub Int16), (lub UInt16), char, String, DateTime, bool, GUID lub Byte []).
...
### -ScriptPath
Ścieżka do pliku skryptu do przesłania.
