...
## Opis
**AzApiManagementDiagnostic** pobiera szczegóły dotyczące diagnostyki skonfigurowanej w usłudze zarządzania interfejsem API w danym zakresie.

## Przykłady

### Przykład 1: Uzyskaj wszystkie funkcje diagnostyczne skonfigurowane w zakresie dzierżawy.
```powershell
...
To polecenie pobiera wszystkie testy diagnostyczne skonfigurowane w usłudze zarządzania interfejsem API.

### Przykład 2: uzyskiwanie wszystkich testów skonfigurowanych w zakresie interfejsu API
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
To polecenie pobiera wszystkie testy diagnostyczne skonfigurowane w `echo-api` zakresie interfejsu API

### Przykład 3: pobieranie diagnostycznego o zakresie interfejsów API określonego przez identyfikator
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
To polecenie pobiera `applicationinsights` diagnostykę skonfigurowaną w interfejsie API `echo-api` .

## PARAMETRÓW

### -ApiId
Identyfikator istniejącego interfejsu API.
Jeśli ta określona, zwróci komunikat o zakresie diagnostyki zakresu API.
...
Jeśli ta określona, zwróci zasady dotyczące zakresu produktów.
Parametry te są opcjonalne.
...
### -ResourceId
Identyfikator zasobu ARM diagnostyki diagnostycznej lub API. Jeśli zostanie określona, spróbuje znaleźć diagnostykę za pomocą identyfikatora. Ten parametr jest wymagany.
