...
> [!IMPORTANT]
> Moduł `AzureRM`, szczegółowo opisany w innych artykułach, nie jest przeznaczony dla platformy .NET Core i nie będzie współdziałać z programem PowerShell Core. Moduły `Az` zawierają aliasy poleceń dla wszystkich poleceń cmdlet `AzureRM`, dlatego ma zastosowanie cała dokumentacja dotycząca usługi `AzureRM`.
