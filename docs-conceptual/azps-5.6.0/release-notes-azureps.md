...
## <a name="560---march-2021"></a>5.6.0 — marzec 2021
#### <a name="azaccounts"></a>Az.Accounts
* Uaktualnij platformę Azure. Identity, aby rozwiązać problem, którego Connect-AzAccount nie powiodło się, gdy jest używane poświadczenie usług ADFS [#13560]

#### <a name="azautomation"></a>Az.Automation
* Rozwiązano problem polegający na tym, że ciąg nie może zostać poprawnie Zserializowany. [#14215]
* Dodano obsługę typu elementu Runbook python3

#### <a name="azcompute"></a>Az.Compute
* Dodano parametr "-EnableHotpatching" do polecenia cmdlet "Set-AzVMOperatingSystem" dla maszyn z systemem Windows. 
* Dodano parametr "-Patchmode" do zestawów parametrów systemu Linux w poleceniu cmdlet "Set-AzVMOperatingSystem". 
* [Istotna zmiana] Istotne zmiany dla użytkowników w publicznej wersji zapoznawczej funkcji poprawek gościa maszyny wirtualnej.
    - Usunięto Właściwość "RebootStatus" z obiektu "Microsoft. Azure. Management. COMPUTE. models. LastPatchInstallationSummary". 
    - Usunięto Właściwość "StartedBy" z obiektu "Microsoft. Azure. Management. COMPUTE. models. LastPatchInstallationSummary".
    - Zmieniono nazwę właściwości "kbid" na "KbId" w obiekcie "Microsoft. Azure. Management. COMPUTE. models. VirtualMachineSoftwarePatchProperties". 
    - Zmieniono nazwę właściwości "poprawki" na "availablePatches" w obiekcie "Microsoft. Azure. Management. COMPUTE. models. VirtualMachineAssessPatchesResult". 
    - Zmieniono nazwę obiektu "Microsoft. Azure. Management. COMPUTE. models. SoftwareUpdateRebootBehavior" na "Microsoft. Azure. Management. COMPUTE. models. VMGuestPatchRebootBehavior".
    - Zmieniono nazwę obiektu "Microsoft. Azure. Management. COMPUTE. models. InGuestPatchMode" na "Microsoft. Azure. Management. COMPUTE. models. WindowsVMGuestPatchMode".
* [Istotna zmiana] Usunięto wszystkie polecenia cmdlet "ContainerService". Interfejs API usługi Container Service został uznany za przestarzały w styczniu 2020. 
...
* Stałe uwierzytelnianie dla `Connect-AzContainerRegistry`

#### <a name="azcosmosdb"></a>AZ. CosmosDB
* Wprowadzono NetworkAclBypass i NetworkAclBypassResourceIds dla poleceń cmdlet konta bazy danych.
* Wprowadzono parametr ServerVersion do aktualizacji-AzCosmosDBAccount.
* Wprowadzono BackupInterval i BackupRetention dla poleceń cmdlet konta bazy danych

#### <a name="azdatafactory"></a>Az.DataFactory
* Zaktualizowano wersję zestawu SDK platformy .NET do 4.14.0

#### <a name="azmigrate"></a>AZ. zmigrować
* AZ. Migrowanie GA
* Włączona Initialize-AzMigrateReplicationInfrastructure jako polecenie cmdlet w module AZ. zmigrować, z zewnętrznego skryptu, który jest obecnie uruchomiony.
* Wykonano kilka parametrów instrukcji New-AzMigrateServerReplication, New-AzMigrateDiskMapping bez uwzględniania wielkości liter.
* Dodano obsługę zmiany urządzenia skalowania w celu obsługi nowych kluczy v3.

#### <a name="azrecoveryservices"></a>Az.RecoveryServices
* Dodano sprawdzanie wartości null dla docelowego konta magazynu w ramach przywracania udziału plików.

#### <a name="azresources"></a>Az.Resources
* Dodano obsługę wdrażania zasobów platformy Azure w języku Bicep
* Rozwiązano problemy z wdrożeniami TemplateSpec w elementach "New-AzTenantDeployment" i "New-AzManagementGroupDeployment"
* Dodano obsługę parametru "-QueryString" w poleceniach cmdlet "test-AZ * Deployments"
* Rozwiązano problem z parametrami dynamicznymi, gdy "New-AZ * Deployments" jest używany z elementem "-QueryString"
* Dodano obsługę parametru "-TemplateParameterObject" podczas używania parametru "-TemplateSpecId" w poleceniach cmdlet "New-AZ * Deployments"
* Rozwiązano niedokładny komunikat o błędzie otrzymany przy próbie wdrożenia nieistniejącej specyfikacji szablonu

#### <a name="azstorage"></a>Az.Storage
* Uaktualniono do Microsoft. Azure. Management. Storage 19.0.0, aby obsługiwał nowy interfejs API w wersji 2021-01-01.
* Obsługiwana reguła dostępu do zasobów w NetworkRuleSet
    - „Update-AzStorageAccountNetworkRuleSet”
    - „Add-AzStorageAccountNetworkRule”
    - "Remove-AzStorageAccountNetworkRule"
* Obsługiwana wersja i typ obiektu BLOB w zasadach zarządzania
    - „Add-AzStorageAccountManagementPolicyAction”
    - "New-AzStorageAccountManagementPolicyFilter"
    - „Set-AzStorageAccountManagementPolicy”
* Obsługiwane konto tworzenia/aktualizowania z AllowSharedKeyAccess
    - „New-AzStorageAccount”
    - „Set-AzStorageAccount”
* Obsługiwane Tworzenie zakresu szyfrowania za pomocą RequireInfrastructureEncryption
    - „New-AzStorageEncryptionScope”
* Obsługiwane synchronicznie blokowe obiekty blob, z zakresem szyfrowania
    - "Copy-AzStorageBlob"
* Rozwiązano problem, który Get-AzStorageBlobContent używać nieprawidłowego separatora katalogu w systemie Linux i MacOS [#14234]

#### <a name="azwebsites"></a>Az.Websites
* Wprowadzono opcję wydawania niestandardowego limitu czasu dla elementu "publish-AzWebApp" 
* Dodano obsługę App Service Environment
...
* @alunmj, Małe pisowni, zmiany formatowania (#14155)
* @chakra146, Aktualizacja Add-AzLoadBalancerInboundNatPoolConfig.md (#14231)
* Martin Ehrnst ( @ehrnst ), Naprawiono literówkę w poleceniu cmdlet (#14112)
* Jan David Narkiewicz ( @jdnark )
  * Przykłady używane New-AzAks, które są starszymi poleceniami cmdlet i aliasem dla New-AzAksCluster. Zmieniono przykłady do użycia New-AzAksCluster, który jest poleceniem cmdlet tej strony dokumentacji. (#14088)
  * Typ Fox: zmieniono SshKeyVaule na SshKeyValue (#14087)
* Ivan Kulezic ( @kukislav ), Dodaj przykłady konfiguracji konserwacji programu SQL mi (#14216)
* @webguynj, Aktualizacja Set-AzNetworkSecurityRuleConfig.md (#14176)
* Yannick dils ( @yannickdils ), dodano dodatkowy plik Cmdlines do przykładu, który stosuje zmiany do modułu równoważenia obciążenia (#14185)

## <a name="550---february-2021"></a>5.5.0 — luty 2021
#### <a name="azaccounts"></a>Az.Accounts
* Śledzony kod CloudError w wyjątku
* Wywołano zdarzenie "ContextCleared" po wykonaniu instrukcji "Clear-AzContext"

#### <a name="azaks"></a>Az.Aks
* Elegancja komunikaty o błędach polecenia cmdlet nie powiodło się.
* Uaktualniono obsługę wyjątków w celu używania Azure PowerShell powiązanych wyjątków.
* Rozwiązano problem polegający na tym, że użytkownik nie mógł użyć podanej nazwy głównej usługi do utworzenia klastra Kubernetes. [#13938]

#### <a name="azautomation"></a>Az.Automation
* Rozwiązano problem polegający na przetwarzaniu operacji "parametr PSCustomObject" i "Array".

#### <a name="azcompute"></a>Az.Compute
* Dodano parametr "-EnableAutomaticUpgrade" do opcji "Set-AzVmExtension" i "Add-AzVmssExtension".
* Usunięto parametr FilterExpression z dokumentacji poleceń cmdlet "Get-AzVMImage". 
* Dodano komunikat o zaniechaniu do poleceń cmdlet ContainerService:
    ...
    - "Remove-AzContainerServiceAgentPoolProfile"
    - "Update-AzContainerService"
* Dodano parametr "-BurstingEnabled" do parametrów "New-AzDiskConfig" i "New-AzDiskUpdateConfig"
* Dodano parametry "-GroupByApplicationId" i "-GroupByUserAgent" do poleceń cmdlet "Export-AzLogAnalyticThrottledRequest" i "Export-AzLogAnalyticRequestRateByInterval".
* Dodano parametr "VMParameterSet" do polecenia cmdlet "Get-AzVMExtension". Dodano nowy parametr "-VM" do tego zestawu parametrów. 

#### <a name="azcontainerregistry"></a>Az.ContainerRegistry
* Dodano polecenia cmdlet do obsługiwanego repozytorium, manifestu i operacji tagów:
...
Obsługiwane — EnableNoPublicIP podczas tworzenia obszaru roboczego datakosteks

#### <a name="azfrontdoor"></a>Az.FrontDoor
* Dodano FrontDoorId do właściwości
* Dodano wykluczenia JSON i obsługę RequestBodyCheck do zarządzanych reguł

#### <a name="azhdinsight"></a>Az.HDInsight
* Dodano nowy parametr "-EnableComputeIsolation" i "-ComputeIsolationHostSku" do polecenia cmdlet "New-AzHDInsightCluster" w celu obsługi funkcji izolacji obliczeniowej
* Dodano Właściwość "ComputeIsolationProperties" i "ConnectivityEndpoints" w klasie AzureHDInsightCluster.

#### <a name="azkeyvault"></a>Az.KeyVault
* Obsługiwane Określanie typu klucza i nazwy krzywej podczas importowania kluczy za pomocą pliku BYOK

#### <a name="aznetwork"></a>Az.Network
* Dodano nowe polecenia cmdlet służące do zamiany starej nazwy produktu "routera wirtualnego" na nową nazwę "serwer tras" w przyszłości.
    - "New-AzRouteServer"
    - "Get-AzRouteServer"
    ...
    - Dodano ostrzeżenie o atrybucie wycofania do starych poleceń cmdlet.
* Poprawka usterki w programie ExpressRouteLink MacSecConfig. Dodano nową właściwość "SciState" do "PSExpressRouteLinkMacSecConfig"
* Zaktualizowana lista formatów i widoki tabeli formatowania dla Get-AzVirtualNetworkGatewayConnectionIkeSa

#### <a name="azpolicyinsights"></a>Az.PolicyInsights
* Wycofane zmiany wprowadzone w programie PowerShell, które zwiększyły limit wierszy żądania. Usunięto niepoprawną instrukcję pomocniczego stronicowania

#### <a name="azrecoveryservices"></a>Az.RecoveryServices
* zmodyfikowano limity weryfikacji zasad zgodnie z usługą kopii zapasowej.
* Dodano nadmiarowość strefy dla magazynów usługi Recovery Service. 
* Azure Site Recovery obsługę grupy umieszczania sąsiedztwa dla programu VMware na platformie Azure i funkcji HyperV dla dostawców platformy Azure.
* Azure Site Recovery obsługę strefy dostępności dla oprogramowania VMware na platformie Azure i funkcji HyperV dla dostawców platformy Azure.
* Azure Site Recovery obsługę funkcji Hyper-v dla UseManagedDisk dla dostawcy platformy Azure

#### <a name="azresources"></a>Az.Resources
* Usunięto typ podmiotu zabezpieczeń dla New-AzRoleAssignment i Set-AzRoleAssignment ponieważ bieżące mapowanie dotyczyło pewnych scenariuszy

#### <a name="azsql"></a>Az.Sql
* Dodano MaintenanceConfigurationId do opcji "New-AzSqlDatabase", "Set-AzSqlDatabase", "New-AzSqlElasticPool" i "Set-AzSqlElasticPool"
* Naprawiono regresję w elemencie "Set-AzSqlServerAudit", gdy podano argument PredicateExpression

#### <a name="azstorage"></a>Az.Storage
* Obsługiwane ustawienia RoutingPreference na koncie magazynu Utwórz/zaktualizuj
    - „New-AzStorageAccount”
    - „Set-AzStorageAccount”
* Uaktualniono platformę Azure. Storage. blob do 12.8.0
* Uaktualniono usługę Azure. Storage. Files. shares do 12.6.0
* Uaktualniono usługę Azure. Storage. Files. datalake do 12.6.0

#### <a name="azwebsites"></a>Az.Websites
* Dodano obsługę importowania certyfikatu magazynu kluczy do WebApp.

### <a name="thanks-to-our-community-contributors"></a>Dziękujemy współautorom z naszej społeczności
* @atul-ram, Aktualizacja Set-AzEventHub.md (#13921)
* Christoph Bergmeister [MVP] ( @bergmeister ), Set-AzDataLakeGen2AclRecursive.MD-Fix literę (directiry-> Directory) (#14082)
* Alexander Schmidt ( @devdeer-alex ), stały przerwany link do wytycznych dotyczących udziału (#14009)
* @JiangYuchun, Aktualizacja Get-AzApplicationGatewayAuthenticationCertificate.md (#13972)
* Sebastian Olsen ( @Spacebjorn ), poprawione przykładowe polecenie (#13901)

## <a name="540---january-2021"></a>5.4.0 — styczeń 2021
#### <a name="azaccounts"></a>Az.Accounts
* Podano prawidłowy identyfikator żądania klienta w komunikacie debugowania [#13745]
* Dodano typ wyjątku Common Azure PowerShell
* Obsługiwany interfejs API magazynu 2019-06-01

#### <a name="azautomation"></a>Az.Automation
* Rozwiązano problem polegający na tym, że opis nie został wypełniony dla harmonogramów zarządzania aktualizacjami

#### <a name="azcosmosdb"></a>AZ. CosmosDB
* Ogólna dostępność modułu "AZ. CosmosDB"
* Ograniczanie New-AzCosmosDBAccount polecenia cmdlet w celu wykonywania wywołań aktualizacji do istniejących kont bazy danych.
* Wprowadzenie do AnalyticalStorageTTL w sqlcontainerer.

#### <a name="aziothub"></a>Az.IotHub
* Naprawiono regresję dotyczącą generowania tokenów SAS

#### <a name="azkeyvault"></a>Az.KeyVault
* Rozwiązano problem w module zarządzania kluczami tajnymi

#### <a name="azlogicapp"></a>Az.LogicApp
* Rozwiązano problem polegający na tym, że element "Get-AzLogicAppTriggerHistory" i "Get-AzLogicAppRunAction" pobiera jedynie pierwszą stronę wyników [#9141]

#### <a name="azmonitor"></a>Az.Monitor
* Dodano polecenia cmdlet dla reguł zbierania danych: 
    - "Get-AzDataCollectionRule"
    - "New-AzDataCollectionRule"
    - "Set-AzDataCollectionRule"
    - "Update-AzDataCollectionRule"
    - "Remove-AzDataCollectionRule"
* Dodano polecenia cmdlet dla skojarzeń reguł zbierania danych
    - "Get-AzDataCollectionRuleAssociation"
    - "New-AzDataCollectionRuleAssociation"
    - "Remove-AzDataCollectionRuleAssociation"

#### <a name="aznetwork"></a>Az.Network
* Dodano nowe polecenia cmdlet dla CRUD of VpnGatewayNATRule.
    - "New-AzVpnGatewayNatRule"
    - "Update-AzVpnGatewayNatRule"
    - "Get-AzVpnGatewayNatRule"
    - "Remove-AzVpnGatewayNatRule"  
* Zaktualizowano polecenia cmdlet służące do ustawiania — obiekt natrule w zasobie bramy vpngateway i kojarzenia go z zasobem VpnSiteLinkConnection.
    - "New-AzVpnGateway"
    - "Update-AzVpnGateway" 
    - "New-AzVpnSiteLinkConnection"
* Zaktualizowano polecenia cmdlet umożliwiające ustawienie wartości Connectionmode dla połączeń bramy Virtual Network.
    - „New-AzVirtualNetworkGatewayConnection”
    - „Set-AzVirtualNetworkGatewayConnection”
* Zaktualizowano polecenie cmdlet "New-AzFirewallPolicyApplicationRule":
    - Dodano docelowy adres URL parametrów
    - Dodano TerminateTLS parametrów
* Dodano nowe polecenia cmdlet dla funkcji usługi Azure firewall Premium
    - "New-AzFirewallPolicyIntrusionDetection"
    - "New-AzFirewallPolicyIntrusionDetectionBypassTraffic"
    - "New-AzFirewallPolicyIntrusionDetectionSignatureOverride"
* Zaktualizowano polecenie cmdlet New-AzFirewallPolicy:
    - Dodano parametr-SkuTier
    - Dodano parametr-Identity
    - Dodano parametr-UserAssignedIdentityId
    - Dodano parametr-IntrusionDetection
    - Dodano parametr-TransportSecurityName
    - Dodano parametr-TransportSecurityKeyVaultSecretId
* Dodano nowe polecenie cmdlet do pobierania skojarzeń zabezpieczeń IKE dla połączeń bramy Virtual Network.
    - "Get-AzVirtualNetworkGatewayConnectionIkeSa"
* Dodano obsługę wielu uwierzytelnień dla p2sVpnGateway
    - Zaktualizowano New-AzVpnServerConfiguration i Update-AzVpnServerConfiguration, aby umożliwić ustawienie wielu parametrów uwierzytelniania.
* Zaktualizowano polecenie cmdlet "New-AzVpnGateway" i "New-AzP2sVpnGateway":
    - Dodano EnableRoutingPreferenceInternetFlag parametrów

#### <a name="azrecoveryservices"></a>Az.RecoveryServices
* Dodano funkcję przywracania między regionami.  
* Zablokowano Pobieranie konfiguracji obciążenia, gdy element docelowy jest grupą dostępności.

#### <a name="azresources"></a>Az.Resources
* Dodano obsługę parametru-QueryString w poleceniach cmdlet New-AZ * Deployments

#### <a name="azsql"></a>Az.Sql
* Wykonano polecenie cmdlet "Start-AzSqlInstanceDatabaseLogReplay" synchronicznej, dodanej flagi AsJob

#### <a name="azstorage"></a>Az.Storage
* Popraw ContinuationToken nigdy null, gdy lista obiektów BLOB with-IncludeVersion
    - „Get-AzStorageBlob”

#### <a name="azwebsites"></a>Az.Websites
* Dodano obsługę App Service certyfikatów zarządzanych
    - "New-AzWebAppCertificate"
    - "Remove-AzWebAppCertificate"
* Rozwiązano problem powodujący usunięcie hasła platformy Docker z pliku "Set-AzWebApp" i "Set-AzWebAppSlot"

### <a name="thanks-to-our-community-contributors"></a>Dziękujemy współautorom z naszej społeczności
* Ivan Akcheurov ( @ivanakcheurov ), aktualizacja Set-AzSecurityWorkspaceSetting.MD (#13877)
* @javiermarasco, Przykład aktualizacji (#13837)
* @jhaprakash26, Aktualizacja Set-AzVirtualNetwork.md (#13857)
* Michael Holmes ( @MichaelHolmesWP ), Update New-AzStorageTableStoredAccessPolicy.MD (#13871)
* Jan Kuba ( @mikejwhat ), zezwól Get-AzLogicAppTriggerHistory i Get-AzLogicAppRunAction na zwrócenie więcej niż 30 wyników (#13846)
* @Willem-J-an, Usuń usterkę powodującą usunięcie hasła Docker z zestawu AppSettings w Set-AzWebApp (gniazdo) (#13866)

## <a name="530---december-2020"></a>5.3.0 — grudzień 2020 r.
#### <a name="azaccounts"></a>Az.Accounts
* Rozwiązano problem polegający na tym, że serwer proxy HTTP nie jest uznawany w programie Windows PowerShell [nr 13647]
* Ulepszono dziennik debugowania długotrwałych operacji w wygenerowanych modułach

#### <a name="azautomation"></a>Az.Automation
* Rozwiązano problem polegający na tym, że parametry polecenia „Start-AzAutomationRunbook” nie mogą przekonwertować ciągu opakowanego w obiekt PSObject na ciąg JSON [nr 13240]
* Naprawiono moduł wypełniania lokalizacji polecenia cmdlet New-AzAutomationUpdateManagementAzureQuery

#### <a name="azcompute"></a>Az.Compute
* Dodano nowy parametr „VM” w nowym zestawie parametrów „VMParameterSet” do poleceń cmdlet „Get-AzVMDscExtensionStatus” i „Get-AzVMDscExtension”. 

#### <a name="azdatabricks"></a>Az.Databricks
* Rozwiązano problem mogący powodować zwrócenie wartości przez polecenie „New-AzDatabricksVNetPeering” przed pełnym aprowidowaniem (https://github.com/Azure/autorest.powershell/issues/610)

#### <a name="azdatafactory"></a>Az.DataFactory
* Rozwiązano problem SupportsShouldProcess w przypadku polecenia „Invoke-AzDataFactoryV2Pipeline”

#### <a name="azdesktopvirtualization"></a>Az.DesktopVirtualization
* Dodano właściwość StartVMOnConnect do puli hostów.

#### <a name="azhdinsight"></a>Az.HDInsight
* Dodano właściwości: Fqdn i EffectiveDiskEncryptionKeyUrl w klasie AzureHDInsightHostInfo.

#### <a name="azkeyvault"></a>Az.KeyVault
* Dodano nowy parametr „-AsPlainText” do polecenia „Get-AzKeyVaultSecret”, aby bezpośrednio zwracać wpis tajny w postaci zwykłego tekstu [nr 13630]
* Dodano obsługę selektywnego przywracania klucza z pełnej kopii zapasowej zarządzanego modułu HSM [nr 13526]
* Rozwiązano pewne drobne problemy [nr 13583] [nr 13584]
* Dodano brakujące obiekty zwracane polecenia „Get-Secret” w module SecretManagement
* Rozwiązano problem, który może spowodować utworzenie magazynu bez domyślnych zasad dostępu [nr 13687]

#### <a name="azkusto"></a>Az.Kusto
* Zaktualizowano wersję interfejsu API do 2020-09-18.

#### <a name="aznetwork"></a>Az.Network
* Rozwiązano problem z poleceniem cmdlet usuwania komunikacji równorzędnej i połączenia w scenariuszu ExpressRouteCircuit
    - Polecenia „Remove-AzExpressRouteCircuitPeeringConfig” i „Remove-AzExpressRouteCircuitConnectionConfig”

#### <a name="azpolicyinsights"></a>Az.PolicyInsights
* Dodano obsługę zwracania wyników z podziałem na strony przez polecenie Get-AzPolicyState

#### <a name="azrecoveryservices"></a>Az.RecoveryServices
* Włączono funkcję usuwania nietrwałego dla języka SQL.
* Naprawiono przywracanie grupy dostępności SQL i usunięto sprawdzanie nazwy kontenera.
* Zmieniono format nazwy kontenera dla elementu kopii zapasowej usługi Azure Files.
* Dodano obsługę funkcji klucza zarządzanego przez klienta dla magazynu usługi Recovery Services.

#### <a name="azresources"></a>Az.Resources
* Rozwiązano problem dotyczący wyjątku NullRef w poleceniach „New-AzureManagedApplication” i „Set-AzureManagedApplication”.
* Zaktualizowano zestaw SDK usługi Azure Resource Manager tak, aby używał najnowszej wersji interfejsu API ogólnie dostępnego elementu DeploymentScripts: 2020-10-01.

#### <a name="azservicefabric"></a>Az.ServiceFabric
* Naprawiono polecenie „Add-AzServiceFabricNodeType”. Dodano typ węzła do klastra usługi Service Fabric przed utworzeniem zestawu skalowania maszyn wirtualnych.

#### <a name="azsql"></a>Az.Sql
* Poprawiono opis parametru polecenia „InstanceFailoverGroup”.
* Zaktualizowano logikę wyodrębniającą elementy schemaName, tableName i columnName z identyfikatora poleceń klasyfikacji danych SQL.
* Poprawiono pola Status i StatusMessage w poleceniu cmdlet „Get-AzSqlDatabaseImportExportStatus” tak, aby były zgodne z dokumentacją
* Dodano polecenia cmdlet inspekcji operacji pomocy technicznej firmy Microsoft (DevOps): Get-AzSqlServerMSSupportAudit, Set-AzSqlServerMSSupportAudit, Remove-AzSqlServerMSSupportAudit

#### <a name="azstorage"></a>Az.Storage
* Dodano obsługę tworzenia/aktualizowania/pobierania/wyświetlania listy elementów EncryptionScope konta magazynu
    - „New-AzStorageEncryptionScope”
    - „Update-AzStorageEncryptionScope”
    - „Get-AzStorageEncryptionScope”
* Dodano obsługę tworzenia kontenera i przekazywania obiektu blob z ustawieniem zakresu szyfrowania
    - „New-AzRmStorageContainer”
    - „New-AzStorageContainer”
    - „Set-AzStorageBlobContent”

### <a name="thanks-to-our-community-contributors"></a>Dziękujemy współautorom z naszej społeczności
* Andreas Wolter (@AndreasWolter) — usunięcie języka marketingowego, lepszy filtr przykładów (nr 13671)
* Tidjani Belmansour (@BelRarr) — aktualizacja pliku Get-AzBillingInvoice.md (nr 13634)
* David Klempfner (@DavidKlempfner)
  * Naprawienie błędu pisowni (nr 13677)
  * Aktualizacja pliku PSMetricNoDetails.cs (nr 13676)
* @kilobyte97 — poprawka usterki polecenia cmdlet dotyczącej usuwania konfiguracji (nr 13655)
* @kongou-ae — aktualizacja pliku Set-AzFirewall.md (nr 13727)
* @MasterKuat — poprawienie zamienionego tytułu i kodu w dokumentacji (nr 13666)
* NickT (@nukeulis) — aktualizacja pliku Set-AzContext.md (nr 13702)
* @PaulHCode — aktualizacja pliku Start-AzJitNetworkAccessPolicy.md i poprawienie przykładu tak, aby wyświetlał prawidłowe, omawiane polecenie cmdlet (nr 13713)
* Ryan Borstelmann (@ryanborMSFT) — usunięcie identyfikatora subskrypcji (nr 13715)
* Shashikant Shakya (@shshakya) — aktualizacja pliku Set-AzSqlDatabase.md (nr 13674)
* Sebastian Olsen (@Spacebjorn) — aktualizacja pliku Get-AzRecoveryServicesBackupItem.md (nr 13719)

## <a name="520---december-2020"></a>5.2.0 — grudzień 2020 r.
#### <a name="azaccounts"></a>Az.Accounts
* Możliwość przeanalizowania czasu ExpiresOn z nieprzetworzonego tokenu, jeśli nie można go pobrać z biblioteki źródłowej
* Ulepszony komunikat ostrzegawczy, gdy uwierzytelnianie interaktywne jest niedostępne

#### <a name="azapimanagement"></a>Az.ApiManagement
* [Zmiana powodująca niezgodność] Domyślnie polecenie „New-AzApiManagementProduct” nie ma limitu subskrypcji.

#### <a name="azcompute"></a>Az.Compute
* Edytowano polecenie Get-AzVm tak, aby filtrowało według wartości „-Name” przed sprawdzeniem ograniczenia przepustowości ze względu na zbyt wiele zasobów. 
* Nowe polecenie cmdlet „Start-AzVmssRollingExtensionUpgrade”.

#### <a name="azcontainerregistry"></a>Az.ContainerRegistry
* Obsługiwany parametr „Name” i wartość z danych wejściowych potoku dla polecenia „Get-AzContainerRegistryUsage” [#13605]
* Dopracowane wyjątki dla polecenia „Connect-AzContainerRegistry”

#### <a name="azdatafactory"></a>Az.DataFactory
* Zaktualizowano wersję zestawu .Net SDK usługi ADF do wersji 4.13.0

#### <a name="azhealthcareapis"></a>Az.HealthcareApis
* Dodano obsługę kluczy zarządzanych przez klienta

#### <a name="aziothub"></a>Az.IotHub
* Rozwiązano problem z tokenem SAS.

#### <a name="azkeyvault"></a>Az.KeyVault
* Obsługiwane jest wartość „all” jako opcja podczas ustawiania zasad dostępu do magazynu kluczy
* Obsługiwana jest nowa wersja modułu SecretManagement [#13366]
* Obsługiwane są parametry ByteArray, String, PSCredential i Hashtable dla elementu „SecretValue” w elemencie SecretManagementModule [#12190]
* [Zmiana powodująca niezgodność] przeprojektowano powierzchnię interfejsu API poleceń cmdlet związanych z zarządzanym modułem HSM.

#### <a name="azmonitor"></a>Az.Monitor
* Zmieniono parametr „Rule” polecenia „New-AzAutoscaleProfile” tak, aby akceptował pustą listę. [#12903]
* Dodano nowe polecenia cmdlet do obsługi tworzenia ustawień diagnostycznych w bardziej elastyczny sposób:
    * „Get-AzDiagnosticSettingCategory”
    * „New-AzDiagnosticSetting”
    * „New-AzDiagnosticDetailSetting”

#### <a name="azrecoveryservices"></a>Az.RecoveryServices
* Wprowadzono zmiany w tekście pomocy i nazwie zestawu parametrów w poleceniu cmdlet „Restore-AzRecoveryServicesBackupItem”.

#### <a name="azresources"></a>Az.Resources
* Dodano obsługę parametru „-Tag” do poleceń „Set-AzTemplateSpec” i „New-AzTemplateSpec”
* Dodano obsługę wyświetlania tagów do domyślnego programu formatującego dla specyfikacji szablonu 

#### <a name="azservicefabric"></a>Az.ServiceFabric
* Dodano przykład do polecenia „Set-AzServiceFabricSetting” z parametrem SettingsSectionDescription
* Zaktualizowano polecenia cmdlet powiązane z aplikacją w celu podkreślenia, że obsługa dotyczy tylko zasobów wdrożonych za pomocą usługi ARM
* Oznaczono do wycofania polecenia cmdlet certyfikatu klastra „Add-AzureRmServiceFabricClusterCertificate” i „Remove-AzureRmServiceFabricClusterCertificate”

#### <a name="azsql"></a>Az.Sql
* Dodano parametr SecondaryType do następujących poleceń: 
    - „New-AzSqlDatabase”
    - „Set-AzSqlDatabase”
    - „New-AzSqlDatabaseSecondary”
* Dodano parametr HighAvailabilityReplicaCount do następujących poleceń: 
    - „New-AzSqlDatabase”
    - „Set-AzSqlDatabase”
* Parametr ReadReplicaCount został ustawiony jako alias parametru HighAvailabilityReplicaCount w następujących poleceniach: 
    - „New-AzSqlDatabase”
    - „Set-AzSqlDatabase”

#### <a name="azstorage"></a>Az.Storage
* Obsługa przekazywania plików na platformę Azure o rozmiarze do 4 TiB
    - „Set-AzStorageFileContent”
* Uaktualniono element Azure.Storage.Blob do 12.7.0
* Uaktualniono element Azure.Storage.Files.Shares do 12.5.0
* Uaktualniono element Azure.Storage.Files.DataLake do 12.5.0

#### <a name="azstoragesync"></a>Az.StorageSync
* Dodano funkcję zasad warstw synchronizacji z zasadami pobierania i trybem lokalnej pamięci podręcznej

#### <a name="azwebsites"></a>Az.Websites
* Zapobieganie duplikowaniu reguł ograniczania dostępu

### <a name="thanks-to-our-community-contributors"></a>Dziękujemy współautorom z naszej społeczności
* Andrew Dawson (@dawsonar802), aktualizacja pliku Get-AzKeyVaultCertificate.md —Pobieranie certyfikatów i zapisywanie ich jako sekcji pliku pfx działającego z programem PowerShell Core (#13557)
* @iviark, aktualizacje BYOK programu Powershell dla interfejsów Healthcare API (#13518)
* John Duckmanton (@johnduckmanton), poprawa pisowni dla operacji TagPatchOperation (#13508)
* Michael James (@mikejwhat)
  * Uporządkowanie pomocy do polecenia Get-AzLogicAppRunHistory (#13513)
* Richard de Zwart (@mountain65)
  * Aktualizacja pliku Update-AzAppConfigurationStore.md (#13485)
  * Aktualizacja pliku New-AzCosmosDBAccount.md (#13490)
* @SteppingRazor, New-AzApiManagementProduct: Zmiana wartości domyślnej parametru SubscriptionsLimit na None (#13457)
* Steve Burkett (@SteveBurkettNZ), poprawienie literówki w parametrze WorkspaceResourceId w przykładzie (#13589)

## <a name="510---november-2020"></a>5.1.0 — listopad 2020 r.
#### <a name="azaccounts"></a>Az.Accounts
* Rozwiązano problem polegający na tym, że parametr TenantId może nie być uznawany w przypadku użycia polecenia „Connect-AzAccount-DeviceCode” [#13477]
* Dodano nowe polecenie cmdlet „Get-AzAccessToken”
* Rozwiązano problem polegający występowaniu błędu, gdy ścieżka profilu użytkownika jest niedostępna
* Rozwiązano problem powodujący występowanie błędu Write-Object podczas wykonywania polecenia Connect-AzAccount [#13419]
* Dodano parametr „ContainerRegistryEndpointSuffix” do poleceń: „Add-AzEnvironment”, „Set-AzEnvironment” 
* Obsługiwane przerywanie logowania przez naciśnięcie klawiszy <kbd>CTRL</kbd>+<kbd>C</kbd>
* Rozwiązano problem powodujący niedziałanie polecenia „Connect-AzAccount-KeyVaultAccessToken” [#13127]
* Poprawiono pustą referencję i brak uwzględniania wielkości liter w metodzie w poleceniu „Invoke-AzRestMethod”

#### <a name="azaks"></a>Az.Aks
* Rozwiązano problem polegający na tym, że użytkownik nie może utworzyć nowego klastra Kubernetes przy użyciu nazwy głównej usługi. [#13012]

#### <a name="azappconfiguration"></a>Az.AppConfiguration
* Ogólna dostępność modułu „Az.AppConfiguration”

#### <a name="azdatafactory"></a>Az.DataFactory
* Ulepszono komunikat o błędzie polecenia „New-AzDataFactoryV2LinkedServiceEncryptedCredential”

#### <a name="azdatalakestore"></a>Az.DataLakeStore
* Zaktualizowano zestaw SDK płaszczyzny danych ADLS do wersji 1.2.4-alpha. Zmiany: https://github.com/Azure/azure-data-lake-store-net/blob/preview-alpha/CHANGELOG.md#version-124-alpha

#### <a name="azdesktopvirtualization"></a>Az.DesktopVirtualization
* Dodano nowe polecenia cmdlet pakietu MSIX i zaktualizowano polecenia cmdlet aplikacji.

#### <a name="azeventhub"></a>Az.EventHub
* Naprawiono polecenia klastra dla klastra EventHub bez tagów
* Zaktualizowano tekst pomocy dla parametru PartnerNamespace poleceń AzEventHubGeoDRConfiguration 

#### <a name="azhdinsight"></a>Az.HDInsight
* Dodano parametry „ResourceProviderConnection” i „PrivateLink” do polecenia cmdlet „New-AzHDInsightCluster” w celu obsługi funkcji połączenia wychodzącego przekazywania i łącza prywatnego
* Dodano parametr „AmbariDatabase” do polecenia cmdlet „New-AzHDInsightCluster” w celu obsługi funkcji niestandardowej bazy danych Ambari
* Dodano wartość akceptacji „AmbariDatabase” do parametru „MetastoreType” polecenia cmdlet „Add-AzHDInsightMetastore”

#### <a name="aziothub"></a>Az.IotHub
* Zezwolono na tagi w poleceniu cmdlet tworzenia usługi IoT Hub.

#### <a name="azkeyvault"></a>Az.KeyVault
* Obsługiwany zaktualizowany tag magazynu kluczy

#### <a name="azlogicapp"></a>Az.LogicApp
* Rozwiązano problem polegający na tym, że polecenie Get-AzLogicAppRunHistory powodowało pobieranie tylko pierwszej strony wyników

#### <a name="aznetwork"></a>Az.Network
* Zaktualizowano poniższe polecenie cmdlet 
    - „New-AzLoadBalancerFrontendIpConfigCommand”, „Set-AzLoadBalancerFrontendIpConfigCommand”, „Add-AzLoadBalancerFrontendIpConfigCommand”:
        - Dodano właściwość PublicIpAddressPrefix
        - Dodano właściwość PublicIpAddressPrefixId
* Aby umożliwić globalne równoważenie obciążenia, dodano nowe właściwości do następujących poleceń cmdlet
    - „New-AzLoadBalancer”:
        - Dodano właściwość Sku Tier
    - „New-AzPuplicIpAddress”:
        - Dodano właściwość Sku Tier
    - „New-AzPublicIpPrefix”:
        - Dodano właściwość Sku Tier
    - „New-AzLoadBalancerBackendAddressConfig”:
        - Dodano właściwość LoadBalancerFrontendIPConfigurationId
* Zaktualizowano planowanie w celu wycofania ostrzeżeń dla następujących poleceń cmdlet   -„New-AzVirtualHubRoute”   -„New-AzVirtualHubRouteTable”   -„Add-AzVirtualHubRoute”   -„Add-AzVirtualHubRouteTable”   -„Get-AzVirtualHubRouteTable”   -„Remove-AzVirtualHubRouteTable”
* Dodano planowanie w celu wycofania ostrzeżeń dotyczących argumentu „RouteTable” dla następujących poleceń cmdlet   -„New-AzVirtualHub”   -„Set-AzVirtualHub”   -„Update-AzVirtualHub”
* Argumenty „-MinScaleUnits” i „-MaxScaleUnits” stały się opcjonalne w poleceniu „Set-AzExpressRouteGateway”
* Dodano nowe polecenia cmdlet w celu obsługi uwierzytelniania wzajemnego i profilów SSL w usłudze Application Gateway
...
#### <a name="azrecoveryservices"></a>Az.RecoveryServices
* Określanie czasu tworzenia kopii zapasowej zasad odbywa się w formacie UTC.
* Modyfikacja ostrzeżenia o zmianie powodującej niezgodność w poleceniu cmdlet Get-AzRecoveryServicesBackupJobDetails.
* Aktualizacja tekstu pomocy przykładowego skryptu dla polecenia cmdlet Set-AzRecoveryServicesBackupProtectionPolicy.

#### <a name="azresources"></a>Az.Resources
* Rozwiązano problem polegający na tym, że parametr What-If powodował wyświetlanie dwóch zakresów grupy zasobów z inną wielkością liter
* Zaktualizowano polecenie „Export-AzResourceGroup”, aby używało zestawu SDK.
* Dodano informacje o kulturze do metod analizy

#### <a name="azsql"></a>Az.Sql
* Rozwiązano problemy polegające na tym, że polecenie Set-AzSqlDatabaseAudit nie obsługiwało bazy danych Hiperskala i nie można było ustalić wersji bazy danych
* Dodano parametr MaintenanceConfigurationId do poleceń „New-AzSqlInstance” i „Set-AzSqlInstance”
* Naprawiono usterkę w poleceniu GetAzureSqlDatabaseReplicationLink.cs polegającą na tym, że w parametrze PartnerServerName była sprawdzana wartość zamiast klucza

#### <a name="azwebsites"></a>Az.Websites
* Dodano obsługę nowych funkcji ograniczania dostępu: ServiceTag, multi-ip i http-headers

### <a name="thanks-to-our-community-contributors"></a>Dziękujemy współautorom z naszej społeczności
* John Q. Martin (@johnmart82), dodanie informacji o wymaganiach wstępnych zapory (#13385)
* Manikandan Duraisamy (@madurais-msft), poprawienie argumentu PublicSubnetName (#13417)
* @mahortas, zaktualizowanie wartości parametru -HostNames (#13349)
* @MariachiForHire, dodanie obsługiwanych wartości TrafficAnalyticsInterval (#13304)
* Michael James (@mikejwhat), zezwalanie na zwracanie ponad 30 wpisów przez polecenie Get-AzLogicAppRunHistory (#13393)
* Shashikant Shakya (@shshakya), zaktualizowanie polecenia Restore-AzSqlInstanceDatabase.md (#13404)

## <a name="500---october-2020"></a>5.0.0 — październik 2020 r.
#### <a name="azaccounts"></a>Az.Accounts
* [Zmiana powodująca niezgodność] Usunięto polecenie „Get-AzProfile” i polecenie „Select-AzProfile”
* Zastąpiono bibliotekę uwierzytelniania usługi Azure Directory biblioteką uwierzytelniania firmy Microsoft (MSAL)

#### <a name="azaks"></a>Az.Aks
* [Zmiana powodująca niezgodność] Usunięto alias „ClientIdAndSecret” w poleceniu „New-AzAksCluster” i „Set-AzAksCluster”.
* [Zmiana powodująca niezgodność] Zmieniono wartość domyślną elementu „NodeVmSetType” w poleceniu „New-AzAksCluster” z „AvailabilitySet” na „VirtualMachineScaleSets”.
* [Zmiana powodująca niezgodność] Zmieniono wartość domyślną elementu „NetworkPlugin'” w poleceniu „New-AzAksCluster” z „none” na „Azure”.
* [Zmiana powodująca niezgodność] Usunięto parametr „NodeOsType” w poleceniu „New-AzAksCluster”, ponieważ obsługuje on tylko jedną wartość systemu Linux.

#### <a name="azbilling"></a>Az.Billing
* Dodano polecenie cmdlet „Get-AzBillingAccount”
* Dodano polecenie cmdlet „Get-AzBillingProfile”
* Dodano polecenie cmdlet „Get-AzInvoiceSection”
* Dodano nowe parametry w poleceniu cmdlet „Get-AzBillingInvoice”
* Usunięto właściwości DownloadUrlExpiry, Type, BillingPeriodNames z odpowiedzi polecenia cmdlet Get-AzBillingInvoice

#### <a name="azcdn"></a>Az.Cdn
* Dodano polecenia cmdlet do obsługi funkcji wielu źródeł i łącza prywatnego 

#### <a name="azcognitiveservices"></a>Az.CognitiveServices
* Zaktualizowano zestaw SDK do 7.4.0 — wersja zapoznawcza.

#### <a name="azcompute"></a>Az.Compute
* Dodano parametr „-VmssId” do polecenia „New-AzVm”
* Dodano parametr „PlatformFaultDomainCount” do polecenia cmdlet „New-AzVmss”.
* Nowe polecenie cmdlet „Get-AzDiskEncryptionSetAssociatedResource”
* Dodano opcjonalne parametry „Tier” i „LogicalSectorSize” do polecenia cmdlet New-AzDiskConfig. 
...
#### <a name="azdatabricks"></a>Az.Databricks
* Naprawiono błąd, który może powodować, że aktualizacja obszaru roboczego usługi Databricks bez elementu `-EncryptionKeyVersion` kończy się niepowodzeniem.

#### <a name="azdatafactory"></a>Az.DataFactory
* Zaktualizowano wersję zestawu .Net SDK usługi ADF do wersji 4.12.0
* Zaktualizowano wersję zestawu SDK klienta szyfrowania usługi ADF do wersji 4.14.7587.7
* Dodano polecenia „Stop-AzDataFactoryV2TriggerRun” i „Invoke-AzDataFactoryV2TriggerRun”

#### <a name="azdesktopvirtualization"></a>Az.DesktopVirtualization
* Wymagaj właściwości Location do tworzenia obiektów ARM najwyższego poziomu.
        * Parametr `ApplicationGroupType` jest wymagany dla polecenia `New-AzWvdApplicationGroup`.
        * Parametr `HostPoolArmPath` jest wymagany dla polecenia `New-AzWvdApplicationGroup`.
        * Dodano parametr `PreferredAppGroupType` dla polecenia `New-AzWvdHostPool`.

#### <a name="azfunctions"></a>Az.Functions
* [Zmiana powodująca niezgodność] Usunięto parametr przełącznika „IncludeSlot” ze wszystkich zestawów parametrów oprócz jednego: „Get-AzFunctionApp”. Polecenie cmdlet obsługuje teraz pobieranie miejsc wdrożenia w wynikach, gdy określono parametr „-IncludeSlot”. 
* Zaktualizowano polecenie „New-AzFunctionApp”:
  - Naprawiono element -DisableApplicationInsights, dzięki czemu nie jest tworzony projekt usługi Application Insights, gdy ta opcja jest określona. [#12728]
  - [Zmiana powodująca niezgodność] Usunięto obsługę tworzenia aplikacji funkcji programu PowerShell 6.2.
  - [Zmiana powodująca niezgodność] Zmieniono domyślną wersję środowiska uruchomieniowego w usłudze Functions w wersji 3 w systemie Windows dla aplikacji funkcji programu PowerShell z 6.2 na 7.0, gdy nie określono parametru RuntimeVersion.
  - [Zmiana powodująca niezgodność] Zmieniono domyślną wersję środowiska uruchomieniowego w usłudze Functions w wersji 3 w systemach Windows i Linux dla aplikacji funkcji programu Node z 10 na 12, gdy nie określono parametru RuntimeVersion.
  - [Zmiana powodująca niezgodność] Zmieniono domyślną wersję środowiska uruchomieniowego w usłudze Functions w wersji 3 w systemie Linux dla aplikacji funkcji języka Python z 3.7 na 3.8, gdy nie określono parametru RuntimeVersion.

#### <a name="azhdinsight"></a>Az.HDInsight
 * Dla polecenia cmdlet New-AzHDInsightCluster:
     - Zastąpiono parametr „DefaultStorageAccountName” parametrem „StorageAccountResourceId”
     - Zastąpiono parametr „DefaultStorageAccountKey” parametrem „StorageAccountKey”
     - Zastąpiono parametr „DefaultStorageAccountType” parametrem „StorageAccountType”
     - Usunięto parametr „PublicNetworkAccessType”
     - Usunięto parametr „OutboundPublicNetworkAccessType”
     - Dodano nowe parametry: „StorageFileSystem” i „StorageAccountManagedIdentity” do obsługi usługi ADLSGen2
     - Dodano nowy parametr „EnableIDBroker” do obsługi brokera tożsamości usługi HDInsight
 ...
 * Dla polecenia cmdlet New-AzHDInsightClusterConfig:
     - Zastąpiono parametr „DefaultStorageAccountName” parametrem „StorageAccountResourceId”
     - Zastąpiono parametr „DefaultStorageAccountKey” parametrem „StorageAccountKey”
     - Zastąpiono parametr „DefaultStorageAccountType” parametrem „StorageAccountType”
     - Usunięto parametr „PublicNetworkAccessType”
     - Usunięto parametr „OutboundPublicNetworkAccessType”
* Dla polecenia cmdlet Set-AzHDInsightDefaultStorage:
    - Zastąpiono parametr „StorageAccountName” parametrem „StorageAccountResourceId”
* Dla polecenia cmdlet Add-AzHDInsightSecurityProfile:
    - Zastąpiono parametr „Domain” parametrem „DomainResourceId”
    - Usunięto wymaganie obowiązkowe dla parametru „OrganizationalUnitDN”

#### <a name="azkeyvault"></a>Az.KeyVault
* [Zmiana powodująca niezgodność] Przestarzały parametr DisableSoftDelete w poleceniach „New-AzKeyVault” i parametr EnableSoftDelete w poleceniu „Update-AzKeyVault”
* [Zmiana powodująca niezgodność] Usunięto atrybut SecretValueText, aby uniknąć bezpośredniego wyświetlania elementu SecretValue [#12266]
* Obsługiwany nowy typ zasobu: zarządzany moduł HSM
    - Operacja CRUD zarządzanego modułu HSM i polecenia cmdlet do obsługi kluczy w zarządzanym module HSM
    - Pełna kopia zapasowa/przywracanie modułu HSM, tworzenie klucza AES, kopia zapasowa/przywracanie domeny zabezpieczeń, RBAC

#### <a name="azmanagedservices"></a>Az.ManagedServices
* [Zmiana powodująca niezgodność] Zaktualizowano konwencje nazewnictwa parametrów i powiązane przykłady

#### <a name="aznetwork"></a>Az.Network
* [Zmiana powodująca niezgodność] Usunięto parametr „HostedSubnet” i dodano zamiast niego parametr „Subnet”
* Dodano nowe polecenia cmdlet dla tras równorzędnych routera wirtualnego
    - „Get-AzVirtualRouterPeerLearnedRoute”
    - „Get-AzVirtualRouterPeerAdvertisedRoute”
* Zaktualizowano polecenie cmdlet New-AzFirewall:
    - Dodano parametr „-SkuTier”
    - Dodano parametr „-SkuName” i dodano Sku jako alias tego parametru
    - Usunięto parametr „-Sku”.
* [Zmiana powodująca niezgodność] Argument „Connectionlink” jest obowiązkowy w poleceniach „Start-AzVpnConnectionPacketCapture” i „Stop-AzVpnConnectionPacketCapture”
* [Zmiana powodująca niezgodność] Zaktualizowano polecenie „New-AzNetworkWatcherConnectionMonitorEndPointObject” w celu usunięcia parametru „-Filter”
* [Zmiana powodująca niezgodność] Zastąpiono polecenie cmdlet „New-AzNetworkWatcherConnectionMonitorEndpointFilterItemObject” poleceniem „New-AzNetworkWatcherConnectionMonitorEndpointScopeItemObject”
* Zaktualizowano polecenie cmdlet „New-AzNetworkWatcherConnectionMonitorEndPointObject”:
    - Dodano parametr „-Type”
    - Dodano parametr „-CoverageLevel”
    - Dodano parametr „-Scope”
* Zaktualizowano polecenie cmdlet „New-AzNetworkWatcherConnectionMonitorProtocolConfigurationObject” przy użyciu nowego parametru „-DestinationPortBehavior”

#### <a name="azrecoveryservices"></a>Az.RecoveryServices
* Naprawianie przywracania obciążeń dla uprawnień współautora.
* Dodano nowe zestawy parametrów i walidacje dla polecenia cmdlet Restore-AzRecoveryServicesBackupItem.

#### <a name="azresources"></a>Az.Resources
* Naprawiono błąd analizy składni
* Zaktualizowano szablon ARM polecenia cmdlet w celu usunięcia podglądu wiadomości z wyników
* Rozwiązano problem polegający na tym, że polecenia cmdlet wdrażania szablonu uległy awarii, jeśli element „-WhatIf” był ustawiony w wyższym zakresie [#13038]
* Rozwiązano problem polegający na tym, że polecenia cmdlet wdrażania szablonu nie zachowują wielkości liter dla parametrów szablonu
* Dodano domyślną wersję interfejsu API do użycia w poleceniu cmdlet „Export-AzResourceGroup”
* Dodano polecenia cmdlet dla specyfikacji szablonu („Get-AzTemplateSpec”, „Set-AzTemplateSpec”, „New-AzTemplateSpec”, „Remove-AzTemplateSpec”, „Export-AzTemplateSpec”)
* Dodano obsługę wdrażania specyfikacji szablonu przy użyciu istniejących poleceń cmdlet wdrażania (za pośrednictwem parametru New-TemplateSpecId) 
* Zaktualizowano polecenie „Get-AzResourceGroupDeploymentOperation”, aby używać zestawu SDK.
...
#### <a name="azstorage"></a>Az.Storage
* Usunięto przestarzałą właściwość RestorePolicy.LastEnabledTime
    - „Enable-AzStorageBlobRestorePolicy”
    - „Disable-AzStorageBlobRestorePolicy”
    - „Get-AzStorageBlobServiceProperty”
    - „Update-AzStorageBlobServiceProperty”
* Zmieniono typ elementu DaysAfterModificationGreaterThan z int na int?
    - „Set-AzStorageAccountManagementPolicy”
    - „Get-AzStorageAccountManagementPolicy”
    - „Add-AzStorageAccountManagementPolicyAction”
    - „New-AzStorageAccountManagementPolicyRule”
* Obsługa tworzenia/aktualizowania udziału plików za pomocą warstwy dostępu
    - „New-AzRmStorageShare”
    - „Update-AzRmStorageShare”
* Obsługa ustawiania/aktualizowania/usuwania listy ACL rekursywnie w elemencie Datalake Gen2 
    -  „Set-AzDataLakeGen2AclRecursive” 
    -  „Update-AzDataLakeGen2AclRecursive” 
    -  „Remove-AzDataLakeGen2AclRecursive”
* Obsługa zasad dostępu kontenera z nowymi uprawnieniami x, t
    -  „Set-AzStorageContainerStoredAccessPolicy”
    -  „Set-AzStorageContainerStoredAccessPolicy”
* Zmieniono dane wyjściowe polecenia cmdlet get/set dla zasad dostępu do kontenera, zmieniając właściwość podrzędną Permission type z enum na String
    -  „Get-AzStorageContainerStoredAccessPolicy”
    -  „Set-AzStorageContainerStoredAccessPolicy”
* Rozwiązano problem z przykładowym skryptem dotyczącym konfiguracji zasad zarządzania przy użyciu formatu JSON
    -  „Set-AzStorageAccountManagementPolicy”

#### <a name="azwebsites"></a>Az.Websites
* Dodano obsługę warstwy cenowej Premium v3
* Zaktualizowano zestaw SDK WebSites do wersji 3.1.0

### <a name="thanks-to-our-community-contributors"></a>Dziękujemy współautorom z naszej społeczności
* @atul-ram, aktualizacja pliku Get-AzDelegation.md (#13176)
* @dineshreddy007, uzyskanie prawidłowego przypisania ról aplikacji w przypadku rejestracji w usłudze Stack HCI przy użyciu tokenu WAC. (#13249)
* @kongou-ae, aktualizacja pliku New-AzOffice365PolicyProperty.md (#13217)
* Lohith Chowdary Chilukuri (@Lochiluk), aktualizacja pliku Set-AzApplicationGateway.md (#13150)
* Matthew Burleigh (@mburleigh)
  * Dodanie linków do poleceń cmdlet programu PowerShell, do których odwołuje się dokument (#13203)
  * Dodanie linków do poleceń cmdlet programu PowerShell, do których odwołuje się dokument (#13190)
  * Dodanie linków do poleceń cmdlet programu PowerShell, do których odwołuje się dokument (#13189)
  * Dodanie linków do poleceń cmdlet, do których istnieją odwołania (#13137)
  * Dodanie linków do poleceń cmdlet programu PowerShell, do których odwołuje się dokument (#13204)
  * Dodanie linków do poleceń cmdlet programu PowerShell, do których odwołuje się dokument (#13205)

## <a name="480---october-2020"></a>4.8.0 — październik 2020 r.
#### <a name="azaccounts"></a>Az.Accounts
* Rozwiązano problem z analizą DateTime we wspólnych bibliotekach [#13045]

#### <a name="azcognitiveservices"></a>Az.CognitiveServices
* Dodano polecenie cmdlet „New-AzCognitiveServicesAccountApiProperty”.
* Dodano obsługę parametru „ApiProperty” dla poleceń „New-AzCognitiveServicesAccount” i „Set-AzCognitiveServicesAccount”

#### <a name="azcompute"></a>Az.Compute
* Rozwiązano problem w elemencie „Update-ASRRecoveryPlan” przez wypełnienie elementu FailoverTypes
* Dodano opcjonalne parametry „-Top” i „-OrderBy” do polecenia cmdlet „Get-AzVmImage”. 

#### <a name="azdatabricks"></a>Az.Databricks
* Ogólna dostępność modułu „Az.Databricks”
* Dodano obsługę komunikacji równorzędnej sieci wirtualnych

#### <a name="azdatafactory"></a>Az.DataFactory
* Poprawiono literówki w komunikatach wyjściowych

#### <a name="azeventhub"></a>Az.EventHub
* Dodano opcjonalny parametr przełącznika „TrustedServiceAccessEnabled” do polecenia cmdlet „Set-AzEventHubNetworkRuleSet”

#### <a name="azhdinsight"></a>Az.HDInsight
* Dodano komunikat ostrzegawczy informujący o planowanym wycofaniu parametrów „PublicNetworkAccessType” i „OutboundPublicNetworkAccessType”
* Dodano komunikat ostrzegawczy z informacją o planowanym zastąpieniu parametru „DefaultStorageAccountName” parametrem „StorageAccountResourceId”
* Dodano komunikat ostrzegawczy z informacją o planowanym zastąpieniu parametru „DefaultStorageAccountKey” parametrem „StorageAccountKey”
* Dodano komunikat ostrzegawczy z informacją o planowanym zastąpieniu parametru „DefaultStorageAccountType” parametrem „StorageAccountType”
* Dodano komunikat ostrzegawczy z informacją o planowanym zastąpieniu parametru „DefaultStorageContainer” parametrem „StorageContainer”
* Dodano komunikat ostrzegawczy z informacją o planowanym zastąpieniu parametru „DefaultStorageRootPath” parametrem „StorageRootPath”

#### <a name="aziothub"></a>Az.IotHub
* Zaktualizowano zestaw SDK urządzeń.

#### <a name="azkeyvault"></a>Az.KeyVault
* Podano dokładną datę usunięcia właściwości SecretValueText

#### <a name="azmanagedservices"></a>Az.ManagedServices
* Zaktualizowano ostrzeżenia o zmianach powodujących niezgodności w poleceniach cmdlet przypisywania i definiowania usług zarządzanych

#### <a name="azmonitor"></a>Az.Monitor
* Rozwiązano problem polegający na niemożności pominięcia komunikatu ostrzegawczego. [#12889]
* Dodano obsługę parametru „SkipMetricValidation” w kryteriach reguły alertu. Umożliwia on utworzenie reguły alertu na podstawie niestandardowej metryki, która nie została jeszcze wyemitowana, co powoduje pominięcie sprawdzania poprawności metryki.

#### <a name="aznetwork"></a>Az.Network
* Dodano zasady usługi Office 365 dotyczące zasobu VPNSite
    - „New-AzO365PolicyProperty”

#### <a name="azrecoveryservices"></a>Az.RecoveryServices
* Dodano weryfikację nazwy kontenera dla kopii zapasowej obciążenia.

#### <a name="azrediscache"></a>Az.RedisCache
* Naprawiono błąd polegający na niepowodzeniu wykonania poleceń cmdlet „New-AzRedisCache” i „Set-AzRedisCache” z powodu problemu z uprawnieniami rejestrowania zasobu Microsoft.Cache RP

#### <a name="azsql"></a>Az.Sql
* Dodano parametr BackupStorageRedundancy do następujących poleceń: 
    - „Restore-AzureRmSqlDatabase”
    - „New-AzSqlDatabaseCopy”
    - „New-AzSqlDatabaseSecondary”
* Wyłączono uwzględnianie wielkości liter dla parametru BackupStorageRedundancy dla wszystkich odwołań bazy danych SQL 
* Zaktualizowane nazwy komunikatów ostrzegawczych parametru BackupStorageRedundancy

#### <a name="azstorage"></a>Az.Storage
* Dodano obsługę włączania/wyłączania/pobierania właściwości usuwania nietrwałego udziału w usłudze plików konta magazynu
    - „Update-AzStorageFileServiceProperty”
    - „Get-AzStorageFileServiceProperty”
* Lista obsługiwanych udziałów plików obejmuje usunięte udziały z konta magazynu i pobieranie użycia pojedynczego udziału plików
    - „Get-AzRmStorageShare”
* Dodano obsługę przywracania usuniętego udziału plików
    - „Restore-AzRmStorageShare”
* Zmieniono polecenia cmdlet umożliwiające modyfikację właściwości usługi Blob Service. Nie będą pobierane oryginalne właściwości z serwera, tylko ustawiane zmodyfikowane właściwości na serwerze.
    - „Enable-AzStorageBlobDeleteRetentionPolicy”
    - „Disable-AzStorageBlobDeleteRetentionPolicy”  
    - „Enable-AzStorageBlobRestorePolicy”
    - „Disable-AzStorageBlobRestorePolicy”
    - „Update-AzStorageBlobServiceProperty”
* Rozwiązano problem z pomocą dotyczącą wartości domyślnej parametru -Kind polecenia New-AzStorageAccount [#12189]
* Rozwiązano problem przez dodanie przykładu pokazującego, jak ustawić poprawną wartość ContentType dla przekazywanego obiektu blob [#12989]
...
#### <a name="azaks"></a>Az.Aks
* Zamieniono użycie starego [interfejsu API AccessProfile](/rest/api/aks/managedclusters/getaccessprofile) na wywołania interfejsów API [ListClusterAdmin](/rest/api/aks/managedclusters/listclusteradmincredentials) i [ListClusterUser](/rest/api/aks/managedclusters/listclusterusercredentials)
