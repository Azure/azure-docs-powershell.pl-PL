...
## Opis
Polecenie cmdlet **Get-AzStorageBlobQueryResult** stosuje prostą instrukcję SQL (Structured Query Language) do zawartości obiektu BLOB i zapisuje poszukiwany podzestaw danych w pliku lokalnym.

## Przykłady

### Przykład 1: kwerenda obiektu BLOB
```powershell
...
To polecenie bada działanie obiektu BLOB succsssfully, korzystając z konfiguracji wejściowej jako pliku CSV, oraz konfiguracji wyjściowej w formacie JSON i zapisuje dane wyjściowe w pliku lokalnym "c:\resultfile.json".

### Przykład 2: zapytanie do migawki obiektu BLOB
```powershell
...
To polecenie po raz pierwszy uzyskuje obiekt BLOB na potrzeby migawki obiektu BLOB, a następnie wysyła zapytanie do migawki obiektu BLOB i pokaże wynik błąd zapytania.
...
### -Force
Wymuś zastąpienie istniejącego pliku.
...
Konfiguracja używana do obsługi tekstu wejściowego zapytania. Tworzenie obiektu konfiguracji z nowymi — AzStorageBlobQueryConfig.
...
Konfiguracja używana do obsługi tekstu wyjściowego kwerendy. Tworzenie obiektu konfiguracji z nowymi — AzStorageBlobQueryConfig.
...
### -PassThru
Zwraca, czy pomyślnie zbadano zapytanie określonego obiektu BLOB.
...
### -QueryString
Ciąg zapytania, Zobacz więcej szczegółów w: https://docs.microsoft.com/en-us/azure/storage/blobs/query-acceleration-sql-reference
...
### -ResultFile
Lokalna ścieżka pliku służąca do zapisania wyniku zapytania.
...
### Azure. Storage. blob. specjalizowane. BlobBaseClient
