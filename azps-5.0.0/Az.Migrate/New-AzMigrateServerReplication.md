...
## STRESZCZENIe
Rozpoczyna replikację dla określonego serwera.

## POLECENIA

### ByIdDefaultUser (domyślny)
```
...
Dotyczy to sytuacji, gdy istnieje tylko jeden dysk, który musi być chroniony.

### Przykład 2: Jeśli istnieje wiele dysków
```powershell
...
Dotyczy to sytuacji, gdy istnieje wiele dysków, które trzeba chronić.
...
### -DiskToInclude
Określa dyski serwera źródłowego, które mają zostać uwzględnione w replikacji.
Aby skonstruować, zobacz sekcję notatki dla właściwości DISKTOINCLUDE i Utwórz tablicę skrótów.
...
### -Inputobject
Określa odnaleziony serwer, który ma zostać zmigrowany.
Obiekt serwera można pobrać przy użyciu polecenia cmdlet Get-AzMigrateServer.
...
### -LicenseType
Określa, czy korzystanie z usługi Azure hybrydowe jest właściwe dla migracji serwera źródłowego.
...
### -MachineId
Określa identyfikator komputera wykrytego serwera, który ma zostać zmigrowany.
...
### -PerformAutoResync
Określa, czy replikacja jest automatycznie naprawiana w przypadku utraty śledzenia zmian w przypadku serwera źródłowego w obszarze replikacja.
...
Określa zestaw dostępności, który ma być stosowany dla maszyny wirtualnej creationSpecifies zestawu dostępności, który ma być wykorzystywany do tworzenia maszyny wirtualnej.
...
### -TargetNetworkId
Określa identyfikator sieci wirtualnej w ramach docelowej subskrypcji platformy Azure, do której należy przeprowadzić migrację serwera.
...
Określa identyfikator grupy zasobów w ramach docelowej subskrypcji platformy Azure, do której należy przeprowadzić migrację serwera.
...
### -TargetSubnetName
Określa nazwę podsieci w docelowym wirtualnym Netowk, do której należy przeprowadzić migrację serwera.
...
### -TargetVMName
Określa nazwę maszyny wirtualnej Azure, która ma zostać utworzona.
...
### -TargetVMSize
Określa jednostkę SKU maszyny wirtualnej Azure, którą należy utworzyć.
...
DISKTOINCLUDE <IVMwareCbtDiskInput [] >: określa dyski serwera źródłowego, które mają zostać uwzględnione w replikacji.
  - `DiskId <String>`: Identyfikator dysku.
  - `IsOSDisk <String>`: Wartość wskazująca, czy dysk jest dyskiem systemu operacyjnego.
  - `LogStorageAccountId <String>`: Identyfikator ARM konta dziennika.
  - `LogStorageAccountSasSecretName <String>`: Poufna nazwa konta dziennika w magazynie kluczy.
  - `[DiskEncryptionSetId <String>]`: Identyfikator ARM DiskEncryptionSet.
  - `[DiskType <DiskAccountType?>]`: Typ dysku.

INPUTobject <IVMwareMachine> : określa odnaleziony serwer, który ma zostać zmigrowany. Obiekt serwera można pobrać przy użyciu polecenia cmdlet Get-AzMigrateServer.
