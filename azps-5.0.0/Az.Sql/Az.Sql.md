---
Module Name: Az.Sql
Module Guid: 150d9544-6348-4373-806f-10cd0b4de4cb
Download Help Link: https://docs.microsoft.com/en-us/powershell/module/az.sql
Help Version: 4.4.0.0
Locale: en-US
content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/Sql/Sql/help/Az.Sql.md
original_content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/Sql/Sql/help/Az.Sql.md
ms.openlocfilehash: aed79203910a89ddbf52b4a25bc2f8dbfe93ac2c
ms.sourcegitcommit: b4a38bcb0501a9016a4998efd377aa75d3ef9ce8
ms.translationtype: MT
ms.contentlocale: pl-PL
ms.lasthandoff: 10/27/2020
ms.locfileid: "94231489"
---
# AZ. SQL module
## Opis
W tym temacie są wyświetlane tematy pomocy dotyczące poleceń cmdlet bazy danych usługi Azure SQL.

## AZ. polecenia cmdlet SQL
### [Dodaj-AzSqlDatabaseToFailoverGroup](Add-AzSqlDatabaseToFailoverGroup.md)
Dodaje co najmniej jeden bazę danych do grupy trybu failover bazy danych SQL Azure.

### [Dodaj-AzSqlElasticJobStep](Add-AzSqlElasticJobStep.md)
Dodanie kroku zadania do zadania

### [Dodaj-AzSqlElasticJobTarget](Add-AzSqlElasticJobTarget.md)
Dodawanie elementu docelowego do grupy docelowej

### [Dodaj-AzSqlInstanceKeyVaultKey](Add-AzSqlInstanceKeyVaultKey.md)
Dodaje klucz magazynu kluczy do udostępnionego wystąpienia zarządzanego. 

### [Dodaj-AzSqlManagedInstanceTransparentDataEncryptionCertificate](Add-AzSqlManagedInstanceTransparentDataEncryptionCertificate.md)
Dodaje przezroczysty certyfikat szyfrowania danych dla danego wystąpienia zarządzanego.

### [Dodaj-AzSqlServerKeyVaultKey](Add-AzSqlServerKeyVaultKey.md)
Dodaje klucz magazynu kluczy do programu SQL Server.

### [Dodaj-AzSqlServerTransparentDataEncryptionCertificate](Add-AzSqlServerTransparentDataEncryptionCertificate.md)
Dodaje przezroczysty certyfikat szyfrowania danych dla danego wystąpienia programu SQL Server.

### [Clear-AzSqlDatabaseAdvancedThreatProtectionSetting](Clear-AzSqlDatabaseAdvancedThreatProtectionSetting.md)
Usuwa z bazy danych Zaawansowane ustawienia ochrony przed zagrożeniami.

### [Clear-AzSqlDatabaseVulnerabilityAssessmentRuleBaseline](Clear-AzSqlDatabaseVulnerabilityAssessmentRuleBaseline.md)
Czyści plan bazowy reguły oceny usterki.

### [Clear-AzSqlDatabaseVulnerabilityAssessmentSetting](Clear-AzSqlDatabaseVulnerabilityAssessmentSetting.md)
Czyści ustawienia oceny usterki bazy danych.

### [Clear-AzSqlInstanceDatabaseVulnerabilityAssessmentRuleBaseline](Clear-AzSqlInstanceDatabaseVulnerabilityAssessmentRuleBaseline.md)
Czyści plan bazowy reguły oceny usterki.

### [Clear-AzSqlInstanceDatabaseVulnerabilityAssessmentSetting](Clear-AzSqlInstanceDatabaseVulnerabilityAssessmentSetting.md)
Czyści ustawienia oceny usterki w zarządzanej bazie danych.

### [Clear-AzSqlInstanceVulnerabilityAssessmentSetting](Clear-AzSqlInstanceVulnerabilityAssessmentSetting.md)
Czyści ustawienia oceny usterki w zarządzanym wystąpieniu.

### [Clear-AzSqlServerAdvancedThreatProtectionSetting](Clear-AzSqlServerAdvancedThreatProtectionSetting.md)
Usuwa zaawansowane ustawienia ochrony przed zagrożeniami z serwera.

### [Clear-AzSqlServerVulnerabilityAssessmentSetting](Clear-AzSqlServerVulnerabilityAssessmentSetting.md)
Czyści ustawienia oceny usterki serwera.

### [Ukończone — AzSqlInstanceDatabaseLogReplay](Complete-AzSqlInstanceDatabaseLogReplay.md)
Kończy usługę powtarzania dziennika dla danej bazy danych.

### [Convert-AzSqlDatabaseVulnerabilityAssessmentScan](Convert-AzSqlDatabaseVulnerabilityAssessmentScan.md)
Konwertuje wyniki skanowania oceny dotyczącego usterki na format programu Excel.

### [Convert-AzSqlInstanceDatabaseVulnerabilityAssessmentScan](Convert-AzSqlInstanceDatabaseVulnerabilityAssessmentScan.md)
Konwertuje wyniki skanowania oceny dotyczącego usterki na format programu Excel.

### [Disable-AzSqlDatabaseSensitivityRecommendation](Disable-AzSqlDatabaseSensitivityRecommendation.md)
Wyłączenie (odrzucanie) rekomendacji dotyczących wrażliwości na kolumny w bazie danych.

### [Disable-AzSqlInstanceActiveDirectoryOnlyAuthentication](Disable-AzSqlInstanceActiveDirectoryOnlyAuthentication.md)
Wyłącza uwierzytelnianie tylko platformy Azure AD dla określonego wystąpienia zarządzanego SQL.

### [Disable-AzSqlInstanceAdvancedDataSecurity](Disable-AzSqlInstanceAdvancedDataSecurity.md)
Wyłącza zaawansowane zabezpieczenia danych w zarządzanym wystąpieniu.

### [Disable-AzSqlInstanceDatabaseSensitivityRecommendation](Disable-AzSqlInstanceDatabaseSensitivityRecommendation.md)
Wyłączenie (odrzucanie) rekomendacji dotyczących wrażliwości na kolumny w bazie danych wystąpienia zarządzanego SQL Azure.

### [Disable-AzSqlServerActiveDirectoryOnlyAuthentication](Disable-AzSqlServerActiveDirectoryOnlyAuthentication.md)
Wyłącza uwierzytelnianie tylko platformy Azure AD dla określonego serwera SQL.

### [Disable-AzSqlServerAdvancedDataSecurity](Disable-AzSqlServerAdvancedDataSecurity.md)
Wyłącza zaawansowane zabezpieczenia danych na serwerze.

### [Disable-AzSqlServerAdvancedThreatProtection](Disable-AzSqlServerAdvancedThreatProtection.md)
Wyłącza zaawansowaną ochronę przed zagrożeniami na serwerze.

### [Enable-AzSqlDatabaseSensitivityRecommendation](Enable-AzSqlDatabaseSensitivityRecommendation.md)
Włącza rekomendacje dotyczące wrażliwości na kolumny (rekomendacje są domyślnie włączone we wszystkich kolumnach) w bazie danych.

### [Enable-AzSqlInstanceActiveDirectoryOnlyAuthentication](Enable-AzSqlInstanceActiveDirectoryOnlyAuthentication.md)
Umożliwia uwierzytelnianie tylko platformy Azure AD dla określonego wystąpienia zarządzanego SQL.

### [Enable-AzSqlInstanceAdvancedDataSecurity](Enable-AzSqlInstanceAdvancedDataSecurity.md)
Włącza zaawansowane zabezpieczenia danych w zarządzanym wystąpieniu.

### [Enable-AzSqlInstanceDatabaseSensitivityRecommendation](Enable-AzSqlInstanceDatabaseSensitivityRecommendation.md)
Włączanie rekomendacji dotyczących wrażliwości na kolumny (rekomendacje są domyślnie włączone we wszystkich kolumnach) w bazie danych wystąpienia zarządzanego SQL Azure.

### [Enable-AzSqlServerActiveDirectoryOnlyAuthentication](Enable-AzSqlServerActiveDirectoryOnlyAuthentication.md)
Umożliwia uwierzytelnianie tylko na platformie Azure AD dla określonego programu SQL Server.

### [Enable-AzSqlServerAdvancedDataSecurity](Enable-AzSqlServerAdvancedDataSecurity.md)
Włączanie zaawansowanych zabezpieczeń danych na serwerze.

### [Enable-AzSqlServerAdvancedThreatProtection](Enable-AzSqlServerAdvancedThreatProtection.md)
Włączanie zaawansowanej ochrony przed zagrożeniami na serwerze.

### [Get-AzSqlCapability](Get-AzSqlCapability.md)
Pobiera funkcje bazy danych SQL dla bieżącej subskrypcji.

### [Get-AzSqlDatabase](Get-AzSqlDatabase.md)
Pobiera co najmniej jeden z baz danych.

### [Get-AzSqlDatabaseActivity](Get-AzSqlDatabaseActivity.md)
Pobiera stan operacji bazy danych.

### [Get-AzSqlDatabaseAdvancedThreatProtectionSetting](Get-AzSqlDatabaseAdvancedThreatProtectionSetting.md)
Pobiera zaawansowane ustawienia ochrony przed zagrożeniami dla bazy danych.

### [Get-AzSqlDatabaseAdvisor](Get-AzSqlDatabaseAdvisor.md)
Pobiera co najmniej jeden klasyfikator dla bazy danych SQL Azure.

### [Get-AzSqlDatabaseAudit](Get-AzSqlDatabaseAudit.md)
Pobiera ustawienia inspekcji bazy danych SQL Azure.

### [Get-AzSqlDatabaseBackupLongTermRetentionPolicy](Get-AzSqlDatabaseBackupLongTermRetentionPolicy.md)
Pobiera zasady przechowywania długoterminowych baz danych.

### [Get-AzSqlDatabaseBackupShortTermRetentionPolicy](Get-AzSqlDatabaseBackupShortTermRetentionPolicy.md)
Pobiera zasady przechowywania krótkoterminowych kopii zapasowych.

### [Get-AzSqlDatabaseDataMaskingPolicy](Get-AzSqlDatabaseDataMaskingPolicy.md)
Pobiera zasady maskowania danych dla bazy danych.

### [Get-AzSqlDatabaseDataMaskingRule](Get-AzSqlDatabaseDataMaskingRule.md)
Pobiera reguły maskowania danych z bazy danych.

### [Get-AzSqlDatabaseExpanded](Get-AzSqlDatabaseExpanded.md)
Pobiera bazę danych i jej rozwinięte wartości właściwości.

### [Get-AzSqlDatabaseFailoverGroup](Get-AzSqlDatabaseFailoverGroup.md)
Pobiera lub wyświetla listę grup pracy awaryjnej bazy danych Azure SQL Database.

### [Get-AzSqlDatabaseGeoBackup](Get-AzSqlDatabaseGeoBackup.md)
Pobiera kopię zapasową bazy danych w postaci geograficznej.

### [Get-AzSqlDatabaseGeoBackupPolicy](Get-AzSqlDatabaseGeoBackupPolicy.md)
Pobiera zasady geograficznej kopii zapasowej bazy danych.

### [Get-AzSqlDatabaseImportExportStatus](Get-AzSqlDatabaseImportExportStatus.md)
Pobiera szczegóły importu lub eksportu bazy danych SQL Azure.

### [Get-AzSqlDatabaseIndexRecommendation](Get-AzSqlDatabaseIndexRecommendation.md)
Pobiera zalecane operacje indeksowania dla serwera lub bazy danych.

### [Get-AzSqlDatabaseInstanceFailoverGroup](Get-AzSqlDatabaseInstanceFailoverGroup.md)
Pobiera lub wyświetla grupy trybu failover wystąpienia.

### [Get-AzSqlDatabaseLongTermRetentionBackup](Get-AzSqlDatabaseLongTermRetentionBackup.md)
Pobiera co najmniej jedno długoterminowe kopie zapasowe przechowywania.

### [Get-AzSqlDatabaseRecommendedAction](Get-AzSqlDatabaseRecommendedAction.md)
Umożliwia wykonanie jednej lub większej liczby zalecanych akcji dla klasyfikatora bazy danych Azure SQL Database.

### [Get-AzSqlDatabaseReplicationLink](Get-AzSqlDatabaseReplicationLink.md)
Pobiera linki replikacji geograficznej między bazą danych SQL Azure a grupą zasobów lub programem SQL Server.

### [Get-AzSqlDatabaseRestorePoint](Get-AzSqlDatabaseRestorePoint.md)
Pobiera różne punkty przywracania, z których można przywrócić magazyn danych SQL.

### [Get-AzSqlDatabaseSensitivityClassification](Get-AzSqlDatabaseSensitivityClassification.md)
Pobiera bieżące typy informacji oraz etykiety liter kolumn w bazie danych.

### [Get-AzSqlDatabaseSensitivityRecommendation](Get-AzSqlDatabaseSensitivityRecommendation.md)
Pobiera Polecane typy informacji oraz etykiety wrażliwości kolumn w bazie danych.

### [Get-AzSqlDatabaseTransparentDataEncryption](Get-AzSqlDatabaseTransparentDataEncryption.md)
Pobiera stan TDE bazy danych.

### [Get-AzSqlDatabaseTransparentDataEncryptionActivity](Get-AzSqlDatabaseTransparentDataEncryptionActivity.md)
Pobiera postęp TDEgo skanowania bazy danych.

### [Get-AzSqlDatabaseUpgradeHint](Get-AzSqlDatabaseUpgradeHint.md)
Pobiera wskazówki dotyczące warstwy cenowej dla bazy danych.

### [Get-AzSqlDatabaseVulnerabilityAssessmentRuleBaseline](Get-AzSqlDatabaseVulnerabilityAssessmentRuleBaseline.md)
Pobiera plan bazowy reguły oceny luki.

### [Get-AzSqlDatabaseVulnerabilityAssessmentScanRecord](Get-AzSqlDatabaseVulnerabilityAssessmentScanRecord.md)
Pobiera wszystkie rekordy, których dotyczy próba oceny, skojarzone z daną bazą danych.

### [Get-AzSqlDatabaseVulnerabilityAssessmentSetting](Get-AzSqlDatabaseVulnerabilityAssessmentSetting.md)
Pobiera ustawienia oceny usterki bazy danych.

### [Get-AzSqlDeletedDatabaseBackup](Get-AzSqlDeletedDatabaseBackup.md)
Pobieranie usuniętej bazy danych, którą można przywrócić.

### [Get-AzSqlDeletedInstanceDatabaseBackup](Get-AzSqlDeletedInstanceDatabaseBackup.md)
Pobieranie usuniętej bazy danych, którą można przywrócić.

### [Get-AzSqlElasticJob](Get-AzSqlElasticJob.md)
Pobiera jeden lub więcej zadań

### [Get-AzSqlElasticJobAgent](Get-AzSqlElasticJobAgent.md)
Pobiera elastycznego agenta zadań SQL Azure

### [Get-AzSqlElasticJobCredential](Get-AzSqlElasticJobCredential.md)
Pobiera jedno lub więcej poświadczeń

### [Get-AzSqlElasticJobExecution](Get-AzSqlElasticJobExecution.md)
Pobiera co najmniej jedno wykonanie zadania

### [Get-AzSqlElasticJobStep](Get-AzSqlElasticJobStep.md)
Pobiera co najmniej jeden krok zadania

### [Get-AzSqlElasticJobStepExecution](Get-AzSqlElasticJobStepExecution.md)
Umożliwia wykonanie co najmniej jednego wykonania kroku zadania

### [Get-AzSqlElasticJobTargetExecution](Get-AzSqlElasticJobTargetExecution.md)
Pobiera co najmniej jedno wykonanie zadania w celu

### [Get-AzSqlElasticJobTargetGroup](Get-AzSqlElasticJobTargetGroup.md)
Pobieranie jednej lub większej liczby grup docelowych zadań

### [Get-AzSqlElasticPool](Get-AzSqlElasticPool.md)
Pobiera pule elastyczne i wartości ich właściwości w bazie danych SQL Azure.

### [Get-AzSqlElasticPoolActivity](Get-AzSqlElasticPoolActivity.md)
Pobiera stan operacji z puli elastycznej.

### [Get-AzSqlElasticPoolAdvisor](Get-AzSqlElasticPoolAdvisor.md)
Pobiera jeden lub więcej doradców dla puli elastycznej SQL Azure.

### [Get-AzSqlElasticPoolDatabase](Get-AzSqlElasticPoolDatabase.md)
Pobiera elastyczną bazę danych w puli elastycznej i jej wartości właściwości.

### [Get-AzSqlElasticPoolRecommendation](Get-AzSqlElasticPoolRecommendation.md)
Pobiera elastyczne rekomendacje.

### [Get-AzSqlElasticPoolRecommendedAction](Get-AzSqlElasticPoolRecommendedAction.md)
Umożliwia wykonanie jednej lub większej liczby zalecanych akcji dla klasyfikatora Elastic Pool SQL platformy Azure.

### [Get-AzSqlInstance](Get-AzSqlInstance.md)
Zwraca informacje o wystąpieniu bazy danych zarządzanej przez usługę Azure SQL.

### [Get-AzSqlInstanceActiveDirectoryAdministrator](Get-AzSqlInstanceActiveDirectoryAdministrator.md)
Pobiera informacje o administratorze usługi Azure AD dla wystąpienia zarządzanego SQL.

### [Get-AzSqlInstanceActiveDirectoryOnlyAuthentication](Get-AzSqlInstanceActiveDirectoryOnlyAuthentication.md)
Pobiera uwierzytelnianie tylko platformy Azure AD dla określonego wystąpienia zarządzanego SQL.

### [Get-AzSqlInstanceAdvancedDataSecurityPolicy](Get-AzSqlInstanceAdvancedDataSecurityPolicy.md)
Pobiera zaawansowane zasady zabezpieczeń danych wystąpienia zarządzanego.

### [Get-AzSqlInstanceDatabase](Get-AzSqlInstanceDatabase.md)
Zwraca informacje o bazie danych wystąpienia zarządzanego SQL Azure.

### [Get-AzSqlInstanceDatabaseBackupLongTermRetentionPolicy](Get-AzSqlInstanceDatabaseBackupLongTermRetentionPolicy.md)
Pobiera zasady przechowywania długoterminowych zarządzanych baz danych

### [Get-AzSqlInstanceDatabaseBackupShortTermRetentionPolicy](Get-AzSqlInstanceDatabaseBackupShortTermRetentionPolicy.md)
Pobiera zasady przechowywania krótkoterminowych kopii zapasowych.

### [Get-AzSqlInstanceDatabaseGeoBackup](Get-AzSqlInstanceDatabaseGeoBackup.md)
Zwraca informacje o bazie danych wystąpienia zarządzanego programu Azure SQL Database.

### [Get-AzSqlInstanceDatabaseLogReplay](Get-AzSqlInstanceDatabaseLogReplay.md)
Pobiera stan usługi powtarzania dziennika.

### [Get-AzSqlInstanceDatabaseLongTermRetentionBackup](Get-AzSqlInstanceDatabaseLongTermRetentionBackup.md)
Wykonywanie kopii zapasowych w długim czasie przechowywania.

### [Get-AzSqlInstanceDatabaseSensitivityClassification](Get-AzSqlInstanceDatabaseSensitivityClassification.md)
Pobiera bieżące typy informacji oraz etykiety liter kolumn w bazie danych wystąpienia zarządzanego SQL Azure.

### [Get-AzSqlInstanceDatabaseSensitivityRecommendation](Get-AzSqlInstanceDatabaseSensitivityRecommendation.md)
Pobiera Polecane typy informacji i etykiety wrażliwości kolumn w bazie danych wystąpienia zarządzanego SQL Azure.

### [Get-AzSqlInstanceDatabaseVulnerabilityAssessmentRuleBaseline](Get-AzSqlInstanceDatabaseVulnerabilityAssessmentRuleBaseline.md)
Pobiera plan bazowy reguły oceny luki.

### [Get-AzSqlInstanceDatabaseVulnerabilityAssessmentScanRecord](Get-AzSqlInstanceDatabaseVulnerabilityAssessmentScanRecord.md)
Pobiera wszystkie rekordy skanowania oceny, które zostały skojarzone z daną zarządzaną bazą danych.

### [Get-AzSqlInstanceDatabaseVulnerabilityAssessmentSetting](Get-AzSqlInstanceDatabaseVulnerabilityAssessmentSetting.md)
Pobiera ustawienia oceny dotyczącego usterki dotyczącej zarządzanej bazy danych.

### [Get-AzSqlInstanceKeyVaultKey](Get-AzSqlInstanceKeyVaultKey.md)
Pobiera klucze magazynu kluczy wystąpienia zarządzanego przez SQL.

### [Get-AzSqlInstanceOperation](Get-AzSqlInstanceOperation.md)
Pobiera operacje wystąpienia zarządzanego przez SQL.

### [Get-AzSqlInstancePool](Get-AzSqlInstancePool.md)
Zwraca informacje o puli wystąpień usługi Azure SQL.

### [Get-AzSqlInstancePoolUsage](Get-AzSqlInstancePoolUsage.md)
Zwraca informacje o użyciu puli wystąpień usługi Azure SQL.

### [Get-AzSqlInstanceTransparentDataEncryptionProtector](Get-AzSqlInstanceTransparentDataEncryptionProtector.md)
Pobiera funkcję ochrony przezroczystego szyfrowania danych (TDE) dla wystąpienia zarządzanego SQL.

### [Get-AzSqlInstanceVulnerabilityAssessmentSetting](Get-AzSqlInstanceVulnerabilityAssessmentSetting.md)
Pobiera ustawienia oceny dotyczącego luki w zabezpieczeniach wystąpienia zarządzanego.

### [Get-AzSqlServer](Get-AzSqlServer.md)
Zwraca informacje o serwerach baz danych SQL.

### [Get-AzSqlServerActiveDirectoryAdministrator](Get-AzSqlServerActiveDirectoryAdministrator.md)
Pobiera informacje o administratorze usługi Azure AD dla programu SQL Server.

### [Get-AzSqlServerActiveDirectoryOnlyAuthentication](Get-AzSqlServerActiveDirectoryOnlyAuthentication.md)
Pobiera uwierzytelnianie tylko usługi Azure AD dla określonego programu SQL Server.

### [Get-AzSqlServerAdvancedDataSecurityPolicy](Get-AzSqlServerAdvancedDataSecurityPolicy.md)
Pobiera zaawansowane zasady zabezpieczeń danych serwera.

### [Get-AzSqlServerAdvancedThreatProtectionSetting](Get-AzSqlServerAdvancedThreatProtectionSetting.md)
Pobiera zaawansowane ustawienia ochrony przed zagrożeniami dla serwera.

### [Get-AzSqlServerAdvisor](Get-AzSqlServerAdvisor.md)
Pobiera jeden lub więcej doradców dla programu Azure SQL Server.

### [Get-AzSqlServerAudit](Get-AzSqlServerAudit.md)
Pobiera ustawienia inspekcji programu Azure SQL Server.

### [Get-AzSqlServerCommunicationLink](Get-AzSqlServerCommunicationLink.md)
Pobiera linki komunikacyjne dotyczące transakcji Elastic Database między serwerami baz danych.

### [Get-AzSqlServerDisasterRecoveryConfiguration](Get-AzSqlServerDisasterRecoveryConfiguration.md)
Pobiera konfigurację odzyskiwania systemu serwera bazy danych.

### [Get-AzSqlServerDisasterRecoveryConfigurationActivity](Get-AzSqlServerDisasterRecoveryConfigurationActivity.md)
Pobiera aktywność dotyczącą konfiguracji odzyskiwania systemu serwera bazy danych.

### [Get-AzSqlServerDnsAlias](Get-AzSqlServerDnsAlias.md)
Pobiera lub wyświetla alias DNS programu Azure SQL Server.

### [Get-AzSqlServerFirewallRule](Get-AzSqlServerFirewallRule.md)
Pobiera reguły zapory dla serwera bazy danych SQL.

### [Get-AzSqlServerKeyVaultKey](Get-AzSqlServerKeyVaultKey.md)
Pobiera klucze magazynu kluczy programu SQL Server.

### [Get-AzSqlServerRecommendedAction](Get-AzSqlServerRecommendedAction.md)
Umożliwia wykonanie jednej lub kilku zalecanych akcji dla usługi Azure SQL Server Advisor.

### [Get-AzSqlServerServiceObjective](Get-AzSqlServerServiceObjective.md)
Umożliwia pobieranie celów usługi dla serwera bazy danych Azure SQL.

### [Get-AzSqlServerTransparentDataEncryptionProtector](Get-AzSqlServerTransparentDataEncryptionProtector.md)
Pobiera funkcję ochrony przezroczystego szyfrowania danych (TDE)

### [Get-AzSqlServerUpgradeHint](Get-AzSqlServerUpgradeHint.md)
Pobiera wskazówki dotyczące warstwy cenowej w celu uaktualnienia serwera bazy danych SQL Azure.

### [Get-AzSqlServerVirtualNetworkRule](Get-AzSqlServerVirtualNetworkRule.md)
Pobiera lub wyświetla regułę wirtualnej sieci programu Azure SQL Server.

### [Get-AzSqlServerVulnerabilityAssessmentSetting](Get-AzSqlServerVulnerabilityAssessmentSetting.md)
Pobiera ustawienia oceny usterki serwera.

### [Get-AzSqlSyncAgent](Get-AzSqlSyncAgent.md)
Zwraca informacje dotyczące agentów synchronizacji Azure SQL.

### [Get-AzSqlSyncAgentLinkedDatabase](Get-AzSqlSyncAgentLinkedDatabase.md)
Zwraca informacje o bazach danych programu SQL Server połączonych z agentem synchronizacji.

### [Get-AzSqlSyncGroup](Get-AzSqlSyncGroup.md)
Zwraca informacje dotyczące grup synchronizacji bazy danych Azure SQL Database.

### [Get-AzSqlSyncGroupLog](Get-AzSqlSyncGroupLog.md)
Zwraca dzienniki grupy synchronizacji bazy danych Azure SQL.

### [Get-AzSqlSyncMember](Get-AzSqlSyncMember.md)
Zwraca informacje o elementach członkowskich synchronizacji bazy danych Azure SQL Database.

### [Get-AzSqlSyncSchema](Get-AzSqlSyncSchema.md)
Zwraca informacje o schemacie synchronizacji bazy danych członka lub centralnej bazie danych.

### [Get-AzSqlVirtualCluster](Get-AzSqlVirtualCluster.md)
Zwraca informacje o wirtualnym klastrze SQL Azure.

### [Invoke-AzSqlDatabaseFailover](Invoke-AzSqlDatabaseFailover.md)
Praca awaryjna bazy danych.

### [Invoke-AzSqlElasticPoolFailover](Invoke-AzSqlElasticPoolFailover.md)
Praca awaryjna z elastyczną pulą.

### [Invoke-AzSqlInstanceFailover](Invoke-AzSqlInstanceFailover.md)
Praca awaryjna wystąpienia zarządzanego przez usługę Azure SQL.

### [Nowe — AzSqlDatabase](New-AzSqlDatabase.md)
Tworzy bazę danych lub elastyczną bazę danych.

### [Nowe — AzSqlDatabaseCopy](New-AzSqlDatabaseCopy.md)
Tworzy kopię bazy danych SQL, która w bieżącym czasie używa migawki.

### [Nowe — AzSqlDatabaseDataMaskingRule](New-AzSqlDatabaseDataMaskingRule.md)
Tworzy regułę maskowania danych dla bazy danych.

### [Nowe — AzSqlDatabaseExport](New-AzSqlDatabaseExport.md)
Eksportuje bazę danych SQL Azure jako plik BACPAC do konta magazynu.

### [Nowe — AzSqlDatabaseFailoverGroup](New-AzSqlDatabaseFailoverGroup.md)
To polecenie powoduje utworzenie nowej grupy trybu failover bazy danych SQL Azure.

### [Nowe — AzSqlDatabaseImport](New-AzSqlDatabaseImport.md)
Importuje plik BACPAC i tworzy nową bazę danych na serwerze.

### [Nowe — AzSqlDatabaseInstanceFailoverGroup](New-AzSqlDatabaseInstanceFailoverGroup.md)
To polecenie tworzy nową grupę trybu failover wystąpienia bazy danych SQL Azure.

### [Nowe — AzSqlDatabaseRestorePoint](New-AzSqlDatabaseRestorePoint.md)
Tworzy nowy punkt przywracania, z którego można przywrócić bazę danych SQL.

### [Nowe — AzSqlDatabaseSecondary](New-AzSqlDatabaseSecondary.md)
Tworzy pomocniczą bazę danych dla istniejącej bazy danych i uruchamia replikację danych.

### [Nowe — AzSqlElasticJob](New-AzSqlElasticJob.md)
Tworzy nowe zadanie

### [Nowe — AzSqlElasticJobAgent](New-AzSqlElasticJobAgent.md)
Tworzy nowego elastycznego agenta zadań.

### [Nowe — AzSqlElasticJobCredential](New-AzSqlElasticJobCredential.md)
Tworzy nowe poświadczenie zadania

### [Nowe — AzSqlElasticJobTargetGroup](New-AzSqlElasticJobTargetGroup.md)
Tworzy nową grupę docelową

### [Nowe — AzSqlElasticPool](New-AzSqlElasticPool.md)
Tworzy pulę Elastic Database dla bazy danych SQL.

### [Nowe — AzSqlInstance](New-AzSqlInstance.md)
Tworzy wystąpienie zarządzane bazy danych Azure SQL Database.

### [Nowe — AzSqlInstanceDatabase](New-AzSqlInstanceDatabase.md)
Tworzy bazę danych wystąpienia zarządzanego SQL Azure.

### [Nowe — AzSqlInstancePool](New-AzSqlInstancePool.md)
Tworzy pulę wystąpień SQL platformy Azure.

### [Nowe — AzSqlServer](New-AzSqlServer.md)
Tworzy serwer bazy danych SQL.

### [Nowe — AzSqlServerCommunicationLink](New-AzSqlServerCommunicationLink.md)
Umożliwia utworzenie łącza komunikacyjnego dla transakcji Elastic Database między dwoma serwerami baz danych SQL.

### [Nowe — AzSqlServerDisasterRecoveryConfiguration](New-AzSqlServerDisasterRecoveryConfiguration.md)
Umożliwia utworzenie konfiguracji odzyskiwania systemu serwera bazy danych.

### [Nowe — AzSqlServerDnsAlias](New-AzSqlServerDnsAlias.md)
To polecenie tworzy nowy alias DNS programu Azure SQL Server.

### [Nowe — AzSqlServerFirewallRule](New-AzSqlServerFirewallRule.md)
Tworzy regułę zapory dla serwera bazy danych SQL.

### [Nowe — AzSqlServerVirtualNetworkRule](New-AzSqlServerVirtualNetworkRule.md)
Umożliwia utworzenie reguły sieci wirtualnej usługi Azure SQL Server. 

### [Nowe — AzSqlSyncAgent](New-AzSqlSyncAgent.md)
Tworzy agenta synchronizacji usługi Azure SQL.

### [Nowe — AzSqlSyncAgentKey](New-AzSqlSyncAgentKey.md)
Tworzy klucz agenta synchronizacji platformy Azure SQL.

### [Nowe — AzSqlSyncGroup](New-AzSqlSyncGroup.md)
Tworzy grupę usługi Azure SQL Database Sync.

### [Nowe — AzSqlSyncMember](New-AzSqlSyncMember.md)
Tworzy członka synchronizacji bazy danych Azure SQL Database.

### [Remove-AzSqlDatabase](Remove-AzSqlDatabase.md)
Usuwa bazę danych SQL Azure.

### [Remove-AzSqlDatabaseAudit](Remove-AzSqlDatabaseAudit.md)
Usuwa ustawienia inspekcji bazy danych SQL Azure.

### [Remove-AzSqlDatabaseDataMaskingRule](Remove-AzSqlDatabaseDataMaskingRule.md)
Usuwa regułę maskowania danych z bazy danych.

### [Remove-AzSqlDatabaseFailoverGroup](Remove-AzSqlDatabaseFailoverGroup.md)
Usuwa grupę trybu failover bazy danych SQL Azure.

### [Remove-AzSqlDatabaseFromFailoverGroup](Remove-AzSqlDatabaseFromFailoverGroup.md)
Usuwa co najmniej jeden z baz danych z grupy usługi Azure SQL Database failover.

### [Remove-AzSqlDatabaseInstanceFailoverGroup](Remove-AzSqlDatabaseInstanceFailoverGroup.md)
Usuwa grupę wystąpień awaryjnych.

### [Remove-AzSqlDatabaseLongTermRetentionBackup](Remove-AzSqlDatabaseLongTermRetentionBackup.md)
Usuwa kopię zapasową długotrwałego przechowywania.

### [Remove-AzSqlDatabaseRestorePoint](Remove-AzSqlDatabaseRestorePoint.md)
Usuwa dany punkt przywracania z bazy danych SQL.

### [Remove-AzSqlDatabaseSecondary](Remove-AzSqlDatabaseSecondary.md)
Kończenie replikacji danych między bazą danych SQL a określoną pomocniczą bazą danych.

### [Remove-AzSqlDatabaseSensitivityClassification](Remove-AzSqlDatabaseSensitivityClassification.md)
Usuwa typy informacji oraz etykiety liter kolumn w bazie danych.

### [Remove-AzSqlElasticJob](Remove-AzSqlElasticJob.md)
Usuwa zadanie.

### [Remove-AzSqlElasticJobAgent](Remove-AzSqlElasticJobAgent.md)
Usuwa elastycznego agenta zadań.

### [Remove-AzSqlElasticJobCredential](Remove-AzSqlElasticJobCredential.md)
Usuwa elastyczne poświadczenie zadania.

### [Remove-AzSqlElasticJobStep](Remove-AzSqlElasticJobStep.md)
Usuwa krok zadania

### [Remove-AzSqlElasticJobTarget](Remove-AzSqlElasticJobTarget.md)
Usuwa element docelowy z grupy docelowej.

### [Remove-AzSqlElasticJobTargetGroup](Remove-AzSqlElasticJobTargetGroup.md)
Usuwa grupę docelową.

### [Remove-AzSqlElasticPool](Remove-AzSqlElasticPool.md)
Usuwa pulę Elastic Database.

### [Remove-AzSqlInstance](Remove-AzSqlInstance.md)
Usuwa wystąpienie bazy danych zarządzanej przez usługę Azure SQL.

### [Remove-AzSqlInstanceActiveDirectoryAdministrator](Remove-AzSqlInstanceActiveDirectoryAdministrator.md)
Umożliwia usunięcie wystąpienia zarządzanego przez SQL administratora usługi Azure AD.

### [Remove-AzSqlInstanceDatabase](Remove-AzSqlInstanceDatabase.md)
Usuwa bazę danych wystąpienia zarządzanego SQL Azure.

### [Remove-AzSqlInstanceDatabaseLongTermRetentionBackup](Remove-AzSqlInstanceDatabaseLongTermRetentionBackup.md)
Usuwa kopię zapasową długotrwałego przechowywania.

### [Remove-AzSqlInstanceDatabaseSensitivityClassification](Remove-AzSqlInstanceDatabaseSensitivityClassification.md)
Usuwa typy informacji oraz etykiety liter kolumn w bazie danych wystąpienia zarządzanego SQL Azure.

### [Remove-AzSqlInstanceKeyVaultKey](Remove-AzSqlInstanceKeyVaultKey.md)
Usuwa klucz magazynu kluczy z wystąpienia zarządzanego przez SQL

### [Remove-AzSqlInstancePool](Remove-AzSqlInstancePool.md)
Usuwa pulę wystąpień SQL platformy Azure.

### [Remove-AzSqlServer](Remove-AzSqlServer.md)
Usuwa serwer bazy danych SQL Azure.

### [Remove-AzSqlServerActiveDirectoryAdministrator](Remove-AzSqlServerActiveDirectoryAdministrator.md)
Usuwa administratora usługi Azure AD dla programu SQL Server.

### [Remove-AzSqlServerAudit](Remove-AzSqlServerAudit.md)
Usuwa ustawienia inspekcji programu Azure SQL Server.

### [Remove-AzSqlServerCommunicationLink](Remove-AzSqlServerCommunicationLink.md)
Umożliwia usunięcie łącza komunikacyjnego dla transakcji Elastic Database między dwoma serwerami.

### [Remove-AzSqlServerDisasterRecoveryConfiguration](Remove-AzSqlServerDisasterRecoveryConfiguration.md)
Usuwa konfigurację odzyskiwania systemu serwera bazy danych SQL.

### [Remove-AzSqlServerDnsAlias](Remove-AzSqlServerDnsAlias.md)
Usuwa alias DNS serwera Azure SQL Server.

### [Remove-AzSqlServerFirewallRule](Remove-AzSqlServerFirewallRule.md)
Usuwa regułę zapory z serwera bazy danych SQL.

### [Remove-AzSqlServerKeyVaultKey](Remove-AzSqlServerKeyVaultKey.md)
Usuwa klucz magazynu kluczy z programu SQL Server.

### [Remove-AzSqlServerVirtualNetworkRule](Remove-AzSqlServerVirtualNetworkRule.md)
Usuwa regułę sieci wirtualnej usługi Azure SQL Server.

### [Remove-AzSqlSyncAgent](Remove-AzSqlSyncAgent.md)
Usuwa agenta synchronizacji usługi Azure SQL.

### [Remove-AzSqlSyncGroup](Remove-AzSqlSyncGroup.md)
Usuwa grupę usługi Azure SQL Database Sync.

### [Remove-AzSqlSyncMember](Remove-AzSqlSyncMember.md)
Usuwa członka synchronizacji bazy danych SQL Azure.

### [Remove-AzSqlVirtualCluster](Remove-AzSqlVirtualCluster.md)
Usuwa klaster wirtualny usługi Azure SQL.

### [Restore-AzSqlDatabase](Restore-AzSqlDatabase.md)
Przywraca bazę danych SQL.

### [Restore-AzSqlInstanceDatabase](Restore-AzSqlInstanceDatabase.md)
Przywraca bazę danych wystąpienia zarządzanego SQL Azure.

### [Życiorys — AzSqlDatabase](Resume-AzSqlDatabase.md)
Wznawia bazę danych hurtowni danych SQL.

### [Set-AzSqlDatabase](Set-AzSqlDatabase.md)
Ustawia właściwości bazy danych lub przenosi istniejącą bazę danych do puli elastycznej.

### [Set-AzSqlDatabaseAdvisorAutoExecuteStatus](Set-AzSqlDatabaseAdvisorAutoExecuteStatus.md)
Modyfikuje stan automatycznego wykonywania klasyfikatora usługi Azure SQL Database.

### [Set-AzSqlDatabaseAudit](Set-AzSqlDatabaseAudit.md)
Zmienia ustawienia inspekcji bazy danych SQL Azure.

### [Set-AzSqlDatabaseBackupLongTermRetentionPolicy](Set-AzSqlDatabaseBackupLongTermRetentionPolicy.md)
Ustawia zasady przechowywania długoterminowych serwerów.

### [Set-AzSqlDatabaseBackupShortTermRetentionPolicy](Set-AzSqlDatabaseBackupShortTermRetentionPolicy.md)
Ustawia zasady przechowywania krótkoterminowej kopii zapasowej.

### [Set-AzSqlDatabaseDataMaskingPolicy](Set-AzSqlDatabaseDataMaskingPolicy.md)
Ustawia maskowanie danych dla bazy danych.

### [Set-AzSqlDatabaseDataMaskingRule](Set-AzSqlDatabaseDataMaskingRule.md)
Ustawia właściwości reguły maskowania danych dla bazy danych.

### [Set-AzSqlDatabaseFailoverGroup](Set-AzSqlDatabaseFailoverGroup.md)
Modyfikuje konfigurację grupy trybu failover bazy danych SQL Azure.

### [Set-AzSqlDatabaseGeoBackupPolicy](Set-AzSqlDatabaseGeoBackupPolicy.md)
Ustawianie zasad tworzenia kopii zapasowej bazy danych.

### [Set-AzSqlDatabaseInstanceFailoverGroup](Set-AzSqlDatabaseInstanceFailoverGroup.md)
Modyfikuje konfigurację grupy trybu failover wystąpienia.

### [Set-AzSqlDatabaseRecommendedActionState](Set-AzSqlDatabaseRecommendedActionState.md)
Aktualizuje stan zalecanej akcji bazy danych SQL Azure.

### [Set-AzSqlDatabaseSecondary](Set-AzSqlDatabaseSecondary.md)
Przełącza pomocniczą bazę danych na podstawową w celu zainicjowania trybu failover.

### [Set-AzSqlDatabaseSensitivityClassification](Set-AzSqlDatabaseSensitivityClassification.md)
Ustawia typy informacji i oznakowań liter kolumn w bazie danych.

### [Set-AzSqlDatabaseTransparentDataEncryption](Set-AzSqlDatabaseTransparentDataEncryption.md)
Modyfikuje Właściwość TDE bazy danych.

### [Set-AzSqlDatabaseVulnerabilityAssessmentRuleBaseline](Set-AzSqlDatabaseVulnerabilityAssessmentRuleBaseline.md)
Ustawia plan bazowy reguły oceny usterki.

### [Set-AzSqlElasticJob](Set-AzSqlElasticJob.md)
Aktualizowanie zadania

### [Set-AzSqlElasticJobAgent](Set-AzSqlElasticJobAgent.md)
Aktualizowanie elastycznego agenta zadań

### [Set-AzSqlElasticJobCredential](Set-AzSqlElasticJobCredential.md)
Aktualizowanie poświadczenia zadania

### [Set-AzSqlElasticJobStep](Set-AzSqlElasticJobStep.md)
Aktualizowanie kroku zadania

### [Set-AzSqlElasticPool](Set-AzSqlElasticPool.md)
Modyfikuje właściwości puli Elastic Database w bazie danych Azure SQL Database.

### [Set-AzSqlElasticPoolAdvisorAutoExecuteStatus](Set-AzSqlElasticPoolAdvisorAutoExecuteStatus.md)
Aktualizuje stan automatycznego wykonania klasyfikatora Elastic Pool SQL platformy Azure.

### [Set-AzSqlElasticPoolRecommendedActionState](Set-AzSqlElasticPoolRecommendedActionState.md)
Aktualizuje stan zalecanej akcji puli elastycznej w usłudze Azure SQL.

### [Set-AzSqlInstance](Set-AzSqlInstance.md)
Ustawia właściwości wystąpienia zarządzanego bazy danych Azure SQL Database.

### [Set-AzSqlInstanceActiveDirectoryAdministrator](Set-AzSqlInstanceActiveDirectoryAdministrator.md)
Inicjuje obsługę administracyjną administratora usługi Azure AD dla wystąpienia zarządzanego SQL.

### [Set-AzSqlInstanceDatabaseBackupLongTermRetentionPolicy](Set-AzSqlInstanceDatabaseBackupLongTermRetentionPolicy.md)
Polecenie cmdlet **Set-AzSqlInstanceDatabaseLongTermRetentionBackup** określa zasady przechowywania długoterminowych zarządzanych baz danych.

### [Set-AzSqlInstanceDatabaseBackupShortTermRetentionPolicy](Set-AzSqlInstanceDatabaseBackupShortTermRetentionPolicy.md)
Ustawia zasady przechowywania krótkoterminowej kopii zapasowej.

### [Set-AzSqlInstanceDatabaseSensitivityClassification](Set-AzSqlInstanceDatabaseSensitivityClassification.md)
Ustawia typy informacji i oznakowań liter kolumn w bazie danych wystąpienia zarządzanego SQL Azure.

### [Set-AzSqlInstanceDatabaseVulnerabilityAssessmentRuleBaseline](Set-AzSqlInstanceDatabaseVulnerabilityAssessmentRuleBaseline.md)
Ustawia plan bazowy reguły oceny usterki.

### [Set-AzSqlInstancePool](Set-AzSqlInstancePool.md)
Ustawia właściwości puli wystąpień usługi Azure SQL.

### [Set-AzSqlInstanceTransparentDataEncryptionProtector](Set-AzSqlInstanceTransparentDataEncryptionProtector.md)
Ustawia ochronę przezroczystego szyfrowania danych (TDE) dla wystąpienia zarządzanego SQL.

### [Set-AzSqlServer](Set-AzSqlServer.md)
Modyfikuje właściwości serwera bazy danych SQL.

### [Set-AzSqlServerActiveDirectoryAdministrator](Set-AzSqlServerActiveDirectoryAdministrator.md)
Inicjuje obsługę administracyjną administratora usługi Azure AD dla programu SQL Server.

### [Set-AzSqlServerAdvisorAutoExecuteStatus](Set-AzSqlServerAdvisorAutoExecuteStatus.md)
Aktualizuje stan automatycznego wykonywania usługi Azure SQL Server Advisor.

### [Set-AzSqlServerAudit](Set-AzSqlServerAudit.md)
Zmienia ustawienia inspekcji programu Azure SQL Server.

### [Set-AzSqlServerDisasterRecoveryConfiguration](Set-AzSqlServerDisasterRecoveryConfiguration.md)
Modyfikuje konfigurację odzyskiwania serwera bazy danych.

### [Set-AzSqlServerDnsAlias](Set-AzSqlServerDnsAlias.md)
Modyfikuje serwer, do którego jest wskazywany alias DNS programu Azure SQL Server.

### [Set-AzSqlServerFirewallRule](Set-AzSqlServerFirewallRule.md)
Modyfikuje regułę zapory na serwerze bazy danych SQL Azure.

### [Set-AzSqlServerRecommendedActionState](Set-AzSqlServerRecommendedActionState.md)
Aktualizuje stan zalecanej akcji programu Azure SQL Server.

### [Set-AzSqlServerTransparentDataEncryptionProtector](Set-AzSqlServerTransparentDataEncryptionProtector.md)
Ustawia funkcję ochrony przezroczystego szyfrowania danych (TDE) dla programu SQL Server.

### [Set-AzSqlServerVirtualNetworkRule](Set-AzSqlServerVirtualNetworkRule.md)
Modyfikuje konfigurację reguły sieci wirtualnej usługi Azure SQL Server.

### [Start — AzSqlDatabaseExecuteIndexRecommendation](Start-AzSqlDatabaseExecuteIndexRecommendation.md)
Uruchamia przepływ pracy, w którym jest uruchamiana zalecana operacja indeksowania.

### [Start — AzSqlDatabaseVulnerabilityAssessmentScan](Start-AzSqlDatabaseVulnerabilityAssessmentScan.md)
Rozpoczyna skanowanie oceny dotyczącego luki w zabezpieczeniach.

### [Start — AzSqlElasticJob](Start-AzSqlElasticJob.md)
Rozpoczyna zadanie, zwracając identyfikator wykonywania zadania, który może być sondowany w celu wyświetlenia jego statusu.

### [Start — AzSqlInstanceDatabaseLogReplay](Start-AzSqlInstanceDatabaseLogReplay.md)
Uruchamia usługę powtarzania dziennika z podanymi parametrami.

### [Start — AzSqlInstanceDatabaseVulnerabilityAssessmentScan](Start-AzSqlInstanceDatabaseVulnerabilityAssessmentScan.md)
Rozpoczyna skanowanie oceny dotyczącego luki w zabezpieczeniach.

### [Start — AzSqlSyncGroupSync](Start-AzSqlSyncGroupSync.md)
Rozpoczyna synchronizację grupy synchronizacji.

### [Zatrzymaj — AzSqlDatabaseActivity](Stop-AzSqlDatabaseActivity.md)
Anuluje operację aktualizacji asynchronicznych w bazie danych.

### [Zatrzymaj — AzSqlDatabaseExecuteIndexRecommendation](Stop-AzSqlDatabaseExecuteIndexRecommendation.md)
Zatrzymuje przepływ pracy, w którym jest uruchamiana zalecana operacja indeksowania.

### [Zatrzymaj — AzSqlElasticJob](Stop-AzSqlElasticJob.md)
Zatrzymuje zadanie o identyfikatorze wykonania zadania

### [Zatrzymaj — AzSqlElasticPoolActivity](Stop-AzSqlElasticPoolActivity.md)
Anuluje operację aktualizacji asynchronicznej w puli elastycznej.

### [Zatrzymaj — AzSqlInstanceDatabaseLogReplay](Stop-AzSqlInstanceDatabaseLogReplay.md)
Anuluje usługę rejestrowania powtarzania, upuszczając bazę danych.

### [Zatrzymaj — AzSqlInstanceOperation](Stop-AzSqlInstanceOperation.md)
Zatrzymuje operacje wystąpienia zarządzanego przez SQL.

### [Zatrzymaj — AzSqlSyncGroupSync](Stop-AzSqlSyncGroupSync.md)
Zatrzymuje synchronizację grupy synchronizacji.

### [Suspend — AzSqlDatabase](Suspend-AzSqlDatabase.md)
Wstrzymuje bazę danych hurtowni danych SQL.

### [Przełącznik-AzSqlDatabaseFailoverGroup](Switch-AzSqlDatabaseFailoverGroup.md)
Wykonuje przejście do trybu failover grupy trybu failover bazy danych SQL Azure.

### [Przełącznik-AzSqlDatabaseInstanceFailoverGroup](Switch-AzSqlDatabaseInstanceFailoverGroup.md)
Wykonuje przejście do trybu failover w grupie praca awaryjna wystąpienia.

### [Update-AzSqlDatabaseAdvancedThreatProtectionSetting](Update-AzSqlDatabaseAdvancedThreatProtectionSetting.md)
Ustawia zaawansowane ustawienia ochrony przed zagrożeniami w bazie danych.

### [Update-AzSqlDatabaseVulnerabilityAssessmentSetting](Update-AzSqlDatabaseVulnerabilityAssessmentSetting.md)
Umożliwia zaktualizowanie ustawień oceny luk w zabezpieczeniach bazy danych.

### [Update-AzSqlInstanceDatabaseVulnerabilityAssessmentSetting](Update-AzSqlInstanceDatabaseVulnerabilityAssessmentSetting.md)
Umożliwia zaktualizowanie ustawień oceny luk w zarządzanej bazie danych.

### [Update-AzSqlInstanceVulnerabilityAssessmentSetting](Update-AzSqlInstanceVulnerabilityAssessmentSetting.md)
Aktualizuje ustawienia oceny usterki wystąpienia zarządzanego.

### [Update-AzSqlServerAdvancedThreatProtectionSetting](Update-AzSqlServerAdvancedThreatProtectionSetting.md)
Ustawia zaawansowane ustawienia ochrony przed zagrożeniami na serwerze.

### [Update-AzSqlServerVulnerabilityAssessmentSetting](Update-AzSqlServerVulnerabilityAssessmentSetting.md)
Umożliwia zaktualizowanie ustawień oceny usterek serwera.

### [Update-AzSqlSyncGroup](Update-AzSqlSyncGroup.md)
Umożliwia zaktualizowanie grupy synchronizacji bazy danych Azure SQL.

### [Update-AzSqlSyncMember](Update-AzSqlSyncMember.md)
Umożliwia zaktualizowanie elementu członkowskiego synchronizacji bazy danych Azure SQL Database.

### [Update-AzSqlSyncSchema](Update-AzSqlSyncSchema.md)
Aktualizowanie schematu synchronizowania bazy danych członka synchronizacji lub bazy danych Centrum synchronizacji.
Pobierze on najnowszą wersję schematu bazy danych z rzeczywistej bazy danych, a następnie użyje go w celu odświeżenia schematu buforowanego przez bazę danych metadanych synchronizacji.
Jeśli zostanie określona wartość "SyncMemberName", spowoduje to odświeżenie schematu bazy danych członka. Jeśli nie, zostanie odświeżony schemat bazy danych centrum.

