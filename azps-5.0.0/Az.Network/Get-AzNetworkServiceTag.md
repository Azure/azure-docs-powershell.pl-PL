...
## Opis
Polecenie cmdlet **Get-AzNetworkServiceTag** pobiera listę zasobów informacji o znacznikach usług.

Pamiętaj, że określone informacje o regionie platformy Azure będą używane jako odwołania dla wersji (nie jako filtru opartego na lokalizacji). Na przykład, nawet jeśli określisz, `-Location eastus2` zostanie wykorzystana lista numerów seryjnych usług ze prefiksem we wszystkich regionach, które będą ograniczone do chmury, do której należy Twój abonament (tzn. Public, USA, Chiny lub Niemcy).
...
Polecenie pobiera listę zasobów informacji o znacznikach usług i zapisuje je w zmiennej `serviceTags` .

### Przykład 2: pobieranie wszystkich prefiksów adresów dla AzureSQL
```powershell
PS C:\> $serviceTags = Get-AzNetworkServiceTag -Location eastus2
...
Pierwsze polecenie pobiera listę zasobów informacji o znacznikach usług i zapisuje je w zmiennej `serviceTags` .
Drugie polecenie filtruje listę, aby wybrać zasób informacyjny dla języka SQL.

### Przykład 3: uzyskiwanie zasobu informacji o znacznikach usługi magazynu dla zachodniego USA 2
```powershell
PS C:\> $serviceTags = Get-AzNetworkServiceTag -Location eastus2
...
Pierwsze polecenie pobiera listę zasobów informacji o znacznikach usług i zapisuje je w zmiennej `serviceTags` .
Poniższe polecenia pokazują różne metody filtrowania listy w celu wybrania informacji o numerach seryjnych w przypadku miejsca do magazynowania w zachodnich Stanach Zjednoczonych 2.

### Przykład 4: uzyskiwanie wszystkich zasobów informacji o tagach usług globalnych
```powershell
PS C:\> $serviceTags = Get-AzNetworkServiceTag -Location eastus2
...
Pierwsze polecenie pobiera listę zasobów informacji o znacznikach usług i zapisuje je w zmiennej `serviceTags` .
Drugie polecenie filtruje listę, aby wybrać tylko te, które nie mają ustawionego regionu.
...
### — Lokalizacja
Lokalizacja, która będzie używana jako odwołanie dla wersji (a nie jako filtru opartego na lokalizacji, zostanie wykorzystana lista numerów seryjnych usług ze prefiksem ze szczegółami dotyczącymi wszystkich regionów, ale ograniczona do chmury, do której należy Twój abonament).
