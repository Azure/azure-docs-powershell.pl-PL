...
## Opis
Polecenie cmdlet **New-AzApiManagementApi** tworzy interfejs API zarządzania interfejsem Azure API.
...
### Przykład 2: Tworzenie interfejsu API przez skopiowanie wszystkich operacji, tagów, produktów i zasad z funkcji echo-API i w ApiVersionSet
```powershell
...
To polecenie tworzy interfejs API `echoapiv3` w ApiVersionSet `xmsVersionSet` i kopiuje wszystkie operacje, Tagi i zasady ze źródłowego interfejsu API `echo-api` . Zastępuje ona SubscriptionRequired, ServiceUrl, ścieżkę, protokoły

### Przykład 3

Tworzy interfejs API. (autogenerowana)
...
### -ApiId
Określa identyfikator interfejsu API, który ma zostać utworzony.
Jeśli nie określisz tego parametru, to polecenie cmdlet wygeneruje identyfikator.
...
### -ApiVersion
Wersja API interfejsu API do utworzenia. Ten parametr jest opcjonalny.
...
Jest to publiczna nazwa interfejsu API, która jest wyświetlana w portalach dewelopera i administratora.
...
### -Path
Określa ścieżkę internetowego interfejsu API, która jest ostatnią częścią publicznego adresu URL interfejsu API i odpowiada polu sufiksu adresu URL interfejsu API sieci Web w portalu administracyjnym.
...
### -ProductId
Określa tablicę identyfikatorów produktów, do których ma zostać dodany nowy interfejs API.
...
### -SourceApiRevision
Wersja API źródłowego interfejsu API. Ten parametr jest opcjonalny.
