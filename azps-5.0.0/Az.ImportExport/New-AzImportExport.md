...
### Przykład 1. Utwórz nowe zadanie ImportExport
```powershell
...
### -BlobListBlobPath
Kolekcja ciągów ścieżek obiektu BLOB.
...
Kolekcja ciągów prefiksu obiektu BLOB.
...
### -CancelRequested
Wskazuje, czy przesłano żądanie anulowania zadania.
...
### -ClientTenantId
Identyfikator dzierżawy klienta składającego żądanie.
...
### -DiagnosticsPath
Katalog wirtualnego obiektu BLOB, do którego będą przechowywane dzienniki kopii i kopie zapasowe plików manifestu dysków (jeśli są włączone).
...
### -DriveList
Lista składająca się z maksymalnie dziesięciu dysków, które składają się na zadanie.
Lista dysków to element wymagany dla zadania importu; nie określono jej w zadaniach eksportowania.
...
Względny identyfikator URI względem obiektu BLOB bloku, który zawiera listę ścieżek obiektu BLOB lub prefiksów ścieżek obiektu BLOB zdefiniowanych powyżej, rozpoczynając od nazwy kontenera.
Jeśli obiekt BLOB znajduje się w kontenerze głównym, identyfikator URI musi zaczynać się od $root.
...
Ścieżka obiektu BLOB wskazująca element BLOB bloku zawierający listę nazw obiektów blob, które nie zostały wyeksportowane ze względu na niewystarczającą ilość miejsca na dysku.
Jeśli wszystkie obiekty blob zostały pomyślnie wyeksportowane, ten element nie jest uwzględniony w odpowiedzi.
...
### -PercentComplete
Całkowity procent ukończenia zadania.
...
### -StorageAccountId
Identyfikator zasobu konta magazynu, z którego będą importowane dane lub do których zostaną one wyeksportowane.
...
DRIVELIST <IDriveStatus [] >: Lista składająca się z maksymalnie dziesięciu dysków składających się na zadanie. Lista dysków to element wymagany dla zadania importu; nie określono jej w zadaniach eksportowania.
