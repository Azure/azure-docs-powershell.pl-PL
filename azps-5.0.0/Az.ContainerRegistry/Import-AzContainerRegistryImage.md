...
### ImportImageByResourceId (domyślny)
```
...
Importowanie BUSYBOX do ACR. Warto 
* "Library/" musi być dodawany przed obrazem źródłowym. "BUSYBOX: Najnowsza" => "Library/BUSYBOX: Najnowsza"
* Poświadczenie jest wymagane, jeśli rejestr źródłowy nie jest publicznie dostępny
...
Zaimportuj BusyBox z ACR źródłowego do elementu docelowego ACR. Warto 
* Poświadczenie jest wymagane, jeśli użytkownik administracyjny dla rejestru źródłowego został wyłączony.
...
### -Mode (tryb)
Po wymuszeniu wszystkie istniejące znaczniki docelowe zostaną zastąpione.
Po wymuszeniu wszystkie istniejące znaczniki docelowe zakończą operację, zanim rozpocznie się kopiowanie.
...
### -Password (hasło)
Hasło używane do uwierzytelniania przy użyciu rejestru źródłowego.
...
Określ obraz według repozytorium ("Witaj na świecie").
Spowoduje to użycie znacznika "Najnowsza".

Określ obrazek według znacznika ("Witaj na świecie: Najnowsza").

Określ obraz na podstawie skrótu manifestu opartego na SHA256 (' hello-world@sha256:abc123 ').
...
Identyfikator zasobu źródłowego rejestru kontenera platformy Azure.
...
### -SourceRegistryUri
Adres rejestru źródłowego (na przykład "mcr.microsoft.com").
...
### -TargetTag
Lista ciągów formularza repozytorium \[ : tag \] .
Gdy znacznik zostanie pominięty, zostanie użyte źródło (lub "Najnowsza", Jeśli znacznik źródłowy jest również pominięty).
...
Lista ciągów nazw repozytoriów w celu wykonania kopii tylko w manifeście.
Nie zostanie utworzony żaden znacznik.
...
### -Username
Nazwa użytkownika do uwierzytelnienia przy użyciu rejestru źródłowego.
