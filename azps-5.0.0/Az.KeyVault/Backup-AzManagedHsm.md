...
## Opis
Pełne wykonywanie kopii zapasowej zarządzanego modułu HSM na koncie magazynu.
Użyj, `Restore-AzManagedHsm` Aby przywrócić kopię zapasową.
...
Polecenie cmdlet spowoduje utworzenie folderu (zwykle nazywanego `mhsm-{name}-{timestamp}` ) w kontenerze magazynu, zapisanie kopii zapasowej w tym folderze i wyjście z folderu o identyfikatorze URI.
