...
## Opis
Polecenie cmdlet **Get-AzIntegrationAccountAssembly** tworzy nowy zestaw na koncie integracji.

## Przykłady

### Przykład 1. Tworzenie nowego zestawu przy użyciu pliku lokalnego
```powershell
...
Tworzy nowy zestaw przy użyciu pliku lokalnego znajdującego się w ścieżce pliku znajdującej się w "$localAssemblyFilePath".

### Przykład 2: Tworzenie nowego zestawu przy użyciu bajtów danych
```powershell
...
Tworzy nowy zestaw przy użyciu tablicy bajtowej zawartej w "$assemblyContent".

### Przykład 3: Tworzenie nowego zestawu za pomocą linku zawartości
```powershell
...
Tworzy nowy zestaw przy użyciu danych bajtowych znajdujących się w adresie URL "$assemblyUrl". Jest to sugerowana metoda tworzenia zestawów o dużym rozmiarze.
