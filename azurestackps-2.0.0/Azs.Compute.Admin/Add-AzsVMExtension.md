...
Użyj linku publicznie dostępnego, aby podać lokalizację rozszerzenia lub identyfikator URI do obiektu blob platformy Azure przy użyciu SasUri.
...
### -Rozszerzenie
Parametry używane do tworzenia nowego obrazu rozszerzenia maszyny wirtualnej.
Aby skonstruować, zobacz sekcję notatki dla właściwości rozszerzenia i Utwórz tablicę skrótów.
...
### -IsSystemExtension
Wskazuje, czy rozszerzenie dotyczy systemu.
...
### -PropertiesPublisher
Wydawca rozszerzenia maszyny wirtualnej
...
### -SourceBlob
Identyfikator URI do obiektu blob platformy Azure lub AzureStack.
...
Prawda, jeśli obsługuje wiele rozszerzeń.
...
### -VmOsType
Docelowy typ systemu operacyjnego maszyny wirtualnej potrzebny do wdrożenia obsługi rozszerzeń.
...
### -VMScaleSetEnabled
Wartość wskazująca, czy rozszerzenie obsługuje funkcję skalowania skali maszyny wirtualnej.
...
### Microsoft. Azure. PowerShell. polecenia. ComputeAdmin. models. Api20151201Preview. IVMExtensionParameters
...
ROZSZERZENIE <IVMExtensionParameters> : parametry użyte do utworzenia nowego obrazu rozszerzenia maszyny wirtualnej.
  - `[ComputeRole <String>]`: Rola COMPUTE
  - `[IsSystemExtension <Boolean?>]`: Wskazuje, czy rozszerzenie dotyczy systemu.
  - `[ProvisioningState <ProvisioningState?>]`: Stan rozbudowy rozszerzenia.
  - `[Publisher <String>]`: Wydawca rozszerzenia maszyny wirtualnej
  - `[SourceBlobUri <String>]`: Identyfikator URI na platformie Azure lub AzureStack BLOB.
  - `[SupportMultipleExtension <Boolean?>]`: True (prawda), jeśli obsługuje wiele rozszerzeń.
  - `[VMScaleSetEnabled <Boolean?>]`: Wartość wskazująca, czy rozszerzenie obsługuje funkcję skali maszyny wirtualnej.
  - `[VmosType <OSType?>]`: Docelowy typ systemu operacyjnego maszyny wirtualnej potrzebny do wdrożenia obsługi rozszerzeń.
