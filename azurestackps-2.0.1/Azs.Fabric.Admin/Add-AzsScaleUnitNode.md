...
### Przykład 1: Add-AzsScaleUnitNode
```powershell
...
Dodaj nowy węzeł jednostka skali do klastra jednostki skali.
...
Aby skonstruować, zobacz sekcję notatki dla właściwości NODELIST i Utwórz tablicę skrótów.
...
Lista danych wejściowych umożliwiających dodanie zestawu węzłów jednostki skali.
Aby skonstruować, zobacz sekcję notatki dla właściwości SCALEUNITNODEPARAMETER i Utwórz tablicę skrótów.
...
### Microsoft. Azure. PowerShell. polecenia. FabricAdmin. models. Api20160501. IScaleOutScaleUnitParametersList
...
NODELIST <IScaleOutScaleUnitParameters [] >: lista węzłów w jednostkach skali.
  - `[BmciPv4Address <String>]`: Adres BMC komputera fizycznego.
  - `[ComputerName <String>]`: Nazwa komputera fizycznego.

SCALEUNITNODEPARAMETER <IScaleOutScaleUnitParametersList> : Lista danych wejściowych umożliwiających dodanie zestawu węzłów jednostki skali.
  - `[AwaitStorageConvergence <Boolean?>]`: Flaga wskazuje, czy operacja powinna czekać na odłączenie miejsca do magazynowania przed powrotem.
  - `[NodeList <IScaleOutScaleUnitParameters[]>]`: Lista węzłów w jednostkach skali.
    - `[BmciPv4Address <String>]`: Adres BMC komputera fizycznego.
    - `[ComputerName <String>]`: Nazwa komputera fizycznego.
